...
static MEMBER_SERVICETYPE s_SDT_u16OrgServiceType = E_SERVICETYPE_INVALID;
static U16 s_SDT_u16OrgPos = INVALID_PROGRAM_POSITION;
MEMBER_SERVICETYPE MApp_SDT_GetOrgServiceType(void)
{
    return s_SDT_u16OrgServiceType;
}
U16 MApp_SDT_GetOrgPos(void)
{
    return s_SDT_u16OrgPos;
}
void MApp_SDT_ResetOrgPosAndOrgServiceType(void)
{
    s_SDT_u16OrgPos = INVALID_PROGRAM_POSITION;
    s_SDT_u16OrgServiceType = E_SERVICETYPE_INVALID;
}
#endif
    ...
    switch ( stRPServiceInfo.u8RunningStatus )
    {
        ...
        case SI_SDT_RUN_NOT_RUN:
            if (stRPServiceInfo.u8Ser_Replacement && stRPServiceInfo.u8LinkageType == SERVICE_REPLACEMENT_SERVICE)
            {
                ...
                #endif
                #ifdef ENABLE_REPLACEMENT_CH_FROM_ORIGINAL
                    s_SDT_u16OrgServiceType = msAPI_CM_GetCurrentServiceType();
                    s_SDT_u16OrgPos = msAPI_CM_GetCurrentPosition(s_SDT_u16OrgServiceType);
                #endif
