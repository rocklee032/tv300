...
void MApp_Init_UIVariable(void)
{
    ...
    eCountry = (EN_OSD_COUNTRY_SETTING)ISDB_DEFAULT_COUNTRY;
   #else
    ...
    MApp_SetOSDCertificationSetting(MApp_GetCertificationSetting(), FALSE);
#endif
...
#if(ENABLE_ATSC)
    INIT_DBINFO(printf("AIR VIR CH          0x%04X  0x%04X\n", RM_AIR_VIR_CH_START_ADR, (U16)RM_AIR_VIR_CH_LEN));
    INIT_DBINFO(printf("AIR CH SETTING       0x%04X  0x%04X\n", RM_AIR_CH_SETTING_START_ADR, (U16)RM_CATV_CH_SETTING_LEN));
    INIT_DBINFO(printf("CATV VIR CH          0x%04X  0x%04X\n", RM_CATV_VIR_CH_START_ADR, (U16)RM_CATV_VIR_CH_LEN));
    INIT_DBINFO(printf("CATV CH SETTING       0x%04X  0x%04X\n", RM_CATV_CH_SETTING_START_ADR, (U16)RM_CATV_CH_SETTING_LEN));
    INIT_DBINFO(printf("RF CH SETTING       0x%04X  0x%04X\n", RM_ATSC_RF_CH_START_ADR, (U16)RM_ATSC_RF_CH_LEN));
    INIT_DBINFO(printf("RRT5 SETTING       0x%04X  0x%04X\n", ATSC_RRT5_SETTING_START_ADDR, RM_DTV_ATSC_CHDATA_END_ADDR-ATSC_RRT5_SETTING_START_ADDR));
    INIT_DBINFO(printf("sizeof(MS_VIRTUAL_CHANNEL)    = %u\n", (U16)sizeof(MS_VIRTUAL_CHANNEL)));
    INIT_DBINFO(printf("sizeof(MS_CHANNEL_SETTING)    = %u\n", (U16)sizeof(MS_CHANNEL_SETTING)));

#endif

    INIT_DBINFO(printf("START 0x%04X  END 0x%04X\n", RM_DB_CH_START_ADDR, RM_DB_CH_END_ADDR));
...
static void MApp_Init_CheckOnTimer(void)
{
    ...
    else if(msAPI_Power_IswakeupsourceRTC())
    {
        ...
        {
            // Get EpgTimer UiSrc...
            E_UI_INPUT_SOURCE eEpgTimer_UiSrc = MApp_EpgTimer_Get_UiSrcType(0);

            // Update input source
            MApp_InputSrc_Set_UiInputSrcType( eEpgTimer_UiSrc );

            // If DTV, Update current service/position
            if( MApp_InputSrc_Is_UiInputSrcTypeDtv(eEpgTimer_UiSrc ) )
            {
                MEMBER_SERVICETYPE srvType = E_SERVICETYPE_INVALID;
                U16 srvPos = INVALID_SERVICE_ID;
                if( msAPI_CM_GetServiceTypeAndPositionWithPCN(u8ReminderPcn, u16ReminderSrv, &srvType, &srvPos) )
                {
...
void MApp_Gop_init(void)
{
  ...
  #if (DISPLAY_LOGO)
    if ( s_PreInit_bShowLogo == FALSE ) // If GOP is not inited
  #endif
    {
        ...
        MApi_GOP_GWIN_SetForceWrite(ENABLE);
...
#if( GOP_SET_ALPHA_INVERSE )
    for (u8GopId = 0; u8GopId<=1; u8GopId++)
    {
        MApi_GOP_GWIN_SetAlphaInverse_EX(u8GopId, TRUE);
    }
#endif
...
#define DEBUG_INIT_AUDIO_TIME(x)    //x
void MApp_PreInit_Audio_Setting_Init(void)
{
    DEBUG_BOOT_TIME(DEBUG_FUNC_TIME_START());

    DEBUG_INIT_AUDIO_TIME( printf("Audio_Setting_Init() at %u\n", MsOS_GetSystemTime()); );
    ...
    DEBUG_INIT_AUDIO_TIME( printf("Audio_Setting_Init() end at %u\n", MsOS_GetSystemTime()); );
...
void MApp_PreInit_PowerOnMusic_Init(void)
{
    ...
    DEBUG_INIT_AUDIO_TIME( printf("PowerOnMusic_Init() at %u\n", MsOS_GetSystemTime()); );

    MApp_InputSource_ChangeAudioSource(INPUT_SOURCE_STORAGE, UI_INPUT_SOURCE_NONE);   // use 183ms

    DEBUG_INIT_AUDIO_TIME( printf("Chg AuSrc done at %u\n", MsOS_GetSystemTime()); );

    MApp_Music_StateInit(BIN_ID_MELODY_ON);

    DEBUG_INIT_AUDIO_TIME( printf("Music_StateInit done at %u\n", MsOS_GetSystemTime()); );

    BOOL bRtn = MApp_Music_PowerON_Melody_Decode_OneTime(); // use 328ms

    DEBUG_INIT_AUDIO_TIME( printf("Decode_OneTime done at %u\n", MsOS_GetSystemTime()); );
...
void MApp_PreInit_Font_Init(void)
{
    ...
    printf("[STR] load font init start: %d\n",MsOS_GetSystemTime());
    if(IsSTREnable())
    {
    ...
    printf("[STR] load font init End: %d\n",MsOS_GetSystemTime());
#else
...
void MApp_PreInit_CheckGoToPowerDown(void)
{
    ...
    #if ((CHIP_SUPPORT_STR) &&(STR_ENABLE))
        MApp_STR_BackupAll();
        MAPP_STR_StoreBackupInfo(TRUE);
    #endif

    #if(ENABLE_MHL)
        msAPI_MHL_CbusControl(MHL_CBUS_FLOATING_OFF);
    #endif
        msAPI_Power_PowerDown_EXEC();
        MDrv_Sys_StopCpu(1);
    }
...
void MApp_PreInit_TurnOn_Backlight(void)
{
    ...
    #if(XC_MLOAD_ENABLE_POS_SEL == XC_MLOAD_EN_POS_AFTER_PANEL_ENABLE)
...
void MApp_PreInit_Environment_Init(void)
{
    ...
    MApi_GOP_GWIN_SetForceWrite(DISABLE);
...
#if(XC_MLOAD_ENABLE_POS_SEL == XC_MLOAD_EN_POS_AFTER_CHANGE_SOURCE)
    MApi_XC_MLoad_Enable(TRUE);
#endif
...
void MApp_PreInit_InitSILib(void)
{
    ...
    if( bInitForISDBT )
    {
        sSIParameter.u32NITScanTimeOut = 10000;
    }
    else
#endif
    {
...
#if DVBC_CERTIFICATION_AUTOSCAN_ENABLE
    MApp_SetCertificationSetting(MApp_GetCertificationSetting());
#endif
...
#if (ENABLE_SECURITY_R2)
void MApp_Init_Security_R2(void)
{
    DEBUG_BOOT_TIME(DEBUG_FUNC_TIME_START());
