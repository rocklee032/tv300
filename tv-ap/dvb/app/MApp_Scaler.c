...
#define XC_SRC_WIN_WIDTH_MIN_MM 64

#define XC_SRC_WIN_HIGH_MAX     2160
#define XC_SRC_WIN_HIGH_MIN     100
#define XC_SRC_WIN_HIGH_MIN_MM  64
#endif

#if( CHIP_FAMILY_TYPE == CHIP_FAMILY_MAZDA )
    #define SCALER_FORCE_DTV_PREDOWN_TO_FULLHD  1
#else
    #define SCALER_FORCE_DTV_PREDOWN_TO_FULLHD  0
#endif
...
#if(ENABLE_HDMI_4K_2K)
// 4K2K
#define MSVD_HDMI_HSTART_4K2K  0xDE
#define MSVD_HDMI_VSTART_4K2K  0x20
#endif
...
MS_VIDEO_Window_Info HDMI_WinInfo[E_HDMI_MAX][E_AR_MAX] =
{
    ...
    {  //576p_50, 3
        ...
        {MSVD_HDMI_HSTART_576p50, MSVD_HDMI_VSTART_576p50, 0, 0, 0, 0},  // Just Scan
...
#if(ENABLE_HDMI_4K_2K)
    {  //4K2k, 18 //Copy from 1080p60
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // Default
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // 16:9
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // 4:3
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // Auto
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // Panorama
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // Just Scan
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // Zoom2
        {MSVD_HDMI_HSTART_4K2K, MSVD_HDMI_VSTART_4K2K, 0, 0, 0, 0},  // Zoom1
    },
#endif
};
...
/*              Functions                                           */
/* ******************************************************************************/
Enu_Scaler_YPbPr_Resolution _MApp_Scaler_Resolution_Remapping_YPbPr(XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_WIN eWindow );
Enu_Scaler_HDMI_Resolution _MApp_Scaler_Resolution_Remapping_HDMI(XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_WIN eWindow );
    ...
    */
    ...
    case MAIN_WINDOW:
            ...
            )
        {
            ...
            //printf("msAPI_MHEG5_IsRunning()=%u\n", msAPI_MHEG5_IsRunning());
              ...
              )
            {
                ...
                BOOLEAN bCurBlueScreenStatus = MApi_XC_Sys_Get_BlueScreenStatus(MAIN_WINDOW);
                //printf("MApi_XC_Sys_Get_BlueScreenStatus()=%u\n", bCurBlueScreenStatus );

                if( bCurBlueScreenStatus == 0 ) // Original Blue screen is Off, need to set blue screen
                {
                ...
                {
                    MApp_Scaler_SetFBLTimingForAspectRatio(enVideoScreen, FALSE);
                }
                else
              #endif
                {
...
static void _MApp_Scaler_CheckHDMode(SCALER_WIN eWindow)
{
    ...
    if( IsSrcTypeDigitalVD(eINPUT_SOURCE_TYPE) )
    {
    ...
    else
    {
        ...
        if(IsSrcTypeHDMI(eINPUT_SOURCE_TYPE))
        {
      ...
      #endif
        if(IsSrcTypeVga(eINPUT_SOURCE_TYPE) || IsSrcTypeYPbPr(eINPUT_SOURCE_TYPE))
        {
      ...
      #if(ENABLE_DTV)
        else if( IsSrcTypeDTV(eINPUT_SOURCE_TYPE) )
        {
      ...
      #endif
      #if(ENABLE_DMP)
        else if( IsSrcTypeStorage(eINPUT_SOURCE_TYPE) )
        {
            u16H_CapSize = MDrv_MVOP_GetHSize();
            u16V_CapSize = MDrv_MVOP_GetVSize();
        }
                                         ...
                                         SCALER_WIN eWindow)
{
    ...
    if( IsSrcTypeHDMI(enInputSourceType) )
    {
        ...
        //if (MApi_XC_GetHdmiSyncMode() == HDMI_SYNC_DE)
        {
            ...
            Enu_Scaler_HDMI_Resolution eHDMIResolution = _MApp_Scaler_Resolution_Remapping_HDMI(pstXC_SetWin_Info, eWindow);

            if( eHDMIResolution >= E_HDMI_MAX )
            {
                eHDMIResolution = E_HDMI480p_60;
            }
            ...
            {                                                   // HDMI mode
                pstXC_SetWin_Info->stCapWin.x = HDMI_WinInfo[eHDMIResolution][u8AspectRatio].u16H_CapStart;
                pstXC_SetWin_Info->stCapWin.y = HDMI_WinInfo[eHDMIResolution][u8AspectRatio].u16V_CapStart;
            }
...
#endif
    {
        ...
        Enu_Scaler_YPbPr_Resolution eYPbpr_Resolution = _MApp_Scaler_Resolution_Remapping_YPbPr(pstXC_SetWin_Info, eWindow);

        if( eYPbpr_Resolution >= E_YPbPr_MAX )
        {
            eYPbpr_Resolution = E_YPbPr480p_60;
        }
        ...
        case E_PQ_ADC_SAMPLING_NOT_SUPPORT:
            ...
            if(IsSrcTypeYPbPr(enInputSourceType))
            {
                ...
                pstXC_SetWin_Info->stCapWin.x = YPbPr_WinInfo[eYPbpr_Resolution][u8AspectRatio].u16H_CapStart;
                pstXC_SetWin_Info->stCapWin.y = YPbPr_WinInfo[eYPbpr_Resolution][u8AspectRatio].u16V_CapStart;
            }
            break;
        ...
        {
            pstXC_SetWin_Info->stCapWin.x = YPbPr_WinInfo[eYPbpr_Resolution][u8AspectRatio].u16H_CapStart * u16ratio / 2;
            pstXC_SetWin_Info->stCapWin.y = YPbPr_WinInfo[eYPbpr_Resolution][u8AspectRatio].u16V_CapStart;
        }
                                            ...
                                            U8 u8AspectRatio, XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_WIN eWindow)
{
    ...
    if( IsVDHasSignal() )
    {
      ...
      #if 0 // Coverity 182652 Dead default in switch
      ...
      #else
        if ( (enVideoSystem_ForCaptureWindow == SIG_PAL)
           ||(enVideoSystem_ForCaptureWindow == SIG_SECAM)
           ||(enVideoSystem_ForCaptureWindow == SIG_PAL_NC) )
        {
            if ( bVdInputIs50Hz == FALSE ) // vd is 60Hz
...
void MApp_Scaler_SetFBLTimingForAspectRatio(EN_ASPECT_RATIO_TYPE enVideoScreen, BOOLEAN bCtrlBlueScreen)
{
...
{
    DEBUG_SCALER_FLOW( printf("_MApp_Scaler_GetPreScalingDown_CheckCapture()\n"); );
  ...
  if(IsStorageInUse() && MApp_MPlayer_IsMoviePlaying())
   {
...
#if( SCALER_FORCE_DTV_PREDOWN_TO_FULLHD )
    if(IsSrcTypeDTV(enInputSourceType))
    {
        if(pXC_SETWIN_INFO->stCapWin.width > 1920 )
        {
         // H pre-scaling down
            pXC_SETWIN_INFO->bPreHCusScaling = TRUE;           ///<assign pre H customized scaling instead of using XC scaling
            pXC_SETWIN_INFO->u16PreHCusScalingSrc = pXC_SETWIN_INFO->stCapWin.width;
            pXC_SETWIN_INFO->u16PreHCusScalingDst = 1920;       ///<pre H customized scaling dst width
            printf("pXC_SETWIN_INFO->stCapWin.width~=%d\n",pXC_SETWIN_INFO->stCapWin.width);
        }

        if(pXC_SETWIN_INFO->stCapWin.height > 1080)
        {    //V pri-post scaling
            pXC_SETWIN_INFO->bPreVCusScaling = TRUE;           ///<assign pre V customized scaling instead of using XC scaling
            pXC_SETWIN_INFO->u16PreVCusScalingSrc = pXC_SETWIN_INFO->stCapWin.height;
            pXC_SETWIN_INFO->u16PreVCusScalingDst = 1080;       ///<pre V customized scaling dst width
            printf("pXC_SETWIN_INFO->stCapWin.height~~=%d\n", pXC_SETWIN_INFO->stCapWin.height);
        }
    }
#endif
    ...
    // Panel is 1366, input is DTV-HEVC-1080p
    if( (devPanel_WIDTH() == 1366)
      &&(IsSrcTypeDTV(enInputSourceType))
      &&(msAPI_VID_Is_CodecH265()) // HEVC
      &&((pXC_SETWIN_INFO->stCapWin.width >= 1728)&&(pXC_SETWIN_INFO->stCapWin.height >= 900)) // 1080p
      &&(!MApi_XC_IsCurrentFrameBufferLessMode()) // FB
      )
    {
        if( pXC_SETWIN_INFO->stCapWin.width > devPanel_WIDTH() )
        {
         // H pre-scaling down
            pXC_SETWIN_INFO->bPreHCusScaling = TRUE;           ///<assign pre H customized scaling instead of using XC scaling
            pXC_SETWIN_INFO->u16PreHCusScalingSrc = pXC_SETWIN_INFO->stCapWin.width;
            pXC_SETWIN_INFO->u16PreHCusScalingDst = devPanel_WIDTH();  ///<pre H customized scaling dst width
            printf("CusPreDownH=%u -> %u\n", pXC_SETWIN_INFO->u16PreHCusScalingSrc, pXC_SETWIN_INFO->u16PreHCusScalingDst);
        }

        if( (!stXC_SetWin_Info[eWindow].bInterlace) ) // P mode
        {
            if( pXC_SETWIN_INFO->stCapWin.height > devPanel_HEIGHT() )
            {    //V pri-post scaling
                pXC_SETWIN_INFO->bPreVCusScaling = TRUE;           ///<assign pre V customized scaling instead of using XC scaling
                pXC_SETWIN_INFO->u16PreVCusScalingSrc = pXC_SETWIN_INFO->stCapWin.height;
                pXC_SETWIN_INFO->u16PreVCusScalingDst = devPanel_HEIGHT();       ///<pre V customized scaling dst width
                printf("CusPreDownV=%u -> %u\n", pXC_SETWIN_INFO->u16PreVCusScalingSrc, pXC_SETWIN_INFO->u16PreVCusScalingDst);
            }
        }
    }
#endif
...
{
    DEBUG_SCALER_FLOW( printf("_MApp_Scaler_GetPreScalingDown_CheckDisplay()\n"); );
...
#if(ENABLE_CHECK_XC_SRC_WIN)
BOOLEAN MApp_Scaler_CheckSrcWin(MS_WINDOW_TYPE* pstSrcWin)
{
    U16 u8WidthMin;
    U16 u8HighMin;
    ...
    if( IsStorageInUse() )
    {
        u8WidthMin = XC_SRC_WIN_WIDTH_MIN_MM;
        u8HighMin = XC_SRC_WIN_HIGH_MIN_MM;
    }
    else
    {
        u8WidthMin = XC_SRC_WIN_WIDTH_MIN;
        u8HighMin = XC_SRC_WIN_HIGH_MIN;
    }


    // Check width
    if( (pstSrcWin->width > XC_SRC_WIN_WIDTH_MAX)
      ||(pstSrcWin->width < u8WidthMin)
      )
    {
    ...
    // Check height
    if( (pstSrcWin->height > XC_SRC_WIN_HIGH_MAX)
      ||(pstSrcWin->height < u8HighMin)
      )
    {
...
BOOLEAN MApp_Scaler_Is_SupportGameMode( XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_WIN eWindow )
{
    if( !IsHDMIInUse() )
        return FALSE;

    Enu_Scaler_HDMI_Resolution u8Resolution = _MApp_Scaler_Resolution_Remapping_HDMI( pstXC_SetWin_Info, eWindow);

    // Only 1080P@50/60 turn on Game
    if( (u8Resolution == E_HDMI1080p_60)
      ||(u8Resolution == E_HDMI1080p_50)
      ||(u8Resolution == E_HDMI720p_60)
      ||(u8Resolution == E_HDMI720p_50)
    #if(GAME_MODE_SUPPORT_SD_P_MODE)
      ||(u8Resolution == E_HDMI576p_50)
      ||(u8Resolution == E_HDMI480p_60)
    #endif
    #if(ENABLE_HDMI_4K_2K)
      ||(u8Resolution == E_HDMI_4K_2K)
    #endif
      )
    {
        return TRUE;
                                            ...
                                            SCALER_WIN eWindow)
{
    ...
    memset(&stMVOPTiming, 0, sizeof(stMVOPTiming));
...
#endif
    {
        ...
        {
            MVOP_Result eMVOP_Result = MDrv_MVOP_GetOutputTiming(&stMVOPTiming);
            if( eMVOP_Result!= E_MVOP_OK)
            {
                PRINT_CURRENT_LINE();
                printf("\nError: eMVOP_Result=%u\n", eMVOP_Result);
                return;
            }

            DEBUG_CAP_WIN( MApp_Scaler_Print_Win("Cap 0:", &(stXC_SetWin_Info[eWindow].stCapWin)); );

            _MApp_Scaler_ProgMVDWin(eWindow, enInputSourceType, &stMVOPTiming, &stXC_SetWin_Info[eWindow]);

            DEBUG_CAP_WIN( MApp_Scaler_Print_Win("Cap 1:", &(stXC_SetWin_Info[eWindow].stCapWin)); );

            _MApp_Scaler_CropCodecSrcWin(&gstVidStatus, &stXC_SetWin_Info[eWindow]);

            DEBUG_CAP_WIN( MApp_Scaler_Print_Win("Cap 2:", &(stXC_SetWin_Info[eWindow].stCapWin)); );
    ...
    #if(ENABLE_CHECK_XC_SRC_WIN)
        if( FALSE == MApp_Scaler_CheckSrcWin( &(stXC_SetWin_Info[eWindow].stCapWin) ) )
        {
            return;
        }
    #endif
    ...
    if((IsStorageInUse()) && (MApp_MPlayer_IsMoviePlaying()) &&
       (E_MPLAYER_MOVIE_PAUSE==MApp_MPlayer_QueryMoviePlayMode()||
        E_MPLAYER_MOVIE_STEP==MApp_MPlayer_QueryMoviePlayMode()))
    {
    ...
    if( IsHDMIInUse() )
    {
        if( MApp_Scaler_Is_SupportGameMode( &(stXC_SetWin_Info[eWindow]), eWindow ) )
        {
            MApi_XC_Sys_Set_GameMode(XC_SYS_GAME_MODE_ON);
        }
        else
        {
            MApi_XC_Sys_Set_GameMode(XC_SYS_GAME_MODE_OFF);
        }
    }
  #endif

    BOOLEAN bEnableLockFreq = TRUE; // Maya default use LockFreq

    // Because MAYA XC driver default turn on LockFreq,
    // SW need to disable it for some special case.
  ...
  #endif
    {
        bEnableLockFreq = TRUE;
    }

    //printf("bEnableLockFreq=%u\n", bEnableLockFreq);
    MApi_XC_Enable_LockFreqOnly(bEnableLockFreq);

#endif // CHIP_FAMILY_MAYA
    ...
    // Because MApi_XC_SetWindow() will set ADC Phase,
    //  Set customer ADC phase must be after MApi_XC_SetWindow(),
    //MApp_Scaler_SetupADC(eWindow);
    // Set customer ADC phase
    if( IsSrcTypeYPbPr(enInputSourceType) )//For customer set  adc phase
    {
...
BOOLEAN MApp_Scaler_Is_SupportZoomAdjustByUser(void)
{
    BOOLEAN bIsSupport = FALSE;


    // Check aspect ratio...
    switch(stSystemInfo[MAIN_WINDOW].enAspectRatio)
    {
        case VIDEOSCREEN_ZOOM2:
        case VIDEOSCREEN_ZOOM1:
            bIsSupport = TRUE;
...
Enu_Scaler_YPbPr_Resolution _MApp_Scaler_Resolution_Remapping_YPbPr(XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_WIN eWindow )
{
    ...
    Enu_Scaler_YPbPr_Resolution eScaler_YPbPr_Resolution = E_YPbPr480i_60;
...
/*
    ...
    printf("Resolution_Remapping_YPbPr\n");
    printf(" u16HSize = %d\n", u16HSize);
    printf(" u16VSize = %d\n", u16VSize);
    printf(" u16InputVFreq = %d\n", u16InputVFreq);
*/

    switch(g_PcadcModeSetting[eWindow].u8ModeIndex)
    {
        case MD_720x480_60I:
            eScaler_YPbPr_Resolution = E_YPbPr480i_60;
            break;
        case MD_720x480_60P:
            eScaler_YPbPr_Resolution = E_YPbPr480p_60;
            break;
        case MD_720x576_50I:
            eScaler_YPbPr_Resolution = E_YPbPr576i_50;
            break;
        case MD_720x576_50P:
            eScaler_YPbPr_Resolution = E_YPbPr576p_50;
            break;
        case MD_1280x720_60P:
            eScaler_YPbPr_Resolution = E_YPbPr720p_60;
            break;
        case MD_1280x720_50P:
            eScaler_YPbPr_Resolution = E_YPbPr720p_50;
            break;
        case MD_1280x720_25P:
            eScaler_YPbPr_Resolution = E_YPbPr720p_25;
            break;
        case MD_1280x720_30P:
            eScaler_YPbPr_Resolution = E_YPbPr720p_30;
            break;
        case MD_1920x1080_60I:
            eScaler_YPbPr_Resolution = E_YPbPr1080i_60;
            break;
        case MD_1920x1080_50I:
            eScaler_YPbPr_Resolution = E_YPbPr1080i_50;
            break;
        case MD_1920x1080_60P:
            eScaler_YPbPr_Resolution = E_YPbPr1080p_60;
            break;
        case MD_1920x1080_50P:
            eScaler_YPbPr_Resolution = E_YPbPr1080p_50;
            break;
        case MD_1920x1080_30P:
            eScaler_YPbPr_Resolution = E_YPbPr1080p_30;
            break;
        case MD_1920x1080_24P:
            eScaler_YPbPr_Resolution = E_YPbPr1080p_24;
            break;
         case MD_1920x1080_25P:
            eScaler_YPbPr_Resolution = E_YPbPr1080p_25;
            break;
       #if (SUPPORT_EURO_HDTV)
        case  MD_1920x1080_50I_EURO:
            eScaler_YPbPr_Resolution = E_YPbPr108oi_25;
            break;
       #endif
        default:
            eScaler_YPbPr_Resolution = E_YPbPr480p_60;
            printf("\nError: YPbPr unknown resolution!\n");
            break;
    }

    return eScaler_YPbPr_Resolution;
}

#define DEBUG_RESOLUTION_MAPPING(x)     //x

Enu_Scaler_HDMI_Resolution _MApp_Scaler_Resolution_Remapping_HDMI(XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_WIN eWindow )
{
    UNUSED(eWindow);

    Enu_Scaler_HDMI_Resolution eScaler_HDMI_Resolution = E_HDMI480i_60;
    U16 u16HSize, u16VSize, u16InputVFreq;
    ...
    DEBUG_RESOLUTION_MAPPING( printf("Resolution_Remapping_HDMI\n"); );
    DEBUG_RESOLUTION_MAPPING( printf(" u16HSize = %d\n", u16HSize); );
    DEBUG_RESOLUTION_MAPPING( printf(" u16VSize = %d\n", u16VSize); );
    DEBUG_RESOLUTION_MAPPING( printf(" u16InputVFreq = %d\n", u16InputVFreq); );

    if((u16HSize>=630 && u16HSize<=650) && (u16VSize>=470 && u16VSize<=490))
    {// 640
        if(pstXC_SetWin_Info->bInterlace)
        {// 480i
           eScaler_HDMI_Resolution = E_HDMI640x480i_60;
        }
        else
        {// 480p
           eScaler_HDMI_Resolution = E_HDMI640x480P_60;
        }
    }
    ...
    {// 720
        if(pstXC_SetWin_Info->bInterlace)
        {// 480i
           eScaler_HDMI_Resolution = E_HDMI480i_60;
        }
        else
        {// 480p
           eScaler_HDMI_Resolution = E_HDMI480p_60;
        }
    }
    ...
    {// 720
        if(pstXC_SetWin_Info->bInterlace)
        {// 576i
           eScaler_HDMI_Resolution = E_HDMI576i_50;
        }
        else
        {// 576p
           eScaler_HDMI_Resolution = E_HDMI576p_50;
        }
    }
    ...
    {// 720
        if(pstXC_SetWin_Info->bInterlace)
        {// 480i
           eScaler_HDMI_Resolution = E_HDMI1440x480i_60;
        }
        else
        {// 480p
           eScaler_HDMI_Resolution = E_HDMI1440x480p_60;
        }
    }
    ...
    {// 720
        if(pstXC_SetWin_Info->bInterlace)
        {// 576i
           eScaler_HDMI_Resolution = E_HDMI1440x576i_50;
        }
        else
        {// 576p
           eScaler_HDMI_Resolution = E_HDMI1440x576p_50;
        }
    }
    ...
    {// 1280
        ...
        if( u16InputVFreq > 550 )
           eScaler_HDMI_Resolution = E_HDMI720p_60;
        else
            eScaler_HDMI_Resolution = E_HDMI720p_50;
    }
    ...
    {// 1920
        ...
        {//1080i
            if( u16InputVFreq > 550 )
               eScaler_HDMI_Resolution = E_HDMI1080i_60;
            else
                eScaler_HDMI_Resolution = E_HDMI1080i_50;
        }
        else
        {
            if( u16InputVFreq > 550 )
            {// 1080P (60)
               eScaler_HDMI_Resolution = E_HDMI1080p_60;
            }
            else if( u16InputVFreq > 450 )
            {// 1080P (50)
               eScaler_HDMI_Resolution = E_HDMI1080p_50;
            }
            else if( u16InputVFreq > 275 )
            {// 1080P (30)
               eScaler_HDMI_Resolution = E_HDMI1080p_30;
            }
            else if( u16InputVFreq > 200 )
            {// 1080P (24P)
               eScaler_HDMI_Resolution = E_HDMI1080p_24;
            }
        }
    }
#if(ENABLE_HDMI_4K_2K)
    else if( IS_IN_RANGE(u16HSize, 2600, 3840) && IS_IN_RANGE(u16VSize, 1300, 2160) )
    {
        eScaler_HDMI_Resolution = E_HDMI_4K_2K;
    }
#endif
    else
    {
        eScaler_HDMI_Resolution = E_HDMI640x480P_60;  //default use 480p
        printf("\nError: HDMI unknown resolution!\n");
    }

    return eScaler_HDMI_Resolution;
}
...
void MApp_Scaler_GetOverScan(INPUT_SOURCE_TYPE_t enInputSrcType, XC_SETWIN_INFO *pstXC_SetWin_Info)
{
    ...
    {
        Enu_Scaler_YPbPr_Resolution eYPbpr_Resolution = _MApp_Scaler_Resolution_Remapping_YPbPr(pstXC_SetWin_Info, MAIN_WINDOW);
        ...
        _u8H_CropRatio_Left = YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8HCrop_Left;
        _u8H_CropRatio_Right = YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8HCrop_Right;
        _u8V_CropRatio_Up = YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8VCrop_Up;
        _u8V_CropRatio_Down = YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8VCrop_Down;
    }
    ...
    {  // make sure this is HDMI mode, not DVI mode
        ...
        Enu_Scaler_HDMI_Resolution eHDMI_Resolution = _MApp_Scaler_Resolution_Remapping_HDMI(pstXC_SetWin_Info, MAIN_WINDOW);

        if( eHDMI_Resolution >= E_HDMI_MAX )
        {
            eHDMI_Resolution = E_HDMI480p_60;
        }

        _u8H_CropRatio_Left = HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8HCrop_Left;
        _u8H_CropRatio_Right = HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8HCrop_Right;
        _u8V_CropRatio_Up = HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8VCrop_Up;
        _u8V_CropRatio_Down = HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8VCrop_Down;
                                       ...
                                       MS_WINDOW_TYPE *pstDstWin)
{
    ...
    {
      #if 0 // CID: 182656 Identical code for different branches
        if(IsATVInUse())
        {
...
void MApp_Scaler_MainWindowSyncEventHandler(INPUT_SOURCE_TYPE_t src, void* para)
{
    ...
    {
    #if( MHEG5_ENABLE )
        if( IsDVBInUse()&&msAPI_MHEG5_IsRunning() )
        {
            MApp_MHEG5_SetGraphARCInfo(SENDARC_AFD_FOR_GE,stSystemInfo[MAIN_WINDOW].enAspectRatio);
            MApp_MHEG5_SetGraphARCInfo(SENDARC_AFD,stSystemInfo[MAIN_WINDOW].enAspectRatio);
        }
...
void MApp_Scaler_SetVidWinInfo(INPUT_SOURCE_TYPE_t enInputSrcType,MS_VIDEO_Window_Info_EXT *pstWindow_info, SCALER_WIN eWindow)
{
    ...
    u8_AspectRatio = _MApp_Scaler_Aspect_Ratio_Remapping(stSystemInfo[eWindow].enAspectRatio);
    ...
    {
        Enu_Scaler_YPbPr_Resolution eYPbpr_Resolution = _MApp_Scaler_Resolution_Remapping_YPbPr(&stXC_SetWin_Info[eWindow], eWindow);
        ...
        YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8HCrop_Left = st_VidWin_Info.u8HCrop_Left;
        YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8HCrop_Right = st_VidWin_Info.u8HCrop_Right;
        YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8VCrop_Up = st_VidWin_Info.u8VCrop_Up;
        YPbPr_WinInfo[eYPbpr_Resolution][u8_AspectRatio].u8VCrop_Down = st_VidWin_Info.u8VCrop_Down;
    ...
    {  // make sure this is HDMI mode, not DVI mode
        ...
        Enu_Scaler_HDMI_Resolution eHDMI_Resolution = _MApp_Scaler_Resolution_Remapping_HDMI(&stXC_SetWin_Info[eWindow], eWindow);
        ...
        HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8HCrop_Left = st_VidWin_Info.u8HCrop_Left;
        HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8HCrop_Right = st_VidWin_Info.u8HCrop_Right;
        HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8VCrop_Up = st_VidWin_Info.u8VCrop_Up;
        HDMI_WinInfo[eHDMI_Resolution][u8_AspectRatio].u8VCrop_Down = st_VidWin_Info.u8VCrop_Down;
    ...
    switch( eScalerMMVideoMuteCase )
    {
        ...
        case E_XC_MM_VIDEO_MUTE_CASE_PHOTO_FULL_WIN:
            ...
            else
            {
                ...
                U16 u16WaitTime = 1000/60*4; // Default is wait 4 frame

                MVOP_Timing stMVOPTiming;

                memset(&stMVOPTiming, 0, sizeof(stMVOPTiming));

                if( E_MVOP_OK == MDrv_MVOP_GetOutputTiming(&stMVOPTiming) )
                {
                    if( stMVOPTiming.u8Framerate == 0 )
                    {
                        stMVOPTiming.u8Framerate = 10;
                    }
                    else if( stMVOPTiming.u8Framerate > 65 )
                    {
                        stMVOPTiming.u8Framerate = 65;
                    }
                    u16WaitTime = (1000/stMVOPTiming.u8Framerate) * 4;
                }

                // wait 4 input vsync before turn on scalar
                MApi_XC_WaitInputVSync(4, u16WaitTime, MAIN_WINDOW);
                ...
                // Don't need to wait timer! unmute right now
