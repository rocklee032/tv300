...
static enPVRFileChange  _enPVRFileChangeStatus = E_PVR_FILE_CHANGE_NONE;
/*************************************************************************************************/
...
{   //progress of MApp_PVR_DiskFormatCreateFolder(), MApp_PVR_DiskFormatCreateBulks(), MApp_PVR_DiskFormatCreateIndexFile()
    ...
    u16BulksNum = msAPI_PVRFS_GetBulksNumToCreate();
    if (u16BulksNum > 0)
        percentage = ( ((U32)msAPI_PVRFS_GetCurrentCreateBulkNum()*100) / u16BulksNum );
...
BOOLEAN MApp_PVR_IsVideoFrameCountStillIncrease(void)
{
    ...
    static U8 u8Cnt = 0;
    static U8 u8TimeoutCnt = 0;
    static U32 u32FrameCnt = 0;
    ...
    u8TimeoutCnt++;
    if (u8TimeoutCnt == WAIT_VIDPICCOUNT_TIME)
    {
        u32Timer = msAPI_Timer_GetTime0();
    }

    if ( ((u8Cnt >= WAIT_VIDPICCOUNT_TIME) || (u8TimeoutCnt >= WAIT_VIDPICCOUNT_TIME))
         && (msAPI_Timer_DiffTimeFromNow(u32Timer) > 1000) )
    {
        u32FrameCnt = 0;
        u8Cnt = 0;
        u32Timer = 0;
...
{
    if ( (_enPVRState == E_PVR_STATE_RECORDnPLAYBACK) && (TRUE == MApp_Playback_IsDiffFile()) )
    {
...
U32 MApp_PVR_GetDurationTime(void)
{
    if ( (_enPVRState == E_PVR_STATE_TIMESHIFT)
         && ( (E_TIMESHIFT_STATE_RECORDING == MApp_TimeShift_StateMachineGet())
              || (E_TIMESHIFT_STATE_RECORDINGnPLAYBACKING == MApp_TimeShift_StateMachineGet())
              || (E_TIMESHIFT_STATE_PLAYBACKING == MApp_TimeShift_StateMachineGet()) ) )
    {
...
{
    enPvrEncryptType enType = E_PVR_ENCRYPTION_NONE;

    if ( ((_enPVRState == E_PVR_STATE_RECORD) || (_enPVRState == E_PVR_STATE_RECORDnPLAYBACK))
         && (E_RECORD_STATE_RECORDING == MApp_Record_StateMachineGet()) )
    {
        enType = MApp_Record_GetEncryptionType(RECORD_PATH_DEFAULT);
        return enType;
    }
    else if ( (_enPVRState == E_PVR_STATE_TIMESHIFT)
              && ( (E_TIMESHIFT_STATE_RECORDINGnPLAYBACKING == MApp_TimeShift_StateMachineGet())
                   ...
                   || (E_TIMESHIFT_STATE_PLAYBACKING == MApp_TimeShift_StateMachineGet()) ) )
    {
        enType = MApp_TimeShift_GetEncryptionType();
        return enType;
    }
    else if ( ((_enPVRState == E_PVR_STATE_PLAYBACK) || (_enPVRState == E_PVR_STATE_RECORDnPLAYBACK))
              && ( (E_PLAYBACK_STATE_PLAYBACKING == MApp_Playback_StateMachineGet())
                   || (E_PLAYBACK_STATE_FASTFORWARD == MApp_Playback_StateMachineGet())
                   || (E_PLAYBACK_STATE_FASTBACKWARD == MApp_Playback_StateMachineGet())
                   || (E_PLAYBACK_STATE_PAUSE == MApp_Playback_StateMachineGet()) ) )
    {
        enType = MApp_Playback_GetEncryptionType();
        return enType;
    }
...
void MApp_PVR_StateMachineInput(enPVRInput enSMInput, U32 u32ExtParameter)
{
    ...
    if (E_PVR_FILE_CHANGE_FORWARD == _enPVRFileChangeStatus)
    {
        _enPVRInput = E_PVR_INPUT_PLAYBACK_FAST_FORWARD;
    }
    else if (E_PVR_FILE_CHANGE_BACKWARD == _enPVRFileChangeStatus)
    {
        _enPVRInput = E_PVR_INPUT_PLAYBACK_FAST_BACKWARD;
    }
    else
    {
...
void MApp_PVR_SetFileChangeStatus(enPVRFileChange enFileChangeStatus)
{
    _enPVRFileChangeStatus = enFileChangeStatus;
}

enPVRFileChange MApp_PVR_GetFileChangeStatus(void)
{
    return _enPVRFileChangeStatus;
}
/***************************************************************************************/
