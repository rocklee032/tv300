...
static U8 MApp_Record_Open(U16 *pu16RecordFileName)
{
  ...
  //printf("MApp_Record_Open \n");
    ...
    if(_pstPvrRecordPath[u8RecordPathNum])
    {
        ...
        if (TRUE == _bRecordAll)
        {
            _pstPvrRecordPath[u8RecordPathNum]->bRecordAll = TRUE;
            _pstPvrRecordPath[u8RecordPathNum]->u8PacketSize = E_PVR_PACKET_188;
        }
        else
        {
...
#endif
            _pstPvrRecordPath[u8RecordPathNum]->bRecordAll = FALSE;
            _pstPvrRecordPath[u8RecordPathNum]->u8PacketSize = E_PVR_PACKET_192;
        }
        ...
        if ( (FALSE == msAPI_CI_CC_GetAuthenticationStatus())
             && (FALSE == msAPI_CM_GetProgramAttribute(bServiceType, wProgramPositon, E_ATTRIBUTE_IS_SCRAMBLED)) )
        {
            _pstPvrRecordPath[u8RecordPathNum]->enEncryptionType = E_PVR_ENCRYPTION_DEFAULT;
            PVR_DBG( printf("[%s:%d][AUTH Failed] Do Record FTA Program  \n", __FUNCTION__, __LINE__); );
        }
        else if ( (msAPI_CM_GetProgramAttribute(bServiceType, wProgramPositon, E_ATTRIBUTE_IS_SCRAMBLED))
                  || (EN_CI_VERSION_CI_PLUS == msAPI_CI_CC_CAM_Mode()) )
        {
            ...
            if ( (E_PVR_API_STATUS_ERROR_CIPLUS_COPY_PROTECTION == enPvrStatus)
                 && (FALSE == msAPI_CM_GetProgramAttribute(bServiceType, wProgramPositon, E_ATTRIBUTE_IS_SCRAMBLED)) )
            {
                _pstPvrRecordPath[u8RecordPathNum]->enEncryptionType = E_PVR_ENCRYPTION_DEFAULT;
                PVR_DBG( printf("[%s:%d][AUTH Success][PVR Error] Do Record FTA Program \n", __FUNCTION__, __LINE__); );
            }
            else if ( (E_PVR_API_STATUS_OK != enPvrStatus)
                      && (E_PVR_API_STATUS_SUCCESS_CIPLUS_COPY_PROTECTION != enPvrStatus) )
            {
                PVR_DBG( printf("[%s:%d] ERROR_CIPLUS_COPY \n", __FUNCTION__, __LINE__); );
            ...
            else if (E_PVR_API_STATUS_SUCCESS_CIPLUS_COPY_PROTECTION == enPvrStatus)
            {
                ...
                {
                    PVR_DBG(printf("[%s:%d] Get CICAM ID Failed \n", __FUNCTION__, __LINE__));
                ...
                if ( (FALSE == msAPI_CI_CC_GetCicamUriLicenseUpdate(&stUriLicense, FALSE))
            ...
            MAPP_CIPVR_RecordStartEventUpdate(u8aCICAMID);
        }
#endif
    ...
    PVR_DBG( printf("[%s:%d] u8CicamCheckLevel=%u, enEncryptionType=%u \n", __FUNCTION__, __LINE__, _pstPvrRecordPath[u8RecordPathNum]->u8CicamCheckLevel, _pstPvrRecordPath[u8RecordPathNum]->enEncryptionType); );
...
static BOOLEAN  MApp_Record_Enable(U8 u8PathNum)
{
  ...
  //printf("MApp_Record_Enable \n");
    ...
    /*[02-1]set pvr record path =======================================================*/
    u8PacketSize = _pstPvrRecordPath[u8PathNum]->u8PacketSize;
    _pstPvrRecordPath[u8PathNum]->u32BufferStart  = PvrGetWriteSdramAdr();                 //<--from system
    _pstPvrRecordPath[u8PathNum]->u32BufferLength = (PVR_WRITE_SDRAM_LEN / (16*u8PacketSize)) * (16*u8PacketSize);
    ...
    if ((_enRecordStatus=msAPI_PVR_RecordPathSet(_pstPvrRecordPath[u8PathNum])) == E_PVR_API_STATUS_OK)
    {
      ...
      //msAPI_PVR_RecordPathAddPID(_pstPvrRecordPath[u8PathNum], g_u16Current_PVR_AudioPID,       MSAPI_DMX_FILTER_TYPE_AUDIO);        //<--from other AP:(03) AUDIO
        ...
        {
            // Channel Lock
            U16 u16Ordinal = msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType());
            _pstPvrRecordPath[u8PathNum]->bIsChannelLocked = IS_LOCK_CH(u16Ordinal);
        }
        #ifdef S3PLUS
...
static BOOLEAN  MApp_Record_Routine(U8 u8PathNum)
{
  ...
  //printf("MApp_Record_Routine \n");
    ...
    {
        if ( E_PVR_API_STATUS_OK != (_enRecordStatus = msAPI_PVR_RecordPathInput(_pstPvrRecordPath[u8PathNum])) )
        {
            if ( (E_PVR_API_STATUS_RECORD_FILE_OVER_SIZE == _enRecordStatus)
                 && ((PVR_FS_N51FAT32 == msAPI_PVRFS_GetFileSystemType())
                    || (PVR_FS_LINUX_FAT == msAPI_PVRFS_GetFileSystemType())) )
            {
                ...
                if ( E_PVR_API_STATUS_OK != msAPI_PVR_RecordChangeFile(u16NewFileName, _pstPvrRecordPath[u8PathNum]) )
                {
                    return FALSE;
                }
            ...
            else if (E_PVR_API_STATUS_RECORD_BUFFER_ERROR == _enRecordStatus)
            {
    ...
    if(_pstPvrRecordPath[u8PathNum]->u8Parental_Control < MApp_EIT_GetCurParentControl())
    {
        _pstPvrRecordPath[u8PathNum]->u8Parental_Control = MApp_EIT_GetCurParentControl();
    }

    msAPI_PVR_RecordPathGatherStatistics(_pstPvrRecordPath[u8PathNum]);
    _enRecordState = E_RECORD_STATE_RECORDING;
    return TRUE;
}
...
static BOOLEAN  MApp_Record_Close(U8 u8PathNum)
{
    ...
    PVR_TRACE(printf("MApp_Record_Close \n"););
    _pstPvrRecordPath[u8PathNum]->u32RecordedPeriod = msAPI_PVR_PlaybackGetTotalTimeByTimeStamp(_pstPvrRecordPath[u8PathNum]->enEncryptionType, _pstPvrRecordPath[u8PathNum]->hWriteFile);
...
U32 MApp_Record_GetTotalTimeSec(U8 u8PathNum)
{
    ...
    u32TotalTime = _pstPvrRecordPath[u8PathNum]->u32RecordedPeriod/10;
        ...
        case E_RECORD_STATE_WAIT:
        {
            ...
            if(_enRecordInput == E_RECORD_INPUT_RECORD_OPEN)
            {
                ...
                else
                {
                    ...
                    PVR_DBG(printf("MApp_Record_Open(): Failed! \n"););
                    _enRecordInput = E_RECORD_INPUT_NONE;
                }
            }
...
BOOLEAN MApp_Record_GetPrevSeqFileName(U16 *u16NewFileName)
{
    U16 StrLen;
    StrLen = UnicodeLen((S8 *)u16NewFileName);
    ...
    if (StrLen >= 6)
    {
        if ( (u16NewFileName[StrLen-6] == '0') && (u16NewFileName[StrLen-5] == '0') && (u16NewFileName[StrLen-4] == '2') )
        {
            u16NewFileName[StrLen-6] = 0;
        }
        else
        {
            if (u16NewFileName[StrLen-4] >= '1')
            {
                u16NewFileName[StrLen-4]--;
            }
            else if ( (u16NewFileName[StrLen-4] == '0') && (u16NewFileName[StrLen-5] >= '1') )
            {
                u16NewFileName[StrLen-4] = '9';
                u16NewFileName[StrLen-5]--;
            }
            else if ( (u16NewFileName[StrLen-4] == '0') && (u16NewFileName[StrLen-5] == '0') && (u16NewFileName[StrLen-6] >= '1') )
            {
                u16NewFileName[StrLen-4] = '9';
                u16NewFileName[StrLen-5] = '9';
                u16NewFileName[StrLen-6]--;
            }
        }
        return TRUE;
