...
static BOOLEAN MApp_TimeShift_RecordOpen(void)
{
    ...
    if (_pstPvrRecordPath != NULL)
    {
        ...
        _pstPvrRecordPath->u8PacketSize = E_PVR_PACKET_192;
        ...
        PVR_DBG(printf("[%s:%d] u8CicamCheckLevel=%u, enEncryptionType=%u \n", __FUNCTION__, __LINE__,
            _pstPvrRecordPath->u8CicamCheckLevel, _pstPvrRecordPath->enEncryptionType));
    }
    else
    {
...
static BOOLEAN MApp_TimeShift_PlaybackOpen(void)
{
    ...
    if(_pstPvrPlaybackPath)
    {
        ...
        u8PlaybackOpen = TRUE;
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
        PVR_DBG(printf("[TimeShift_PlaybackOpen] u8CicamCheckLevel=%u \n", _pstPvrPlaybackPath->pstPvrRecordPath->u8CicamCheckLevel));
        switch (_pstPvrPlaybackPath->pstPvrRecordPath->u8CicamCheckLevel)
        {
            case EN_CI_VERSION_CI_PLUS:
            {
                ...
                {
                    PVR_DBG(printf("[TimeShift_PlaybackOpen] Open Error(%u) \n", enPvrStatus));
                ...
                if (FALSE == msAPI_CI_CC_GetCICAMID(u8aCICAMID))
                {
                    PVR_DBG(printf("[TimeShift_PlaybackOpen] Get CICAM ID Failed \n"));
                    _enTimeShiftPlaybackState = E_TIMESHIFT_PLAYBACK_STATE_DISABLE;
                    _enTimeShiftPlaybackSpeed = E_PLAY_SPEED_1X;
                ...
                {
                    PVR_DBG(printf("[TimeShift_PlaybackOpen] CICAM ID Is Not Same \n"));
                    _enTimeShiftPlaybackState = E_TIMESHIFT_PLAYBACK_STATE_DISABLE;
                    _enTimeShiftPlaybackSpeed = E_PLAY_SPEED_1X;
                    _enTimeShiftStatus = E_PVR_API_STATUS_ERROR_RETENTION_LIMIT_EXPIRED;
                    msAPI_PVR_PlaybackPathClose(_pstPvrPlaybackPath);
                ...
                else
                {
                    ...
                    if ( (TRUE == bRetentionLimitExpire)
                         && (MApp_TimeShift_GetRecordEndTimeSec() <= u32JumpTime) )
                    {
                        PVR_DBG(printf("[TimeShift_PlaybackOpen] Is Expire, Can't Play(%u:%u) \n", MApp_TimeShift_GetRecordEndTimeSec(), u32JumpTime));
                        _enTimeShiftPlaybackState = E_TIMESHIFT_PLAYBACK_STATE_DISABLE;
                        _enTimeShiftPlaybackSpeed = E_PLAY_SPEED_1X;
                        _enTimeShiftStatus = E_PVR_API_STATUS_ERROR_RETENTION_LIMIT_EXPIRED;
                        msAPI_PVR_PlaybackPathClose(_pstPvrPlaybackPath);
                    ...
                    {
                        PVR_DBG(printf("[TimeShift_PlaybackOpen] Is Not Expire, Can Play(%u:%u) \n", MApp_TimeShift_GetRecordEndTimeSec(), u32JumpTime));
                    }
                }
            }
            break;
...
static BOOLEAN MApp_TimeShift_PlaybackEnable(void)
{
    ...
    #else
    {
      ...
      //msAPI_PVR_PlaybackPathAddPID(_pstPvrPlaybackPath, PID_TDT_TOT,                     MSAPI_DMX_FILTER_TYPE_SECTION_1K);   //<--from other AP:(12) TDT/TOT
        ...
        {
            U32 u32TotalTime = MApp_TimeShift_GetRecordEndTimeSec();
            if (u32JumpTargetTime <= u32TotalTime)
            {
                msAPI_PVR_PlaybackPathJumpForward(_pstPvrPlaybackPath, u32JumpTargetTime*1000, E_JUMP_BUFFER_RESET);
            }
        }
#endif
...
U32 MApp_TimeShift_GetPlaybackTimeSec(void)
{
    ...
    {
        if ( (0 != u32OriginPlayTime) && (u32PlayTime > u32TotalTime) )
        {
    ...
    {
        u32OriginPlayTime = msAPI_PVR_GetFileEndTimeStamp();
    }
...
void MApp_TimeShift_StateMachineRunning(void)
{
    ...
    switch (_enTimeShiftState)
    {
        ...
        case E_TIMESHIFT_STATE_PLAYBACKING:
        {
            ...
            {
                if(_enTimeShiftState == E_TIMESHIFT_STATE_RECORDING || _enTimeShiftState == E_TIMESHIFT_STATE_RECORDINGnPLAYBACKING)
                {
...
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
BOOLEAN MApp_TimeShift_RetentionLimitMonitor(U32 u32PlaybackTime, U32* u32JumpTime)
{
    BOOLEAN ret = FALSE;
    U32 u32JumpTargetTime = 0;
    ret = MAPP_CIPVR_RetentionLimitMonitor(u32PlaybackTime, &u32JumpTargetTime);
