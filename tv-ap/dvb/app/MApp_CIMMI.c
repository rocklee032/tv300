...
#define DEBUG_CIMMI(x)      //x
#define DEBUG_CIMMI_OP(x)   //x
...
static BOOL MApp_CI_UpdateCIOpInfo( CI_OP_INFO* stCiOpInfo, U8 u8ProfileType );
...
BOOL MApp_CI_CiTuneSetMuxDVBC(const DESC_DEL_SYS* ustDesciptor);
...
#endif
#if (ENABLE_S2)
BOOL MApp_CI_CiTuneSetMuxDVBS(const DESC_DEL_SYS *ustDesciptor);
BOOL MApp_CI_CiTuneSetMuxDVBS2(const DESC_DEL_SYS *ustDesciptor);
#endif
...
static BOOL g_bIsOPNitManagementSend = FALSE;
...
// mantis 1543832, message can't display normal by MMI close command
#if ENABLE_CI_PLUS && MEDIASET_PREMIUM_SMART_CAM
            if(msAPI_CI_GetCardState() == EN_MODULE_READY)
            {
                enCIEvtState = STATE_CIMMI_WAIT;
...
BOOLEAN MMI_ParseInformaton(CIMMI_INFO * pInfo) //[out] pInfo for current displaying infomation.
{
    ...
    switch(pMMI->enType)
    {
        ...
        case EN_MMI_TYPE_MENU:
            ...
            if(bNULLStringFlag == TRUE)
            {
                bNULLStringFlag = FALSE;
                pInfo->content.menu_data.ListCnt--;
            }
            break;
...
void MMI_MessageHandle(void)
{
    ...
    #endif
        //avoid CI Message freshen active OSD
        !((MApp_ZUI_GetActiveOSD() == E_OSD_SCREEN_SAVER) ||(MApp_ZUI_GetActiveOSD() == E_OSD_EMPTY) ||(MApp_ZUI_GetActiveOSD() == E_OSD_CIMMI))
        || (STATE_TOP_DTV_SCAN == MApp_TopStateMachine_GetTopState()) ||(STATE_TOP_ATV_SCAN == MApp_TopStateMachine_GetTopState())
       )
    {
        return;
    }
...
#endif
void MApp_CI_Deteck_cb(void)
{
    switch (msAPI_CI_GetCardState())
    {
        case EN_NO_MODULE://Card Remove
        {
            msAPI_Demod_SetupTSPath(E_DEMOD_TSP_PATH_CASE_CI_BY_PASS);
            break;
        }
...
static BOOL MApp_CI_EnterCIOpServiceMode(U8 u8Index)
{
    ...
    if (MApp_CI_IsOpMode())
    {
        ...
        {
#if ENABLE_CI_PLUS_V1_4
            // [Fix OPMode Power Off/on] Not Op Session, Need ReSend OPNitManagement
            if (FALSE == g_bIsOPNitManagementSend)
            {
                MApp_CI_OP_SendOPNitManagement();
            }
#endif
            printf("[%s:%d] Exit \r\n", __PRETTY_FUNCTION__, __LINE__);
            return FALSE;
...
#if ENABLE_CI_PLUS_V1_4
        if (EN_OP_TYPE_1 == stOpCacheInfo.u8ProfileType)
        {
            PRINT_CURRENT_LINE();
            MApp_ChannelChange_DisableChannel(TRUE, MAIN_WINDOW);
            msAPI_CM_SwitchCMDB(TRUE);
        }
        else if (EN_OP_TYPE_2 == stOpCacheInfo.u8ProfileType)
        {
...
#if ENABLE_CI_PLUS_V1_4
    if(EN_OP_TYPE_2 == stOpCacheInfo.u8ProfileType)
    {
        WORD wOldPosition = msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType());
        if(wOldPosition != wNewPosition)
        {
            MApp_ChannelChange_DisableChannel(TRUE, MAIN_WINDOW);
        }
    }
...
static BOOL MApp_CI_ExitCIOpServiceMode(void)
{
    ...
    DEBUG_CIMMI_OP( printf("[%s:%d] Enter \r\n", __PRETTY_FUNCTION__, __LINE__); );
    if (MApp_CI_IsOpMode())
    {
        DEBUG_CIMMI_OP( printf("[%s:%d] Doing \r\n", __PRETTY_FUNCTION__, __LINE__); );
        MApp_ChannelChange_DisableChannel(TRUE,MAIN_WINDOW);
        ...
        if( TRUE == msAPI_CI_CardDetect() )
        {
            ...
            if( ( TRUE == msAPI_CI_OP_GetStatus( &stCiOpStatus ) ) && ( TRUE == msAPI_CI_OP_GetInfo( &stCiOpInfo ) ) )
            {
                ...
                */
                ...
                {
                    // Send operator_exit()
                    DEBUG_CIMMI_OP( printf("[CI+][OP] Send operator_exit()!\r\n"); );
                    msAPI_CI_OP_SendExit();
                }
                else
                {
                    DEBUG_CIMMI_OP( printf("[CI+][OP] The OP(%02X) doesn't belong to the CI+ CAM: 0x%08lX | 0x%08lX!\n", stCiOpStatus.u8InitializedFlag,
                                          MApp_CI_GetCurrentCicamId(), stCiOpInfo.u32CicamIdentifier); );
                }
            }
        }
...
#if ENABLE_CI_PLUS_V1_4
        g_bIsOPNitManagementSend = FALSE;
#endif
    ...
    DEBUG_CIMMI_OP( printf("[%s:%d] Exit \r\n", __PRETTY_FUNCTION__, __LINE__); );
...
BOOL MApp_CI_CiTuneSetMuxDVBC(const DESC_DEL_SYS* ustDesciptor)
{
    ...
    switch (ustDesciptor->stDCDS.u8Modulation)
    {
        case 1:
...
BOOL MApp_CI_CiTuneSetMuxDVBS(const DESC_DEL_SYS *ustDesciptor)
{
    ...
    if (0 == ustDesciptor->stDSDS.u8Modulation_system)
    {
        printf("[CI+][%s] Trying to tune to DVB-S(%lu KHz)...\n", __FUNCTION__, ((ustDesciptor->u32CentreFreq*10)/1000));
    }
    else if (1 == ustDesciptor->stDSDS.u8Modulation_system)
    {
    ...
    g_stCiTuneInfo.stTuningContent.stCiTuneMux.u32SymbRate = ustDesciptor->u32Symbol_rate/10;
    g_stCiTuneInfo.stTuningContent.stCiTuneMux.bPolarity = ustDesciptor->stDSDS.u8Polarization;
    g_stCiTuneInfo.stTuningContent.stCiTuneMux.u8ModulationMode = (ustDesciptor->stDSDS.u8Modulation_type&0x03);
    ...
    if( TRUE == msAPI_CI_OP_GetInfo( &stCiOpInfo ) )
    {
        ...
        if( TRUE == msAPI_CI_OP_GetStatus( &stCiOpStatus ) )
        {
                 ...
                 && (TRUE == stCiOpInfo.u8InfoValid) )
            {
                ...
                MApp_CI_UpdateCIOpInfo(&stCiOpInfo, stCiOpStatus.u8ProfileType);
            }
        }
        else
        {
...
#if ENABLE_CI_PLUS_V1_4
        MApp_CI_EnterCiOperatorProfileTyep2(stCiOpInfo.u32CicamIdentifier);
#endif
    }
    else
    {
...
static BOOL MApp_CI_UpdateCIOpInfo(CI_OP_INFO* stCiOpInfo, U8 u8ProfileType)
{
    return msAPI_OP_UpdateCIOpInfo(stCiOpInfo, u8ProfileType);
}
static BOOL MApp_CI_GetOpCicamIdByIndex(U8 u8Index , U32* u32CicamId)
{
    MW_OPCACHE_INFO stOpCacheInfo;
...
static BOOL MApp_CI_UpdateCICamNIT(U32 u32CicamId, U8 *pu8NitSection, U16 u16SecLen, U16 u16SecCount)
{
    MW_OPCACHE_INFO stOpCachedInfo;

    if ( FALSE == msAPI_OP_GetOpCacheInfoByIndex(DB_CI_PLUS_SETTING.u8OpIndex, &stOpCachedInfo) )
    {
        printf("[%d][CI+][OP] GetOpCacheInfoByIndex fail \n", __LINE__);
        return FALSE;
    }

    if (u32CicamId == stOpCachedInfo.stCiOpInfo.u32CicamIdentifier)
    {
...
void MApp_CI_TuneServicePlayAV( U16 u16VideoPID, U16 u16VideoType, U16 u16AudioPID, U16 u16AudioType, U16 u16PCRPID)
{
    ...
    MApp_ChannelChange_Set_Audio_Decoder_System((AUDIOSTREAM_TYPE)u16AudioType, FALSE);
...
static void MApp_CI_EventCommandCcUri(void)
{
    ...
    msAPI_CI_CC_UriMappingConvert(bScramble, bRecording, FALSE, pstUriLicense,
                                  &u8URI_DOT, &u8URI_RCT, &u8URI_EMI,
                                  &u8URI_APS, &u8URI_ICT, &u8URI_RL);
...
U8 MApp_CI_GetCurrentOpIndexByCicamId(U32 u32CicamId)
{
    return msAPI_OP_GetOpCacheIndexByCicamId(u32CicamId);
}

void MApp_CI_EnterCiOperatorProfileTyep2(U32 u32CicamIdentifier)
{
    U8 u8Index = MApp_CI_GetCurrentOpIndexByCicamId(u32CicamIdentifier);

    if (INVALID_OPCACHE_INDEX != u8Index)
    {
        MW_OPCACHE_INFO stOpCachedInfo;
        memset(&stOpCachedInfo, 0, sizeof(MW_OPCACHE_INFO));
        msAPI_OP_GetOpCacheInfoByIndex(u8Index, &stOpCachedInfo);

        if (EN_OP_TYPE_2 == stOpCachedInfo.u8ProfileType)
        {
            EN_DELIVERY_SYS_TYPE eProfileDtvSysType = E_DELIVERY_SYS_NONE;
            MApp_CI_GetOpDtvSysTypeByIndex(u8Index, &eProfileDtvSysType);
            if ( ((E_DELIVERY_SYS_CDSD == eProfileDtvSysType) && (EN_DVB_C_TYPE != MApp_DVBType_GetCurrentType()))
                 || ((E_DELIVERY_SYS_TDSD == eProfileDtvSysType) && (EN_DVB_T_TYPE != MApp_DVBType_GetCurrentType())) )
            {
                  printf("[CI+][OP] DTV System Type Don't Match, OP=%u, System=%u \n", eProfileDtvSysType, MApp_DVBType_GetCurrentType());
                  return;
            }
            else
            {
                if (E_DELIVERY_SYS_NONE != eProfileDtvSysType)
                {
                    DB_CI_PLUS_SETTING.u8OpIndex = u8Index;
                    DB_CI_PLUS_SETTING.u8Opv2InstallSource = (U8)UI_INPUT_SOURCE_TYPE;
                    DB_CI_PLUS_SETTING.bInOpMenu = TRUE;
                    if (FALSE == MApp_CI_IsOpMode())
                    {
                        printf("[CI+][OP] Auto Enter OP Profile[%u] \n", u8Index);
                        MApp_CI_EnterCiOperatorProfile(u8Index);
                    }
                }
            }
        }
...
EN_OP_TYPE MApp_CI_GetOperatorProfileType(void)
{
    CI_OP_STATUS stCiOpStatus;
    MW_OPCACHE_INFO stOpCachedInfo;
    EN_OP_TYPE eProfileType = EN_OP_TYPE_RESERVED;

    memset(&stCiOpStatus, 0, sizeof(CI_OP_STATUS));
    memset(&stOpCachedInfo, 0, sizeof(MW_OPCACHE_INFO));

    if (TRUE == msAPI_CI_OP_GetStatus(&stCiOpStatus))
    {
        eProfileType = (EN_OP_TYPE)(stCiOpStatus.u8ProfileType);
        //printf("[CI+][OP] Get From CI: eProfileType=%u \n", eProfileType);
    }
    else
    {
        msAPI_OP_GetOpCacheInfoByIndex(DB_CI_PLUS_SETTING.u8OpIndex, &stOpCachedInfo);
        eProfileType = (EN_OP_TYPE)(stOpCachedInfo.u8ProfileType);
        //printf("[CI+][OP] Get From Cache: eProfileType=%u \n", eProfileType);
    }

    return eProfileType;
}

void MApp_CI_OP_SendOPNitManagement(void)
{
         ...
         || (EN_OSDT_SCHEME_TYPE_HTTP == MApp_SI_GetOSDTSchemeType()) )
    {
        ...
        g_bIsOPNitManagementSend = msAPI_CI_OP_SendOPNitManagement(EN_OP_NIT_ERROR_FIELD_OSDT_ERROR_NOT_SUPPORTED, 0, NULL);
    }

    memset(stOpLCNConflict, 0, sizeof(stOpLCNConflict));
    U8 u8NumofConflictService = MApp_CI_GetNumOfConflictService();
    if (0 < u8NumofConflictService)
    {
        printf("[CI+][OP] sendOPNitManagement with EN_OP_NIT_ERROR_FIELD_NO_ISSUE (LCN COLLISION) \r\n");
        g_bIsOPNitManagementSend = msAPI_CI_OP_SendOPNitManagement(EN_OP_NIT_ERROR_FIELD_NO_ISSUE, u8NumofConflictService, stOpLCNConflict);
    }
}

void MApp_CI_CheckOpCacheInfo(void)
{
    msAPI_OP_CheckOpCacheInfo();
}

BOOLEAN MApp_CI_GetProfileTypeByIndex(U8 u8Index , U8* u8ProfileType)
{
    MW_OPCACHE_INFO stOpCacheInfo;
    MW_DTV_CI_OPCACHE_DB_FUNCTION("%s:%d\n",__PRETTY_FUNCTION__,__LINE__);
    ...
    if (TRUE == msAPI_OP_GetOpCacheInfoByIndex(u8Index, &stOpCacheInfo))
    {
        *u8ProfileType = stOpCacheInfo.u8ProfileType;
        return TRUE;
    }
