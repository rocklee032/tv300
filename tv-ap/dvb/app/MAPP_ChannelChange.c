...
#endif
#ifdef ENABLE_REPLACEMENT_CH_FROM_ORIGINAL
extern MEMBER_SERVICETYPE MApp_SDT_GetOrgServiceType(void);
extern U16 MApp_SDT_GetOrgPos(void);
extern void MApp_SDT_ResetOrgPosAndOrgServiceType(void);
#endif
#if ENABLE_CI_PLUS && MEDIASET_PREMIUM_SMART_CAM
extern BOOL g_bIsServiceVideoChange;
#endif
...
static BOOLEAN g_bChannelChangeIsBusy = 0;
...
typedef struct
{
    ...
    // Video ...
    U16 u16VideoPID;
    VDEC_CodecType eVDEC_CodecType;

    // Audio
    U16 u16AudioPID;
    AUDIOSTREAM_TYPE eAUDIOSTREAM_TYPE;
    // Audio-AD
    U16 u16AudioPID_AD;
  ...
  #if MHEG5_ENABLE
    U16 bIsPIDFromMHEG5 : 1;
  #endif
}StruChannelChangeInfo;

StruChannelChangeInfo g_ChChg_stChChangeInfo;

//--------------------------------------------------------------------------------

BOOLEAN MApp_ChannelChange_EnbalePasreNITOther( void );
    ...
    switch ( u8KeyCode )
    {
        case KEY_CHANNEL_PLUS:
                ...
                )
            {
                ...
                {
                #ifdef ENABLE_REPLACEMENT_CH_FROM_ORIGINAL
                    if((MApp_SDT_GetOrgServiceType() == E_SERVICETYPE_INVALID) || (MApp_SDT_GetOrgPos() == INVALID_PROGRAM_POSITION))
                    {
                    ...
                    {
                        bNewServiceType = bServiceType = MApp_SDT_GetOrgServiceType();
                        wNextProgramPosition = msAPI_CM_GetNextProgramPosition(bServiceType, MApp_SDT_GetOrgPos(), FALSE, E_ALL_LIST,&bNewServiceType);
                    }
                #else
                    bNewServiceType = bServiceType = msAPI_CM_GetCurrentServiceType();
                    wNextProgramPosition = msAPI_CM_GetNextProgramPosition(bServiceType, msAPI_CM_GetCurrentPosition(bServiceType), FALSE, E_ALL_LIST,&bNewServiceType);
                ...
                {
                #ifdef ENABLE_REPLACEMENT_CH_FROM_ORIGINAL
                    MApp_SDT_ResetOrgPosAndOrgServiceType();
                #endif
              ...
              )
            {
                ...
                WORD wPrevProgramPosition;
...
#endif
                  {
                    ...
                    {
                        bNewServiceType = bServiceType = MApp_SDT_GetOrgServiceType();
                        wPrevProgramPosition = msAPI_CM_GetPrevProgramPosition(bServiceType, MApp_SDT_GetOrgPos(), FALSE, E_ALL_LIST,&bNewServiceType);
                    }
                #else
                      bNewServiceType = bServiceType = msAPI_CM_GetCurrentServiceType();
                      wPrevProgramPosition = msAPI_CM_GetPrevProgramPosition(bServiceType, msAPI_CM_GetCurrentPosition(bServiceType), FALSE, E_ALL_LIST,&bNewServiceType);
...
#if( ENABLE_ATSC && (ATSC_CC != NO_CC) )// fix mantis 0905460
...
void MApp_ChannelChange_Set_Audio_Decoder_System(/*WORD wAudType*/AUDIOSTREAM_TYPE eAUDIOSTREAM_TYPE, BOOL bForceAD )
{
    /* set decoder system and reload firmware code */
    //msAPI_Timer_Delayms(500);

    DEBUG_AU_LOAD_CODE( printf("Set_Audio_Decoder_System(eAUDIOSTREAM_TYPE=%u)\n", eAUDIOSTREAM_TYPE); );

    BOOL bIsFirstAudSys = TRUE;
    ...
    if( bForceAD )
    {
        switch(eAUDIOSTREAM_TYPE)
        {
            case E_AUDIOSTREAM_MPEG:
                eAUDIOSTREAM_TYPE = E_AUDIOSTREAM_MPEG_AD;
                break;

            case E_AUDIOSTREAM_AC3:
                eAUDIOSTREAM_TYPE = E_AUDIOSTREAM_AC3_AD;
                break;

            case E_AUDIOSTREAM_AC3P:
                eAUDIOSTREAM_TYPE = E_AUDIOSTREAM_AC3P_AD;
                break;

            case E_AUDIOSTREAM_MPEG4:
            case E_AUDIOSTREAM_AAC:
                eAUDIOSTREAM_TYPE = E_AUDIOSTREAM_AACP_AD;
                break;

            default:
                printf("\nWarning: Unknown audio AD = %u\n", eAUDIOSTREAM_TYPE);
                return;
                break;
        }
    ...
    if( bIsFirstAudSys )
    {
        if( eAUDIOSTREAM_TYPE == E_AUDIOSTREAM_AC3
         || eAUDIOSTREAM_TYPE == E_AUDIOSTREAM_AC3P
         || eAUDIOSTREAM_TYPE == E_AUDIOSTREAM_AAC
         || eAUDIOSTREAM_TYPE == E_AUDIOSTREAM_MPEG4
          )
        {
...
/*********************************************************************************/
void MApp_ChannelChange_DisableChannel (BOOLEAN u8IfStopDsmcc, SCALER_WIN eWindow ) //for change channel
{
    DEBUG_ENABLE_CHANNEL(printf("MApp_ChannelChange_DisableChannel(win=%u)\n", eWindow););

    BOOL bCloseMHEG5 = TRUE;

  #if (!MHEG5_ENABLE)
    UNUSED(u8IfStopDsmcc);
  #endif
...
// For Nordig 9.45
#if ENABLE_DVB // NTV_FUNCTION_ENABLE
            ...
            for(j = 0; j < MAX_AUD_ISOLANG_NUM; j++)
            {
                ...
                if(aAudioStreamInfo.aISOLangInfo[j].bAudType == 0x00)
                {
                    if(u8AudLangCodeIdxL == MApp_GetSILanguage((EN_LANGUAGE)stGenSetting.g_SoundSetting.enSoundAudioLan1))
                    {
                        ...
                        if (msAPI_CM_GetAudioLangIndex(bServiceType,wCurrentPosition) == 0xFF)
                        {
                            msAPI_CM_SetAudioLangIndex(bServiceType,wCurrentPosition,g_u8AudLangSelected);
                        }
                        else
                        {
                            g_u8AudLangSelected = msAPI_CM_GetAudioLangIndex(bServiceType,wCurrentPosition);
                        }
                      #endif
                        return;
                    }
...
void MApp_Audio_SwitchtoADAudio(BOOL ADEnable)
{
    ...
    if ((ADEnable)&&(stGenSetting.g_SoundSetting.bEnableAD)&&( INVALID_PID != stAdAudioStreamInfo.wAudPID ))
    {
        ...
        /* set decoder system and reload firmware code */
        {
            ...
            MApp_ChannelChange_Set_Audio_Decoder_System( msAPI_CM_Get_AudStreamType_AD_By_AudInfo( &stAdAudioStreamInfo ), TRUE);
        }
        ...
        #if 1 // Use new API
            MApp_Audio_Set_LRMode( MApp_Audio_Get_LRMode() );
        #else
    ...
    else if ( INVALID_PID != stAudioStreamInfo.wAudPID )
    {
        ...
        /* set decoder system and reload firmware code */
        {
            ...
            MApp_ChannelChange_Set_Audio_Decoder_System( msAPI_CM_Get_AudStreamType_By_AudInfo( &stAudioStreamInfo ), FALSE);
        }
...
void MApp_ChannelChange_Reset_ScreenSaver(void)
{
...
{
    DEBUG_ENABLE_CHANNEL( PRINT_CURRENT_LINE(); );
    DEBUG_ENABLE_CHANNEL( printf("MApp_ChannelChange_SetVideoAndAudioPid(bServiceType=%u, wPosition=%u)\n", bServiceType, wPosition); );

#if(DEBUG_CHG_SRC_TIME)
    msDebug_PrintChgSrcTime("ChChange_Special - Start");
#endif
    ...
    //init enCurScreenSaver[] to EN_SCREENSAVER_NULL when channel change
    MApp_ChannelChange_Reset_ScreenSaver();
    ...
    if( MApp_ChannelChange_EnbalePasreNITOther())
    {
...
#if 0//(ENABLE_CHCHANGETIME)
    printf("[ch change time]EnableChannel end = %ld\n", msAPI_Timer_DiffTimeFromNow(gU32ChChangeTime));
#endif

#if(DEBUG_CHG_SRC_TIME)
    msDebug_PrintChgSrcTime("ChChange_Special - End");
#endif

}
#endif

void MApp_ChChg_EnCh_Reset_CC_VChip(void)
{
#if(ENABLE_ATSC) // atsc
    if (IsAtscInUse() || IsATVInUse())
    {
...
#endif
    {
    #if ((BRAZIL_CC)||(ATSC_CC == ATV_CC)) // Move enable_channel
     #if BRAZIL_CC
        if(IS_SBTVD_APP)
     #endif
        {
...
void MApp_ChChg_EnCh_Reset_TTX(void)
{
#if(ENABLE_TTX&&ENABLE_DVB)
    MApp_TTX_Reset_TTX_Info();
#endif
}

void MApp_ChChg_EnCh_CheckSwitchDtvSrc(SCALER_WIN eWindow)
{
    // Should refine...
...
#if(ENABLE_DVB)
VDEC_CodecType MApp_ChChg_VIDEO_TYPE_to_VDEC_CodecType(VIDEO_TYPE eVideoType)
{
    VDEC_CodecType eVDEC_CodecType = E_VDEC_CODEC_TYPE_MPEG2;

    if(eVideoType == E_VIDEOTYPE_MPEG)
    {
        eVDEC_CodecType = E_VDEC_CODEC_TYPE_MPEG2;
    }
    else if(eVideoType == E_VIDEOTYPE_H264)
    {
        eVDEC_CodecType = E_VDEC_CODEC_TYPE_H264;
    }
    ...
    return eVDEC_CodecType;
}

void MApp_ChChg_EnCh_Setup_VDEC(VDEC_CodecType eVDEC_CodecType)
{
  #if MHEG5_ENABLE
    g_eCurrent_VideoType = eVDEC_CodecType;
  #endif
    ...
    // If audio pid is invalid, set VDEC freerun
    if( (INVALID_PID == g_ChChg_stChChangeInfo.u16AudioPID)
      ||(0           == g_ChChg_stChChangeInfo.u16AudioPID)
      )
    {
        // set mvd to freerun in video only program
        msAPI_VID_Command( MSAPI_VID_FREERUN );
...
void MApp_ChChg_EnCh_Setup_Video(void)
{
    //BOOLEAN bVDECInited = FALSE;

    if( INVALID_PID != g_ChChg_stChChangeInfo.u16VideoPID )
    {
        /* set video PID & start filter */
        //printf("Channel change, Video Pid 0x%x, u8VidFid %bu\n",wVideo_PID,u8VidFid);

        msAPI_DMX_StartFilter( g_ChChg_stChChangeInfo.u16VideoPID, MSAPI_DMX_FILTER_TYPE_VIDEO, MApp_Dmx_GetFid(EN_VIDEO_FID)  );
        //DMX_FILTER_STATUS eDMXFLTSTA = msAPI_DMX_StartFilter( g_ChChg_stChChangeInfo.u16VideoPID, MSAPI_DMX_FILTER_TYPE_VIDEO, MApp_Dmx_GetFid(EN_VIDEO_FID)  );
        //printf("a Channel change, eDMXFLTSTA %bu,Video Pid 0x%x, u8VidFid %bu\n", eDMXFLTSTA, msAPI_DMX_GetFilterPID(u8VidFid),u8VidFid);
    ...
    #endif
        {
            ...
            {
                /* start video decoder */
                MApp_ChChg_EnCh_Setup_VDEC(g_ChChg_stChChangeInfo.eVDEC_CodecType);

                //bVDECInited = TRUE;
            }
        }

        fEnableMvdTimingMonitor = TRUE; // Enable DTV timing monitor

    }
    else
    {
        // No video PID!
         fEnableMvdTimingMonitor = FALSE; // Disable DTV timing monitor
    }

}
#endif

void MApp_ChChg_Set_Audio_SoundMode(U8 u8LRAudioMode)
{
    if (u8LRAudioMode == 0)
    {
        MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_SoundMode, MSAPI_AUD_MODE_STEREO, 0);
    }
    else if (u8LRAudioMode == 1)
    {
        MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_SoundMode, MSAPI_AUD_MODE_RR, 0);
        MApi_AUDIO_SetMpegInfo(Audio_MPEG_infoType_SoundMode, MSAPI_AUD_MPEG_SOUNDMODE_RR, 0);
    }
    else if (u8LRAudioMode == 2)
    {
        MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_SoundMode, MSAPI_AUD_MODE_LL, 0);
        MApi_AUDIO_SetMpegInfo(Audio_MPEG_infoType_SoundMode, MSAPI_AUD_MPEG_SOUNDMODE_LL, 0);
    }
...
void MApp_ChChg_Set_Audio_Spdif(void)
{
    if( stGenSetting.g_SysSetting.fSPDIFMODE == 0 )
    {
        MApi_AUDIO_SPDIF_SetMode(MSAPI_AUD_SPDIF_PCM);
...
#define IS_AUDIO_PID_VALID( auPID  )    ( ( (auPID) != 0) && ( (auPID) != INVALID_PID) )

static void MApp_ChChg_EnCh_Setup_Audio(SCALER_WIN eWindow, BOOLEAN bIsAtv)
{
    UNUSED(eWindow);
    ...
    MEMBER_SERVICETYPE eServiceType = g_ChChg_stChChangeInfo.eServiceType;
    U16 u16AudioPID = g_ChChg_stChChangeInfo.u16AudioPID;
    AUDIOSTREAM_TYPE eAUDIOSTREAM_TYPE = g_ChChg_stChChangeInfo.eAUDIOSTREAM_TYPE;
    BOOLEAN bIsAudioPIDValid = IS_AUDIO_PID_VALID(u16AudioPID);

    U16 u16AudioPID_AD = g_ChChg_stChChangeInfo.u16AudioPID_AD;
    ...
    if ( g_PIDfromMHEG5 )
    {
      ...
      #endif
        {
            MApp_ChannelChange_Set_Audio_Decoder_System(eAUDIOSTREAM_TYPE, FALSE);
    ...
    #endif
        {
            ...
            g_u16Current_AudioPID = u16AudioPID;
            g_u16Current_AudioType = eAUDIOSTREAM_TYPE;
        }
    }
    else
#endif // #if MHEG5_ENABLE
      ...
      #endif
        {
          ...
          #endif
            {
                ...
                if( bIsAudioPIDValid )
                {
                    ...
                    //eDMXFLTSTA = MApp_Demux_Start_Filter_Audio( stAudioStreamInfo.wAudPID );
                    MApp_Demux_Start_Filter_Audio( u16AudioPID );
                  ...
                  #endif
                    {
                        ...
                        // Reset DMX and decode start
                        MApi_DMX_AVFifo_Reset(DMX_FILTER_TYPE_AUDIO, TRUE);
                        MApi_AUDIO_SetCommand(MSAPI_AUD_DVB_DECCMD_PLAY);
                        MApi_DMX_AVFifo_Reset(DMX_FILTER_TYPE_AUDIO, FALSE);

                        MApp_ChChg_Set_Audio_SoundMode(g_u8LRAudioMode);
                        ...
                        switch( eAUDIOSTREAM_TYPE )
                        {
                            ...
                            default:
                                printf("\nWarning: eAUDIOSTREAM_TYPE=%u is invalid!\n", eAUDIOSTREAM_TYPE);
                                break;
                        }

                        MApp_ChChg_Set_Audio_Spdif();
                    }
                    // solved scramble->ATV no audio bug.
                }
...
#if( ENABLE_DVB)
void MApp_ChChg_EnCh_Setup_SI_SDT(void)
{
    // Re-Init SDT...
    MApp_SDT_ResetRunningStatus();
    ...
    if( IsS2InUse() )
    {
        if(msAPI_CM_GetScanType(g_ChChg_stChChangeInfo.eServiceType, g_ChChg_stChChangeInfo.u16CurPosition))
        {
...
#if(ENABLE_CI)
void MApp_ChChg_Setup_CI(void)
{
#if ENABLE_CI_PLUS

    msAPI_CI_NotifyCurrentServiceInfo(g_ChChg_stChChangeInfo.u16TransportStream_ID, g_ChChg_stChChangeInfo.u16Service_ID);

  #if ENABLE_CI_PLUS_V1_4
    DTV_CHANNEL_INFO ChannelInfo;

    if( msAPI_CM_GetProgramInfo( g_ChChg_stChChangeInfo.eServiceType, g_ChChg_stChChangeInfo.u16CurPosition, &ChannelInfo) == TRUE)
    {
...
#if(MHEG5_ENABLE)
void MApp_ChChg_EnCh_Setup_MHEG5(void)
{
    if ( !g_PIDfromMHEG5 && !_bBackDataProgram)
    {
        g_bMHEG5Service = FALSE;
  ...
  #endif
    {
        g_u16Current_VideoPID = g_ChChg_stChChangeInfo.u16VideoPID;
        g_eCurrent_VideoType = g_eCodecType;
    }
...
void MApp_ChChg_Reset_UIFunction(void)
{
}
...
void MApp_ChannelChange_EnableChannel_2(SCALER_WIN eWindow)
{
    ...
    if( MApp_PVR_IsPlaybacking() )
    {
        printf("\nError: Can't enable channel in PVR playing!\n");
        // Stop PVR here??
        return;
    }
#endif
    ...
    // Check if already enable channel
    if( fEnableSignalMonitor == TRUE )
    {
        printf("Enable_channel: fEnableSignalMonitor already=1\n");
        g_ChChg_bInFunc_EnableChannel = FALSE;
    ...
    // Reset CI
#if (ENABLE_CI_PLUS && MEDIASET_PREMIUM_SMART_CAM)
    if ( IsDTVInUse() && !g_bIsServiceVideoChange)
    {
    ...
    // Clear old channel change info
    memset( &g_ChChg_stChChangeInfo, 0, sizeof(g_ChChg_stChChangeInfo));

    if( IsSrcTypeATV(eInputSrcType) )
    {
        g_ChChg_stChChangeInfo.bIsChAtv = TRUE;

     #if 0 // Should not switch source here!!
        ...
        CHANNELCHANGE_DBINFO(printf("Enable ATV channel\n"));

      #if (ENABLE_PIP)
        if( (MApp_Get_PIPMode() == EN_PIP_MODE_OFF) || UI_IS_AUDIO_SOURCE_IN(eWindow) )
      #endif
        {
            MApp_ChChg_EnCh_Setup_Audio(eWindow, TRUE);
        }
    }
#if ENABLE_DVB
    else
    {
        ...
        g_ChChg_stChChangeInfo.bIsChAtv = FALSE;
        ...
        {
            g_ChChg_stChChangeInfo.bNoProg = TRUE;
            g_ChChg_bInFunc_EnableChannel = FALSE;
            return;
        }
        ...
        g_ChChg_stChChangeInfo.eServiceType = bServiceType;
        g_ChChg_stChChangeInfo.u16CurPosition = wCurrentPosition;

        DEBUG_ENABLE_CHANNEL(printf("bServiceType =%u\n", bServiceType););
        DEBUG_ENABLE_CHANNEL(printf("wCurrentPosition =%u\n", wCurrentPosition););

        g_ChChg_stChChangeInfo.bIsProgDeleted = msAPI_CM_GetProgramAttribute(bServiceType, wCurrentPosition, E_ATTRIBUTE_IS_DELETED);

        if( g_ChChg_stChChangeInfo.bIsProgDeleted == TRUE )
        {
            g_ChChg_bInFunc_EnableChannel = FALSE;
            return;
        }
        ...
        g_ChChg_stChChangeInfo.u16Service_ID = msAPI_CM_GetService_ID(bServiceType, wCurrentPosition);
        g_ChChg_stChChangeInfo.u16TransportStream_ID = msAPI_CM_GetTS_ID(bServiceType, wCurrentPosition);

    #if (ENABLE_CI_PLUS)
        g_ChChg_stChChangeInfo.bIsCIOpMode = MApp_CI_IsOpMode();
    #endif

    #if MHEG5_ENABLE
        g_ChChg_stChChangeInfo.bIsPIDFromMHEG5 = g_PIDfromMHEG5;
    #endif
        ...
        VIDEO_TYPE eVideoType = msAPI_CM_GetProgramVideoType(bServiceType, wCurrentPosition);
        VDEC_CodecType eVDEC_CodecType = MApp_ChChg_VIDEO_TYPE_to_VDEC_CodecType(eVideoType);

        g_ChChg_stChChangeInfo.u16VideoPID = wVideo_PID;
        g_ChChg_stChChangeInfo.eVDEC_CodecType = eVDEC_CodecType;

        DEBUG_ENABLE_CHANNEL(printf("u16VideoPID=0x%X\n", wVideo_PID););
        //printf("wVideo_PID=%d\n",wVideo_PID);
        ...
        // Setup audio lang...
    #if (ENABLE_CI_PLUS)
        if( TRUE == g_ChChg_stChChangeInfo.bIsCIOpMode )
        {
            MApp_ChannelChange_SearchDefaultOPAudioLang();
        ...
        memset(&stAudioStreamInfo, 0, sizeof(stAudioStreamInfo));
        msAPI_CM_GetAudioStreamInfo(bServiceType, wCurrentPosition, &stAudioStreamInfo, g_u8AudLangSelected);

        memset(&stAdAudioStreamInfo, 0, sizeof(stAdAudioStreamInfo));
        msAPI_CM_GetAudioStreamInfo(bServiceType, wCurrentPosition, &stAdAudioStreamInfo, g_u8AdAudSelected);

        U16 u16AudioPID = INVALID_PID;
        BOOLEAN bIsAudioPIDValid = TRUE;
        AUDIOSTREAM_TYPE eAUDIOSTREAM_TYPE = E_AUDIOSTREAM_MPEG;

      #if MHEG5_ENABLE
        if( g_PIDfromMHEG5 )
        {
            u16AudioPID = (g_u16MHEG5_AudioPID&~0xE000);

            switch( (g_u16MHEG5_AudioPID&0xE000) )
            {
                case 0x8000:
                    eAUDIOSTREAM_TYPE=E_AUDIOSTREAM_AC3;
                    break;
                case 0x2000:
                    eAUDIOSTREAM_TYPE=E_AUDIOSTREAM_MPEG4;
                    break;
                ...
                case 0x6000:
                    ...
                    break;
                default:
                    eAUDIOSTREAM_TYPE=E_AUDIOSTREAM_MPEG;
                    break;
            }
        }
        else
      #endif
        {
            u16AudioPID = stAudioStreamInfo.wAudPID;
            eAUDIOSTREAM_TYPE = msAPI_CM_Get_AudStreamType_By_AudInfo( (&stAudioStreamInfo) );
        }

        // Check audio PID
        if( ( u16AudioPID == INVALID_PID ) || ( u16AudioPID == 0) )
            bIsAudioPIDValid = FALSE;

        g_ChChg_stChChangeInfo.u16AudioPID = u16AudioPID;
        g_ChChg_stChChangeInfo.eAUDIOSTREAM_TYPE = eAUDIOSTREAM_TYPE;
        g_ChChg_stChChangeInfo.u16AudioPID_AD = stAdAudioStreamInfo.wAudPID;

        DEBUG_ENABLE_CHANNEL(printf("u16AudioPID=0x%X\n", u16AudioPID););
        DEBUG_ENABLE_CHANNEL(printf("u16AudioPID_AD=0x%X\n", g_ChChg_stChChangeInfo.u16AudioPID_AD););


        // Setup MHEG5: Get info ...
    #if MHEG5_ENABLE
        ...
        // Setup video ...
        MApp_ChChg_EnCh_Setup_Video();


    #if( ENABLE_DVB)
        // Setup SI ...
        MApp_ChChg_EnCh_Setup_SI_SDT();
    #endif


        // If cur source is not DTV, Change to last DTV source
        //MApp_ChChg_EnCh_CheckSwitchDtvSrc(eWindow);


        // Setup PCR filter
        U16 wPCR_PID = msAPI_CM_GetPCR_PID(bServiceType,wCurrentPosition);
        //printf("wPCR_PID=%d\n",wPCR_PID);
        if ( INVALID_PID != wPCR_PID )
        {
            /* set pcr PID & start filter */
    ...
    #if(MHEG5_ENABLE)
        // Setup MHEG5
        MApp_ChChg_EnCh_Setup_MHEG5();
    #endif


        // Setup AV Sync ...

        BOOLEAN bIsProgStillPicture = msAPI_CM_GetProgramAttribute(bServiceType,wCurrentPosition, E_ATTRIBUTE_IS_STILL_PICTURE);

      #if (ENABLE_PIP)
        if( (MApp_Get_PIPMode() == EN_PIP_MODE_OFF) || UI_IS_AUDIO_SOURCE_IN(eWindow) )
      #endif
        {
            // If video valid and audio valid, enable AV Sync
            if( wVideo_PID != INVALID_PID && (bIsProgStillPicture == FALSE) )
            {
                //if( INVALID_PID != stAudioStreamInfo.wAudPID )
                if( bIsAudioPIDValid )
                {
        ...
        {
            // Setup audio ...
            MApp_ChChg_EnCh_Setup_Audio(eWindow, FALSE);
        }
        ...
        MApp_ParentalRating_EnableChannel();
        u32TVCheckScrambleTimer = msAPI_Timer_GetTime0();
...
//        WORD wAudPID;
        ...
        if ( g_PIDfromMHEG5 )
        {
            ...
            MApp_ChannelChange_Set_Audio_Decoder_System(wAudType, FALSE);
            MApi_AUDIO_SetCommand(MSAPI_AUD_DVB_DECCMD_PLAY);
        }
        else
    #endif
        {
        ...
        #endif
            {
                ...
                {
                    /* set decoder system and reload firmware code */
                    MApp_ChannelChange_Set_Audio_Decoder_System(eAudStreamType, FALSE);
...
#if 0//((BRAZIL_CC)||(ATSC_CC == ATV_CC)) // Move to enable_channel
#if BRAZIL_CC
    if(IS_SBTVD_APP)
#endif
    {
...
//static Bool bFontLoadF=1;
    ...
    switch ( enChannelChangeState )
    {
        case STATE_CHANNELCHANGE_INIT:
        {
            DEBUG_CH_CHG_STATE( printf( " STATE_CHANNELCHANGE_INIT\n" ) );
            // TV<-> Radio switch is enable when there is not searched Radio pr. And when show installation guide the banner can't display.
            enChannelChangeState = STATE_CHANNELCHANGE_DISPLAY;
            g_bChannelChangeIsBusy = TRUE;

        #if(ENABLE_DTV)
            g_u8LRAudioMode = E_AUDIO_LRMODE_STEREO; //Init for Audio language
        #endif
        }
        break;
        ...
        {
            DEBUG_CH_CHG_STATE( printf("STATE_CHANNELCHANGE_SHOWINFO:\n"));
        ...
        case STATE_CHANNELCHANGE_DISPLAY:
        {
            DEBUG_CH_CHG_STATE( printf( " STATE_CHANNELCHANGE_DISPLAY:\n" ) );
...
void MApp_ChannelPosition_Restore(void)
{
    if( IsDVBInUse() )
    {
        ...
        else if( (g_eCurrentUserServiceType != g_eCurrentRealServiceType)
              || (g_wCurrentTVUserServicePosition != g_wCurrentTVRealServicePosition)
       #if NORDIG_FUNC //for Nordig Spec v2.0
...
BOOLEAN MApp_ChannelChange_Is_ChannelChangeBusy(void)
{
    if( !IsDTVInUse() )
        return FALSE;

    return g_bChannelChangeIsBusy;
}
