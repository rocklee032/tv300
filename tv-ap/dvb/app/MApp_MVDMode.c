...
//    if(msAPI_VID_GetDispRdy())
    ...
    #if(DEBUG_CHG_SRC_TIME)
        if( g_Debug_u32Time_VDEC_CheckDispInfoRdy == 0 )
        {
            g_Debug_u32Time_VDEC_CheckDispInfoRdy = g_Debug_u32Time_ChangeSrc;
            msDebug_PrintChgSrcTime("VDEC DispInfoRdy");
        }
    #endif


        BOOLEAN bProgramIsVideo = MApp_Is_CurDtvProgVideo();

        BOOLEAN bVdecIsWithValidStream = FALSE;
        // Check VDEC valid stream
        VDEC_Result eVDEC_IsWithValidStream = MApi_VDEC_IsWithValidStream();

        if( E_VDEC_OK != eVDEC_IsWithValidStream )
        {
            //printf("eVDEC_IsWithValidStream=%u\n", eVDEC_IsWithValidStream );
            bVdecIsWithValidStream = FALSE;
        }
        else
        {
            bVdecIsWithValidStream = TRUE;

        #if(DEBUG_CHG_SRC_TIME)
            if( g_Debug_u32Time_VDEC_IsWithValidStream == 0 )
            {
                g_Debug_u32Time_VDEC_IsWithValidStream = g_Debug_u32Time_ChangeSrc;
                msDebug_PrintChgSrcTime("VDEC WithValidStream");
            }
        #endif
        }


        if( ( bProgramIsVideo && (!bVdecIsWithValidStream) )
...
static BOOL g_bBypasSeqChgCheck = FALSE;
#endif
//*************************************************************************
...
void MApp_AVCH264_BypasSeqChgCheck(BOOL bBypassCheck)
{
    g_bBypasSeqChgCheck = bBypassCheck;
}
...
void MApp_AVCH264_TimingMonitor( SCALER_WIN eWindow )
{
    ...
    if(TRUE == bH264TimingCheck || (E_VDEC_OK == MApi_VDEC_IsSeqChg() && !g_bBypasSeqChgCheck) )
    {
        ...
        if(E_VDEC_OK == MApi_VDEC_CheckDispInfoRdy())
        {
            ...
            // Check resolution...
            BOOLEAN bIsResolutionOK = TRUE;
            if( (NewVidStatus.u16HorSize == 0)
              ||(NewVidStatus.u16VerSize == 0)
              )
            {
                bIsResolutionOK = FALSE;
            }
            else
            {
                bIsResolutionOK = TRUE;
            }
            ...
            // Check if program is still picture
            BOOL bStillPic = FALSE;
            if( FALSE == MApp_Is_CurDtvProgVideo() )
                bStillPic = TRUE;


            // Check frame rate...
            BOOLEAN bIsFrameRateOK = TRUE;
            if( ( (!bStillPic)&& (NewVidStatus.u32FrameRate < 10000) ) //for still picture video, do not check fr<10fps
              ||(NewVidStatus.u32FrameRate > 200000)
              )
            {
                bIsFrameRateOK = FALSE;
            }
            else
            {
                bIsFrameRateOK = TRUE;
            ...
            if( bIsResolutionOK && bIsFrameRateOK  )
            {
            #if ENABLE_MVD_TWICEFRC
                //let MVOP do double FRC if 25P/30P signal, it will fix the subtitle of VE ouput shaking issue
                //because the VE do not has good FRC capability
...
#if(DEBUG_CHG_SRC_TIME)
    msDebug_PrintChgSrcTime("HVD_SetMode - Start" );
#endif
        ...
        if( (gstVidStatus.u16HorSize >= 1900)&&(gstVidStatus.u16VerSize >= 1060) )
        {
            ...
            else // Non-Interlace
            {
                if( gstVidStatus.u32FrameRate >= 48500 ) // >= 48.5 Hz
                {
                    ...
                    {
                        if( devPanel_WIDTH() > 1366 ) // 1366 panel don't need this patch: Test at Milan_128
                        {
                            bDTVUseFBL = TRUE;
                        }
                    }
