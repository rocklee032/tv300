...
static BOOL _bIsDiffFile = FALSE;
static U32  u32PrePlayTime = 0;
static enPlaySpeed _enTempSpeed = E_PLAY_SPEED_1X;
static BOOL _bUpdateUI = FALSE;
...
static BOOLEAN MApp_Playback_Open(U16 *pu16PlaybackFileName)
{
    ...
    if(_pstPvrPlaybackPath)
    {
...
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
        PVR_DBG(printf("[Playback Open] u8CicamCheckLevel=%u \n", _pstPvrPlaybackPath->pstPvrRecordPath->u8CicamCheckLevel););

        enPVRState PVRState = MApp_PVR_StateMachineGet();
        if (E_PVR_STATE_RECORD == PVRState)
        {
            MApp_Record_GetCurrentFileName(RECORD_PATH_DEFAULT, _pu16RecordFileName);
            if (FALSE == memcmp(_pu16RecordFileName, _pu16PlaybackFileName, RECORD_NAME_LENGTH))
            {
                _bIsDiffFile = FALSE;
                PVR_DBG(printf("[Playback Open] Record & Playback Is Same File \n"));
            }
            else
            {
                _bIsDiffFile = TRUE;
                PVR_DBG(printf("[Playback Open] Record & Playback Is Diff File \n"));
            }
            memset(_pu16RecordFileName, 0, sizeof(_pu16RecordFileName));
        }
        ...
        switch (_pstPvrPlaybackPath->pstPvrRecordPath->u8CicamCheckLevel)
        {
            ...
            {
                MAPP_CIPVR_CIPVRFS_PlaybackFileRead(_pstPvrPlaybackPath->pstPvrRecordPath->enEncryptionType);
                enPvrApiStatus enPvrStatus = MAPP_CIPVR_CIPlusPVRHandling(PVR_PATH_PLAYBACK, FALSE, 0xFFFF);
                if ((E_PVR_API_STATUS_OK != enPvrStatus))
                {
                    PVR_DBG(printf("[Playback Open] Open Error(%u) \n", enPvrStatus));
                ...
                {
                    PVR_DBG(printf("[Playback Open] Get CICAM ID Failed \n"));
                    _enPlaybackState = E_PLAYBACK_STATE_WAIT;
                    _enPlaybackSpeed = E_PLAY_SPEED_1X;
                ...
                {
                    PVR_DBG(printf("[Playback Open] CICAM ID Is Not Same \n"));
                    _enPlaybackState = E_PLAYBACK_STATE_WAIT;
                    _enPlaybackSpeed = E_PLAY_SPEED_1X;
                    _enPlaybackStatus = E_PVR_API_STATUS_ERROR_CIPLUS_COPY_PROTECTION;
                    msAPI_PVR_PlaybackPathClose(_pstPvrPlaybackPath);
                ...
                else
                {
                    ...
                    if ( (TRUE == bRetentionLimitExpire)
                         && (MApp_Playback_GetTotalTimeSec() <= u32JumpTime) )
                    {
                        PVR_DBG(printf("[Playback Open] Is Expire, Can't Play(%u:%u) \n", MApp_Playback_GetTotalTimeSec(), u32JumpTime));
                        _enPlaybackState = E_PLAYBACK_STATE_WAIT;
                        _enPlaybackSpeed = E_PLAY_SPEED_1X;
                    ...
                    {
                        PVR_DBG(printf("[Playback Open] Is Not Expire, Can Play(%u:%u) \n", MApp_Playback_GetTotalTimeSec(), u32JumpTime));
                    }
                }
            }
            break;
...
//    U8 u8PlaybackEnable = FALSE;
    ...
    if ((_enPlaybackStatus=msAPI_PVR_PlaybackPathSet(_pstPvrPlaybackPath)) == E_PVR_API_STATUS_OK)
    {
        ...
        if (MAPP_CIPVR_RetentionLimitMonitor(msAPI_PVR_GetFileStartTimeStamp(), &u32JumpTargetTime))
        {
            U32 u32TotalTime = MApp_Playback_GetTotalTimeSec();
            if (u32JumpTargetTime <= u32TotalTime)
            {
                ...
                msAPI_PVR_PlaybackPathJumpToTime(_pstPvrPlaybackPath, u32JumpTargetTime*1000);
                PVR_DBG(printf("[Playback Enable] Is Expire %us Jump Time \n", u32JumpTargetTime));
            }
        }
        else
        {
            PVR_DBG(printf("[Playback Enable] Is Not Expire \n"));
        }
...
static BOOLEAN  MApp_Playback_Routine(void)
{
    ...
    if(_enPlaybackState == E_PLAYBACK_STATE_FASTBACKWARD)
    {
       // U32 playTime = MApp_Playback_GetPlaybackTimeSec();
        ...
        if (E_PVR_FILE_CHANGE_BACKWARD == MApp_PVR_GetFileChangeStatus())
        {
            MApp_PVR_SetFileChangeStatus(E_PVR_FILE_CHANGE_NONE);
    ...
    else if(_enPlaybackState == E_PLAYBACK_STATE_FASTFORWARD)
    {
        ...
        if (E_PVR_FILE_CHANGE_FORWARD == MApp_PVR_GetFileChangeStatus())
        {
            MApp_PVR_SetFileChangeStatus(E_PVR_FILE_CHANGE_NONE);
            MApi_DMX_GetFileInTimeStamp(&u32TempTimeCounter);
        }
...
static BOOLEAN MApp_Playback_Resume(void)
{
  ...
  //printf("MApp_Playback_Resume \n");
    ...
    if(*MApp_Dmx_GetFid(EN_PES_FID) != MSAPI_DMX_INVALID_FLT_ID)
    {
        DEBUG_CC_DEMUX( PRINT_CURRENT_LINE(); );
        DEBUG_CC_DEMUX( printf("Close CC filter - start\n"); );
        ...
        DEBUG_CC_DEMUX( printf("*MApp_Dmx_GetFid(EN_PES_FID)=%u\n", (*MApp_Dmx_GetFid(EN_PES_FID)) ); );
        DEBUG_CC_DEMUX( msAPI_DMX_Print_AllFilter(); );
        DEBUG_CC_DEMUX( printf("Close CC filter - End\n"); );
    }
...
{
    U32 u32TotalTime;
    u32TotalTime = _pstPvrRecordPath->u32RecordedPeriod/10;
    return u32TotalTime;
}
...
U32 MApp_Playback_GetPlaybackTimeSec(void)
{
    ...
    u32TotalTime = MApp_Playback_GetTotalTimeSec();

    // avoid play time get error
    if ( (0 == msAPI_PVR_GetFileStartTimeStamp())
         && (E_PVR_FILE_CHANGE_NONE == MApp_PVR_GetFileChangeStatus()) )
    {
        if ( (0 != u32PrePlayTime) && (u32PlayTime > u32TotalTime) )
        {
    ...
    {
       u32PrePlayTime = msAPI_PVR_GetFileEndTimeStamp();
    }
...
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
BOOLEAN MApp_Playback_RetentionLimitMonitor(U32 u32PlaybackTime, U32* u32JumpTime)
{
    BOOLEAN bRet = FALSE;
    U32 u32JumpTargetTime = 0;

    bRet = MAPP_CIPVR_RetentionLimitMonitor(u32PlaybackTime, &u32JumpTargetTime);
    *u32JumpTime = u32JumpTargetTime;
    if (TRUE == bRet)
    {
        PVR_DBG(printf("[MApp_Playback_RetentionLimitMonitor] Need Jump To %u s \n", u32JumpTargetTime););
    }
    return bRet;
}
#endif

BOOLEAN MApp_Playback_IsDiffFile(void)
{
    return _bIsDiffFile;
}
...
void MApp_Playback_StateMachineRunning(void)
{
    ...
    switch (_enPlaybackState)
    {
        ...
        case E_PLAYBACK_STATE_FASTBACKWARD:
        {
            ...
            {
                PVR_DBG(printf("[RL] E_PLAYBACK_INPUT_PLAYBACK_JUMP_TO_TIME \n"));
                msAPI_PVR_SetRetentionLimitJumpBackward(TRUE);
                ...
                msAPI_PVR_SetRetentionLimitJumpBackward(FALSE);
                _bPlaybackStepIn = FALSE;
            }
            ...
            else if(_enPlaybackInput == E_PLAYBACK_INPUT_PLAYBACK_FAST_FORWARD)
            {
                ...
                if (E_PVR_FILE_CHANGE_FORWARD == MApp_PVR_GetFileChangeStatus())
                {
                    ...
                    MApp_Playback_SetUIUpdate(TRUE);
                }
                else
                {
                    if(_bPlaybackStepIn)
...
void MApp_Playback_SetUIUpdate(BOOLEAN bUpdate)
{
    _bUpdateUI = bUpdate;
}

BOOLEAN MApp_Playback_GetUIUpdate(void)
{
    return _bUpdateUI;
}
/***************************************************************************************/
