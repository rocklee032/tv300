...
// Debug message
#define EPG_TIMER_DEBUG     0

#if (EPG_TIMER_DEBUG)
    #define TIMER_DBG( s )      { {printf s;} }
    #define DEBUG_EPGTIMER(x)   x
#else
    #define TIMER_DBG( s )
    #define DEBUG_EPGTIMER(x)
#endif
...
#if ENABLE_CI_PLUS
extern BOOLEAN g_bOpModeSwitchSource;
extern E_UI_INPUT_SOURCE g_eOpModeSource;
 #endif
...
static MS_EPG_TIMER * _pstEpgTimer = NULL;
...
{
    //PRINT_CURRENT_LINE();
    DEBUG_DVB_EPG( printf("_MApp_EpgTimer_Initdata"); );

    _pstEpgTimer = (MS_EPG_TIMER*)GetScheduleListBufAddr();

    DEBUG_DVB_EPG( printf("_pstEpgTimer=0x%X\n", _pstEpgTimer); );

    DEBUG_DVB_EPG( printf("sizeof(ST_TIME)=%u\n", sizeof(ST_TIME) ); );
    DEBUG_DVB_EPG( printf("EPG_TIMER_MAX_EVT_NAME_LEN=%u\n", EPG_TIMER_MAX_EVT_NAME_LEN ); );

    DEBUG_DVB_EPG( printf("sizeof(MS_EPG_TIMER)=%u(0x%X)\n", sizeof(MS_EPG_TIMER), sizeof(MS_EPG_TIMER) ); );
    DEBUG_DVB_EPG( printf("EPG_TIMER_MAX_NUM=%u\n", EPG_TIMER_MAX_NUM ); );
    DEBUG_DVB_EPG( printf("sizeof(MS_EPG_TIMER)*EPG_TIMER_MAX_NUM=%u(0x%X)\n", sizeof(MS_EPG_TIMER)*EPG_TIMER_MAX_NUM, sizeof(MS_EPG_TIMER)*EPG_TIMER_MAX_NUM ); );

    return TRUE;
}

#define _MApp_EpgTimer_CloseXdataWindow(u32PrvAddress)
/*
static void _MApp_EpgTimer_CloseXdataWindow(U32 u32PrvAddress)
{
    UNUSED(u32PrvAddress);
}
*/
...
void MApp_EPG_PrintEventName(const U16* pu16EvtName)
{
    U8 i;
    U16 u16Tmp;
    ...
    printf("EvtName={");
    for( i = 0; i < EPG_TIMER_MAX_EVT_NAME_LEN; ++ i)
    {
        u16Tmp = pu16EvtName[i];

        printf("%02X", u16Tmp);
        if( u16Tmp >= 0x20 && u16Tmp <= 127 )
        {
            printf("(%c)", u16Tmp);
        }
        printf(",");
...
//            memcpy(stEpgTimerParam.stEpgTimer.au16EvtName, p_string_buffer, sizeof(stEpgTimerParam.stEpgTimer.au16EvtName));
            ...
            DEBUG_DVB_EPG( printf("Get Evt-%u: ", MApp_EpgUI_Func_Get_CurFocusEvtIdx() ); );
            DEBUG_DVB_EPG( MApp_EPG_PrintEventName(stEpgTimerParam.stEpgTimer.au16EvtName); );
...
static void MApp_EpgTimer_SetTimerAsNew( void )
{
    ...
    if (TRUE == MApp_CI_IsOpMode())
    {
        ...
        msAPI_CM_GetProgramInfo(E_SERVICETYPE_DTV, srvPos, &stChannelInfo);
        if (TRUE == stChannelInfo.stCHAttribute.bIsOpService)
        {
            stEpgTimerParam.stEpgTimer.bIsOpService = TRUE;
        }
        else
        {
            stEpgTimerParam.stEpgTimer.bIsOpService = FALSE;
        }
    }
#endif
...
*/
/*
BOOLEAN MApp_EpgTimer_Get_EpgTimer(U8 u8Idx, MS_EPG_TIMER* pstMS_EPG_TIMER)
{
    if( u8Idx >= EPG_TIMER_MAX_NUM )
    {
        return FALSE;
    }

    if( _pstEpgTimer == NULL )
        return FALSE;

    if( u8Idx >= MApp_EpgUI_GetNumOfTimers() )
    {
        return FALSE;
    }

    *pstMS_EPG_TIMER = _pstEpgTimer[u8Idx];

    return TRUE;
}
*/

BOOLEAN MApp_EpgTimer_Set_EpgTimer(U8 u8Idx, MS_EPG_TIMER* pstMS_EPG_TIMER)
{
    if( u8Idx >= EPG_TIMER_MAX_NUM )
    {
        return FALSE;
    ...
    _pstEpgTimer[u8Idx] = *pstMS_EPG_TIMER;

    return TRUE;
}

const MS_EPG_TIMER* MApp_EpgTimer_Get_EpgTimer_Pointer(U8 u8Idx)
{
    if( u8Idx >= EPG_TIMER_MAX_NUM )
    {
        return NULL;
    ...
    return &(_pstEpgTimer[u8Idx]);
}

EN_EPGUI_TIMER_TYPE MApp_EpgTimer_Get_TimerType(U8 u8Idx)
{
    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(u8Idx);

    if( pstMS_EPG_TIMER == NULL )
    {
        return EN_EPGUI_TIMER_NULL;
    }

    return (EN_EPGUI_TIMER_TYPE)(pstMS_EPG_TIMER->enTimerType);
}

EN_EPG_TIMER_REPEAT_MODE MApp_EpgTimer_Get_RepeatMode(U8 u8Idx)
{
    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(u8Idx);

    if( pstMS_EPG_TIMER == NULL )
    {
        return EN_EPGRM_MAXNUM;
    }

    return (EN_EPG_TIMER_REPEAT_MODE)(pstMS_EPG_TIMER->enRepeatMode);
}

E_UI_INPUT_SOURCE MApp_EpgTimer_Get_UiSrcType(U8 u8Idx )
{
    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(u8Idx);
    ...
    return (E_UI_INPUT_SOURCE)(pstMS_EPG_TIMER->u8UiSrcType);
}

ST_TIME MApp_EpgTimer_Get_StartDate(U8 u8Idx)
{
    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(u8Idx);
    ...
    {
        ST_TIME stST_TIME;
        memset( &stST_TIME, 0, sizeof(stST_TIME));
        return stST_TIME;
    }

    return (pstMS_EPG_TIMER->stStartDate);
}

ST_TIME MApp_EpgTimer_Get_EndDate(U8 u8Idx)
{
    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(u8Idx);
    ...
    return (pstMS_EPG_TIMER->stEndDate);
}

BOOLEAN MApp_EpgTimer_Get_PCN_ServiceID(U8 u8Idx, U16* pu16PCN, U16 * pu16ServiceID)
{
    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(u8Idx);
    ...
    *pu16PCN = pstMS_EPG_TIMER->u8PCN;
    *pu16ServiceID = pstMS_EPG_TIMER->u16SrvID;

    return TRUE;
}

BOOLEAN MApp_EpgTimer_Get_EventName(U8 u8Idx, U16* pu16EventNameBuf, U8 u8EventNameBufLen)
{
    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(u8Idx);
    ...
    // Copy event name to pu16EventNameBuf
    Str16_StrNCpy(pu16EventNameBuf, pstMS_EPG_TIMER->au16EvtName, u8EventNameBufLen);
...
static void _MApp_EpgTimer_DelPastTimeItems(U32 u32TimeActing )
{
    ...
    for(; i > 0; i--)
    {
        ...
        if((EN_EPGUI_TIMER_REMINDER == _pstEpgTimer[idxTimerItem].enTimerType) && (u32ItemStartDate > (u32TimeActing+EPG_REMINDER_TIME)))
        {
            continue;
        }
    #if ENABLE_CI_PLUS
...
void MApp_EpgTimer_InitTimerSettings( BOOLEAN bScanEnd )
{
    ...
    memset( (char*)MsOS_PA2KSEG1(SCHEDULE_LIST_ADR), 0x00, SCHEDULE_LIST_LEN);
...
void MApp_EpgTimer_ExecTimerUpAct(U8 u8TimeUpIndex)
{
    ...
    // Close old source ...
    if( IsDTVInUse() == FALSE ) // If current source is not DTV
    {
      #if ENABLE_DMP
    ...
    BOOLEAN bIsSourceChanged = FALSE;

    // If current source is not EpgTimer source, then SwitchSource
    ...
    //Channel change Begin...
    {
      ...
      #if ENABLE_CI_PLUS
        if( (g_eOpModeSource == MApp_InputSrc_Get_UiInputSrcType())
          &&(MApp_CI_IsOpMode() == FALSE)
          && g_bOpModeSwitchSource
          )
        {
            MApp_CI_EnterCiOperatorProfile(DB_CI_PLUS_SETTING.u8OpIndex);
            g_bOpModeSwitchSource = FALSE;
            g_eOpModeSource = UI_INPUT_SOURCE_NONE;
        }
       #endif

        MEMBER_SERVICETYPE srvType = E_SERVICETYPE_INVALID, CurSrvType = E_SERVICETYPE_INVALID;
        U16 srvPos = INVALID_PROGRAM_POSITION, CurSrvPos = INVALID_PROGRAM_POSITION;
        ...
        else if( IsDTVInUse() )
        {
            ...
            if( msAPI_CM_GetServiceTypeAndPositionWithEpgTimer( &epgTimerAct, &srvType, &srvPos) )
            {
                if( (srvType != E_SERVICETYPE_INVALID)
                  &&(srvPos != INVALID_PROGRAM_POSITION)
                  &&((CurSrvType != srvType) || (CurSrvPos != srvPos))
                  )
                {
                    // Set current channel as previous
                    dmSetLastWatchedOrdinal();
