...
// This value should defined in tuner driver
...
#if ENABLE_DVBC
    #if (ENABLE_ZIGGO || ENABLE_COMHEM || ENABLE_STOFA)
...
BOOLEAN MApp_Scan_Is_SaveByLCNTag(void)
{
...
#if ENABLE_TDT
    if(OSD_COUNTRY_SETTING == OSD_COUNTRY_SPAIN)
    {
        SI_TOT_MONITOR_ALL_INFO stAllTOTinfo;
        U16 u16timeoffset = 0;
        U8 u8HourTimeOffset = 0;
        U8 u8TimeZonetmp = 0;
        stAllTOTinfo = MApp_SI_GetTOTInfoAllData();
        if(SCAN_TYPE_MANUAL == g_enScanType)
        {
            if(stAllTOTinfo.aLTOTInfo.u16LTO != 0x0000)
            {
                u16timeoffset = BE2ME16(stAllTOTinfo.aLTOTInfo.u16LTO);
                u8HourTimeOffset = LOBYTE(u16timeoffset);
                u8TimeZonetmp = (U8)stGenSetting.g_Time.en_Clock_TimeZone + u8HourTimeOffset*2;
                if(u8TimeZonetmp != (U8)stGenSetting.g_Time.en_Clock_TimeZone)
                {
                    stGenSetting.g_Time.en_Clock_TimeZone = (EN_MENU_Clock_TimeZone)u8TimeZonetmp;
...
#if (ASTRA_LCN_ENABLE)
    if (msAPI_CM_GetServiceListIdCount() && (!bisSetChannelListId) && stGenSetting.g_SysSetting.bIsAstraSat)
    {
...
//#endif
    ...
    if (  stGenSetting.stScanMenuSetting.u8ScanType == SCAN_TYPE_AUTO
      ...
      #endif
      #if(ENABLE_BAT_SCAN)
       || stGenSetting.stScanMenuSetting.u8ScanType == S2_SCAN_TYPE_BAT
      #endif
      #endif
        )
    {
...
static BOOL MApp_Scan_IsEnableNitQuickScan(void)
{
    if(IsDVBCInUse())
    {
#if ENABLE_ZIGGO
...
static void _MApp_DTV_Scan_TableConfig(SCAN_TABLE_CONFIG *pstScanConfig)
{
...
#if (ENABLE_DVBC && ENABLE_NIT_QUICK_SCAN)
    if(MApp_Scan_IsEnableNitQuickScan() && ((g_enScanType == SCAN_TYPE_NETWORK))) //|| (g_enScanType == SCAN_TYPE_AUTO)))// && OSD_COUNTRY_SETTING == E_NETHERLANDS)
    {
    ...
    )// && OSD_COUNTRY_SETTING == E_NETHERLANDS)
    {
        ...
        _stScanConfig.bEnableNITQuickScan=TRUE;
        pstScanConfig->bNetworkIDFilterEn = TRUE;
        pstScanConfig->bNetworkID = msAPI_CM_GetDefaultNetworkIDByOSDCountry();
    }
    #endif
    #if(ENABLE_BAT_SCAN)
    if(IsS2InUse() && (g_enScanType == S2_SCAN_TYPE_BAT))
    {
        pstScanConfig->bNetworkID = msAPI_CM_GetDefaultNetworkIDByOSDCountry();
        pstScanConfig->bEnableBATParse = TRUE;
    }
    #endif
#endif
...
#if (ENABLE_NIT_QUICK_SCAN || ENABLE_S2_NIT_QUICK_SCAN)
    static U16 _u16TSID;
#endif
...
*/
        ...
        case STATE_SCAN_SEARCH_RF_CHANNEL:
            ...
            if( ScanResult == FE_LOCK )
            {
                  ...
                  &&(DVB_T2_DEMOD == MApp_DTV_Scan_T2_ScanCheckSignal()))
                {
                    ...
                    if((SCAN_TYPE_MANUAL == g_enScanType)&&(_TotalPlpIDNum==1))
                    {
                        extern MS_U8 u8PlpIDList[32];
                        MApp_SI_FreeT2DelSysDesc();
                        MApp_SI_AddT2DelSysDesc(u8PlpIDList[0],1);
                        stTPSetting.u8PLPID=u8PlpIDList[0];
                        msAPI_Tuner_SetCurrentTPSetting(u8PlpIDList[0]);
                    }
                    else
                    {
        ...
        case STATE_BLIND_START_SET_FIRST_SAT:
        {
            ...
            if(_GetSatInforByID( _lnb_satelliteID[_u8CurScanSATPos],&sat_info))
            {
                ...
                {
                    stGenSetting.g_SysSetting.bIsAstraSat = TRUE;
                }
              #endif
        ...
        case STATE_FIND_NIT_FREQ:
        {
            ...
            while (u16FreqCnt)
            {
                if(MApp_SI_GetNextSatParameter(&siCDSInfo,NULL,NULL,NULL, &u16FreqCnt, TRUE))
                {
                    ...
                    carrierParam.u8Polarity = siCDSInfo.stDSDS.u8Polarization;
        ...
        case STATE_SCAN_GET_PROGRAMS:   //Truman for digital only, analog scan shouldn't go into this state
        ...
        #if(ASTRA_HD_ENABLE)
                  if((!stGenSetting.g_SysSetting.bIsAstraSat) || (stGenSetting.g_SysSetting.bIsAstraSat && (!MApp_Get_IsNITQuickScan())))
                  {
            ...
            if(!stGenSetting.g_SysSetting.bIsAstraSat)
            {
                    if( MApp_Dmx_GetScanTables(&_stScanConfig, &u8NumOfVchFound ) == FALSE )
                    {
                        u16GetProgramsCnt++;
            ...
            if (IsS2InUse())
            {
                ...
                if( u8NumOfVchFound > 0 )
                {
                    ...
                    if(_stScanConfig.bEnableNITQuickScan)
                    {
                        ...
                        if(stGenSetting.g_SysSetting.bIsAstraSat)
                        {
                            enScanState=STATE_S2_SCAN_NIT_QUICK_SCAN_GET_NIT; // channel info from SGT
                        }
                        else
                        #endif
                        {
                            enScanState=STATE_SCAN_NIT_QUICK_SCAN_GET_NIT;  // channel info from SDT
                        }
                    }
                    else
                #endif //#if(ENABLE_S2_FAST_SCAN)
                    {
                        #if(ENABLE_BAT_SCAN)
                        if(S2_SCAN_TYPE_BAT == g_enScanType)
                        {
                            enScanState = STATE_SCAN_BAT_QUICK_SCAN;
                        }
                        else
                        #endif
        ...
        #if(ENABLE_BAT_SCAN)
            case STATE_SCAN_BAT_QUICK_SCAN:
            {
                printf("STATE_SCAN_BAT_QUICK_SCAN\n");

                U16 j,k;
                MS_BAT_SERVICE_LIST_INFO stBatInfo;
                DESC_DEL_SYS desc;

                // 1. get the cur sevices from sdt
                SI_SHORT_DTV_CHANNEL_INFO *pastVirtualCh=MApp_SI_Get_PastVirtualCh();
                msAPI_Tuner_CheckSignalStrength(&wSignalQualityStrength);
                wSignalQualityStrength = (wSignalQualityStrength&0x7F)<<8;
                wSignalQualityStrength |= (msAPI_Tuner_GetSignalQualityPercentage()&0xFF);

                // 2. get the all bat sevices with LCN
                while(MApp_SI_GetNextBatService(&stBatInfo, TRUE))
                {

                    //3. get the nit sevices by tsid
                    if(MApp_SI_GetSatParameterById(&desc,stBatInfo.wTransportStream_ID)== TRUE)
                    {
                        BOOLEAN bFull=FALSE;
                        SI_DTVPROGRAMID stDtvIDTable;
                        SI_DTV_CHANNEL_INFO stDtvPgmData;
                        ...
                        stDtvIDTable.wNetwork_ID = desc.u16Nid;
                        stDtvIDTable.wTransportStream_ID = desc.u16Tsid;
                        stDtvIDTable.wOriginalNetwork_ID = desc.u16Onid;
                        stDtvIDTable.u32S2Frequency = (desc.u32CentreFreq)/100;
                        stDtvIDTable.u16SymbolRate = (desc.u32Symbol_rate&0xfffffff)/10 ;
                        stDtvIDTable.u8Polarity = desc.stDSDS.u8Polarization;
                        stDtvIDTable.SatelliteId = _lnb_satelliteID[_u8CurScanSATPos];
                        printf("wTransportStream_ID %d u32Frequency %d u32SymbRate %d\n",stDtvIDTable.wTransportStream_ID,stDtvIDTable.u32Frequency,stDtvIDTable.u32SymbRate);

                        for(i=0;stBatInfo.stLcnInfo[i].u16ServiceID != 0;i++)
                        {
                            printf("u16ServiceID %d \n",stBatInfo.stLcnInfo[i].u16ServiceID);

                            msAPI_SI_FillProgramDataWithDefault(&stDtvPgmData);
                            stDtvPgmData.wService_ID = stBatInfo.stLcnInfo[i].u16ServiceID;
                            stDtvPgmData.wTS_LCN     = stBatInfo.stLcnInfo[i].u16LCNNumber;
                            stDtvPgmData.wLCN        = stBatInfo.stLcnInfo[i].u16LCNNumber;
                            stDtvPgmData.stCHAttribute.bVisibleServiceFlag=stBatInfo.stLcnInfo[i].u8fVisable;
                            stDtvPgmData.stCHAttribute.bServiceType = E_TYPE_DTV; // dtv is deflaut
                            stDtvPgmData.satid = _lnb_satelliteID[_u8CurScanSATPos];

                            //4.get service type from bat
                            for(j=0;stBatInfo.stServiceList[j].u16ProgramNumber !=0; j++)
                            {
                                if(stBatInfo.stLcnInfo[i].u16ServiceID == stBatInfo.stServiceList[j].u16ProgramNumber)
                                {
                                    stDtvPgmData.stCHAttribute.bServiceType = stBatInfo.stServiceList[j].u8ServiceType;
                                    stDtvPgmData.stCHAttribute.bServiceTypePrio = stBatInfo.stServiceList[j].u8ServicePriority;
                                    printf("bServiceType %d ,bServiceTypePrio %d \n",stDtvPgmData.stCHAttribute.bServiceType,stDtvPgmData.stCHAttribute.bServiceTypePrio);
                                }
                            }

                            //5. get cur service name
                            if(stBatInfo.wTransportStream_ID == pastVirtualCh[0].wTransportStream_ID && stBatInfo.wOriginalNetwork_ID == pastVirtualCh[0].wOriginalNetwork_ID)
                            {
                                stDtvPgmData.stCHAttribute.wSignalStrength = wSignalQualityStrength;

                                for(k =0;pastVirtualCh[k].wService_ID != 0;k++)
                                {
                                    if(stBatInfo.stLcnInfo[i].u16ServiceID ==  pastVirtualCh[k].wService_ID)
                                    {
                                        printf(" get cur service name \n");
                                        MApp_SI_GetDtvPmgData(&pastVirtualCh[k], k, &stDtvPgmData);
                                        break;
                                    }
                                }
                            }
                            ...
                            {
                                printf(" get cur default name \n");
                                if(stDtvPgmData.stCHAttribute.bServiceType == E_TYPE_DATA)snprintf((char*)stDtvPgmData.bChannelName,SI_MAX_SERVICE_NAME,"DATA\0");
                                else if(stDtvPgmData.stCHAttribute.bServiceType == E_TYPE_RADIO)snprintf((char*)stDtvPgmData.bChannelName,SI_MAX_SERVICE_NAME,"RADIO\0");
                            ...
                            if(msAPI_SI_AddProgram(&stDtvIDTable, &stDtvPgmData,&bFull, TRUE))
                            {
                                ...
                                {
                                    if(stDtvPgmData.stCHAttribute.bServiceType == E_TYPE_DTV) u16ScanDtvChNum++;
                                    else if(stDtvPgmData.stCHAttribute.bServiceType == E_TYPE_RADIO) u16ScanRadioChNum++;
                ...
                MApp_SI_FreeBatServiceDesc();

                enScanState = STATE_SCAN_END;
                printf("STATE_SCAN_BAT_QUICK_SCAN end u16ScanDataChNum %d u16ScanRadioChNum %d u16ScanDtvChNum %d\n",u16ScanDataChNum,u16ScanRadioChNum,u16ScanDtvChNum);
            }
            break;
        #endif
            ...
            {
                //printf("STATE_SCAN_NIT_QUICK_SCAN_GET_NIT\n");
                if(IsDVBCInUse())
                {
                    DESC_DEL_SYS desc;
                ...
                {
                    DESC_DEL_SYS desc;
                    if(MApp_SI_GetNextSatParameter(&desc, NULL,NULL,NULL,NULL, FALSE))
                    {
                        _u16TSID=desc.u16Tsid;
                        enScanState = STATE_SCAN_NIT_QUICK_SCAN_WAIT_SDT;
                        break;
            ...
            {
                //printf("STATE_SCAN_NIT_QUICK_SCAN_WAIT_SDT\n");
                DESC_DEL_SYS desc;
                U16 u16RemainFreq;
                ...
                #if ENABLE_GET_SDT_WITHOUT_ORDER
                BOOL bEnableGetSdtWithoutOrder = TRUE;
                BOOL bWaitSDTTimeout = FALSE;

                #if ENABLE_STOFA
                if(OSD_COUNTRY_SETTING == OSD_COUNTRY_DENMARK)
                    bEnableGetSdtWithoutOrder = FALSE;
                #endif

                if(bEnableGetSdtWithoutOrder)
                {
                ...
                if(IsDVBCInUse())
                {
                    bResult = MApp_SI_GetNextCableParameter(&desc, &pServiceListInfo,&pLcnInfo,&u16ServiceNumber,&u16RemainFreq, TRUE);
                }
                else if(IsS2InUse())
                {
                    bResult = MApp_SI_GetNextSatParameter(&desc, &pServiceListInfo,&pLcnInfo,&u16ServiceNumber,&u16RemainFreq, TRUE);
                }
                ...
                if(bResult)
                {
                    ...
                    if(IsDVBCInUse())
                    {
                        ...
                        stDtvIDTable.QamMode = desc.stDCDS.u8Modulation-1;
                    ...
                    else if(IsS2InUse())
                    {
                        stDtvIDTable.SatelliteId =_lnb_satelliteID[_u8CurScanSATPos];
                        stDtvIDTable.u32S2Frequency = desc.u32CentreFreq/100;
                        stDtvIDTable.u16SymbolRate = (MS_U16)(desc.u32Symbol_rate/10) ;
                        stDtvIDTable.u8Polarity = desc.stDSDS.u8Polarization;
                        stDtvIDTable.cRFChannelNumber = 0x88;
                    }
    ...
    */
                    ...
                    #if ENABLE_GET_SDT_WITHOUT_ORDER
                    if(bEnableGetSdtWithoutOrder && !bWaitSDTTimeout)
                    {
                ...
                #if ENABLE_GET_SDT_WITHOUT_ORDER
                if(bEnableGetSdtWithoutOrder && (!bWaitSDTTimeout) && (!u16RemainFreq))
                {
                    enScanState = STATE_SCAN_END;
                    break;
                }
        ...
        case STATE_S2_SCAN_NIT_QUICK_SCAN_GET_NIT:
        {
            ...
            if(MApp_SI_GetNextSatParameter(&siCDSInfo,NULL,NULL,NULL, &u16FreqCnt,FALSE))
            {
                carrierParam.u32S2Frequency  = (siCDSInfo.u32CentreFreq)/100;  // 111.11111GHZ --->111111MHZ
                carrierParam.u16SymbolRate = ((siCDSInfo.u32Symbol_rate&0xfffffff))/10;// 111.1111M --->111111K
                carrierParam.u8Polarity = siCDSInfo.stDSDS.u8Polarization;
       ...
       case STATE_S2_SCAN_NIT_QUICK_SCAN_WAIT_SGT:
       {
            ...
            if(MApp_SI_GetNextSatParameter(&siCDSInfo, NULL,NULL,NULL,NULL,TRUE))
            {
...
#if (ENABLE_UPDATE_MULTIPLEX_VIA_NIT)
#define DEBUG_SCAN_UPDATE_MUX_ACT(x)        //x
#define DEBUG_SCAN_UPDATE_MUX_STATE(x)      //x

void MApp_DTV_Scan_Retune_Confirm_Yes(void)
{
    DEBUG_SCAN_UPDATE_MUX_ACT(printf("%s\n", __FUNCTION__));
...
void MApp_DTV_Scan_NetworkChange_Cornfim_OK(void)
{
    ...
    if(_u32CheckMuxUpdateTimer )  //norway spec. says scan start without confirmation
    {
        ...
        if( IsDVBCInUse() &&
            ...
            stGenSetting.stScanMenuSetting.u8ScanType == SCAN_TYPE_NETWORK)
        {
            MEMBER_SERVICETYPE eServiceType = msAPI_CM_GetCurrentServiceType();
            WORD wCurPosition = msAPI_CM_GetCurrentPosition(eServiceType);
            WORD wNID = msAPI_CM_GetNetwork_ID(eServiceType, wCurPosition);

            MApp_CadtvManualTuning_SetNID(wNID);

            DEBUG_SCAN_UPDATE_MUX_ACT(printf("Network id=%d\n", wNID));

            msAPI_Tuner_GetCurTPSetting(&stTPSetting);

            MApp_CadtvManualTuning_SetFrequency(stTPSetting.u32Frequency);

            DVBCNetworkIdScanType = 0;
        }
#endif
...
EN_RET MApp_DTV_Scan_Update_Mux( void )
{
    ...
    switch( enScanState )
    {
        case STATE_SCAN_INIT:
            ...
            if(IsDVBCInUse())
            {
                if(OSD_COUNTRY_SETTING == OSD_COUNTRY_SWEDEN && stGenSetting.stScanMenuSetting.u8ScanType == SCAN_TYPE_NETWORK)
                {
            ...
            if( MApp_DTV_Scan_Init() == TRUE )
            {
                ...
                enScanState = STATE_SCAN_SEARCH_RF_CHANNEL;
                DEBUG_SCAN_UPDATE_MUX_STATE(printf("STATE_SCAN_SEARCH_RF_CHANNEL %d\n", __LINE__));
            #if ENABLE_DVBC
                if(IsDVBCInUse())
                {
                    ...
                    {
                        enScanState = STATE_SCAN_END;
                        DEBUG_SCAN_UPDATE_MUX_STATE(printf("STATE_SCAN_END %d\n", __LINE__));
                        break;
                    }
            ...
            if(IsDVBCInUse())
            {
                ...
                while(MApp_SI_GetNextCableParameter(&desc, NULL,NULL,NULL,NULL, TRUE))
                {
                    if(msAPI_CM_Is_TSExist(desc.u16Onid, desc.u16Tsid, &dummy) == FALSE)
                    {
                        ...
                        stTPSetting.u8Modulation = desc.stDCDS.u8Modulation-1;
        ...
        case STATE_SCAN_SEARCH_RF_CHANNEL:
            ...
            {
                enScanState = STATE_SCAN_GET_PROGRAMS;
                DEBUG_SCAN_UPDATE_MUX_STATE(printf("STATE_SCAN_GET_PROGRAMS %d\n", __LINE__));
            }
            else
            {
                if((MApp_DTV_Scan_GetLossSignalState() == LOSS_SIGNAL_RETURN_ORIGINAL_RF) && (OSD_COUNTRY_SETTING == OSD_COUNTRY_NORWAY))
                ...
                {
                    enScanState = STATE_SCAN_NEXT_CHANNEL;
                    DEBUG_SCAN_UPDATE_MUX_STATE(printf("STATE_SCAN_NEXT_CHANNEL %d\n", __LINE__));
                }
            }
            break;
        case STATE_SCAN_GET_PROGRAMS:   //Truman for digital only, analog scan shouldn't go into this state
            ...
            {
                enScanState = STATE_SCAN_SAVE_PROGRAMS;
                DEBUG_SCAN_UPDATE_MUX_STATE(printf("STATE_SCAN_SAVE_PROGRAMS %d\n", __LINE__));
            }
            else
            {
                enScanState = STATE_SCAN_NEXT_CHANNEL;
        ...
        case STATE_SCAN_END:
            ...
            if(_u32CheckMuxUpdateTimer)//cancel
            {
                ...
                enDVBScanRetVal =EXIT_GOTO_PREVIOUS;
                enScanState = STATE_SCAN_INIT;
                DEBUG_SCAN_UPDATE_MUX_STATE(printf("STATE_SCAN_INIT %d\n", __LINE__));
                g_enScanType = (EN_SCAN_TYPE) stGenSetting.stScanMenuSetting.u8ScanType;
...
//1281446: [SQC][Marlon_120B][1st_DVBT][Antenna Ready HD (Finland)] Standby channel update ->
            ...
            if((MApp_DTV_Scan_GetLossSignalState() == LOSS_SIGNAL_RETURN_ORIGINAL_RF) &&
#if ENABLE_DVBC
                (!IsDVBCInUse()) &&
#endif
                ((OSD_COUNTRY_SETTING == OSD_COUNTRY_NORWAY)||(OSD_COUNTRY_SETTING == OSD_COUNTRY_SWEDEN)))
...
static BOOLEAN _MApp_DTV_Scan_GetTPSettingFromNITParams(MS_TP_SETTING *pstTPSetting)
{
    ...
    while(MApp_SI_GetNextCableParameter(&desc, NULL,NULL,NULL,&u16RemainFreq, TRUE))
    {
        ...
        pstTPSetting->u8Modulation = desc.stDCDS.u8Modulation - 1;
...
static BOOLEAN _MApp_Scan_DVBC_QuickInstall_AddOneDTVPchVchs(void)
{
    ...
    if (u8NumOfVch>0)
    {
        ...
        if (MApp_SI_GetNextCableParameter(&desc,NULL,NULL,NULL, &u16RemainFreq, TRUE))
        {
            ...
            stDtvIDTable.QamMode = desc.stDCDS.u8Modulation;
            ResetSDTParsingParameter(); //reset Params for next GetNextCableParameter
        }
