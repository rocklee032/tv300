...
BOOLEAN MApp_Audio_SetEWSAudio(MS_EWSAUDIOINFO mEwsAudioinfo)
{
    ...
    MApp_ChannelChange_Set_Audio_Decoder_System( eAudType, FALSE );
...
void MApp_Audio_Mute_Task(void)
{
    // If sync_video is on
    if( msAPI_AUD_Is_MuteClientActive(E_AU_MUTE_CL_ALIGN_VIDEO) )
    {
        // Check if video is mute
        if( MApi_XC_IsBlackVideoEnable(MAIN_WINDOW) == FALSE )
        {
            msAPI_AUD_Set_Mute(E_AU_MUTE_CL_ALIGN_VIDEO, AU_MUTE_DISABLE, E_AUDIOMUTESOURCE_ACTIVESOURCE);
        }
    }
...
void MApp_Audio_SearchAdAudio_ATSC(void)
{
    ...
    U8 u8AudioLangNum = MApp_ChanProc_Get_CurCh_AudLangNum();
    U8 u8i;
    u8i =0;
            ...
            if( eCurAudStreamType == AUD_STREAM_TYPE_AC3 )
            {
                if (!msAPI_AUD_DECODER_GetMAD_LOCK() && (msAPI_AUD_DECODER_GetMAD_PCMBufLevel(E_AUDIOSTREAM_AC3_)==0))
                {
                    ...
                    if ( ++u8AudioType > (PreferSoundTrack_Number+1) )
                    {
                        ...
                        g_u8AudLangSelected %= u8CurChAudLangNum;
                        u8AudioType = 0;
...
#if (ENABLE_DTV)
EnuAppAudioLRMode MApp_Audio_Get_LRMode(void)
{
    if( g_u8LRAudioMode >= E_AUDIO_LRMODE_UNKNOWN )
        return E_AUDIO_LRMODE_UNKNOWN;

    return (EnuAppAudioLRMode)g_u8LRAudioMode;
}

void MApp_Audio_Set_LRMode(EnuAppAudioLRMode eAppAudioLRMode)
{
    switch(eAppAudioLRMode)
    {
        case E_AUDIO_LRMODE_STEREO:
            MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_SoundMode, MSAPI_AUD_MODE_STEREO, 0);
            MApi_AUDIO_SetMpegInfo(Audio_MPEG_infoType_SoundMode, MSAPI_AUD_MPEG_SOUNDMODE_LR, 0);
            break;

        case E_AUDIO_LRMODE_RR:
            MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_SoundMode, MSAPI_AUD_MODE_RR, 0);
            MApi_AUDIO_SetMpegInfo(Audio_MPEG_infoType_SoundMode, MSAPI_AUD_MPEG_SOUNDMODE_RR, 0);
            break;

        case E_AUDIO_LRMODE_LL:
            MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_SoundMode, MSAPI_AUD_MODE_LL, 0);
            MApi_AUDIO_SetMpegInfo(Audio_MPEG_infoType_SoundMode, MSAPI_AUD_MPEG_SOUNDMODE_LL, 0);
            break;
