...
void MApp_OAD_Set_PowerOnCheck(BOOLEAN bEnable)
{
    ...
    if( IsDVBInUse()&& MApp_OAD_IsUserSelectOn())
    {
...
{
#if(ENABLE_DVB)
    if( SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW) // What's this meaning?
      && MApp_OAD_IsUserSelectOn()
      && msAPI_Power_IswakeupsourceRTC()
      )
    {
        if((msAPI_CM_CountProgram(E_SERVICETYPE_DTV, E_PROGACESS_INCLUDE_NOT_VISIBLE_ALSO)+ \
            msAPI_CM_CountProgram(E_SERVICETYPE_RADIO, E_PROGACESS_INCLUDE_NOT_VISIBLE_ALSO)+ \
...
void MApp_OAD_SetInfo_ForBL(U16 u16PID, U32 u32Freq, U8 u8BandWidth)
{
    ...
    U8 u8Country = stGenSetting.stTvSetting.eCountry;
...
#if (EEPROM_DB_STORAGE==EEPROM_SAVE_NONE)
   {
        ...
        dl_info.u8DL_OAD_Country = u8Country;//(U8)msAPI_CM_GetCountry();
...
//    static U8 u8ScanCount;
    ...
    if (enOADScanState == STATE_OAD_SCAN_END)
    {
        ...
        u8OadRFCh = 2;//msAPI_DFT_GetFirstPhysicalChannelNumber();
...
LPTSTR MApp_OAD_GetOadRfChannel(void)
{
    ...
    #endif
        {
          #if (ENABLE_DVB) //ENABLE_T_C_COMBO
            U8 u8_idx = 0;

            if( IsDVBInUse() && (!IsDVBCInUse()) )
            {
                U8 RFChannelName[4] = {0};
                if(TRUE == msAPI_DFT_GetPhysicalChannelName(u8OadRFCh, RFChannelName, 4)) // DVBT
                {
    ...
    switch ( eAppState )
    {
        ...
        case EN_OAD_APP_STATE_DOWNLOADING:
        {
            ...
            if (OSD_COUNTRY_SETTING == OSD_COUNTRY_NEWZEALAND || (IS_NORDIC_COUNTRY(OSD_COUNTRY_SETTING)))
            {
                ...
                else if( (MApp_OAD_GetProgress()==0)
                       &&(FALSE != IsServiceTypeValid(ePrev_ServiceTypePower) )
                       &&( msAPI_CM_GetCurrentServiceType() != ePrev_ServiceTypePower)
                       )
                {
