...
#define CI_OPERATORING_MAX_TIME      6000
#define PVR_INVISIBLE_SKIP_CAP       3
#define CI_PINCODESTATUS_MAX_TIME    6000
...
typedef struct
{
    ...
    /// rest license count
    U8 u8Count;
}RETENTION_LIMIT_BLOCK;

typedef struct
{
    U8 u8UriLicenseCount;
    U8 u8Index;
    RETENTION_LIMIT_BLOCK m_RetentionLimitBlock[MAX_EVENTINFO_NUM];
}URI_LICENSE_REGION_INFO;
...
INTERFACE BOOL MAPP_CIPVR_CIPVRFS_PlaybackFileRead(enPvrEncryptType enEncryptionType);
...
INTERFACE BOOL MAPP_CIPVR_RecordStartEventUpdate(U8* u8aCICAMID);
INTERFACE BOOL MAPP_CIPVR_GetUriLicenseRegionInfo(URI_LICENSE_REGION_INFO *stUriLicenseRegionInfo);
INTERFACE BOOL MAPP_CIPVR_RetentionLimitBlockUpdates(RETENTION_LIMIT_BLOCK *stRLBlockInfo);
#undef INTERFACE
