...
//#endif
#if ENABLE_EWBS
#define EWBS_LocationCode_LENGTH 4
#endif
...
typedef enum _EN_OSD_CERTIFICATION_SETTING
{
    ...
    OSD_CERTIFICATION_UPC                   ,//= E_UPC,
    OSD_CERTIFICATION_CABLEREADY            ,//= E_CABLEREADY,
...
#if ENABLE_EWBS
typedef enum _EN_OSD_EWBS_AREA_TYPE_SETTING
{
    OSD_AREA_TYPE_WIDE_AREA_ZONE,
    OSD_AREA_TYPE_REGION,
    OSD_AREA_TYPE_CITY,
    OSD_AREA_TYPE_NUMS
}EN_OSD_EWBS_AREA_TYPE_SETTING;
#endif
...
typedef enum
{
    E_CERTIFICATION_INIT              = 0,

    E_CERTIFICATION_OTHER             = BIT0,
    E_CERTIFICATION_UNITYMEDIA        = BIT1,
    E_CERTIFICATION_UPC               = BIT2,
    E_CERTIFICATION_ZIGGO             = BIT3,
    E_CERTIFICATION_CABLEREADY        = BIT4,
    E_CERTIFICATION_COMHEM            = BIT5,
    E_CERTIFICATION_STOFA             = BIT6,
    E_CERTIFICATION_VIRGINMEDIA       = BIT7,

}EN_CERTIFICATION_STATUS;
...
#if ENABLE_ISDBT
    E_ANTENNA_SOURCE_TYPE enLastWatchedAntennaType;
#endif
  #if(!DVB_T_C_DIFF_DB)
...
#if DVBC_CERTIFICATION_AUTOSCAN_ENABLE
    U32 u32CertificationSetting;
#endif

#if ENABLE_BAT_SCAN
    EN_SI_BOUQUET_ID eBouquetID;
#endif

#if(ENABLE_GAMEMODE)
    U8 u8GameMode; // 0: Off, 1: On
#endif
...
#if (ENABLE_EWBS)
    BOOLEAN g_bEwbsMode:1;    // ENABLE : on  (Available all EWBS function).
                             //DISABLE: off (Disable all EWBS function).
    WORD wMonitoringServiceID;
    WORD wMonitoringONID;
    WORD wMonitoringTSID;

    U8 u8EWBSLocationCode[EWBS_LocationCode_LENGTH];
    BOOLEAN g_bEwbsAutomaticStartup:1;    // ENABLE : on  (TV wakes up from standby while receiving EWBS)
                                          // DISABLE: off (TV does not wake up from standby while receiving EWBS)
    BOOLEAN g_bEwbsNoOperationOff:1;      // ENABLE : on  (TV  should run into standby when waking up within 5 minutes without operation).
                                          // DISABLE: off (TV  should not run into standby when waking up within 5 minutes without operation).
    BOOLEAN g_bEwbsBuzzerNotification:1;  // ENABLE : on  (Buzzer should alarm when EWBS shows).
                                          // DISABLE: off (Buzzer should not alarm when EWBS shows).
#endif
...
typedef struct
{
    ...
    U16 au16DimVal[REGION5_DIMENSION_NUM];

#elif defined(ENABLE_PARSE_SAME_DIMENSION_IN_RRT5_CAD) && ENABLE_PARSE_SAME_DIMENSION_IN_RRT5_CAD
    U16 u8DimVal0;
    U16 u8DimVal1;
    U16 u8DimVal2;
    U16 u8DimVal3;
...
#if ENABLE_ISDBT
#define LAST_WATCHED_ANTENNA_TYPE (stGenSetting.g_SysSetting.enLastWatchedAntennaType)
#endif
#if(!DVB_T_C_DIFF_DB)
...
#if ENABLE_BAT_SCAN
#define GET_OSD_MENU_REGION()                 stGenSetting.g_SysSetting.eBouquetID
#define SET_OSD_MENU_REGION(x)                 stGenSetting.g_SysSetting.eBouquetID = (x)
#endif
...
#if DVBC_CERTIFICATION_AUTOSCAN_ENABLE
INTERFACE void MApp_SetCertificationSetting(EN_OSD_CERTIFICATION_SETTING eCertificationType);
INTERFACE EN_OSD_CERTIFICATION_SETTING MApp_GetCertificationSetting(void);
INTERFACE void MApp_EnableCertificationFlag(void);
#endif
