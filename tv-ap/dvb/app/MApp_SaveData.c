...
void MApp_CheckFlash(void)
{
    ...
    //MApp_SaveData_Dump_WhiteBalance();
    ...
    else if(IsYPbPrInUse())
    {
        wRmAddress=(U32)(RM_OVERSCAN_DATA_START_ADR+(g_PcadcModeSetting[MAIN_WINDOW].u8ModeIndex -MD_720x480_60I+EN_FACTORY_OverScan_YPBPR480I)*SIZE_OVERSCAN_DATA);
            ...
            ||( IsSrcTypeStorage(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW))
              &&!IsSrcTypeJpeg(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)))// storage type except JPEG
            )
    {
...
void MApp_SaveData_Dump_WhiteBalance(void)
{
    U8 u8DataSrcIdx;
    T_MS_WHITEBALANCE * pstWhiteBalance;

    U8 u8ColorTempIdx;
    T_MS_COLOR_TEMP * pstMS_COLOR_TEMP;

    PRINT_CURRENT_LINE();

    for(u8DataSrcIdx = 0; u8DataSrcIdx < DATA_INPUT_SOURCE_NUM; u8DataSrcIdx++)
    {
        pstWhiteBalance = &(G_WHITE_BALANCE_SETTING[u8DataSrcIdx]);

        printf("u8DataSrcIdx=%u\n", u8DataSrcIdx);

        for(u8ColorTempIdx = 0; u8ColorTempIdx < MS_COLOR_TEMP_COUNT; u8ColorTempIdx++)
        {
            pstMS_COLOR_TEMP = &(pstWhiteBalance->astColorTemp[u8ColorTempIdx]);

            printf(" ColorTmp[%u]:", u8ColorTempIdx);
            printf(" Offset: %u %u %u;", pstMS_COLOR_TEMP->cRedOffset, pstMS_COLOR_TEMP->cGreenOffset, pstMS_COLOR_TEMP->cBlueOffset);
            printf(" Gain: %u %u %u;", pstMS_COLOR_TEMP->cRedColor, pstMS_COLOR_TEMP->cGreenColor, pstMS_COLOR_TEMP->cBlueColor);
            printf(" Scale: %u %u %u\n", pstMS_COLOR_TEMP->cRedScaleValue, pstMS_COLOR_TEMP->cGreenScaleValue, pstMS_COLOR_TEMP->cBlueScaleValue);
        }
