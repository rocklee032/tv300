...
#define DEBUG_XC_PATH(x)      //do { msDebug_ANSI_SetColorText(E_FONT_BACKGROUND_COLOR_RED); x; msDebug_ANSI_AllAttrOffText(); } while(0)
...
//    {INPUT_SOURCE_DVI,          INPUT_SOURCE_HDMI   ,AUDIO_SOURCE_ATV},
  ...
  #if(INPUT_HDMI_VIDEO_COUNT>3)
    {INPUT_SOURCE_HDMI4,        INPUT_SOURCE_HDMI4  ,AUDIO_SOURCE_HDMI4},
  #endif
...
void MApp_InputSource_SetSystemmInfo( E_UI_INPUT_SOURCE enUiInputSourceType, MS_SYS_INFO *penMsSysInfo , E_DATA_INPUT_SOURCE *penDataInpSrcType)
{
   ...
   //         break;
    ...
    INPUTSOURCE_DBG( printf( "Result enInputSourceType = %u(%s)\n", penMsSysInfo->enInputSourceType, MApp_InputSrc_Get_InputSrcName(penMsSysInfo->enInputSourceType) ) );
...
void _MApp_ChangeSrc_Step_02_DestroyPreSource( E_UI_INPUT_SOURCE eOldUiInputSrc,
                                               E_UI_INPUT_SOURCE eNewUiInputSrc,
                                               SCALER_WIN eWindow )
{
    ...
    // Warning
    UNUSED(eOldUiInputSrc);
    UNUSED(eNewUiInputSrc);
    UNUSED(eWindow);
    UNUSED(eOldInputSrc);
...
void MApp_ChangeSrc_XCCreatePath_MainSubWin(SCALER_WIN eWindow, INPUT_SOURCE_TYPE_t eInputSrc)
{
    DEBUG_XC_PATH( printf("MApp_ChangeSrc_XCCreatePath_MainSubWin(win=%u, src=%u(%s))\n", eWindow, eInputSrc, MApp_InputSrc_Get_InputSrcName(eInputSrc)); );

    if( eInputSrc >= INPUT_SOURCE_NONE )
    {
        return ;
    }
  ...
  #if(ENABLE_DTV)
    if(IsSrcTypeDTV(eInputSrc))
    {
        PathInfo.path_thread = MApp_DTV_Handler;
    }
    else
  #endif
    if(IsSrcTypeAnalog(eInputSrc) || IsSrcTypeHDMI(eInputSrc))
    {
  ...
  #if ENABLE_DMP
    else if (IsSrcTypeStorage(eInputSrc))
    {
    ...
    PathInfo.src = eInputSrc;

    if ( eWindow == MAIN_WINDOW )
    {
        PathInfo.dest = OUTPUT_SCALER_MAIN_WINDOW;
        if(IsSrcTypeStorage(eInputSrc))
        {
    ...
    ******************************************************************************/
    if( PathInfo.src < INPUT_SOURCE_NONE )
    {
        DEBUG_XC_PATH(printf("\nCreate path: src = %d(%s) dest = %d\n", PathInfo.src, MApp_InputSrc_Get_InputSrcName(PathInfo.src), PathInfo.dest); );
        S16 s16PathId = MApi_XC_Mux_CreatePath( &PathInfo, sizeof(XC_MUX_PATH_INFO) );
        if (s16PathId == -1)
        {
            printf("\nError: Create path fail! src = %d  dest = %d\n", PathInfo.src, PathInfo.dest);
        }
        else
        {
...
void MApp_ChangeSrc_XCCreatePath_OUTCVBS1(INPUT_SOURCE_TYPE_t u8NewCVBS1OutSource)
{
    DEBUG_XC_PATH( printf("MApp_ChangeSrc_XCCreatePath_OUTCVBS1(u8NewCVBS1OutSource=%u(%s))\n", u8NewCVBS1OutSource, MApp_InputSrc_Get_InputSrcName(u8NewCVBS1OutSource)); );
    ...
    DEBUG_XC_PATH( printf(" Create CVBS1 path: src = %d  dest = %d \n",PathInfo.src ,PathInfo.dest ); );
    s16PathId = MApi_XC_Mux_CreatePath(&PathInfo, sizeof(XC_MUX_PATH_INFO) );

    if( s16PathId == -1 )
    {
        printf("\nError: Create CVBS1 path fail! src = %d  dest = %d\n",PathInfo.src ,PathInfo.dest );
    }
    else
    {
        MApi_XC_Mux_EnablePath( (U16)s16PathId );
...
void MApp_ChangeSrc_XCCreatePath_OUTCVBS2(INPUT_SOURCE_TYPE_t u8NewCVBS2OutSource)
{
    DEBUG_XC_PATH( printf("MApp_ChangeSrc_XCCreatePath_OUTCVBS2(u8NewCVBS2OutSource=%u(%s))\n", u8NewCVBS2OutSource, MApp_InputSrc_Get_InputSrcName(u8NewCVBS2OutSource)); );
    ...
    DEBUG_XC_PATH(printf(" Create CVBS2 path: src = %d  dest = %d \n", PathInfo.src, PathInfo.dest ); );
    s16PathId = MApi_XC_Mux_CreatePath(&PathInfo, sizeof(XC_MUX_PATH_INFO) );

    if (s16PathId == -1)
    {
        printf("\nError: Create CVBS2 path fail! src = %d  dest = %d \n", PathInfo.src, PathInfo.dest );
    }
    else
    {
        MApi_XC_Mux_EnablePath( (U16)s16PathId );
                                               ...
                                               SCALER_WIN eWindow )
{
    ...
    printf(" OldUiSrc=%u(%s), eOldInputSrc=%u(%s)\n", _s_eOldUiInputSourceType, MApp_InputSrc_Get_UiInputSrcName(_s_eOldUiInputSourceType),
        eOldInputSrc, MApp_InputSrc_Get_InputSrcName(eOldInputSrc));
#else
    ...
    /////////////////////////////////////////
    {
        if( eOldInputSrc < INPUT_SOURCE_NUM )
        {
            ...
            DEBUG_XC_PATH( printf("==>Delete Main Path, SRC =%u, Dest=%u\n", eOldInputSrc, eDelPathDest); );
            ...
            {
                DEBUG_XC_PATH( printf(" Delete path success src =%d dst=%d ", eOldInputSrc, OUTPUT_SCALER_MAIN_WINDOW ));
            }
        }
    ...
    DEBUG_CHG_SRC_TIME2();
      ...
      if ( IsSrcTypeDTV(eOldInputSrc) )
      {
         ...
         if( msAPI_CI_CardDetect() )
         {
            ...
            WORD wCurrentPosition, wServiceID;
            U8 u8PMTVersion;

            bServiceType = msAPI_CM_GetCurrentServiceType();
            wCurrentPosition = msAPI_CM_GetCurrentPosition(bServiceType);

            wServiceID = msAPI_CM_GetService_ID(bServiceType, wCurrentPosition);
            u8PMTVersion = MApp_SI_Get_PSISIVersion(E_VER_PMT);

            msAPI_CI_SendEmptyPmt(wServiceID, u8PMTVersion);
        }
    ...
    printf(" eNewInputSrc=%u(%s)\n", eNewInputSrc, MApp_InputSrc_Get_InputSrcName(eNewInputSrc));
    //=============================================================
    ...
    if(IsS2InUse())
    {
        ...
        //msAPI_Timer_Delayms(100); // Why delay ?
        Int_DigiTuner_Tp_Sat();
    }
    else
    {
    #if 0//(FRONTEND_DEMOD_S2_TYPE == DEMOD_AVL6211)
    ...
    if((u8PreCVBS1OutSource != u8NewCVBS1OutSource)&&(u8PreCVBS1OutSource!= INPUT_SOURCE_NONE))
    {
        ...
        {
            DEBUG_XC_PATH(printf(" delete CVBS1 path success src = %d  dest = %d \n",u8PreCVBS1OutSource ,OUTPUT_CVBS1));
        }
        DEBUG_CHG_SRC_TIME2();
    ...
    if((u8PreCVBS2OutSource != u8NewCVBS2OutSource)&&(u8PreCVBS2OutSource!= INPUT_SOURCE_NONE))
    {
        ...
        {
            DEBUG_XC_PATH(printf(" delete CVBS2 path success src = %d  dest = %d \n",u8PreCVBS2OutSource ,OUTPUT_CVBS2));
        }
    ...
    //////////////////////////////////////////////
    MApp_ChangeSrc_XCCreatePath_MainSubWin(eWindow, stSystemInfo[eWindow].enInputSourceType);

    // Create CVBS out path...
    if( (u8NewCVBS1OutSource != INPUT_SOURCE_NONE)&&(u8NewCVBS1OutSource != u8PreCVBS1OutSource) )
    {
        MApp_ChangeSrc_XCCreatePath_OUTCVBS1(u8NewCVBS1OutSource);
    }
    if( u8NewCVBS1OutSource == INPUT_SOURCE_TV )
    {
    ...
    {
        MApp_ChangeSrc_XCCreatePath_OUTCVBS2(u8NewCVBS2OutSource);
    }
  ...
  // digital IP
      ...
      )
    {
        ...
        if( IsDVBInUse() )
        {
            ...
            {
                //msAPI_CI_ReInitial(); ////fix HTT console can't response after switch source
            }
        }
    #endif
...
void MApp_InputSource_ChangeInputSource_2(SCALER_WIN eWindow)
{
      ...
      )
    {
        if (source_type == UI_INPUT_SOURCE_ISDBT)
        {
            ...
            if (LAST_WATCHED_ANTENNA_TYPE == ANTENNA_DTV_TYPE)
            {
...
char* MApp_InputSrc_Get_InputSrcName(INPUT_SOURCE_TYPE_t eInputSourceType)
{
    switch(eInputSourceType)
    {
        case INPUT_SOURCE_VGA:   ///<0   VGA input
            return "VGA";

        case INPUT_SOURCE_TV:   ///<1   TV input
            return "VGA";

        case INPUT_SOURCE_CVBS:   ///<2   AV 1
            return "CVBS";

        case INPUT_SOURCE_SVIDEO:  ///<11 S-video 1
            return "SV";

        case INPUT_SOURCE_YPBPR: ///<16 Component 1
            return "YPbPr";

        case INPUT_SOURCE_SCART: ///<20 Scart 1
            return "SCART1";

        case INPUT_SOURCE_SCART2: ///<21 Scart 2
            return "SCART2";

        case INPUT_SOURCE_HDMI: ///<23 HDMI 1
            return "HDMI1";

        case INPUT_SOURCE_HDMI2: ///<24 HDMI 2
            return "HDMI2";

        case INPUT_SOURCE_HDMI3:  ///<25 HDMI 3
            return "HDMI3";

        case INPUT_SOURCE_HDMI4:  ///<26 HDMI 4
            return "HDMI4";

        case INPUT_SOURCE_DTV:
            return "DTV";

        case INPUT_SOURCE_STORAGE: ///<34 Storage:
            return "STORAGE";
