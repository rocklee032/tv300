...
#if ENABLE_EWBS
static EN_OSD_EWBS_AREA_SETTING m_eEWBSAreaSetting;
#endif
...
static code TimeZone2Country stTimeZone2Country[]=
{
    ...
    {TIMEZONE_PHILIPPINES,  OSD_COUNTRY_PHILIPPINES},
#endif
};
#endif
//*******************************************************************************
...
BOOLEAN MApp_DTV_Is_DemodLock(void)
{
#if( ENABLE_ATSC )
    if( IsAtscInUse() )
    {
...
BOOLEAN MApp_DTV_Is_VDECLock(void)
{
#if( ENABLE_DTV )
    if( IsDTVInUse() )
    {
...
/*BYTE*/ EN_OSDAPI_LANGUAGE MApp_GetLanguageForFont(void)
{
...
#if ENABLE_EWBS
EN_OSD_EWBS_AREA_SETTING MApp_GetEWBSAreaSetting(void)
{
    return m_eEWBSAreaSetting;
}
#endif
...
{
 #if ENABLE_ZIGGO && ENABLE_SUBTITLE_SWITCH_MENU
   if(msAPI_SI_IsZiggo_Spec_Country(MApp_GetSICountry(eOSDCountrySetting)))
   {
    ...
    switch(eOSDCountrySetting)
    {
        ...
        {
            stGenSetting.g_SysSetting.SubtitleDefaultLanguage = LANGUAGE_MALAYSIA;
            stGenSetting.g_SysSetting.SubtitleDefaultLanguage_2= LANGUAGE_ENGLISH;
            stGenSetting.g_SoundSetting.enSoundAudioLan1 = LANGUAGE_MALAYSIA;
            stGenSetting.g_SoundSetting.enSoundAudioLan2 = LANGUAGE_ENGLISH;
        }
        break;
        default:
...
static EN_MENU_TIMEZONE MApp_GetTimeZoneBySITimeZone(EN_SI_TIMEZONE eTimeZone)
{
    switch(eTimeZone)
    {
        ...
        case SI_TIMEZONE_PHILIPPINES:    return TIMEZONE_PHILIPPINES;
#endif
...
EN_SI_COUNTRY_SETTING MApp_GetSICountry(EN_OSD_COUNTRY_SETTING eOSDCountrySetting)
{
    ...
    switch(eOSDCountrySetting)
    {
        ...
        case OSD_COUNTRY_PHILIPPINES:          eSICountry= SI_COUNTRY_PHILIPPINES; break;
        default:
        ASSERT(0);
        break;
    }
...
{
#if ASTRA_HD_ENABLE
    if(ISDVBS2Source()&& stGenSetting.g_SysSetting.bIsAstraSat) // && MApp_GetSICountry(OSD_COUNTRY_SETTING) == SI_COUNTRY_GERMANY)
    {
...
EN_SI_TIMEZONE MApp_GetSITimeZone(EN_MENU_TIMEZONE eTimeZone)
{
    switch(eTimeZone)
    {
        ...
        case TIMEZONE_PHILIPPINES:          return SI_TIMEZONE_PHILIPPINES;
      #endif
...
EN_LANGUAGE MApp_GetLanguageBySILanguage(EN_SI_LANGUAGE eLanguage)
{
    switch(eLanguage)
    {
        ...
        case SI_LANGUAGE_VALENCIAN:         return LANGUAGE_VALENCIAN;
...
static void MApp_GetDefaultAudioSoundTrackPID(U16 *pu16AudioPID, U8 u8AudioLangIdx)
{
    ...
    {
        U8 u8AudLangNum = MApp_ChanProc_Get_Ch_AudLangNum(pstVirtualCh);

        //for ( u8Loop_i = 0; u8Loop_i < pstVirtualCh->u8AudLangNum; u8Loop_i++ )
        for ( u8Loop_i = 0; u8Loop_i < u8AudLangNum; u8Loop_i++ )
        {
...
U8 MApp_ReMapAudioLanguage_SelectProcess(BOOLEAN bSelectProcess)
{
    ...
    else
    {
        ...
        //2 11.2.2.2.1. Select process by Priority value
        U8 u8CurChAudLangNum = MApp_ChanProc_Get_CurCh_AudLangNum();

        //for(iu8Loop_i = 0; iu8Loop_i< MApp_ChanProc_GetCurChannelPointer()->u8AudLangNum; iu8Loop_i++)
        for(iu8Loop_i = 0; iu8Loop_i< u8CurChAudLangNum; iu8Loop_i++)
        {
...
void MApp_ATSC_ReMapAudioLanguage(void)
{
    ...
    for(enAudLangNum = SOUND_AUDIO_LAN_ENGLISH; enAudLangNum < SOUND_AUDIO_LAN_NUM; enAudLangNum++)
    {
        ...
        for(u8SrcAuLangIdx = 0; u8SrcAuLangIdx < u8AudLangNum; u8SrcAuLangIdx ++)
        {
        ...
        if( u8DestAuLangIdx == u8AudLangNum )
            return;
    }
...
U16 MApp_SetPreferAudioSoundTrackPIDNew(void)
{
    ...
    //for ( iu8Loop_i = 0; iu8Loop_i < pstVirtualCh->u8AudLangNum; iu8Loop_i++ )
    for ( u8SoundTrackIdx = 0; u8SoundTrackIdx < u8AudLangNum; u8SoundTrackIdx++ )
    {
        if ( astgSoundTrack[u8SoundTrackIdx].u8AudLangIdx == stGenSetting.g_SoundSetting.enSoundAudioLan )
        {
            if ( astgSoundTrack[u8SoundTrackIdx].u8AudPidInPMT )
            {
                u16AudioStreamType = astgSoundTrack[u8SoundTrackIdx].u16AudStreamType;
                DEBUG_ATSC_AUDIO_STREAM_TYPE( printf("2 u16AudioStreamType=%u\n", u16AudioStreamType); );

                u8AudioLanguageType = astgSoundTrack[u8SoundTrackIdx].u8AudLangIdx;
                u16AudioPID_temp = astgSoundTrack[u8SoundTrackIdx].u16AudioPid;
                g_u8AudLangSelected = u8SoundTrackIdx;
...
U16 MApp_SetPreferAudioSoundTrackPID(U8 u8Enable)
{
    ...
    for ( u8SoundTrackIdx = 0; u8SoundTrackIdx < u8AudLangNum; u8SoundTrackIdx++ )
    {
        ...
        if(astgSoundTrack[u8SoundTrackIdx].u8AudAc3Bsmod == EN_PSIP_AC3_BSMOD_VI)
        {
            bViFound = TRUE;
            if ( astgSoundTrack[u8SoundTrackIdx].u8AudPidInPMT )
            {
                ...
                if (u8Enable)
                {
                    MApp_GetDefaultAudioSoundTrackPID(&u16AudioPID_temp, u8SoundTrackIdx);
                }
                return u16AudioPID_temp;
            }
        }
...
MEMBER_COUNTRY MApp_Sys_Get_Country(void)
{
#if(ENABLE_DTV)
    return stGenSetting.stTvSetting.eCountry;
#else
    return E_CHINA;
#endif
}

BOOLEAN MApp_Is_CurDtvProgVideo(void)
{
#if(ENABLE_ATSC)
    if( IsAtscInUse() )
    {
    ...
    if( IsDVBInUse() )
    {
        ...
        if( msAPI_CM_GetProgramAttribute(bServiceType, u16CurPosition, E_ATTRIBUTE_IS_STILL_PICTURE) == TRUE )
            return FALSE; // Program is picture
        else
            return TRUE; // Program is video
    }
#endif
...
void MApp_Sys_Print_Char_N(char ch, U8 u8Cnt)
{
    U8 i;
...
void MApp_Sys_PrintMVOPInfo(void)
{
