...
static U32 _u32TempValidAudioIndex=0;
static U32 _u32TempValidSubtitleIndex=0;

#define _TEMPVAILDAUDIOINDEX    _u32TempValidAudioIndex
#endif
...
static void _MApp_SI_Parse_Update_Component(E_COMPONENT_TYPE eType, BYTE *lang)
{
    ...
    U32 *puVaildAudioIndex = &_TEMPVAILDAUDIOINDEX;

    if(eType == E_COMPONENT_MPEG_AUDIO)
    {
        msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_MPEG,bISOLangIndex,puVaildAudioIndex);
        if(bISOLangIndex == SI_LANGUAGE_ENGLISH)
        {
            msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_MPEG,SI_LANGUAGE_UND,puVaildAudioIndex);
        }
    }
    else if(eType == E_COMPONENT_AC3_AUDIO)
    {
        msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_AC3,bISOLangIndex,puVaildAudioIndex);
        msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_AC3P,bISOLangIndex,puVaildAudioIndex);
        if(bISOLangIndex == SI_LANGUAGE_ENGLISH)
        {
            msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_AC3,SI_LANGUAGE_UND,puVaildAudioIndex);
            msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_AC3P,SI_LANGUAGE_UND,puVaildAudioIndex);
        }
    }
    else if(eType == E_COMPONENT_AAC_AUDIO)
    {
        msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_AAC,bISOLangIndex,puVaildAudioIndex);
        msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_MPEG4,bISOLangIndex,puVaildAudioIndex);
        if(bISOLangIndex == SI_LANGUAGE_ENGLISH)
        {
            msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_AAC,SI_LANGUAGE_UND,puVaildAudioIndex);
            msAPI_CM_SetAudioStreamValidIndex(bServiceType,wCurrentPosition,E_AUDIOSTREAM_MPEG4,SI_LANGUAGE_UND,puVaildAudioIndex);
        }
    }
#if ENABLE_SUBTITLE
...
static BOOL _MApp_SI_Parse_Restart_Component(void)
{
    ...
    if( IS_EIT_COMPONENT_COUNTRY(OSD_COUNTRY_SETTING) && _TEMPVAILDAUDIOINDEX && (msAPI_CM_Get_AudioValidIndex() != _TEMPVAILDAUDIOINDEX))
    {
        ...
        else
        {
            ...
            if(((wONID == 0x222A) || (wONID == 0x233A)))
            {
                U32 oldValidIndex=msAPI_CM_Get_AudioValidIndex();
                BYTE bISOLangIndex;
                ...
                if(bNeedUpdate && TRUE == msAPI_CM_GetAudioStreamInfo(bServiceType, wCurrentPosition, &aAudioStreamInfo, g_u8AudLangSelected))
                {
                    ...
                    msAPI_CM_ArrangeAudioStreamInfo(bServiceType,wCurrentPosition,_TEMPVAILDAUDIOINDEX);
                    AudioNum = msAPI_CM_GetAudioStreamCount(bServiceType, wCurrentPosition);
                   if(AudioNum == 0)
                    {
                        msAPI_CM_Reset_AudioValidIndex();
...
//printf("u16Service_ID %x %x %d\n",u16Service_ID,u16CurService_ID,MApp_Dmx_GetOADScanMode());
    ...
    {
        u16TotalDescriptorLength = TSGetBitsFromU16(&pu8Section[10], 0, 0x0fff);
        /*[4]check whether has CA Descriptor in program layer =================*/
        pu8Descriptor = MApp_Dmx_FindDescriptorTag(pu8Section + 12, TAG_CaD, u16TotalDescriptorLength);
        
        if( pu8Descriptor != NULL )
        {
            msAPI_CM_UpdateProgramScrambWithServiceID(u16Service_ID,TRUE);
        }
