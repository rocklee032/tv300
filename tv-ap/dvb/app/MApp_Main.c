...
/***************************************************************************************/
#if( CHIP_FAMILY_MAZDA == CHIP_FAMILY_TYPE )
...
MS_BOOL MApp_PreInit_State(void)
{
    ...
    switch ( enPreInit_Sub_State )
    {
        ...
        case EN_PRE_INIT_STAGE_SECURITY_R2_INIT:
        #if(INIT_SEC_R2_BEOFRE_CHANGE_SRC)
          #if (ENABLE_SECURITY_R2)
            MApp_Init_Security_R2();
          #endif
        #endif
            break;
...
BOOLEAN MApp_Is_BeforeFirstTimeChangeSource(void)
{
    if( (enPreInit_Sub_State <= EN_PRE_INIT_STAGE_BEFORE_CHANGE_SOURCE) )
    {
        return TRUE;
    }
