...
#define TV_MODE_WINDOW_CHECK_SCRABLE_PERIOD       2000
...
BOOLEAN MApp_TV_SelectSubtileLang(void)
{
    ...
    if ( msAPI_CM_GetSubtitleIndex(msAPI_CM_GetCurrentServiceType(), msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType())) == 0xFF)
    {
        msAPI_CM_SetSubtitleIndex(msAPI_CM_GetCurrentServiceType(), msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType()),u8SubtitleItemSelIdx);
    }
    else
    {
        u8SubtitleItemSelIdx = msAPI_CM_GetSubtitleIndex(msAPI_CM_GetCurrentServiceType(), msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType()));

        if(u8SubtitleItemSelIdx > 0)
            u8SubtitleMenuIdx= u8SubtitleItemSelIdx - 1;
        else
            u8SubtitleMenuIdx = 0;
...
BOOLEAN MApp_TV_MHEG_Loading_Monitor(void)
{
            ...
            /*MApp_ZUI_GetActiveOSD() == E_OSD_EMPTY*/) //ZUI: ( enCurTVWin == DISP_WIN_NONEOSD ) )
    {
        ...
        if (msAPI_IsCIMHEGRun()==E_MHEG_CI_TRIGGER)
        {
            ...
            MApp_MHEG5_Dsmcc_StartTime_Update();
...
void MApp_TV_DSMCC_Trigger_Monitor(void)
{
    ...
    if (msAPI_MHEG5_GetBinStatus()  && (enCheckMHEGLoadingStatus == EN_MHEG5_MONITOR) && (MApp_SI_Get_PMT_PID() != MSAPI_DMX_INVALID_PID))
    {
        ...
        MApp_MHEG5_Dsmcc_StartTime_Reset();
    }
}
#endif
                  ...
                  else
                  {
                         ...
                         )
                      {
                          ...
                          if (stGenSetting.g_SysSetting.fEnableTTXSubTitle && !MApp_TTX_IsChInfoOn()&&(!bIsBlocked || (bIsBlocked && bStopMonitorBlock))&&(enTVRetVal != EXIT_GOTO_INPUTSOURCE)
                            && (msAPI_Scaler_GetScreenMute(MAIN_WINDOW) == E_SCREEN_MUTE_INIT))
                    #endif
                          {
                ...
                )
        {
            ...
            else
            {
                ...
                else if (MApp_Scramble_GetCurStatus() && msAPI_CI_CardDetect())
                {
                    ...
                    {
                        //Fix mantis: 1467680....
                        if ((msAPI_Timer_DiffTimeFromNow(u32TVCheckScrambleTimer) > TV_MODE_WINDOW_CHECK_SCRABLE_PERIOD)) //unit = ms
                            SYS_SCREEN_SAVER_TYPE(eWindow) = EN_SCREENSAVER_SCRAMBLED_PROGRAM;
                    }
    ...
    //#if(ENABLE_DVB)
                ...
                )
        {
            ...
            MS_DEBUG_MSG(printf("EN_SCREENSAVER_BLOCKRATING: Someting need to do??\n"));
                 ...
                 && (MApp_TV_IsProgramRunning())
                 && (msAPI_CM_GetCurrentServiceType() == E_SERVICETYPE_RADIO)
                 && (msAPI_CM_GetVideoPID(msAPI_CM_GetCurrentServiceType(),msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType())) == INVALID_PID)
                 && (SYS_SCREEN_SAVER_TYPE(eWindow) != EN_SCREENSAVER_AUDIOONLY)
                 && (MApp_SI_Get_PSISIVersion(E_VER_PMT)!=INVALID_VERSION_NUM)
...
//==========================================================================
#if (MHEG5_ENABLE)
    if ( MApp_MHEG5_Dsmcc_StartTime_Get() > 0//&&MApp_IsSrcHasSignal(MAIN_WINDOW)
      #if (ENABLE_PIP)
       &&(MApp_Get_PIPMode() == EN_PIP_MODE_OFF)
      #endif
       )
...
static void MApp_TV_Monitor(SCALER_WIN eWindow)
{
  ...
  #if ENABLE_PVR
    if (  (MApp_PVR_StateMachineGet() == E_PVR_STATE_RECORD ||MApp_PVR_StateMachineGet() == E_PVR_STATE_RECORDnPLAYBACK || MApp_PVR_StateMachineGet() == E_PVR_STATE_TIMESHIFT)
       && (MApp_Get_MonitorParental() == TRUE)
...
BOOLEAN MApp_TV_ProcessMessageBoxKey(U8 key)
{
    ...
    if (MApp_ZUI_ACT_ExecuteWndAction(EN_EXE_QUERY_IS_POWER_OFF_COUNTDOWN_MSG_BOX))
    {
        ...
        else if((enIndicateTimerSource  == TS_TYPE_SLEEPTIMER_NOSIGNAL)&&(key != KEY_POWER))
        {
            key = KEY_EXIT;
            enIndicateTimerSource = TS_TYPE_NONE;
        }
...
void MApp_TV_Process_Key_Select_For_OSD_TenKey(void)
{
#if ENABLE_ATSC
...
#endif
    {
       if((bIsBlocked == FALSE) || (bStopMonitorBlock == TRUE)||(MApp_PVR_IsRecording()&&(u8KeyCode==KEY_STOP)))   //cannot operate PVR in block program
         {
  ...
  if((u8KeyCode!=KEY_INFO)&&(u8KeyCode!=KEY_SELECT)&&(u8KeyCode!=KEY_MENU)&&(u8KeyCode!=KEY_NULL))
    ...
    switch ( u8KeyCode )
    {
        ...
        case DSC_KEY_PWROFF:
            ...
            )
            {
                ...
                {
                    if((msAPI_Scaler_GetScreenMute(MAIN_WINDOW) & (E_SCREEN_MUTE_RATING|E_SCREEN_MUTE_BLOCK)))
                    {
                        MApp_ChannelChange_CheckBlockChannelPW();
                    }
                }
        ...
        case KEY_SELECT:
            ...
            {
            #if 1
                MApp_TV_Process_Key_Select_For_OSD_TenKey();
            #else
            #if ENABLE_ATSC
                if(IsAtscInUse()||IsATVInUse())
                {
                ...
                //MApp_ZUI_ACT_ExecuteWndAction(EN_EXE_SHOW_TV_BANNER);
            #endif
            } // (MApp_ZUI_GetActiveOSD() == E_OSD_TENKEY_NUMBER)
        #if ENABLE_PVR
          #ifndef PVR_8051
        ...
        #endif // #if(ENABLE_DVB_OR_ATVONLY)
            }
            else
            {
                MApp_ZUI_ACT_StartupOSD(E_OSD_MESSAGE_BOX);
    ...
    break;
        ...
        case KEY_EPG:
              ...
              if (IsAtscInUse() || IsATVInUse())
              {
                  U16 u16SrvNum = MApp_ChanProc_GetNumOfServices(MAIN_LIST);
...
EN_RET MApp_TV(void)
{
    ...
    switch ( enTVState )
    {
        case STATE_TV_INIT:
          ...
          #if ENABLE_CI_PLUS_V1_4
            if (DB_CI_PLUS_SETTING.u8Opv2InstallSource == (U8)UI_INPUT_SOURCE_TYPE && EN_OP_TYPE_2 == MApp_CI_GetOperatorProfileType())
            {
