...
#ifndef ENABLE_SGP_MDA_CHAR_MAPPING
#define ENABLE_SGP_MDA_CHAR_MAPPING     DISABLE
#endif
...
static EN_CHARACTER_CODE_TABLE MApp_CharTable_Mapping_By_Country(void)
{
    switch (OSD_COUNTRY_SETTING)
    {
        ...
        #endif
            {
                if(OSD_COUNTRY_SETTING == OSD_COUNTRY_POLAND || OSD_COUNTRY_SETTING == OSD_COUNTRY_ROMANIA)
                {
                    return ISO_8859_02;
                }
                else if(OSD_COUNTRY_SETTING == OSD_COUNTRY_HUNGARY || OSD_COUNTRY_SETTING == OSD_COUNTRY_SWITZERLAND)
                {
                    return ISO_8859_09;
                }
                else
...
#if ENABLE_DMP
  #if (ENABLE_DTMB_CHINA_APP || ENABLE_ATV_CHINA_APP || ENABLE_DVB_TAIWAN_APP || ENABLE_DVBC_PLUS_DTMB_CHINA_APP||CHINESE_SIMP_FONT_ENABLE ||CHINESE_BIG5_FONT_ENABLE)
    #if ENABLE_SGP_MDA_CHAR_MAPPING
    if(IsStorageInUse())
    #endif
    {
        if(u81stByte > 0x1F)
