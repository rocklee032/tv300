...
static BOOLEAN g_ParentalRating_bGetTimeout = FALSE;
...
U8 MApp_ParentalRating_Get_Rating(void)
{
    ...
    if ((MApp_PVR_StateMachineGet() == E_PVR_STATE_RECORDnPLAYBACK && MApp_PVR_IsPlaybacking())
        ||((MApp_PVR_StateMachineGet() == E_PVR_STATE_TIMESHIFT) && (MApp_TimeShift_IfPause() == FALSE))
        )
    {
        u8Parental_Control = MApp_EITFileIn_GetCurParentControl();
    }
  #else
...
void MApp_ParentalRating_BlockStatus_Monitor(void)
{
    ...
    if(g_ParentalRating_u8PreParental_Control == u8Parental_Control)
    {
        ...
        if(g_ParentalRating_bResetAge)
        {
            ...
            if (msAPI_Timer_DiffTimeFromNow(g_ParentalRating_u32LockTimer) > 2500 )
            {
                 ...
                 g_ParentalRating_bGetTimeout = TRUE;
            }
        }
    #endif
        return;
    ...
    else
    {
        // Need unlock...
        #if ENABLE_CHG_CH_DEFAULT_LOCK
        if( TRUE == _bParentalBlock || g_ParentalRating_bGetTimeout )
        #else
        if( TRUE == _bParentalBlock )
        #endif
        {
            #if ENABLE_CHG_CH_DEFAULT_LOCK
            if(g_ParentalRating_bGetTimeout)
            {
                g_ParentalRating_bGetTimeout = FALSE;
            }
            #endif
            MApp_ParentalControl_SetBlockStatus(FALSE);
            _bParentalBlock = FALSE;
