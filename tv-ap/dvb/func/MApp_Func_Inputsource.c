...
#if ENABLE_CI_PLUS
BOOLEAN g_bOpModeSwitchSource = FALSE;
E_UI_INPUT_SOURCE g_eOpModeSource = UI_INPUT_SOURCE_NONE;
 #endif
...
#if ENABLE_CI_PLUS
    if(IsDTVInUse() && MApp_CI_IsOpMode() == TRUE)
    {
        MApp_CI_ExitCiOperatorProfile();
        g_bOpModeSwitchSource = TRUE;
        g_eOpModeSource = UI_INPUT_SOURCE_TYPE;
    }
#endif
     ...
     #if ENABLE_CI_PLUS
        if(g_eOpModeSource == MApp_InputSrc_Get_UiInputSrcType() && MApp_CI_IsOpMode() == FALSE && g_bOpModeSwitchSource)
        {
