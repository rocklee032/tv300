...
extern BOOLEAN g_bIsLockedOpen;//modify by munez
...
void MApp_ZUI_ACT_AppShowMessageBox(void)
{
    ...
    #endif
    {
        ...
        #if ENABLE_EWBS
        if(MApp_GetEwbsFlag() && stGenSetting.stMiscSetting.g_bEwbsMode)
        {
            MApp_ZUI_API_ShowWindow(HWND_MSGBOX_COMMON, SW_HIDE);
            MApp_ZUI_API_KillTimer(HWND_MSGBOX_COMMON_BG, 0);
        }
        ...
        #endif
        {
            MApp_ZUI_API_ShowWindow(HWND_EWBS_FRAME, SW_HIDE);
        }
        #endif
    #endif
    ...
    switch(act)
    {
       ...
       //     return TRUE;
        ...
        case EN_EXE_SHOW_PASSWORD_INPUT_MSGBOX:
            ...
            MApp_ZUI_API_ShowWindow(HWND_MSGBOX_COMMON_BTN_OK,SW_HIDE);
            MApp_ZUI_API_ShowWindow(HWND_MSGBOX_COMMON_BTN_OK, SW_SHOW);//modify by munez
            MApp_ZUI_API_ShowWindow(HWND_MSGBOX_USB_LIST_PANE, SW_HIDE);
        #if (UI_SKIN_SEL == UI_SKIN_1366X768X565)
            MApp_ZUI_API_ShowWindow(HWND_MSGBOX_COMMON_CONFIRM_MSG_BAR,SW_HIDE);
        #endif
...
#endif //(ENABLE_ATSC)
            {
...
#endif //((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
                {
                        ...
                        MApp_MenuFunc_Setup_BlockSysPassWord_Get())
                    {
                      ...
                      #endif
                      #if(ENABLE_PVR && (MEMORY_MAP == MMAP_128MB))
                        if(MApp_MenuFunc_Setup_BlockSysLockMode_Get() && g_bIsPvrPlay == TRUE)
                        {
                            g_bIsPvrPlay = FALSE;
                            g_bIsLockedOpen = TRUE;
...
#endif
                    MApp_ZUI_ACT_ExecuteMessageBoxAction(EN_EXE_CLOSE_CURRENT_OSD);//modify by munez
#if ENABLE_DMP
#if ENBALE_PARENTALRATING_MM
                    if(IsStorageInUse())
                    {
