...
void MApp_ZUI_SwUpdate_ProgressBar(U8 percent)
{
    if(_eCommonDlgMode == EN_COMMON_DLG_MODE_USB_UPGRADING)
    {
#if 1 //Only update the "%" number when software update
         USB_Upgrade_Percent = percent;
         MApp_ZUI_API_InvalidateWindow(HWND_MENU_DLG_COMMON_TEXT2);
         _MApp_ZUI_API_WindowProcOnIdle();
#else //Old method update all the things, it will bad when read string from flash while erasing flash
