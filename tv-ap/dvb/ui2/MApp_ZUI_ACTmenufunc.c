...
#if ENABLE_BAT_SCAN
extern U16 MApp_ZUI_ACT_GetRegionStringID(EN_REGION eRegion);
#endif
extern EN_MENU_STATE _enTargetMenuState;
extern LPTSTR MApp_ZUI_ACT_GetTimezoneString(EN_MENU_TIMEZONE timezone);
                ...
                if (act == EN_EXE_INC_COMMON_ADJ_ITEM5)
                {
                    stGenSetting.g_Time.cOnTimerChannel =
                        MApp_GetPrevNextProgramIndex_ByProgIndex(
                                    FALSE, stGenSetting.g_Time.cOnTimerChannel, FALSE);
                }
                else if (act == EN_EXE_DEC_COMMON_ADJ_ITEM5)
                {
                    stGenSetting.g_Time.cOnTimerChannel =
                        MApp_GetPrevNextProgramIndex_ByProgIndex(
                                    TRUE, stGenSetting.g_Time.cOnTimerChannel, FALSE);
                }
                break;
...
BOOLEAN MApp_ZUI_ACT_ExecuteMenuItemAction(U16 act)
{
    switch(act)
    {
    ...
    #if ENABLE_GAMEMODE
        case EN_EXE_ONOFF_GAMEMODE:
        {
            U8 u8GameModeStatus = !stGenSetting.g_SysSetting.u8GameMode;
            MApp_MenuFunc_Set_GameMode(u8GameModeStatus);
            MApp_ZUI_API_InvalidateWindow(HWND_MENU_PICTURE_GAMEMODE_OPTION);
            return TRUE;
        }
    #endif
#endif
     ...
     /*   case EN_EXE_GOTO_AUTOSLEEP_TIMER_PAGE:
        ...
        case EN_EXE_INC_TIMEZONE:
            ...
            stGenSetting.g_Time.u8TimeZoneMinutes = 0;
          ...
          #endif
            {
                ...
                else if(MENU_TIMEZONE>=TIMEZONE_GMT_12point75_START && MENU_TIMEZONE<=TIMEZONE_GMT_12point75_END)
                {
                    stGenSetting.g_Time.en_Clock_TimeZone=EN_Clock_TimeZone_48;
                    stGenSetting.g_Time.u8TimeZoneMinutes = 45;
                }
            #endif
            ...
            #if ENABLE_TIMEZONE_MINUTES
            MApp_SI_SetTimeZone_Minutes(stGenSetting.g_Time.u8TimeZoneMinutes);
            #endif
  ...
  if(IS_SBTVD_APP)
...
#endif
    {
        ...
        stGenSetting.g_BlockSysSetting.u8ParentalControl=
           (U8) MApp_ZUI_ACT_DecIncValue_Cycle(
                act==EN_EXE_INC_PARENTAL_GUIDANCE,
                stGenSetting.g_BlockSysSetting.u8ParentalControl,u8Min, u8Max, 1);
#endif
    }
...
//===
    ...
    #if ENABLE_EWBS
        case EN_EXE_EWBS_SELECT:
        {
            if (MApp_ZUI_API_GetFocus() == HWND_MENU_EWBS_INFO_EWBS)
            {
                CEC_UI_MSG( printf("\n [%d] HWND_MENU_EWBS_INFO_EWBS(%d) \n", __LINE__, DB_EWBS_SETTING.g_bEwbsMode) );

                if ( stGenSetting.stMiscSetting.g_bEwbsMode == ENABLE)
                {
                    // Move to MApp_EWBS_Set_Enable()
                    MApp_EWBS_Set_Enable(DISABLE);
                ...
                {
                    // Move to MApp_EWBS_Set_Enable()
                    MApp_EWBS_Set_Enable(ENABLE);

                    MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_MONITORING_CHANNEL, ENABLE);
                    MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_AREA_CODE, ENABLE);
                    MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_AUTOMATIC_STARTUP, ENABLE);
                    MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_NO_OPERATION_OFF, ENABLE);
                    MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_BUZZER_NOTIFICATION, ENABLE);
                }
            }
            else if(MApp_ZUI_API_GetFocus() == HWND_MENU_EWBS_INFO_MONITORING_CHANNEL)
            {
                MApp_ZUI_ACT_ExecuteMainMenuAction(EN_EXE_GOTO_MONITORING_CHANNEL_LIST);
            }
            else if(MApp_ZUI_API_GetFocus() == HWND_MENU_EWBS_INFO_AREA_CODE)
            {
            ...
            else if(MApp_ZUI_API_GetFocus() == HWND_MENU_EWBS_INFO_AUTOMATIC_STARTUP)
            {
                CEC_UI_MSG( printf("\n [%d] HWND_MENU_EWBS_INFO_AUTOMATIC_STARTUP \n", __LINE__) );

                if (stGenSetting.stMiscSetting.g_bEwbsAutomaticStartup == TRUE)
                    stGenSetting.stMiscSetting.g_bEwbsAutomaticStartup = FALSE;
                else
                    stGenSetting.stMiscSetting.g_bEwbsAutomaticStartup = TRUE;
            }
            else if(MApp_ZUI_API_GetFocus() == HWND_MENU_EWBS_INFO_NO_OPERATION_OFF)
            {
                CEC_UI_MSG( printf("\n [%d] HWND_MENU_EWBS_INFO_NO_OPERATION_OFF \n", __LINE__) );

                if (stGenSetting.stMiscSetting.g_bEwbsNoOperationOff == TRUE)
                    stGenSetting.stMiscSetting.g_bEwbsNoOperationOff = FALSE;
                else
                    stGenSetting.stMiscSetting.g_bEwbsNoOperationOff = TRUE;
            }
            else if(MApp_ZUI_API_GetFocus() == HWND_MENU_EWBS_INFO_BUZZER_NOTIFICATION)
            {
                CEC_UI_MSG( printf("\n [%d] HWND_MENU_EWBS_INFO_BUZZER_NOTIFICATION \n", __LINE__) );
                if (stGenSetting.stMiscSetting.g_bEwbsBuzzerNotification == TRUE)
                    stGenSetting.stMiscSetting.g_bEwbsBuzzerNotification = FALSE;
                else
                    stGenSetting.stMiscSetting.g_bEwbsBuzzerNotification = TRUE;
            }

            MApp_ZUI_API_InvalidateAllSuccessors(HWND_MENU_EWBS_INFO_PAGE);
            return TRUE;
        }
    #endif
...
U16 _MApp_ZUI_ACT_GetLanguageStringID(EN_LANGUAGE lang, BOOLEAN bDefaultEnglish)
{
    ...
    #if ENABLE_TDT
        case LANGUAGE_UND:
            u16TempID=en_str_Other_language;
            break;
    #endif
        case LANGUAGE_IRISH:
...
LPTSTR MApp_ZUI_ACT_GetMenuItemDynamicText(HWND hwnd)
{
    ...
    switch (hwnd)
    {
        ...
        #if ENABLE_BAT_SCAN
        case HWND_MENU_DLG_REGION_SELECT_TYPE_TEXT:
        {
            u16TempID = MApp_ZUI_ACT_GetRegionStringID(msAPI_GetRegion());
            break;
        }
        #endif
...
#if ( UI_SKIN_SEL == UI_SKIN_1920X1080X565 )
    #if(ENABLE_GAMEMODE)
    	case HWND_MENU_PICTURE_GAMEMODE_OPTION:
    	{
    	    if(MApp_MenuFunc_Get_GameMode()==DISABLE)
    	    {
                u16TempID=en_str_OFF_GameMode;
    	    }
    	    else
    	    {
                u16TempID=en_str_ON_GameMode;
    	    }
    	    break;
    	}
    #endif
        ...
        case HWND_MENU_TIME_SET_TIMEZONE_OPTION:
        {
          ...
          #endif
            {
                ...
                return MApp_ZUI_ACT_GetTimezoneString(stGenSetting.g_Time.enTimeZone);
            }
        ...
        case HWND_MENU_OPTION_COUNTRY_OPTION:
        {
            ...
            if (EnCountry < E_COUNTRY_NUM)
                u16TempID = MApp_ZUI_ACT_GetCountryStringID(EnCountry);
            break;
        }
    ...
    #if ENABLE_EWBS
        case HWND_MENU_EWBS_INFO_EWBS_OPTION:
            if (stGenSetting.stMiscSetting.g_bEwbsMode)
                u16TempID = en_str_On;
            else
                u16TempID = en_str_Off;
            break;

        case HWND_MENU_EWBS_INFO_AUTOMATIC_STARTUP_OPTION:
            if (stGenSetting.stMiscSetting.g_bEwbsAutomaticStartup)
                u16TempID = en_str_On;
            else
                u16TempID = en_str_Off;
            break;

        case HWND_MENU_EWBS_INFO_NO_OPERATION_OFF_OPTION:
            if (stGenSetting.stMiscSetting.g_bEwbsNoOperationOff)
                u16TempID = en_str_On;
            else
                u16TempID = en_str_Off;
            break;

        case HWND_MENU_EWBS_INFO_BUZZER_NOTIFICATION_OPTION:
            if (stGenSetting.stMiscSetting.g_bEwbsBuzzerNotification)
                u16TempID = en_str_On;
            else
                u16TempID = en_str_Off;
            break;
        ...
        {
            return MApp_ZUI_API_GetU16String(u8Temp_EWBS_Num1);
        }

        case  HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_2:
        {
            return MApp_ZUI_API_GetU16String(u8Temp_EWBS_Num2);
        }

        case  HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_3:
        {
            return MApp_ZUI_API_GetU16String(u8Temp_EWBS_Num3);
        }

        case  HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_4:
        {
            return MApp_ZUI_API_GetU16String(u8Temp_EWBS_Num4);
        }
    #endif
...
GUI_ENUM_DYNAMIC_LIST_STATE MApp_ZUI_ACT_QueryMainMenuItemStatus(HWND hwnd)
{
    ...
    #if ( UI_SKIN_SEL == UI_SKIN_1920X1080X565 )
        case HWND_MENU_PICTURE_GAMEMODE:
        #if(ENABLE_GAMEMODE)
            if((!MApp_IsSrcHasSignal(MAIN_WINDOW))||
                (MApi_XC_Sys_Is_SupportGameMode() == FALSE))
...
////==================================================================
    ...
    //==========================================================================
        ...
        #if (ENABLE_DVBC)
          #if ENABLE_ZIGGO
            if(IsDVBCInUse() && Mapp_SI_Get_Ziggo_ENABLE() == TRUE)
            {
                return EN_DL_STATE_DISABLED;
            }
            else
