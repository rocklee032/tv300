...
BOOLEAN _GetSatInforByID(MS_U8 u8SATId,MS_SAT_PARAM *pSATParam)//»ñµÃÎÀÐÇµÄÐÅÏ¢
{
    MS_BOOL bRet =TRUE;
...
static BOOLEAN _AddSatInfor(MS_U8* u8SatID)//Ìí¼ÓÎÀÐÇÐÅÏ¢
{
    return MApi_DB_SAT_AddSAT(u8SatID,&SatInfor);
}

static BOOLEAN _SaveSatInfor(MS_U8 u8SATIdx)//´æ´¢ÎÀÐÇµÄÐÅÏ¢
{
...
static LPTSTR _GetTPListTxt(MS_U16 idx)
{
    ...
    memset( &S2_tp_info, 0, sizeof(DTVPROGRAMID_M)); //For coverity
...
static void _AddScanSatelliteAndTP(void)
{
    ...
    if(u8CurEditListType == TYPE_TPLIST)
    {
        ...
        memset( &stCarrier, 0, sizeof(DTVPROGRAMID_M)); //For coverity
    ...
    switch(S2_enScanType)
    {
        ...
        case SCAN_BAT_FAST:
        {
            ...
            {
                SET_OSD_MENU_REGION(SI_REGION_NETHERLANDS);
            }
    ...
    switch (key)
    {
        ...
        case VK_SELECT:
        {
            ...
            if(_bAddSATOrTP)
            {
                ...
                {
                    MS_U8 u8SatID = 0xFF; //For coverity
...
LPTSTR MApp_ZUI_ACT_GetDishSetupMenuDynamicText(HWND hwnd)
{
    ...
    switch (hwnd)
    {
        ...
        case HWND_MENU_DISHSETUP_SETLIMIT_MOVESTEP_TEXT:
        {
            ...
            sprintf((char*)u8Str, "(%d)", _u8MoveSteps);
