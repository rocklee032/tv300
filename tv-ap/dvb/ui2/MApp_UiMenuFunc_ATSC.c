...
BOOLEAN MApp_UiMenuFunc_ToggleRRT_DimensionValues(U8 u8ValueIndex)
{
    if( (u8ValueIndex<1)||(u8ValueIndex>REGION5_MAX_RATING_OPTION)||
        (g_u8CurrentRRT_Dimension>=REGION5_DIMENSION_NUM)
      ) // error checking
    {
        return FALSE;
    }
...
//*************************************************************************************************
void MApp_UiMenuFunc_ConvertTempChannelProgramNumber2Str(MS_VIRTUAL_CHANNEL * pstVirCh, U8 *pu8Str)
{
    MS_VIRTUAL_CHANNEL_NUM *pstVirtualChNum = &(pstVirCh->stVirtualChNum);
    U8 u8NoOfDigit;
    ...
    {
        if( !MApp_ChanProc_Is_ChannelAtv(pstVirCh) )
        {
