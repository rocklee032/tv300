...
static LPTSTR _MApp_SubtitleLanguage_MapFullLangName(U8 UI_Lang_IDX)
{
    ...
    {
        if(u32StringCodes == STRING_TO_U32('u','n','d') || u32StringCodes == STRING_TO_U32('U','N','D'))
        {
            ...
            if(TRUE == MApp_CI_GetOpIso639LangCodeByCicamId(MApp_CI_GetCurrentOpCicamId(), au8IsoLang))
            {
                ...
                LPTSTR str_lang = MApp_ZUI_ACT_GetMenuLanguageStringByIndex(MApp_GetLanguageBySILanguage((EN_SI_LANGUAGE)eLangcodeIndex));
                if(str_lang == 0)
                {
                    u32StringCodes = STRING_TO_U32('U','N','D');
                }
                else
                {
                    return str_lang;
    ...
    switch(act)
    {
        ...
        case EN_EXE_SUBTITLE_LANG_DOWN:
            ...
            if(u8SubtitleItemSelIdx != 0)
            {
              ...
              #if (ENABLE_SUBTITLE_SWITCH_MENU && !ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
