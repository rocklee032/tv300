...
void MApp_ZUI_ACT_AppShowCadtvManualTuning(void)
{
  ...
  #if ENABLE_ZIGGO
    if(Mapp_SI_Get_Ziggo_ENABLE() == FALSE)
  #endif
    {
...
BOOLEAN MApp_ZUI_ACT_ExecuteCadtvManualTuningAction(U16 act)
{
    switch(act)
    {
        ...
        case EN_EXE_GOTO_MAINMENU:
            ...
            if((MApp_DVBC_SCAN_GetCurrentType() == OSD_DVBC_TYPE_NETWOEKSCAN)
               &&(MApp_CadtvManualTuning_GetNID() == 0 || MApp_CadtvManualTuning_GetNID() == DEFAULT_DVBC_NID))
            {
                MApp_CadtvManualTuning_SetNID(0);
                MApp_ZUI_API_InvalidateWindow(HWND_MENU_DLG_DVBC_SCAN_NETWORKID_OPTION);
            }
            else
          #endif
            {
        ...
        case EN_EXE_INC_DVBC_SCAN_TYPE:
                ...
                {
                    eDVBScanType = OSD_DVBC_TYPE_NETWOEKSCAN;
                }
                else
            #endif
                {
    ...
    switch(hwnd)
    {
        ...
        case HWND_MENU_DLG_DVBC_SCAN_NETWORKID_OPTION:
           if(DVBCNetworkIdScanType == 0)
           {
                ...
                {
                  #if DVBC_CERTIFICATION_AUTOSCAN_ENABLE
                    if(MApp_CadtvManualTuning_GetNID() == 0)
                    {
                        U16 u16DefaultStr[6] = {0x30,0x30,0x30,0x30,0x30,0};
                        memcpy(CHAR_BUFFER,u16DefaultStr,sizeof(u16DefaultStr));
                        return CHAR_BUFFER;
                    }
                    else
                  #endif
...
void MApp_DVBC_SCAN_SetCurrentType(EN_OSD_DVBC_SCAN_TYPE eScanType)
{
     eDVBScanType = eScanType;
}
