...
EN_LANGUAGE _ZUI_TBLSEG stNzOsdAudLangList[] =
{
  ...
  #if 1 //(ENABLE_INDIA_OSD) // mantis 1483750: [Marlon U01][DVB][Freeview NZ_SI] - nz_si02_109
    LANGUAGE_HINDI,
  #endif
...
#if(DVBC_CERTIFICATION_AUTOSCAN_ENABLE)
U16 _ZUI_TBLSEG stAutoTuningCertificationList[] =
{
    OSD_CERTIFICATION_OTHER,
    OSD_CERTIFICATION_UNITYMEDIA,
    OSD_CERTIFICATION_ZIGGO,
    OSD_CERTIFICATION_UPC,
    OSD_CERTIFICATION_VIRGINMEDIA,
    OSD_CERTIFICATION_CABLEREADY,
#if ENABLE_COMHEM
    OSD_CERTIFICATION_COMHEM,
#endif
#if ENABLE_STOFA
    OSD_CERTIFICATION_STOFA,
#endif
};
...
ST_OSD_SELECTION_STRING_MAPPING_LIST _ZUI_TBLSEG stAutoTuningUpcCountryList[] =
{
    {OSD_COUNTRY_AUSTRIA, en_str_Australia},
    {OSD_COUNTRY_HUNGARY, en_str_Hungary},
    {OSD_COUNTRY_NETHERLANDS, en_str_Netherlands},
    {OSD_COUNTRY_POLAND, en_str_Poland},
    {OSD_COUNTRY_ROMANIA, en_str_Romanian},
    {OSD_COUNTRY_SWITZERLAND, en_str_Switzerland},
};

ST_OSD_SELECTION_STRING_MAPPING_LIST _ZUI_TBLSEG stAutoTuningVirginMediaCountryList[] =
{
    {OSD_COUNTRY_IRELAND, en_str_Ireland},
};


ST_OSD_SELECTION_STRING_MAPPING_LIST _ZUI_TBLSEG stAutoTuningCableReadyCountryList[] =
{
    {OSD_COUNTRY_FINLAND, en_str_Finland},
};

ST_OSD_SELECTION_STRING_MAPPING_LIST _ZUI_TBLSEG stAutoTuningComhemCountryList[] =
{
    {OSD_COUNTRY_SWEDEN, en_str_Sweden},
};

ST_OSD_SELECTION_STRING_MAPPING_LIST _ZUI_TBLSEG stAutoTuningStofaCountryList[] =
{
    {OSD_COUNTRY_DENMARK, en_str_Denmark},
};
...
{
    if (TuningCountry >= E_COUNTRY_NUM)
...
void MApp_ZUI_ACT_SetTuningCountryIndex(U8 u8Index)
{
        ...
        if(IsDVBCInUse())
        {
            switch(OSD_CERTIFICATION_SETTING)
            {
                ...
                {
                    if (u8Index >= COUNTOF(stAutoTuningCountryList))
                    {
                ...
                {
                    if (u8Index >= COUNTOF(stAutoTuningUnityMediaCountryList))
                    {
                        _eTuningCountry = DEFAULT_TUNING_UNITYMEDIA_COUNTRY;
                    }
                    else
                    {
                        _eTuningCountry = (EN_OSD_COUNTRY_SETTING)stAutoTuningUnityMediaCountryList[u8Index].u16SelectionItem;
                    }
                    break;
                }
                case OSD_CERTIFICATION_ZIGGO:
                {
                    if (u8Index >= COUNTOF(stAutoTuningZiggoCountryList))
                    {
                        _eTuningCountry = DEFAULT_TUNING_ZIGGO_COUNTRY;
                    }
                    else
                    {
                        _eTuningCountry = (EN_OSD_COUNTRY_SETTING)stAutoTuningZiggoCountryList[u8Index].u16SelectionItem;
                    }
                    break;
                }
                case OSD_CERTIFICATION_UPC:
                {
                    if (u8Index >= COUNTOF(stAutoTuningUpcCountryList))
                    {
                        _eTuningCountry = DEFAULT_TUNING_UPC_COUNTRY;
                    }
                    else
                    {
                        _eTuningCountry = (EN_OSD_COUNTRY_SETTING)stAutoTuningUpcCountryList[u8Index].u16SelectionItem;
                    }
                    break;
                }
                case OSD_CERTIFICATION_VIRGINMEDIA:
                {
                    if (u8Index >= COUNTOF(stAutoTuningVirginMediaCountryList))
                    {
                        _eTuningCountry = DEFAULT_TUNING_VIRGINMEDIA_COUNTRY;
                    }
                    else
                    {
                        _eTuningCountry = (EN_OSD_COUNTRY_SETTING)stAutoTuningVirginMediaCountryList[u8Index].u16SelectionItem;
                    }
                    break;
                }
                case OSD_CERTIFICATION_CABLEREADY:
                {
                    if (u8Index >= COUNTOF(stAutoTuningCableReadyCountryList))
                    {
                        _eTuningCountry = DEFAULT_TUNING_CABLEREADY_COUNTRY;
                    }
                    else
                    {
                        _eTuningCountry = (EN_OSD_COUNTRY_SETTING)stAutoTuningCableReadyCountryList[u8Index].u16SelectionItem;
                    }
                    break;
                }
                case OSD_CERTIFICATION_COMHEM:
                {
                    if (u8Index >= COUNTOF(stAutoTuningComhemCountryList))
                    {
                        _eTuningCountry = DEFAULT_TUNING_COMHEM_COUNTRY;
                    }
                    else
                    {
                        _eTuningCountry = (EN_OSD_COUNTRY_SETTING)stAutoTuningComhemCountryList[u8Index].u16SelectionItem;
                    }
                    break;
                }
                case OSD_CERTIFICATION_STOFA:
                {
                    if (u8Index >= COUNTOF(stAutoTuningStofaCountryList))
                    {
                        _eTuningCountry = DEFAULT_TUNING_STOFA_COUNTRY;
                    }
                    else
                    {
                        _eTuningCountry = (EN_OSD_COUNTRY_SETTING)stAutoTuningStofaCountryList[u8Index].u16SelectionItem;
                    }
                    break;
                }
                default:
...
U8 MApp_ZUI_ACT_GetTuningCountryIndexMax(void)
{
        ...
        if(IsDVBCInUse())
        {
            switch(OSD_CERTIFICATION_SETTING)
            {
                ...
                case OSD_CERTIFICATION_UPC:
                {
                    return (U8)(sizeof(stAutoTuningUpcCountryList)/sizeof(stAutoTuningUpcCountryList[0])) - 1;
                }
                case OSD_CERTIFICATION_VIRGINMEDIA:
                {
                    return (U8)(sizeof(stAutoTuningVirginMediaCountryList)/sizeof(stAutoTuningVirginMediaCountryList[0])) - 1;
                }
                case OSD_CERTIFICATION_CABLEREADY:
                {
                    return (U8)(sizeof(stAutoTuningCableReadyCountryList)/sizeof(stAutoTuningCableReadyCountryList[0])) - 1;
                }
                case OSD_CERTIFICATION_COMHEM:
                {
                    return (U8)(sizeof(stAutoTuningComhemCountryList)/sizeof(stAutoTuningComhemCountryList[0])) - 1;
                }
                case OSD_CERTIFICATION_STOFA:
                {
                    return (U8)(sizeof(stAutoTuningStofaCountryList)/sizeof(stAutoTuningStofaCountryList[0])) - 1;
                }
                default:
                    break;
            }
    ...
    for (u8Index = 0; u8Index <= MApp_ZUI_ACT_GetTuningCountryIndexMax(); u8Index++)
    {
        ...
        if(IsDVBCInUse())
        {
            switch(OSD_CERTIFICATION_SETTING)
            {
                ...
                case OSD_CERTIFICATION_UNITYMEDIA:
                {
                    if (_eTuningCountry == (EN_OSD_COUNTRY_SETTING)stAutoTuningUnityMediaCountryList[u8Index].u16SelectionItem)
                    {
                        return u8Index;
                    }
                    break;
                ...
                case OSD_CERTIFICATION_ZIGGO:
                {
                    if (_eTuningCountry == (EN_OSD_COUNTRY_SETTING)stAutoTuningZiggoCountryList[u8Index].u16SelectionItem)
                    {
                        return u8Index;
                    }
                    break;
                ...
                case OSD_CERTIFICATION_UPC:
                {
                    if (_eTuningCountry == (EN_OSD_COUNTRY_SETTING)stAutoTuningUpcCountryList[u8Index].u16SelectionItem)
                    {
                        return u8Index;
                    }
                    break;
                ...
                case OSD_CERTIFICATION_VIRGINMEDIA:
                {
                    if (_eTuningCountry == (EN_OSD_COUNTRY_SETTING)stAutoTuningVirginMediaCountryList[u8Index].u16SelectionItem)
                    {
                        return u8Index;
                    }
                    break;
                ...
                case OSD_CERTIFICATION_CABLEREADY:
                {
                    if (_eTuningCountry == (EN_OSD_COUNTRY_SETTING)stAutoTuningCableReadyCountryList[u8Index].u16SelectionItem)
                    {
                        return u8Index;
                    }
                    break;
                ...
                case OSD_CERTIFICATION_COMHEM:
                {
                    if (_eTuningCountry == (EN_OSD_COUNTRY_SETTING)stAutoTuningComhemCountryList[u8Index].u16SelectionItem)
                    {
                        return u8Index;
                    }
                    break;
                ...
                case OSD_CERTIFICATION_STOFA:
                {
                    if (_eTuningCountry == (EN_OSD_COUNTRY_SETTING)stAutoTuningStofaCountryList[u8Index].u16SelectionItem)
                    {
                        return u8Index;
                    }
                    break;
...
{
    if (stAutoTuningCertificationList[u8Index] >= OSD_CERTIFICATION_NUMS)
    {
        MS_DEBUG_MSG(printf("Set Certification Overflow: %u\n", (U16)stAutoTuningCertificationList[u8Index]));
        _eTuningCertification = DEFAULT_TUNING_CERTIFICATION;
    }
    else
    {
        _eTuningCertification = (EN_OSD_CERTIFICATION_SETTING)stAutoTuningCertificationList[u8Index];
    }
...
U8 MApp_ZUI_ACT_GetTuningCertificationIndex(void)
{
    ...
    {
        if (_eTuningCertification == (EN_OSD_CERTIFICATION_SETTING)stAutoTuningCertificationList[u8Index])
        {
            return u8Index;
        }
    }
...
LPTSTR MApp_ZUI_ACT_GetCertificationStringByIndex(U8 u8Index)
{
    ...
    switch(stAutoTuningCertificationList[u8Index])
    {
        case OSD_CERTIFICATION_OTHER:
            u16TempID = en_str_Other;
            break;

        case OSD_CERTIFICATION_UNITYMEDIA:
            u16TempID = en_str_Unitymedia;
            break;

        case OSD_CERTIFICATION_ZIGGO:
            u16TempID = en_str_Ziggo;
            break;

        case OSD_CERTIFICATION_UPC:
            return MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8*)"UPC", STRING_BUFFER_LENGTH);
            break;

        case OSD_CERTIFICATION_VIRGINMEDIA:
            return MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8*)"Virgin Media", STRING_BUFFER_LENGTH);
            break;

        case OSD_CERTIFICATION_CABLEREADY:
            return MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8*)"Cable Ready", STRING_BUFFER_LENGTH);
            break;

        case OSD_CERTIFICATION_COMHEM:
            return MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8*)"ComHem", STRING_BUFFER_LENGTH);

        case OSD_CERTIFICATION_STOFA:
            return MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8*)"Stofa", STRING_BUFFER_LENGTH);
    }
...
LPTSTR MApp_ZUI_ACT_GetCountryStringByIndex(U8 u8Index)
{
    ...
    if(IsDVBCInUse())
    {
        switch(OSD_CERTIFICATION_SETTING)
        {
            ...
            case OSD_CERTIFICATION_UPC:
            {
                u16TempID = stAutoTuningUpcCountryList[u8Index].u16StringIndex;
                break;
            }
            case OSD_CERTIFICATION_VIRGINMEDIA:
            {
                u16TempID = stAutoTuningVirginMediaCountryList[u8Index].u16StringIndex;
                break;
            }
            case OSD_CERTIFICATION_CABLEREADY:
            {
                u16TempID = stAutoTuningCableReadyCountryList[u8Index].u16StringIndex;
                break;
            }
            case OSD_CERTIFICATION_COMHEM:
            {
                u16TempID = stAutoTuningComhemCountryList[u8Index].u16StringIndex;
                break;
            }
            case OSD_CERTIFICATION_STOFA:
            {
                u16TempID = stAutoTuningStofaCountryList[u8Index].u16StringIndex;
                break;
            }
