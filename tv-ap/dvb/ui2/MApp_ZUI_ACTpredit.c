...
static void _MApp_ProgramEdit_Handler(U16 u16ActiveProgOrder, U16 u16ShiftProgIndex, U16 u16TotalSrvCount, U8 u8SrvCommand)
{
    ...
    switch (u8SrvCommand)
    {
        ...
        case PROGRAM_EDIT_COMMAND_DELETE:
        {
        ...
        #endif
            {
                ...
                if (TRUE == MApp_CI_IsOpMode())
                {
                    ...
                    msAPI_CM_GetProgramInfo(u8Type, u16CurPosition, &stChannelInfo);
                    if (TRUE == stChannelInfo.stCHAttribute.bIsOpService)
                    {
                        printf("[CI+][OP] OP Service Can't Be Delete \n");
                        return;
                    }
                }
            #endif
        ...
        #if ENABLE_DTV
            if( (u8Type==SERVICE_TYPE_DTV)
              || (u8Type==SERVICE_TYPE_RADIO)
          #if NORDIG_FUNC //for Nordig spec v2.0
              || (u8Type==SERVICE_TYPE_DATA)
          #endif
              )
            {
                for(Index = 0;Index < MApp_EpgUI_GetNumOfTimers();)
                {
                    const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(Index);
                    ...
                    msAPI_CM_GetServiceTypeAndPositionWithPCN( pstMS_EPG_TIMER->u8PCN, pstMS_EPG_TIMER->u16SrvID, &srvType, &srvPos);
...
#if ( ENABLE_RCSRDS_C )     // RCS/RDS-C certi. case [TC16082600575], forbid dele behavior.
    if ( msAPI_CM_GetCountry() == E_ROMANIA )
        return;
#endif  //ENABLE_RCSRDS_C
