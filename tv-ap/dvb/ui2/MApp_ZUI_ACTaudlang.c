        ...
        if (str_lang == 0)
        {
            ...
            if(msAPI_SI_GetISOLangCodeFromIndex((EN_SI_LANGUAGE)stAudInfo.aISOLangInfo[0].bISOLangIndex, au8Code) == TRUE)
            {
                ...
                if( MApp_CI_IsOpMode() == TRUE)
                {
                    ...
                    {
                        U8 au8IsoLang[MAX_ISO639CODE_LENGTH];
                        EN_SI_LANGUAGE eLangcodeIndex = SI_LANGUAGE_UND;
                        if(TRUE == MApp_CI_GetOpIso639LangCodeByCicamId(MApp_CI_GetCurrentOpCicamId(), au8IsoLang))
                        {
                            eLangcodeIndex = msAPI_SI_GetLanguageByISO639LangCode(au8IsoLang);
                            str_lang = MApp_ZUI_ACT_GetMenuLanguageStringByIndex(MApp_GetLanguageBySILanguage((EN_SI_LANGUAGE)eLangcodeIndex));
                            if(str_lang == 0)
                            {
                                str[0] = 'U';
                                str[1] = 'N';
...
GUI_ENUM_DYNAMIC_LIST_STATE MApp_ZUI_ACT_QueryAudioLanguageItemStatus(HWND hwnd)
{
    ...
    if( _MApp_ZUI_ACT_AudioLanguageWindowMapToIndex(hwnd) < MApp_ChanProc_Get_CurCh_AudLangNum() )
    {
...
BOOLEAN MApp_ZUI_ACT_ExecuteAudioLanguageAction(U16 act)
{
    ...
    switch(act)
    {
        ...
        #if 1 // Use new API
            MApp_Audio_Set_LRMode( (EnuAppAudioLRMode)g_u8LRAudioMode );
        #else
            if(g_u8LRAudioMode == 0)
            {
        ...
        case EN_EXE_AUDIO_LANG_DOWN:                ///< audio language down
        {
            ...
            if(IsAtscInUse() || IsATVInUse())
            {
                U8 u8AudLangNum = MApp_ChanProc_Get_CurCh_AudLangNum();
          ...
          #if ( ENABLE_DVB )
            {
                if ( IsDVBInUse() )
                {
                    ...
                    {   
                        ...
                        if (u8CurrentItemIndex != msAPI_CM_GetAudioLangIndex(msAPI_CM_GetCurrentServiceType(), msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType())))
                        {
                            msAPI_CM_SetAudioLangIndex(msAPI_CM_GetCurrentServiceType(), msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType()),u8CurrentItemIndex);
                        }
                      #endif
                      #if(ENABLE_DVB)
...
{
    //if (u8index >= MApp_ChanProc_GetCurChannelPointer()->u8AudLangNum)
    if (u8index >= MApp_ChanProc_Get_CurCh_AudLangNum() )
        return FALSE;
