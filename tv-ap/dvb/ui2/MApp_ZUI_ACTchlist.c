...
#if ENABLE_EWBS
static BOOLEAN _MApp_ZUI_ACT_IsMonitoringMode(void)
{
    return (_eChannelListMode == MODE_CHLIST_TV_MONITORING);
}
#endif
...
void MApp_ZUI_ACT_AppShowChannelList(void)
{
  ...
  #if ENABLE_CI_PLUS
    U8 u8OpNum = MApp_CI_GetValidOpCacheCount();    //Evie++:get num of op index
  #endif
...
*/
    ...
    #if ENABLE_EWBS
        case MODE_CHLIST_TV_MONITORING:
            _MApp_ChannelList_Init(SERVICE_TYPE_DTV);
            MApp_ChannelList_SetMode(MODE_CHLIST_TV_MONITORING);
            break;
    #endif
    ...
    if ( (u8OpNum>0) && (MApp_CI_IsOpMode()==FALSE) && IsDTVInUse() )
    {
        ...
        {
            MApp_ZUI_API_ShowWindow(HWND_CHLIST_BOTTOM_HALF_OP_MENU_BTN, SW_HIDE); // Hide OP Menu Button
        }
        else
#endif
        {
    ...
    else if ( (u8OpNum>0) && (MApp_CI_IsOpMode()==TRUE) && IsDTVInUse() )
    {
        ...
        if (EN_OP_TYPE_2 == MApp_CI_GetOperatorProfileType())
        {
            //[OP-0031] OPv2 Auto Enter OP, Send OPNitManagement, But CICAM Not Ready To Accept
            //          So Need Show Channel List, Send Again
...
BOOLEAN MApp_ZUI_ACT_ExecuteChannelListAction(U16 act)
{
    ...
    switch (act)
    {
        ...
        case EN_EXE_CHLIST_SELECT:
            {
                ...
                #endif
                    {
                #if(ENABLE_DVB_OR_ATVONLY)
                    #if ENABLE_EWBS
                    if(_MApp_ZUI_ACT_IsMonitoringMode())
                        {
                            MEMBER_SERVICETYPE bServiceType = msAPI_CM_GetCurrentServiceType();
                            WORD wCurrentPosition = msAPI_CM_GetPositionbyListPosition(u16ListOrdinal,(MEMBER_SERVICETYPE *)&bServiceType);
                            WORD wService_ID = msAPI_CM_GetService_ID(bServiceType, wCurrentPosition);
                            WORD wTS_ID = msAPI_CM_GetTS_ID(bServiceType, wCurrentPosition);
                            WORD wON_ID = msAPI_CM_GetON_ID(bServiceType, wCurrentPosition);
                            stGenSetting.stMiscSetting.wMonitoringServiceID = wService_ID;
                            stGenSetting.stMiscSetting.wMonitoringTSID = wTS_ID;
                            stGenSetting.stMiscSetting.wMonitoringONID = wON_ID;
                        }else
                    #endif
        ...
        case EN_EXE_SORT_SELECT:
        {
            ...
            WORD CurrentS2idx = msAPI_CM_GetPhysicalidx(cbServiceType,cwCurrentPosition);
    ...
    switch(key)
    {
       ...
       case VK_UP:
        ...
        case VK_VOLUME_MINUS:
            MApp_FuncExec_AdjustVolume(key);
            return TRUE;
            break;
...
LPTSTR MApp_ZUI_ACT_GetChannelListDynamicText(HWND hwnd)
{
    ...
    switch(hwnd)
    {
       ...
       case HWND_MENU_CHLIST_SOFTKEYBOARD_TITLE_M:
        ...
        #if ENABLE_EWBS
            else if(_MApp_ZUI_ACT_IsMonitoringMode())
            {
                u16TempID = en_str_Monitoring_List;
            }
        #endif
          #if ENABLE_CI_PLUS
...
#if ENABLE_CI_PLUS_V1_4
            U8 u8ProfileType = 0;
            MApp_CI_GetProfileTypeByIndex(wProgramPosition, &u8ProfileType);
            if (EN_OP_TYPE_2 == u8ProfileType)
            {
                break;
            }
#endif
        ...
        case HWND_CHLIST_ITEM9_TEXT:
        {
           ...
           if (IsAtscInUse() || IsATVInUse())
           {
                ...
                MS_VIRTUAL_CHANNEL stVIRTUAL_CHANNEL_Tmp;
                memset(&stVIRTUAL_CHANNEL_Tmp, 0, sizeof(MS_VIRTUAL_CHANNEL));
                if( !msAPI_ChProc_ATSC_Get_VirCh_By_MainListIdx( u16MainListIdx, &stVIRTUAL_CHANNEL_Tmp) )
                {
                    printf("\nError: Get VirCh failed! u16MainListIdx=%u\n", u16MainListIdx);
                }

        //Channel number
        #if 1
                //MS_VIRTUAL_CHANNEL_NUM stMS_VIRTUAL_CHANNEL_NUM_Tmp;
                //memset( &stMS_VIRTUAL_CHANNEL_NUM_Tmp, 0, sizeof(MS_VIRTUAL_CHANNEL_NUM));
                //msAPI_ChProc_ATSC_Get_VirChNum_By_ProgPos( u16ProgPosition, &stMS_VIRTUAL_CHANNEL_NUM_Tmp );
                MApp_UiMenuFunc_ConvertProgramNumber2Str( &(stVIRTUAL_CHANNEL_Tmp.stVirtualChNum), au8Section);
        #else
            #if ENABLE_ATSC_CHLIST_DISP_TOGETHER
        ...
        //Channel name
        #if 1
                pu8String = stVIRTUAL_CHANNEL_Tmp.acServiceName;
        #else
            #if ENABLE_ATSC_CHLIST_DISP_TOGETHER
                if (u16ProgPosition < stChSettingDisp[ANT_AIR].u16MainListSrvNum)
                {
    ...
    #endif  //ENABLE_ATSC
            {
                ...
                #if ENABLE_CI_PLUS_V1_4
                    U8 u8ProfileType = 0;
                    MApp_CI_GetProfileTypeByIndex(u16ProgPosition, &u8ProfileType);
                    if (EN_OP_TYPE_2 == u8ProfileType)
                    {
                        break;
                    }
...
//printf("DynamicBitmap: %u\n", hwnd);
    ...
    switch(hwnd)
    {
        ...
        if (IsAtscInUse() || IsATVInUse())
        {
            ...
            U16 u16MainListIdx = msAPI_ChProc_ATSC_Get_MainListIdx_By_ProgPos(wProgramPosition);
            MEMBER_SERVICETYPE eServiceType = msAPI_ChProc_ATSC_Get_ServiceType_By_MainListIdx(u16MainListIdx);
        ...
        case HWND_CHLIST_ITEM9_LOCK:
            //printf("HWND_CHLIST_ITEM0_LOCK ~ 9\n");
            {
                ...
                {
                    U16 u16MainListIdx = msAPI_ChProc_ATSC_Get_MainListIdx_By_ProgPos(wProgramPosition);
                    BOOLEAN bLock = msAPI_ChProc_ATSC_Get_Attr_Lock_By_MainListIdx(u16MainListIdx);
        ...
        case HWND_CHLIST_ITEM9_FAV:
            //printf("HWND_CHLIST_ITEM0_FAV ~ 9\n");
            {
                ...
                {
                    U16 u16MainListIdx = msAPI_ChProc_ATSC_Get_MainListIdx_By_ProgPos(wProgramPosition);
                    BOOL bIsFav = msAPI_ChProc_ATSC_Get_Attr_Fav_By_MainListIdx(u16MainListIdx);
...
//printf("DynamicColor: %u\n", hwnd);
    ...
    #endif
    {
        ...
        msAPI_CM_GetServiceTypeAndPositionWithIDs(stGenSetting.stMiscSetting.wMonitoringTSID, stGenSetting.stMiscSetting.wMonitoringONID, stGenSetting.stMiscSetting.wMonitoringServiceID, TRUE, &peMonitorServiceType, &pwMonitorPosition, TRUE);
        if(!(_MApp_ZUI_ACT_IsMonitoringMode()))
    #endif
        {
        ...
        {
            if(g_wProgramPosition == pwMonitorPosition)
            {
                if(u16SrvPageCurIndex == (U16)_MApp_ZUI_ACT_ChannelListWindowMapToIndex( MApp_ZUI_API_GetFocus())) //is the high light program
                    return CH_LIST_CURRENT_CN_TEXT_HL_COLOR;
                else
