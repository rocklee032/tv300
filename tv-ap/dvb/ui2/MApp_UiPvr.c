...
S16 MApp_UiPvr_PlaybackGetABLoopPercentage(void)
{
    U32 u32PlaybackTime    = MApp_Playback_GetPlaybackTimeSec();
    U32 u32TotalTime        = MApp_Playback_GetTotalTimeSec();
    U32 u32RecordStartTime = MApp_Playback_GetRecordStartTimeSec();
    U8 u8StartPercentage = 0;
    U8 u8CurPercentage = 0;
    S16 s16Percentage = 0;

    if((MApp_UiPvr_GetABLoop_Status() == E_ABLOOP_NONE)
        || (u32PlaybackTime == 0)|| (u32TotalTime == 0)|| (u32PlaybackTime > u32TotalTime)
        || (_u32ABLoopStrTime < u32RecordStartTime) || (u32PlaybackTime+u32RecordStartTime < _u32ABLoopStrTime))
    {
        u8StartPercentage = 0;
        u8CurPercentage = 0;
    }
    ...
    {
        u8StartPercentage = (U8)((_u32ABLoopStrTime - u32RecordStartTime)*100/u32TotalTime);
        u8CurPercentage = (U8)(u32PlaybackTime*100/u32TotalTime);
    }
    s16Percentage = (S16)(u8StartPercentage << 8) + u8CurPercentage;
    return s16Percentage;
}
...
static void MApp_UiPvr_GetFileName(U16* _pu16Str, ST_TIME* pstTime)
{
  ...
  #else//PVR_AEON
    LPTSTR CurrentChNumberStr = MApp_ZUI_GetCurrentChNumber();
    if (CurrentChNumberStr != NULL)
    {
        UnicodeCat((S8*)_pu16Str, (S8*)CurrentChNumberStr);
    }
    u8len = UnicodeLen((S8*)_pu16Str);
    _pu16Str[u8len++] = CHAR_SPACE;
    _pu16Str[u8len] = 0;
...
void MApp_UiPvr_RecordStart(U16 u16CiPinCode)
{
    ...
    U8 i = 0;
    PVR_TRACE(printf("[PVR][Record]FileName = "););
    for (i=0; i<RECORD_NAME_LENGTH; i++)
    {
        printf("%c", g_pu16TempFileName[i]);
    }
    printf("\n");
...
void MApp_UiPvr_PlaybackStart(void)
{
  ...
  //enPlaybackState  PlaybackState  = MApp_Playback_StateMachineGet();      //<---only get state here
    ...
    {
        PVR_TRACE(printf("Unknow input to MApp_UiPvr_PlaybackStart()\n"));
    }
...
static void MApp_UiPvr_UpdateUI(void)
{
    ...
    PVR_TRACE(printf("MApp_UiPvr_UpdateUI, UI PVR state:%u\n", PVRState));
    ...
    switch(PVRState)
    {
        ...
        {
            PVR_TRACE(printf("UI record state:%u\n", RecordState));
        ...
        case E_PVR_STATE_RECORDnPLAYBACK:
        {
            ...
            /*[03-2]Playback state ====================================================*/
            PVR_TRACE(printf("UI playback state:%u\n", PlaybackState));
        ...
        {
            PVR_TRACE(printf("UI timeshift state:%u\n", TimeShiftState));
...
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
BOOL MApp_RetentionLimitCheck(U32 u32LoopA, U32 u32LoopB, U32 *u32JumpTargetTime)
{
    U8 u8Index = 0;
    ...
    u8Index = stUriLicenseRegionInfo.u8Index;

    PVR_TRACE(printf("[MApp_RetentionLimitCheck] Cur=%u, A=%u, B=%u \n",
                    stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Cur, u32LoopA, u32LoopB));
    if (u32LoopA > stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Cur)
    {
        *u32JumpTargetTime = u32LoopA;
    }
    else if ( (u32LoopA <= stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Cur)
              && (stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Cur < u32LoopB) )
    {
        *u32JumpTargetTime = stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Cur + PVR_INVISIBLE_SKIP_CAP;
    }
    else if (stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Cur >= u32LoopB)
    {
        return FALSE;
    }
    return TRUE;
...
*/
            ...
            (msAPI_Timer_DiffTimeFromNow(_u32PvrUiLastInfoUpdateTime) >= PVR_UI_ALL_UPDATE_PERIOD_ms))
    {
        ...
        MApp_UiPvr_DrawRetentionLimitBlocks();
#endif
        MApp_UiPvr_SetPercentProgressBar();

        if (TRUE == MApp_Playback_GetUIUpdate())
        {
            MApp_Playback_SetUIUpdate(FALSE);
            MApp_UiPvr_UpdateUI();
        }
            ...
            //if(PlaybackState == E_PLAYBACK_STATE_FASTFORWARD)
            {
                ...
                MApp_UiPvr_UpdateUI();
...
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
        else if ( (u32PlaybackTime != 0) && (MApp_Playback_RetentionLimitMonitor(u32PlaybackTime, &u32JumpTargetTime))
                  && ((PlaybackState == E_PLAYBACK_STATE_PLAYBACKING)
                     ...
                     || (PlaybackState == E_PLAYBACK_STATE_SLOWFORWARD)) )
        {
            if (E_PLAYBACK_STATE_FASTFORWARD == PlaybackState)
            {
                MApp_PVR_SetFileChangeStatus(E_PVR_FILE_CHANGE_FORWARD);
            }
            else if (E_PLAYBACK_STATE_FASTBACKWARD == PlaybackState)
            {
                if (0 != u32JumpTargetTime)
                {
                    MApp_PVR_SetFileChangeStatus(E_PVR_FILE_CHANGE_BACKWARD);
                }
            }
            MApp_PVR_StateMachineInput(E_PVR_INPUT_PLAYBACK_JUMP_TO_TIME, u32JumpTargetTime);
            MApp_UiPvr_UpdateUI();
        }
#endif
        ...
        else if(PlaybackState == E_PLAYBACK_STATE_FASTBACKWARD)
        {
            ...
            else if ( (u32PlaybackTime == msAPI_PVR_GetFileStartTimeStamp())
                      || ( (u32PlaybackTime - 1) <= msAPI_PVR_GetFileStartTimeStamp()) )
            {
                if (MApp_PVR_IsVideoFrameCountStillIncrease() == FALSE)  //play until run out
                {
                    ...
                    if ( (msAPI_PVRFS_GetFileSystemType() == PVR_FS_N51FAT32)
                         || (msAPI_PVRFS_GetFileSystemType() == PVR_FS_LINUX_FAT) )
                    {
                        if (MApp_Record_GetPrevSeqFileName(u16NewFileName))
                        {
                            if (msAPI_PVRFS_FileExist(u16NewFileName))
                            {
                                PVR_AUTO_ACTION_DBG(printf("[PVR_UI]:AUTO#10 PLAY PREV FILE\n"));
        ...
        else if( (u32TotalTime == u32PlaybackTime)
                 || ( (PlaybackState == E_PLAYBACK_STATE_FASTFORWARD)
                      && ((u32PlaybackTime == 0) || (u32TotalTime - 1) <= u32PlaybackTime) ) )   //no need UI_PVR_SAFE_TIMEGAP_TO_BORDER_SEC in playback mode
        {
            if (MApp_PVR_IsVideoFrameCountStillIncrease() == FALSE)  //play until run out
            {
                U16 u16NewFileName[RECORD_NAME_LENGTH];
                   ...
                   msAPI_PVRFS_GetFileSystemType() == PVR_FS_LINUX_FAT)
                {
                    if(MApp_Record_GetNextSeqFileName(u16NewFileName))
                    {
                        if(msAPI_PVRFS_FileExist(u16NewFileName))
                        {
                            ...
                            {
                                MApp_PVR_SetFileChangeStatus(E_PVR_FILE_CHANGE_FILE);
                            }
                            MApp_UiPvr_PlaybackStart();
                            if (E_PLAYBACK_STATE_FASTFORWARD != PlaybackState)
                            {
        ...
        else if(PlaybackState == E_PLAYBACK_STATE_PLAYBACKING)
        {
            ...
            {
                #if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
                    if (TRUE == MApp_RetentionLimitCheck(_u32ABLoopStrTime-u32RecordStartTime, _u32ABLoopEndTime-u32RecordStartTime, &u32JumpTargetTime))
                    {
                        PVR_TRACE(printf("[CI+ PVR][RL] A-B Loop Jump To %us \n", u32JumpTargetTime));
                        MApp_PVR_StateMachineInput(E_PVR_INPUT_PLAYBACK_JUMP_TO_TIME, u32JumpTargetTime);
                    }
                    else
                    {
                        PVR_TRACE(printf("[CI+ PVR][RL] A-B LOOP Stop \n"));
                        MApp_UiPvr_ABLoopStop();
                        MApp_UiPvr_UpdateUI();
                    }
                #else
        ...
        && (msAPI_Timer_DiffTimeFromNow(_u32PvrUiLastInfoUpdateTime) >= PVR_UI_ALL_UPDATE_PERIOD_ms))
    {
                ...
                (TimeShiftState == E_TIMESHIFT_STATE_PLAYBACKING)             )
        {
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
            U32 u32JumpTargetTime = 0;
            enTimeShiftPlaybackState timeshiftPlaybackState = MApp_TimeShift_Playback_StateMachineGet();
#endif
            ...
            #endif //==================================================================//
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
            else if((MApp_TimeShift_RetentionLimitMonitor(u32PlaybackTime, &u32JumpTargetTime))
                && ((timeshiftPlaybackState == E_PLAYBACK_STATE_PLAYBACKING)
                    || (timeshiftPlaybackState == E_PLAYBACK_STATE_FASTFORWARD)
                    || (timeshiftPlaybackState == E_PLAYBACK_STATE_FASTBACKWARD)
                    || (timeshiftPlaybackState == E_PLAYBACK_STATE_SLOWFORWARD)))
            {
                MApp_PVR_StateMachineInput(E_PVR_INPUT_PLAYBACK_JUMP_TO_TIME, u32JumpTargetTime);
            }
#endif
    ...
    #endif
        {
            PVR_TRACE(printf("MApp_ZUI_GetActiveOSD=Empty\n");)
            switch(TimeShiftState)
            {
                case E_TIMESHIFT_STATE_RECORDING:
                case E_TIMESHIFT_STATE_RECORDINGnPLAYBACKING:
...
{
    PVR_TRACE(printf("MApp_UiPvr_VirtualKeyProcessor:%d\n", current));
...
static EN_PVR_KEY_INPUT_RETURN _MApp_UiPvr_KeyProcessor(void)
{
    ...
    switch (u8KeyCode)
    {
        ...
        {
            PVR_TRACE(printf("MApp_UiPvr_KeyProcessor- KEY_INDEX\n"));
        ...
        {
            PVR_TRACE(printf("MApp_UiPvr_KeyProcessor- KEY_UPDATE\n"));
        ...
        case KEY_PLAY:          //start timeshift or record or playback
        {
            ...
            else if((PlaybackState == E_PLAYBACK_STATE_PLAYBACKING))
            {
              ...
              //U32 u32RecordEndTime    = MApp_Playback_GetRecordEndTimeSec();
                ...
                if(MApp_UiPvr_GetABLoop_Status() == E_ABLOOP_NONE)          //set A
                {
                    ...
                    PVR_TRACE(printf("[PVR_UI]: SET AB LOOP START TIME: %02lu:%02lu S\n",(_u32ABLoopStrTime)/60, (_u32ABLoopStrTime)%60));
                }
                else if(MApp_UiPvr_GetABLoop_Status() == E_ABLOOP_A_set)    //set B
                {
                    if(u32PlaybackTime + u32RecordStartTime<= (_u32ABLoopStrTime+UI_PVR_SAFE_TIMEGAP_TO_BORDER_SEC))
                    ...
                    PVR_TRACE(printf("[PVR_UI]: SET AB LOOP END   TIME: %02lu:%02lu S\n",(_u32ABLoopEndTime)/60, (_u32ABLoopEndTime)%60));
                    MApp_PVR_StateMachineInput(E_PVR_INPUT_PLAYBACK_JUMP_BACKWARD, MApp_UiPvr_ABLoopGetJumpTime());
                }
            }
            //>>>AB_LOOP_PROCESS_HERE
                    ...
                    (PlaybackState == E_PLAYBACK_STATE_WAIT))
            {
                ...
                {
                    PVR_AUTO_ACTION_DBG(printf("[PVR_UI]:OPEN FILE NAME IS NULL \n");)
                }
                ...
                if(*g_pu16TempFileName != 0)
                {
                    ...
                    else
                    {
                        ...
                        g_bIsPvrPlay = TRUE;
                        if(MApp_MenuFunc_Setup_BlockSysLockMode_Get() && FALSE == MApp_UiPvr_Browser_KeyProcessor(KEY_PLAY))
                        {
        ...
        case KEY_FF:            //fast foward
        {
            ...
            PVR_TRACE(printf("MApp_UiPvr_KeyProcessor- KEY_FF\n"));
                 ...
                 && (TimeShiftState != E_TIMESHIFT_STATE_WAIT))
            {
                ...
                {
                  #if 0 // no need to use swtich case, only (PVRState == E_PVR_STATE_TIMESHIFT)
                    switch (PVRState)
                    {
                        case E_PVR_STATE_TIMESHIFT:
                            MApp_ZUI_API_SetFocus(HWND_PVR_TIMESHIFT_ICON_PLAY);
        ...
        case KEY_RECORD:        //start to record current program
        {
            ...
            else
            {
                ...
                PVR_TRACE(printf("MApp_UiPvr_KeyProcessor- KEY_RECORD\n"));
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
...
extern BOOLEAN g_bIsLockedOpen;
extern BOOLEAN g_bIsPvrPlay;
extern BOOLEAN MApp_ParentalRating_CompareRating(U8 u8UserRating, U8 u8ProgRating);
...
//  enRecordState    RecordState    = MApp_Record_StateMachineGet();        //<---only get state here
...
#endif
    {
        ...
        case KEY_PLAY:
        {
            static BOOL bIsFindFileName = FALSE;
            if(g_PvrBrowser_TotalItem == 0)
            {
                isKeyProcess = TRUE;
                break;
            ...
            if(bIsFindFileName == FALSE)
            {
                ...
                bIsFindFileName = TRUE;
            }

            if(MApp_MenuFunc_Setup_BlockSysLockMode_Get())
            {
                if(g_bIsLockedOpen == FALSE && bIsFindFileName == TRUE)
                {
                    U8  hReadFile;
                    U8  _IsParentalLock;

                    hReadFile = msAPI_PVRFS_FileOpen(g_pu16TempFileName, PVRFS_OPEN_FOR_READ);
                    if (INVALID_FILE_HANDLE == hReadFile)
                        return FALSE;
                    msAPI_PVRFS_FileRead(hReadFile, PvrGetReadMetadataBufAddr(), META_DATA_SIZE);
                    msAPI_PVRFS_FileClose(hReadFile);
                    _IsParentalLock = MApp_ParentalRating_CompareRating(stGenSetting.g_BlockSysSetting.u8ParentalControl, msAPI_PVR_GetMeataDataParentalControl());
                    if ( (g_bIsPvrPlay == TRUE)
                       &&( (TRUE == msAPI_PVR_GetMeataDataLockInfo()) || (TRUE == _IsParentalLock) )
                       )
                    {
                        return FALSE;
                    }
                }//add by joe
                g_bIsLockedOpen = FALSE;
            }
...
static void MApp_UiPvr_Browser_AddProgramme(U16* _pu16Filename, U32 recStartTime)
{
    ...
    if (CurrentChNumberStr != NULL)
    {
        UnicodeCpy((S8*)E.u16ChannelName, (S8*)CurrentChNumberStr);
        u8len = UnicodeLen((S8*)E.u16ChannelName);
    }
    E.u16ChannelName[u8len++] = CHAR_SPACE;
...
/***************************************************************************************/
#if ((ENABLE_CI_PLUS == 1) && (ENABLE_CIPLUS_PVR == 1))
void MApp_UiPvr_DrawRetentionLimitBlocks(void)
{
    URI_LICENSE_REGION_INFO stUriLicenseRegionInfo;
    memset(&stUriLicenseRegionInfo, 0, sizeof(URI_LICENSE_REGION_INFO));
    MAPP_CIPVR_GetUriLicenseRegionInfo(&stUriLicenseRegionInfo);

    UI_REGION_INFO BarRegionInfo;
    memset(&BarRegionInfo, 0 ,sizeof(BarRegionInfo));
    U8 u8Index = 0;
    U8 u8CountIndex = 0;
    U32 u32TotalTime = MApp_Playback_GetTotalTimeSec();
    if (u32TotalTime == 0 || stUriLicenseRegionInfo.u8UriLicenseCount > MAX_EVENTINFO_NUM)
    {
        return;
    }

    u8Index = stUriLicenseRegionInfo.u8Index;
    PVR_DBG(printf("[RL][%u](Beg:%u, Cur:%u, End:%u, Total=%u) \n",
                   u8Index,
                   stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Begin,
                   stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32Cur,
                   stUriLicenseRegionInfo.m_RetentionLimitBlock[u8Index].u32End,
                   u32TotalTime););

    BarRegionInfo.u8RegionCount = stUriLicenseRegionInfo.u8UriLicenseCount;
    for (u8CountIndex = 0; u8CountIndex < stUriLicenseRegionInfo.u8UriLicenseCount; u8CountIndex++)
    {
        BarRegionInfo.stRetentionLimitInfo[u8CountIndex].bLicense = stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].bLicense;

        if (stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32Cur == stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32End)
        {
            BarRegionInfo.stRetentionLimitInfo[u8CountIndex].bExpire = TRUE;
        }
        else if ( (stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32Cur >= stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32Begin)
                  && (stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32Cur < stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32End) )
        {
            BarRegionInfo.stRetentionLimitInfo[u8CountIndex].bExpire = FALSE;
        }

        MS_U8 u8TempPercent = 0;
        U32 u32Begin = stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32Begin;
        U32 u32End = stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32End;
        U32 u32Cur = stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u32Cur;

        // Region
        u8TempPercent = (U8)(u32Begin * 100 / u32TotalTime);
        if (u8TempPercent > 100)
        {
            u8TempPercent = 100;
        }

        BarRegionInfo.stRetentionLimitInfo[u8CountIndex].u16RetentionLimitRegion |= u8TempPercent << 8;
        u8TempPercent = (U8)(u32End * 100 / u32TotalTime);
        if (u8TempPercent > 100)
        {
            u8TempPercent = 100;
        }
        BarRegionInfo.stRetentionLimitInfo[u8CountIndex].u16RetentionLimitRegion |= u8TempPercent;
        ...
        BarRegionInfo.stRetentionLimitInfo[u8CountIndex].u16RetentionLimitPercent|= u8TempPercent << 8;
        u8TempPercent = (U8)(u32Cur * 100 / u32TotalTime);
        if (u8TempPercent > 100)
        {
            u8TempPercent = 100;
        }
        BarRegionInfo.stRetentionLimitInfo[u8CountIndex].u16RetentionLimitPercent |= u8TempPercent;

        // Playback Count
        BarRegionInfo.stRetentionLimitInfo[u8CountIndex].u8RetentionLimitCount = stUriLicenseRegionInfo.m_RetentionLimitBlock[u8CountIndex].u8Count;
    }

    MApp_ZUI_CTL_RectBorderSetBarInfo(BarRegionInfo);
}
#endif

void MApp_UiPvr_SetPercentProgressBar(void)
{
    S16 aS16TempArr[2];
    memset(aS16TempArr, 0 ,sizeof(aS16TempArr));
    aS16TempArr[0] = MApp_UiPvr_PlaybackGetPlaybackPercentage();
    aS16TempArr[1] = MApp_UiPvr_PlaybackGetABLoopPercentage();
    MApp_ZUI_CTL_SetPercentProgressBar(aS16TempArr);
}
/***************************************************************************************/
