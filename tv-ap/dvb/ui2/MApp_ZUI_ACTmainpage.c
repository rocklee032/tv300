...
#endif
#if ENABLE_EWBS
#include "MApp_ZUI_ACTEWBSconfirmfunc.h"
#endif
...
extern BOOLEAN bEWBS_Vaild;

extern U8 u8Temp_EWBS_Num1 ;
extern U8 u8Temp_EWBS_Num2 ;
extern U8 u8Temp_EWBS_Num3 ;
extern U8 u8Temp_EWBS_Num4 ;
...
#if ENABLE_EWBS
EN_OSD_EWBS_AREA_TYPE_SETTING eEwbsAreaType = OSD_AREA_TYPE_WIDE_AREA_ZONE;
#endif
extern BOOLEAN _MApp_ZUI_API_WindowProcOnIdle(void);
...
#if ENABLE_TIMEZONE_MINUTES
    {TIMEZONE_NZST_CI, en_str_TimeZone_Text_G88},
#endif
...
#if ENABLE_BAT_SCAN
ST_OSD_SELECTION_STRING_MAPPING_LIST _ZUI_TBLSEG stRegionList[] =
{
    {E_REGION_NORTHLAND,en_str_Northland},
    {E_REGION_AUCKLAND,en_str_Auckland},
    {E_REGION_WAIKATO,en_str_Waikato},
    {E_REGION_ROTORUA,en_str_Rotorua},
    {E_REGION_BAY_OF_PLENTY,en_str_Bay_of_plenty},
    {E_REGION_TAUPO,en_str_Taupo},
    {E_REGION_GISBORNE,en_str_Gisborne},
    {E_REGION_NAPIER,en_str_Napier},
    {E_REGION_HASTINGS,en_str_Hastings},
    {E_REGION_TARANAKI,en_str_Taranaki},
    {E_REGION_WANGANUI,en_str_Wanganui},
    {E_REGION_WANAWATU,en_str_Wanawatu},
    {E_REGION_WAIRARAPA,en_str_Wairarapa},
    {E_REGION_WELLINGTON,en_str_Wellington},
    {E_REGION_WEST_COAST,en_str_West_coast},
    {E_REGION_CANTERBURY,en_str_Canterbury},
    {E_REGION_SOUTHEM_LAKES,en_str_Southem_lakes},
    {E_REGION_OTAGO,en_str_Otago},
    {E_REGION_SOUTHLAND,en_str_Southland},
    {E_REGION_ALL_FAT,en_str_All_Free_Channels},
};
#endif
...
#if ENABLE_EWBS
extern void MApp_ZUI_ACT_SetEWBSAreaIndex(U8 u8Index);
extern U8 MApp_ZUI_ACT_GetEWBSAreaIndexMax(void);
extern U8 MApp_ZUI_ACT_GetEWBSAreaIndex(void);
extern LPTSTR MApp_ZUI_ACT_GetEWBSAreaStringByIndex(U8 u8Index);
extern void MApp_ZUI_ACT_SetEWBSDefaultAreaIndex(void);
#endif
...
#if ENABLE_TIMEZONE_MINUTES
    MApp_ZUI_API_GetString(stTimezoneList[u8Index].u16StringIndex);

    U16 u16ItemSel = stTimezoneList[u8Index].u16SelectionItem;

    if( u16ItemSel >= TIMEZONE_GMT_12point75_START && u16ItemSel <= TIMEZONE_GMT_12point75_END)
    {
        U16 u16Len = msAPI_OSD_u16Strlen(CHAR_BUFFER);

        CHAR_BUFFER[u16Len++] = CHAR_DOT;
        CHAR_BUFFER[u16Len++] = CHAR_7;
        CHAR_BUFFER[u16Len++] = CHAR_5;
...
LPTSTR MApp_ZUI_ACT_GetTimezoneString(EN_MENU_TIMEZONE timezone)
{
    U8 u8Index = 0;
    for (u8Index = 0; u8Index <= MApp_ZUI_ACT_GetTimezoneMaxIndex(); u8Index++)
    {
        ...
        {
            return MApp_ZUI_ACT_GetTimezoneStringByIndex(u8Index);
        }
    }
    return MApp_ZUI_API_GetString(Empty);
}
...
#if ENABLE_BAT_SCAN
U8 MApp_ZUI_ACT_GetRegionMaxIndex(void)
{
    return (U8)(sizeof(stRegionList)/sizeof(stRegionList[0]))-1;
}
void MApp_ZUI_ACT_SetRegionByIndex(U8 u8Index)
{
    if (u8Index > MApp_ZUI_ACT_GetRegionMaxIndex())
    {
        u8Index = 0;
    }
    msAPI_SetRegion((EN_REGION)stRegionList[u8Index].u16SelectionItem);
    SET_OSD_MENU_REGION(msAPI_Region2BouquetID(msAPI_GetRegion()));
}
U8 MApp_ZUI_ACT_GetRegionIndex(EN_REGION eRegion)
{
    U8 u8Index = 0;
    for (u8Index = 0; u8Index <= MApp_ZUI_ACT_GetRegionMaxIndex(); u8Index++)
    {
        if (eRegion == stRegionList[u8Index].u16SelectionItem)
        {
            return u8Index;
        }
    }
...
U16 MApp_ZUI_ACT_GetRegionStringID(EN_REGION eRegion)
{
    U8 u8Index = 0;
    for (u8Index = 0; u8Index <= MApp_ZUI_ACT_GetRegionMaxIndex(); u8Index++)
    {
        if (eRegion == (EN_REGION)stRegionList[u8Index].u16SelectionItem)
        {
            return stRegionList[u8Index].u16StringIndex;
        }
    }
    return Empty;
}
LPTSTR MApp_ZUI_ACT_GetRegionStringByIndex(U8 u8Index)
{
    if (u8Index > MApp_ZUI_ACT_GetRegionMaxIndex())
    {
        return MApp_ZUI_API_GetString(Empty);
    ...
    return MApp_ZUI_API_GetString(stRegionList[u8Index].u16StringIndex);
}
#endif
...
LPTSTR MApp_ZUI_ACT_GetMenuLanguageStringByIndex(U8 u8Index)
{
...

    {
        ...
        switch(u8Index)
        {
            ...
            case LANGUAGE_VALENCIAN:
                return MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8*)"Valencian", STRING_BUFFER_LENGTH);

            case LANGUAGE_QAA:
            #if ENABLE_TDT
                return MApp_ZUI_API_GetString(en_str_Original_Version);
            #endif
...
{
#if DVBC_CERTIFICATION_AUTOSCAN_ENABLE
    if(Mapp_SI_Get_Ziggo_ENABLE() == TRUE)
    {
        MApp_DVBC_SCAN_SetCurrentType(OSD_DVBC_TYPE_NETWOEKSCAN);
    }
#endif
...
void MApp_ZUI_ACT_AppShowMainMenu()
{
    ...
    default:
        {
        #if ENABLE_EWBS
            if(_eChannelListMode == MODE_CHLIST_TV_MONITORING)
            {
                MApp_ZUI_API_ShowWindow(HWND_MENU_EWBS_INFO_PAGE, SW_SHOW);
                MApp_ZUI_API_SetFocus(HWND_MENU_EWBS_INFO_MONITORING_CHANNEL);
                _eChannelListMode = MODE_CHLIST_TV;
            }else
        #endif
            {
                MApp_ZUI_ACT_ShowMainMenuBackground(HWND_MENU_BOTTOM_BALL_FOCUS_CHANNEL);
...
void MApp_ZUI_ACT_MainPageMenuKey(void)
{
  ...
  //      MApp_ZUI_ACT_ShowMainMenuBackground(HWND_MENU_BOTTOM_BALL_FOCUS_OPTION);
   ...
   else if(MApp_ZUI_API_IsSuccessor(HWND_MENU_OPTION_MFC_PAGE, MApp_ZUI_API_GetFocus()))
   {
...
#if ENABLE_EWBS
    else if(MApp_ZUI_API_IsSuccessor(HWND_MENU_EWBS_INFO_PAGE, MApp_ZUI_API_GetFocus()))
    {
        // printf("\n    [%s,%d] \n",__FILE__,__LINE__);
        MApp_ZUI_API_ShowWindow(HWND_MENU_EWBS_INFO_PAGE, SW_HIDE);
        MApp_ZUI_API_ShowWindow(HWND_MENU_CHANNEL_PAGE, SW_SHOW);
        MApp_ZUI_ACT_ShowMainMenuBackground(HWND_MENU_BOTTOM_BALL_FOCUS_CHANNEL);
        MApp_ZUI_CTL_DynamicListSetItemFocus(HWND_MENU_CHANNEL_PAGE_LIST, HWND_MENU_EWBS_INFO);
    }
	else if(MApp_ZUI_API_IsSuccessor(HWND_MENU_DLG_AREA_CODE_TYPE_SELECT_MENU, MApp_ZUI_API_GetFocus()))
    {
       // printf("\n    [%s,%d] \n",__FILE__,__LINE__);
        MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_AREA_CODE_TYPE_SELECT_MENU, SW_HIDE);
        MApp_ZUI_API_ShowWindow(HWND_MENU_EWBS_INFO_PAGE, SW_SHOW);
        MApp_ZUI_API_SetFocus(HWND_MENU_EWBS_INFO_AREA_CODE);
    }
	else if(MApp_ZUI_API_IsSuccessor(HWND_MENU_DLG_INPUT_EWBS_LOCATION_COMMON, MApp_ZUI_API_GetFocus()))
    {
       // printf("\n    [%s,%d] \n",__FILE__,__LINE__);
        if(MApp_GetOSDCountrySetting() == E_PHILIPPINES)
        {
            MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_INPUT_EWBS_LOCATION_COMMON, SW_HIDE);
            MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_AREA_CODE_TYPE_SELECT_MENU, SW_SHOW);
            MApp_ZUI_API_SetFocus(HWND_SELECTED_DIGIT_BG);
        }
        else
        {
            MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_INPUT_EWBS_LOCATION_COMMON, SW_HIDE);
    ...
    else if(MApp_ZUI_API_IsSuccessor(HWND_MENU_DLG_EWBS_CONFIRM, MApp_ZUI_API_GetFocus()))
    {
       // printf("\n    [%s,%d] \n",__FILE__,__LINE__);
        if(eEwbsAreaType == OSD_AREA_TYPE_WIDE_AREA_ZONE)
        {
            ...
            MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_AREA_CODE_TYPE_SELECT_MENU, SW_SHOW);
            MApp_ZUI_API_SetFocus(HWND_SELECTED_NAME_BG);
        }
        else
        {
            eEwbsAreaType--;
            MApp_ZUI_API_InvalidateAllSuccessors(HWND_MENU_DLG_EWBS_CONFIRM);
        }
    }
#endif
}
...
void _MApp_ZUI_ACT_OpenCommonDialog(COMMON_DLG_MODE dlg)
{
    ...
    {
#if ENABLE_PVR
        case EN_COMMON_DLG_MODE_PVR_STOP_CONFIRM:
...
BOOLEAN MApp_ZUI_ACT_ExecuteMainMenuAction(U16 act)
{
    ...
    EN_OSD_COUNTRY_SETTING enOsdCountry = OSD_COUNTRY_SETTING;
#endif
...
*/
    ...
    switch(act)
    {
        ...
        case EN_EXE_GOTO_SUBPAGE:
        {
            ...
            switch(hwnd)
            {
                ...
                case HWND_MENU_LOCK_RRTDIMENTIONSUBPAGE_ITEM9:
                    ...
                    {
                        if(
                        #if(ENABLE_ATSC_RRT_NEW_STRUCT)
                            ( g_stVChipRatingInfo.au16DimVal[g_u8CurrentRRT_Dimension] != 0 )
                        #else
                            (g_stVChipRatingInfo.u8DimVal0 && g_u8CurrentRRT_Dimension == 0) ||
        ...
        case EN_EXE_FUNCTION:
        {
            ...
            #if( ENABLE_ATSC_MENU_LOCK_UI && ((ATSC_CC == ATV_CC)||(ATSC_CC == DTV_CC)||ENABLE_ISDB_ATV_VCHIP) )
                case HWND_MENU_LOCK_TVRATINGSUBPAGE_ITEM_TV_NONE_ALL:
    ...
    //**********************************************
    ...
    #endif
            {
                U8 u8Min = EN_F4_LockSystem_Min;
                U8 u8Max = EN_F4_LockSystem_Max;
                U8 u8Ctl = stGenSetting.g_BlockSysSetting.u8ParentalControl;
    #if ENABLE_ZIGGO_PARENTAL_RULE
            #if(ENABLE_ISDBT)
                if( !IS_COUNTRY_USE_SBTVD() )
            #endif
                {
                    u8Max = EN_F4_LockSystem_AllLock;
                }
    #endif
                if(u8Ctl >= u8Min && u8Ctl <= u8Max)
                {
                    U16 u16Step = (HWND_MENU_SINGLELIST_ITEM3 - HWND_MENU_SINGLELIST_ITEM2)*(u8Ctl - u8Min);

                    MApp_ZUI_CTL_DynamicListSetItemFocus(HWND_MENU_SINGLELIST_COMMON_PAGE_LIST, HWND_MENU_SINGLELIST_ITEM2 + u16Step);
                }
                else
                {
                    MApp_ZUI_API_SetFocus(HWND_MENU_SINGLELIST_ITEM1);
        ...
        case EN_EXE_EXECUTE_APP:
            {
                ...
                if ( IsDTVInUse() )
                {
                    if(MApp_PVR_IsRecording())
                    {
                        ...
                        _MApp_ZUI_ACT_OpenCommonDialog(EN_COMMON_DLG_MODE_PVR_STOP_CONFIRM);
        ...
        #if ENABLE_EWBS
			if (MApp_ZUI_API_IsSuccessor(HWND_MENU_DLG_EWBS_CONFIRM, MApp_ZUI_API_GetFocus())){
                MApp_ZUI_ACT_ExecuteEWBSConfirmDialogAction(EN_EXE_CLOSE_EWBS_CONFIRM);
            }
        #endif
            #if(DVBC_CERTIFICATION_AUTOSCAN_ENABLE)
        ...
        #if ENABLE_BAT_SCAN
	    case EN_EXE_POP_REGION_SELECT:
	    {
	        //printf("EN_EXE_POP_REGION_SELECT\n");
	        MApp_ZUI_API_ShowWindow(HWND_MENU_CHANNEL_PAGE, SW_HIDE);
	        MApp_ZUI_API_ShowWindow(HWND_MENU_MASK_BACKGROUND, SW_HIDE);
	        MApp_ZUI_API_ShowWindow(HWND_MENU_TRANSPARENT_BG,SW_SHOW);
	        MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_REGION_SELECT_MENU, SW_SHOW);
	        MApp_ZUI_API_SetFocus(HWND_MENU_DLG_REGION_SELECT_MENU);
	        SET_OSD_MENU_REGION(msAPI_Region2BouquetID(msAPI_GetRegion()));
            break;
	    }
	    case EN_EXE_SELECT_REGION_CONFIRM:
	    {
	        U8 u8CurIndex;
	        u8CurIndex = MApp_ZUI_ACT_GetRegionIndex(msAPI_GetRegion());
	        MApp_ZUI_ACT_SetRegionByIndex(u8CurIndex);
	        MApp_ZUI_ACT_ExecuteMainMenuAction(EN_EXE_CLOSE_CURRENT_OSD);
		//printf("EN_EXE_SELECT_REGION_CONFIRM\n");
            break;
	    }
	    case EN_EXE_SELECT_REGION_NEXT:
        case EN_EXE_SELECT_REGION_PREV:
        {
            U8 u8Index,u8CurIndex;
            #define MAXREGIONINDEX  MApp_ZUI_ACT_GetRegionMaxIndex()
            #define MINREGIONINDEX  0
            u8CurIndex = MApp_ZUI_ACT_GetRegionIndex(msAPI_GetRegion());
            if(EN_EXE_SELECT_REGION_NEXT == act)
            {
                u8Index = (u8CurIndex == MAXREGIONINDEX)? MINREGIONINDEX:(u8CurIndex + 1);
                //printf("EN_EXE_SELECT_REGION_NEXT\n");
            }
            else
            {
                u8Index = (u8CurIndex == MINREGIONINDEX)? MAXREGIONINDEX:(u8CurIndex - 1);
                //printf("EN_EXE_SELECT_REGION_PREV\n");
            }
            MApp_ZUI_ACT_SetRegionByIndex(u8Index);
            MApp_ZUI_API_InvalidateWindow(HWND_MENU_DLG_REGION_SELECT_MENU);
            //printf("u8index = %d,stGenSetting.g_SysSetting.eBouquetID = 0x%x\n",u8Index,stGenSetting.g_SysSetting.eBouquetID);
            break;
        }
        #endif
    #endif
...
*/
        ...
        case EN_EXE_GOTO_AUTO_TUNING:
        {
            ...
            else
            {
            ...
            #endif  //DVBC_CERTIFICATION_AUTOSCAN_ENABLE
                {
                    ...
                    if (EN_OP_TYPE_2 == MApp_CI_GetOperatorProfileType())
                    {
                        MApp_CI_ExitCiOperatorProfile();
                        MApp_CI_DeleteOpCacheByIndex(DB_CI_PLUS_SETTING.u8OpIndex);
                        MApp_CI_CheckOpCacheInfo();
...
#if (ENABLE_EWBS)
        case EN_EXE_GOTO_EWBS_INFO_PAGE:
        {
            MApp_ZUI_API_ShowWindow(HWND_MENU_CHANNEL_PAGE, SW_HIDE);
            MApp_ZUI_API_ShowWindow(HWND_MENU_MASK_BACKGROUND, SW_HIDE);
            MApp_ZUI_API_ShowWindow(HWND_MENU_TRANSPARENT_BG, SW_SHOW);
            ...
            if ( stGenSetting.stMiscSetting.g_bEwbsMode == DISABLE)
            {
                MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_MONITORING_CHANNEL, DISABLE);
                MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_AREA_CODE, DISABLE);
                MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_AUTOMATIC_STARTUP, DISABLE);
                MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_NO_OPERATION_OFF, DISABLE);
                MApp_ZUI_API_EnableWindow(HWND_MENU_EWBS_INFO_BUZZER_NOTIFICATION, DISABLE);
            }
            MApp_ZUI_API_SetFocus(HWND_MENU_EWBS_INFO_EWBS);
        }
        return TRUE;

        case EN_EXE_GOTO_INPUT_EWBS_LOCATION_COMMON_DLG:
        {
            MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_AREA_CODE_TYPE_SELECT_MENU, SW_HIDE);
            MApp_ZUI_API_ShowWindow(HWND_MENU_MASK_BACKGROUND, SW_HIDE);
            MApp_ZUI_API_ShowWindow(HWND_MENU_TRANSPARENT_BG, SW_SHOW);
            MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_INPUT_EWBS_LOCATION_COMMON, SW_SHOW);

            u8Temp_EWBS_Num1 = stGenSetting.stMiscSetting.u8EWBSLocationCode[0] ;
            u8Temp_EWBS_Num2 = stGenSetting.stMiscSetting.u8EWBSLocationCode[1] ;
            u8Temp_EWBS_Num3 = stGenSetting.stMiscSetting.u8EWBSLocationCode[2] ;
            u8Temp_EWBS_Num4 = stGenSetting.stMiscSetting.u8EWBSLocationCode[3] ;

            MApp_ZUI_API_SetFocus(HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_1);
        }
        return TRUE;

        case EN_EXE_EWBS_AREA_TYPE_SWITCH:
            eEwbsAreaType++;
            return TRUE;

        case EN_EXE_GOTO_EWBS_CONFIRM_DLG:
        {
            if(eEwbsAreaType < OSD_AREA_TYPE_NUMS)
            {
                MApp_ZUI_ACT_SetEWBSArea(EWBS_AREA_SETTING); //initial temp country

                MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_AREA_CODE_TYPE_SELECT_MENU, SW_HIDE);
                //MApp_ZUI_API_ShowWindow(HWND_MENU_MASK_BACKGROUND, SW_HIDE);
                MApp_ZUI_API_ShowWindow(HWND_MENU_TRANSPARENT_BG, SW_SHOW);
                MApp_ZUI_API_ShowWindow(HWND_MENU_DLG_EWBS_CONFIRM, SW_SHOW);
                MApp_ZUI_CTL_Grid_SetIndex(HWND_MENU_DLG_EWBS_CONFIRM_COUNTRY_GRID, MApp_ZUI_ACT_GetEWBSAreaIndex, MApp_ZUI_ACT_SetEWBSAreaIndex, 0, MApp_ZUI_ACT_GetEWBSAreaIndexMax());
                MApp_ZUI_CTL_Grid_SetFnGetTextByIndex(HWND_MENU_DLG_EWBS_CONFIRM_COUNTRY_GRID, MApp_ZUI_ACT_GetEWBSAreaStringByIndex);

                MApp_ZUI_API_SetFocus(HWND_MENU_DLG_EWBS_CONFIRM_COUNTRY_GRID_02);
                MApp_ZUI_ACT_SetEWBSDefaultAreaIndex();
            }
            else
           {
                MApp_ZUI_ACT_ExecuteMainMenuAction(EN_EXE_CLOSE_CURRENT_OSD);
        ...
        case EN_EXE_INPUT_EWBS_NUM_KEY0:
        case EN_EXE_INPUT_EWBS_NUM_KEY1:
        case EN_EXE_INPUT_EWBS_NUM_KEY2:
        case EN_EXE_INPUT_EWBS_NUM_KEY3:
        case EN_EXE_INPUT_EWBS_NUM_KEY4:
        case EN_EXE_INPUT_EWBS_NUM_KEY5:
        case EN_EXE_INPUT_EWBS_NUM_KEY6:
        case EN_EXE_INPUT_EWBS_NUM_KEY7:
        case EN_EXE_INPUT_EWBS_NUM_KEY8:
        case EN_EXE_INPUT_EWBS_NUM_KEY9:
        {
            U16 u16Number= act - EN_EXE_INPUT_EWBS_NUM_KEY0;
            switch(MApp_ZUI_API_GetFocus())
            {
                case HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_1:
                    u8Temp_EWBS_Num1 = u16Number;
                    MApp_ZUI_API_SetFocus(HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_2);
                    break;

                case HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_2:
                    u8Temp_EWBS_Num2 = u16Number;
                    MApp_ZUI_API_SetFocus(HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_3);
                    break;

                case HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_3:
                    u8Temp_EWBS_Num3 = u16Number;
                    MApp_ZUI_API_SetFocus(HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_4);
                    break;

                case HWND_MENU_DLG_INPUT_EWBS_LOCATION_PASSWORD_INPUT0_TEXT_4:
                    u8Temp_EWBS_Num4 = u16Number;
                    MApp_ZUI_API_SetFocus(HWND_MENU_DLG_INPUT_EWBS_LOCATION_COMMON_BTN_YES);
                    break;
            ...
            MApp_ZUI_API_InvalidateAllSuccessors(HWND_MENU_DLG_INPUT_EWBS_LOCATION_CODE_PANE0);

        }
        return TRUE;

        case EN_EXE_INPUT_EWBS_EWBS_CANEL:
            u8Temp_EWBS_Num1 = stGenSetting.stMiscSetting.u8EWBSLocationCode[0] ;
            u8Temp_EWBS_Num2 = stGenSetting.stMiscSetting.u8EWBSLocationCode[1] ;
            u8Temp_EWBS_Num3 = stGenSetting.stMiscSetting.u8EWBSLocationCode[2] ;
            u8Temp_EWBS_Num4 = stGenSetting.stMiscSetting.u8EWBSLocationCode[3] ;
        ...
        case EN_EXE_INPUT_EWBS_EWBS_DONE:
            stGenSetting.stMiscSetting.u8EWBSLocationCode[0] = u8Temp_EWBS_Num1;
            stGenSetting.stMiscSetting.u8EWBSLocationCode[1] = u8Temp_EWBS_Num2;
            stGenSetting.stMiscSetting.u8EWBSLocationCode[2] = u8Temp_EWBS_Num3;
            stGenSetting.stMiscSetting.u8EWBSLocationCode[3] = u8Temp_EWBS_Num4;
        ...
        case EN_EXE_GOTO_SELECT_COUNTRY:
        {
            ...
            if(IsDVBCInUse())
            {
                ...
                MApp_SetCertificationSetting(OSD_CERTIFICATION_SETTING);
                MApp_EnableCertificationFlag();
                MApp_ZUI_API_ShowWindow(HWND_MENU_TRANSPARENT_BG, SW_HIDE);
                ...
                MApp_ZUI_API_SetFocus(HWND_MENU_DLG_TUNE_CONFIRM_TUNE_TYPE);
                MApp_ZUI_ACT_SetTuningCountryIndex(MApp_ZUI_ACT_GetTuningCountryIndex());
            }
            return TRUE;
        }
    #endif
        ...
        case EN_EXE_GOTO_TUNING_CONFIRM:
            ...
            #endif
                {
                    EN_OSD_COUNTRY_SETTING enCountry = OSD_COUNTRY_SETTING;
                    if (enCountry >= E_COUNTRY_NUM)
                        break;

                    MApp_ZUI_ACT_SetTuningCountry(enCountry); //initial temp country
...
*/
        ...
        #endif
#if ENABLE_PVR
            else if(_eCommonDlgMode == EN_COMMON_DLG_MODE_PVR_STOP_CONFIRM)
            {
                if( MApp_PVR_IsPlaybacking())
                {
                    MApp_UiPvr_PlaybackStop();
                ...
                MApp_ZUI_API_ShowWindow(_hwndCommonDlgTargetFocus, SW_SHOW);
                MApp_ZUI_API_SetFocus(_hwndCommonDlgTargetFocus);
                MApp_ZUI_ACT_ExecuteMainMenuAction(EN_EXE_EXECUTE_APP);
            }
#endif
            else
            {
    ...
    #if ENABLE_EWBS
        case EN_EXE_GOTO_MONITORING_CHANNEL_LIST:
             MApp_ZUI_ACT_TransitionEffectBegin(EN_EFFMODE_CLOSE, E_ZUI_STATE_TERMINATE);
            _enTargetMenuState = STATE_MENU_GOTO_MONITORING_CHANNEL_LIST;
            return TRUE;
    #endif
...
#if ENABLE_EWBS
    if (MApp_ZUI_API_IsSuccessor(HWND_MENU_DLG_EWBS_CONFIRM_AREA_TYPE, hwnd))
        return MApp_ZUI_ACT_GetEWBSConfirmDynamicText(hwnd);

    if (MApp_ZUI_API_IsSuccessor(HWND_MENU_DLG_EWBS_CONFIRM_COUNTRY_GRID, hwnd))
        return MApp_ZUI_CTL_GridGetDynamicText(HWND_MENU_DLG_EWBS_CONFIRM_COUNTRY_GRID,hwnd);
#endif
...
S32 MApp_ZUI_ACT_Mainpage_DynamicListWinProc(HWND hWnd, PMSG pMsg)
{
    ...
    switch (pMsg->message)
    {
        ...
        case MSG_NOTIFY_HIDE:
        {
            ...
            memset(&rect_first, 0, sizeof(RECT));
            memset(&rect_second, 0, sizeof(RECT));
            ...
            if (hWnd != HWND_INVALID)
            {
                for (child = hWnd+1; child <= last_succ; child++)
                {
                    ...
                    DRAW_TEXT_OUT draw;
                    memset(&draw, 0, sizeof(DRAW_TEXT_OUT));
