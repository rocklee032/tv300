...
{
    DEBUG_UI_EPG(printf("MApp_EpgUI_Func_Add_EvtToOSDBuffer(%u,%u)\n", u8UiSrvIdx, u16EvtIdx););
...
{
    DEBUG_UI_EPG(printf("MApp_EpgUI_Func_Channel_Add_EvtToOSDBuffer(%u,%u)\n",u16Idx, u16EvtIdx););
...
void MApp_EpgUI_Func_Add_MatchEvent_Sche(U8 u8UiSrvIdx)
{
    ...
    DEBUG_UI_EPG( printf("MApp_EpgUI_Func_Add_MatchEvent_Sche(%u)\n", u8UiSrvIdx); );

    u32SearchEvtTime = stTime.PageStartTime;

    DEBUG_UI_EPG( printf(" u32SearchEvtTime=%u\n", u32SearchEvtTime); );
    DEBUG_UI_EPG( printf(" stTime.PageEndTime=%u\n", stTime.PageEndTime); );

    while ( u32SearchEvtTime < stTime.PageEndTime )
    {
        if ( MW_EN_EPG_FUNC_STATUS_SUCCESS !=
                    MApp_Epg_EventFindByTime(msAPI_SI_ToSI_Service_Type(stFocus.enSvt),
                                             au16SrvListOSD[u8UiSrvIdx], u32SearchEvtTime, &SearchEvtIdx) )
        {
            break;
        }
...
void MApp_EpgUI_Func_Add_MatchOneEvent(U8 u8UiSrvIdx)
{
    ...
    // Page Matched Event
    if ( (stTime.EventEnd > stTime.SystemTime && stTime.EventStart <= stTime.SystemTime )
       &&(enRtnValue == MW_EN_EPG_FUNC_STATUS_SUCCESS) )
    {
        if ( !(MApp_EpgUI_Func_Add_EvtToOSDBuffer(u8UiSrvIdx, SearchEvtIdx)) )
        {
            EPG_UI (DBG_HANDLER, ("Add_MatchEvent_Sche>> UI Event List is full \n"));
        }
        else
        {
            EPG_UI (DBG_HANDLER, ("u8UiSrvIdx = %bu, ListEvtCnt = %2bu\n",
    ...
    else
    {
        ...
        if ( MApp_EpgUI_Func_Add_EvtToOSDBuffer (u8UiSrvIdx, MW_INVALID_EVENT_IDX) )
        {
            EPG_UI (DBG_HANDLER, ("u8UiSrvIdx = %bu, ListEvtCnt = %2bu\n",
                                u8UiSrvIdx, astScheOSD[u8UiSrvIdx].ListEvtCnt));
        }
...
void MApp_EpgUI_Func_Add_NumberEvent(U8 u8UiSrvIdx)
{
    ...
    while ( EventSearchResult == MW_EN_EPG_FUNC_STATUS_SUCCESS )
    {
        ...
        {
        #if (ENABLE_TDT)
            //TDT special stream : following event start time earlier than present event start time
            if(OSD_COUNTRY_SETTING == OSD_COUNTRY_SPAIN)
            {
                ...
                if (pstPfSrvHdr != NULL)
                {
                    MApp_Epg_GetEventTime(pstPfSrvHdr->au16PfEvtIdx[0], &stPresentEvtTime);
                    MApp_Epg_GetEventTime(pstPfSrvHdr->au16PfEvtIdx[1], &stFollowingEvtTime);
                    if(stFollowingEvtTime.u32StartTime < stPresentEvtTime.u32StartTime)
                    {
                        SearchEvtIdx = pstPfSrvHdr->au16PfEvtIdx[1];
                        MApp_EpgUI_Func_Channel_Add_EvtToOSDBuffer(u32EvtCount, SearchEvtIdx);
                        u32EvtCount++;
...
#endif
    {
        ...
        for( i=0 ; i < SCHEDULE_LIST_PER_PAGE ; i++)
        {
            if( (i+_u8ScheduleStartingIdx) < emptyIndex)
            {
                ...
                EN_EPG_TIMER_REPEAT_MODE eRepeatMode = MApp_EpgTimer_Get_RepeatMode(i+_u8ScheduleStartingIdx);

                //if(_pstEpgTimer[i+_u8ScheduleStartingIdx].enRepeatMode != EN_EPGRM_ONCE)
                if( eRepeatMode != EN_EPGRM_ONCE )
                {
...
//            gbEPG_RECORDER_PANEL_CUR_FOCUS = TRUE;
    ...
    #endif
        {
            ...
            if ( DTVProgTtl == 0 && RadioProgTtl == 0)
            {
                break;
            }
        ...
        case EN_EXE_EPG_RECORDER_ALTERNATIVE_OK:
        {
            ...
            else if ((m_pstAlterIdx >= 0) )
            {
                //m_pstEpgTimer[m_pstAlterIdx] = m_AlternateEpgTimer.stEpgTimer;

                if( MApp_EpgTimer_Set_EpgTimer(m_pstAlterIdx, &(m_AlternateEpgTimer.stEpgTimer) ) )
                {
                    MApp_EpgTimer_SaveTimerSetting();
                }
            }
    ...
    switch(key)
    {
        ...
        case VK_INFO:
        {
            if ( (MApp_ZUI_API_GetFocus() == HWND_EPG_PRO_GUIDE_TITLE)
               ||(MApp_ZUI_API_GetFocus() == HWND_EPG_PRO_GUIDE_TITLE_TIME)
...
LPTSTR MApp_ZUI_ACT_GetEpgDynamicText(HWND hwnd)
{
    ...
    switch(hwnd)
    {
    ...
    #endif // #if ENABLE_EPG_INFO_DISP_GENRE_TYPE
        ...
        case HWND_EPG_SCHEDULE_LIST_ITEM_3_TITLE:
        {
            ...
            *str=0;
        ...
        #if 1
            if( FALSE == MApp_EpgTimer_Get_EventName(idx+_u8ScheduleStartingIdx, str, EPG_TIMER_MAX_EVT_NAME_LEN) )
            {
                MApp_ZUI_API_LoadString(en_str_No_Title, str);
            }
        #else
            BOOLEAN bUseEventName = FALSE;
            const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(idx+_u8ScheduleStartingIdx);

            if( pstMS_EPG_TIMER != NULL )
            {
                //if( _pstEpgTimer[idx+_u8ScheduleStartingIdx].bNoTitle )
                if( pstMS_EPG_TIMER->bNoTitle )
                {
                    bUseEventName = TRUE;
                }
            }
            ...
            if( bUseEventName )
            {
                ...
                MApp_ZUI_API_Strcpy(str, (U16*) pstMS_EPG_TIMER->au16EvtName );
            }
            else
            {
                MApp_ZUI_API_LoadString(en_str_No_Title, str);
        ...
        case HWND_EPG_SCHEDULE_LIST_ITEM_3_TIME:
        {
            ...
            *str=0;
            ...
            __MApp_UlongToString(pstMS_EPG_TIMER->stStartDate.u8Hour, str, 2);
            str+=2;

            *str=':';
            str+=1;

            __MApp_UlongToString(pstMS_EPG_TIMER->stStartDate.u8Min,str,2);
            str += 2;

            if( pstMS_EPG_TIMER->enTimerType == EN_EPGUI_TIMER_RECORDER )
            {
                *str='-';
                str+=1;

                __MApp_UlongToString(pstMS_EPG_TIMER->stEndDate.u8Hour, str, 2);
                str+=2;

                *str=':';
                str+=1;

                __MApp_UlongToString(pstMS_EPG_TIMER->stEndDate.u8Min,str,2);
                str += 2;
            }
            *str=0;
        ...
        case HWND_EPG_SCHEDULE_LIST_ITEM_3_DATE:
        {
            ...
            *str=0;
            ...
            ST_TIME stEpgTimerStartDate = MApp_EpgTimer_Get_StartDate(idx+_u8ScheduleStartingIdx);

            eDayOfWeek = (DAYOFWEEK) MApp_GetDayOfWeek(
                                                stEpgTimerStartDate.u16Year,
                                                stEpgTimerStartDate.u8Month,
                                                stEpgTimerStartDate.u8Day  );
            ...
            *str++ = CHAR_SPACE;
            ...
            __MApp_UlongToString( stEpgTimerStartDate.u8Day, str, 2 );
            str += 2;

            *str++ = CHAR_SPACE;
            _MApp_ZUI_ACT_LoadMonthString( stEpgTimerStartDate.u8Month-1, str);
            str += 3;
        ...
        case HWND_EPG_SCHEDULE_LIST_ITEM_3_PROGRAMME:
        {
            ...
            *str=0;
            ...
            if( FALSE == MApp_EpgTimer_Get_PCN_ServiceID(idx+_u8ScheduleStartingIdx, &u16PCN, &u16ServiceID) )
            {
                break;
            }

            if( FALSE == msAPI_CM_GetServiceTypeAndPositionWithPCN( u16PCN, u16ServiceID, &srvType, &srvPos) )
            {
                break;
            }
        ...
        #if 1
            if( FALSE == MApp_EpgTimer_Get_EventName(idx, str, EPG_TIMER_MAX_EVT_NAME_LEN) )
            {
                MApp_ZUI_API_LoadString(en_str_No_Title, str);
            }
        #else
            const MS_EPG_TIMER* pstMS_EPG_TIMER = MApp_EpgTimer_Get_EpgTimer_Pointer(idx);
            if( pstMS_EPG_TIMER == NULL )
            {
                break;
            }
        ...
        case HWND_EPG_RECORDER_SCHEDULE_LIST_ITEM_4_TIME:
        {
            ...
            *str=0;
            ...
            ST_TIME stEpgTimerStartDate = MApp_EpgTimer_Get_StartDate(idx);

            __MApp_UlongToString(stEpgTimerStartDate.u16Year, str, 4);
            str+=4;
            *str='/';
            str+=1;

            __MApp_UlongToString(stEpgTimerStartDate.u8Month, str, 2);
            str+=2;
            *str='/';
            str+=1;
            ...
            *str=' ';
            ...
            __MApp_UlongToString(stEpgTimerStartDate.u8Hour, str, 2);
            str+=2;
            *str=':';
            str+=1;

            __MApp_UlongToString(stEpgTimerStartDate.u8Min, str, 2);
            str += 2;
            *str=0;
        ...
        case HWND_EPG_RECORDER_SCHEDULE_LIST_ITEM_4_DATE:
        {
            ...
            *str=0;
            ...
            ST_TIME stEpgTimerEndDate = MApp_EpgTimer_Get_EndDate(idx);

            __MApp_UlongToString(stEpgTimerEndDate.u16Year, str, 4);
            str+=4;
            *str='/';
            str+=1;

            __MApp_UlongToString(stEpgTimerEndDate.u8Month, str, 2);
            str+=2;
            *str='/';
            str+=1;

            __MApp_UlongToString(stEpgTimerEndDate.u8Day, str, 2);
            str+=2;
            *str=' ';
            str+=1;

            __MApp_UlongToString(stEpgTimerEndDate.u8Hour, str, 2);
            str+=2;
            *str=':';
            str+=1;

            __MApp_UlongToString(stEpgTimerEndDate.u8Min,str,2);
            str += 2;
            *str=0;
        ...
        case HWND_EPG_RECORDER_SCHEDULE_LIST_ITEM_4_PROGRAMME:
        {
            ...
            *str=0;
            ...
            if( FALSE == MApp_EpgTimer_Get_PCN_ServiceID(idx, &u16PCN, &u16ServiceID) )
            {
                break;
            }
        ...
        case HWND_EPG_RECORDER_SCHEDULE_LIST_ITEM_4_MODE:
        {
            ...
            *str=0;
            ...
            EN_EPG_TIMER_REPEAT_MODE eEpgTimerRepeatMode = MApp_EpgTimer_Get_RepeatMode(idx);

            switch(eEpgTimerRepeatMode)
            {
                case  EN_EPGRM_DAILY:
        ...
        case HWND_EPG_PVR_ALTERNATE_DYNAMIC_TXT_1:
        {
            ...
            if ( FALSE == msAPI_CM_GetServiceTypeAndPositionWithPCN(m_AlternateEpgTimer.stEpgTimer.u8PCN, m_AlternateEpgTimer.stEpgTimer.u16SrvID, &srvType, &srvPos) )
                break;

            if ( MW_EN_EPG_FUNC_STATUS_SUCCESS != MApp_Epg_EventFindByID(msAPI_SI_ToSI_Service_Type(srvType),srvPos,m_AlternateEpgTimer.stEpgTimer.u16EventID,&AlternateEvtIdx) )
                break;

            if ( MW_EN_EPG_FUNC_STATUS_SUCCESS != MApp_Epg_GetEventTime(AlternateEvtIdx, &stEvtTime) )
                break;
...
U16 MApp_ZUI_ACT_GetEpgDynamicBitmap(HWND hwnd, DRAWSTYLE_TYPE ds_type)
{
    switch(hwnd)
    {
        ...
        case HWND_EPG_SCHEDULE_LIST_ITEM_3_ICON:
        {
            ...
            EN_EPGUI_TIMER_TYPE eEPGUI_TIMER_TYPE = MApp_EpgTimer_Get_TimerType(idx+_u8ScheduleStartingIdx);

            if( eEPGUI_TIMER_TYPE == EN_EPGUI_TIMER_REMINDER)
            {
                if ( ds_type == DS_FOCUS )
            ...
            if( eEPGUI_TIMER_TYPE == EN_EPGUI_TIMER_RECORDER)
            {   if ( ds_type == DS_FOCUS )
...
//kk test    printf ("(%d/%d) ListEvtIdx = %d\n", (int)u8ListEvtIdx, (int)u16ChannelEvtList.ListEvtIndex, (int)u16ChannelEvtList.ListEvt[u16ListFirstItemIndex+u8ListEvtIdx]);
    ...
    pText = _ZUI_MALLOC(sizeof(DRAW_TEXT_OUT_DYNAMIC));
    if (pText == NULL)
    {
        __ASSERT(0);
        goto END;
