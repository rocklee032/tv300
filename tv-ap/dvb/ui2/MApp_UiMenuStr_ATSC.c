...
//#define GetDimLockOption(p, n)  ((p)->u8DownloadableRatingD ## n)
// ==>
//#define GetDimLockOption(p, n)  ((p)->au168DownloadableRating[(n-1)])
    {
        U8 u8DimIdx;
        U16 u16DimLockOption;

        for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
        {
            u16DimLockOption = pEpgRating->au168DownloadableRating[u8DimIdx];

            //if (GetDimLockOption(pEpgRating, 1))
            if( u16DimLockOption )
            {
                //for (i = 0; i < GetValuesDefined(0); ++i)
                for (i = 0; i < GetValuesDefined(u8DimIdx); ++i)
                {
                    //if ( GETBIT( GetDimLockOption(pEpgRating, 1), i) )
                    if ( GETBIT( u16DimLockOption, i) )
                    {
                        ...
                        if (precede_common&& stGenSettingVchipRegion5.stRegin5Dimension[u8DimIdx].stRatingText[i][0])
                        {
                        ...
                        strncat((char*)pu8Str, (char*)stGenSettingVchipRegion5.stRegin5Dimension[u8DimIdx].stRatingText[i], buf_size - strlen((char *)pu8Str));
                        precede_common = TRUE;
                    }
                }
            }
...
void UiMenuStr_GetVChip2RatingString(U8 *pu8Str, U16 u16StrBufSize)
{
    ...
    U8 u8DimIdx;
    U16 u16DimLockOption1;

    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
    {
        u16DimLockOption1 = g_stVChipRatingInfo.au16DimVal[u8DimIdx];

        //if (GetDimLockOption1(0))
        if( u16DimLockOption1 )
        {
            //for (i = 0; i < GetValuesDefined(0); ++i)
            //for (i = GetValuesDefined(0); i >= 0; --i)
            for (i = GetValuesDefined(u8DimIdx); i >= 0; --i)
            {
                //if( GETBIT(GetDimLockOption1(0), i) )
                if( GETBIT(u16DimLockOption1, i) )
                {
                    if (precede_common&& stGenSettingVchipRegion5.stRegin5Dimension[u8DimIdx].stRatingText[i][0])
                    {
                        // Check string buffer over flow
                        if( (int)(strlen((char*)pu8Str) + 2) < (u16StrBufSize-1) )
                        {
                            strcat((char*)pu8Str, ", ");
                        }
                    }
                    ...
                    // Check string buffer over flow
                    int iCurStrSize = strlen((char*)pu8Str);
                    U8 u8StrCpySize = strlen((char *)(stGenSettingVchipRegion5.stRegin5Dimension[u8DimIdx].stRatingText[i]));

                    if( ( iCurStrSize + u8StrCpySize) >= (u16StrBufSize-1) )
                    {
                        u8StrCpySize = u16StrBufSize - 1 - iCurStrSize;
                    }

                    strncat((char*)pu8Str, (char*)stGenSettingVchipRegion5.stRegin5Dimension[u8DimIdx].stRatingText[i], u8StrCpySize );
...
#else // ENABLE_ATSC_RRT_NEW_STRUCT
...
#endif // ENABLE_ATSC_RRT_NEW_STRUCT
...
BOOLEAN MApp_VChip_Is_VChipRatingInfo_Has_DimVal(void)
{
#if( ENABLE_ATSC_RRT_NEW_STRUCT )
    U8 u8DimIdx;
    ...
    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
    {
        ...
        if( (g_stVChipRatingInfo.au16DimVal[u8DimIdx] > 0) && (g_stVChipRatingInfo.au16DimVal[u8DimIdx] < GetValuesDefined1(u8DimIdx)) )
            return TRUE;
    }
...
static void UiMenuStr_GetRatingString(U8 *pu8Str, U16 u16StrBufSize)
{
    ...
    if (g_stVChipRatingInfo.RatingType == VCHIP_RATING_TYPE_REG5)
    {
        if (bExtra)
        {
            ...
            {
            #if 1
                if( MApp_VChip_Is_VChipRatingInfo_Has_DimVal() )
            #else
                if ( (g_stVChipRatingInfo.u8DimVal0 > 0 && (g_stVChipRatingInfo.u8DimVal0 < GetValuesDefined1(0)))
                   ||(g_stVChipRatingInfo.u8DimVal1 > 0 && (g_stVChipRatingInfo.u8DimVal1 < GetValuesDefined1(1)))
                   ||(g_stVChipRatingInfo.u8DimVal2 > 0 && (g_stVChipRatingInfo.u8DimVal2 < GetValuesDefined1(2)))
...
U8* EPGPrevProgramNumberText(void)
{
    ...
    u16PreProgIndx = MApp_GetPrevNextProgramIndex(TRUE);
    ...
    if(IsDTVInUse() || IsATVInUse())
    {
        ...
        msAPI_ChProc_ATSC_Get_VirCh_By_MainListIdx(u16PreProgIndx, &stVirCh_Tmp);
        MApp_UiMenuFunc_ConvertTempChannelProgramNumber2Str( &(stVirCh_Tmp), au8Section);
    #else
...
U8* EPGNextProgramNumberText(void)
{
    ...
    #if 1
        MS_VIRTUAL_CHANNEL stVirCh_Tmp;
        msAPI_ChProc_ATSC_Get_VirCh_By_MainListIdx(NextProgIndx, &stVirCh_Tmp);
        MApp_UiMenuFunc_ConvertTempChannelProgramNumber2Str( &(stVirCh_Tmp), au8Section);
    #else
