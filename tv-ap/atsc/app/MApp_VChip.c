...
BOOLEAN MApp_VChip_IsEventHasRatingInfo(MS_EPG_RATING *pstCurEvn)
{
#if(ENABLE_ATSC_RRT_NEW_STRUCT)

    U8 u8DimIdx;

    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
    {
        if( pstCurEvn->au168DownloadableRating[u8DimIdx] != 0 )
            return TRUE;
    }
...
void MApp_VChip_EPGRating2RatingInfo(MS_EPG_RATING *pstCurEvn, MS_VCHIP_RATING_INFO *pstVChipRatingInfo)
{
    ...
    if (pstCurEvn->fRatingRxIsOK)
    {
        ...
        {
            if( MApp_VChip_IsEventHasRatingInfo(pstCurEvn) )
            {
                // Copy rating info from EPG-Event to DB
            #if(ENABLE_ATSC_RRT_NEW_STRUCT)
                {
                    U8 u8DimIdx;
                    ...
                    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
                    {
                        pstVChipRatingInfo->au16DimVal[u8DimIdx] = pstCurEvn->au168DownloadableRating[u8DimIdx];
                    }
                }

            #else // #if(ENABLE_ATSC_RRT_NEW_STRUCT)
            ...
            #endif //#if(ENABLE_ATSC_RRT_NEW_STRUCT)
...
static BOOLEAN MApp_VChip_EIA608DataToVChipRatingInfo(MS_VCHIP_RATING_INFO *pstVChipRatingInfo)
{
    ...
    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
    {
        pstVChipRatingInfo->au16DimVal[u8DimIdx] = 0;
    }
...
#if(ENABLE_ATSC_RRT)
BOOLEAN MApp_VChip_Compare_VCHIP_RATING_INFO_DimVal(MS_VCHIP_RATING_INFO *pstRatingInfo_1, MS_VCHIP_RATING_INFO *pstRatingInfo_2)
{
#if(ENABLE_ATSC_RRT_NEW_STRUCT)

    U8 u8DimIdx;

    //for( u8DimIdx = 0; u8DimIdx < 10; u8DimIdx += 1)
    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
    {
        if( pstRatingInfo_1->au16DimVal[u8DimIdx] != pstRatingInfo_2->au16DimVal[u8DimIdx] )
            return TRUE;
    }

#else

    if( pstRatingInfo_1->u8DimVal0 != pstRatingInfo_2->u8DimVal0 ||
        pstRatingInfo_1->u8DimVal1 != pstRatingInfo_2->u8DimVal1 ||
        pstRatingInfo_1->u8DimVal2 != pstRatingInfo_2->u8DimVal2 ||
        pstRatingInfo_1->u8DimVal3 != pstRatingInfo_2->u8DimVal3 ||
        pstRatingInfo_1->u8DimVal4 != pstRatingInfo_2->u8DimVal4 ||
        pstRatingInfo_1->u8DimVal5 != pstRatingInfo_2->u8DimVal5 ||
        pstRatingInfo_1->u8DimVal6 != pstRatingInfo_2->u8DimVal6 ||
        pstRatingInfo_1->u8DimVal7 != pstRatingInfo_2->u8DimVal7 ||
        pstRatingInfo_1->u8DimVal8 != pstRatingInfo_2->u8DimVal8 ||
        pstRatingInfo_1->u8DimVal9 != pstRatingInfo_2->u8DimVal9
      )
    {
        return TRUE;
    }
...
BOOLEAN MApp_VChip_Compare_VCHIP_RATING_INFO(MS_VCHIP_RATING_INFO *pstRatingInfo_1, MS_VCHIP_RATING_INFO *pstRatingInfo_2)
{
    if( pstRatingInfo_1->RatingType  != pstRatingInfo_2->RatingType ||
        pstRatingInfo_1->RatingLevel != pstRatingInfo_2->RatingLevel ||
        pstRatingInfo_1->TV_FVSLD    != pstRatingInfo_2->TV_FVSLD ||

    #if(ENABLE_ATSC_RRT)
      #if 1 // Use new API
        MApp_VChip_Compare_VCHIP_RATING_INFO_DimVal(pstRatingInfo_1, pstRatingInfo_2) ||
      #else
      ...
      #endif
    #endif
        pstRatingInfo_1->MPAALevel  != pstRatingInfo_2->MPAALevel ||
        pstRatingInfo_1->CanEngLevel != pstRatingInfo_2->CanEngLevel ||
        pstRatingInfo_1->CanFreLevel != pstRatingInfo_2->CanFreLevel
      )
    {
        return TRUE;
    }
...
void MApp_VChip_Copy_VCHIP_RATING_INFO(MS_VCHIP_RATING_INFO *pstRatingInfo_Dest, MS_VCHIP_RATING_INFO *pstRatingInfo_Src)
{
    if( (pstRatingInfo_Dest == NULL)
      ||(pstRatingInfo_Src == NULL )
      )
    {
        return;
    }
...
#if 1
    *pstRatingInfo_Dest = *pstRatingInfo_Src;

#else
    pstRatingInfo_Dest->RatingType = pstRatingInfo_Src->RatingType;
    ...
    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
    {
        pstRatingInfo_Dest->au16DimVal[u8DimIdx] = pstRatingInfo_Src->au16DimVal[u8DimIdx];
    }

  #else
    pstRatingInfo_Dest->u8DimVal0  = pstRatingInfo_Src->u8DimVal0;
    pstRatingInfo_Dest->u8DimVal1  = pstRatingInfo_Src->u8DimVal1;
    pstRatingInfo_Dest->u8DimVal2  = pstRatingInfo_Src->u8DimVal2;
    pstRatingInfo_Dest->u8DimVal3  = pstRatingInfo_Src->u8DimVal3;
    pstRatingInfo_Dest->u8DimVal4  = pstRatingInfo_Src->u8DimVal4;
    pstRatingInfo_Dest->u8DimVal5  = pstRatingInfo_Src->u8DimVal5;
    pstRatingInfo_Dest->u8DimVal6  = pstRatingInfo_Src->u8DimVal6;
    pstRatingInfo_Dest->u8DimVal7  = pstRatingInfo_Src->u8DimVal7;
    pstRatingInfo_Dest->u8DimVal8  = pstRatingInfo_Src->u8DimVal8;
    pstRatingInfo_Dest->u8DimVal9  = pstRatingInfo_Src->u8DimVal9;
  #endif

    pstRatingInfo_Dest->RatingLevel= pstRatingInfo_Src->RatingLevel;
    pstRatingInfo_Dest->MPAALevel  = pstRatingInfo_Src->MPAALevel;
    pstRatingInfo_Dest->CanEngLevel= pstRatingInfo_Src->CanEngLevel;
    pstRatingInfo_Dest->CanFreLevel= pstRatingInfo_Src->CanFreLevel;
    pstRatingInfo_Dest->TV_FVSLD   = pstRatingInfo_Src->TV_FVSLD;
#endif
}
        ...
        if (MApp_EpgDB_GetCurEvent(&stEventInfo) == TRUE)
        {
            ...
            else // 608 only or have Eit but no EIT rating
            {
                if((fSearchRatingfromPMT) &&(stPMTRatinginfo.fRatingRxIsOK))
                {
                    ...
                    if(
                    #if 1 // Use new API
                        MApp_VChip_Compare_VCHIP_RATING_INFO(&stEITRatingInfo, pstVChipRatingInfo)
                    #else
                    ...
                    )
                    {
                        ...
                        for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
                        {
                            pstVChipRatingInfo->au16DimVal[u8DimIdx] = stEITRatingInfo.au16DimVal[u8DimIdx];
                        }
	    ...
	    //Rating in PMT
            if(fSearchRatingfromPMT)
            {
                ...
                if(stPMTRatinginfo.fRatingRxIsOK)
                {
                    ...
                    {
                    #if 1 // Use new API
                        // Copy rating info from stEITRatingInfo to pstVChipRatingInfo
                        MApp_VChip_Copy_VCHIP_RATING_INFO(pstVChipRatingInfo, &stEITRatingInfo);
                    #else
                        pstVChipRatingInfo->RatingType = stEITRatingInfo.RatingType;
                        pstVChipRatingInfo->u8DimVal0  = stEITRatingInfo.u8DimVal0;
                        pstVChipRatingInfo->u8DimVal1  = stEITRatingInfo.u8DimVal1;
...
#define ValuesDefined(n)    (1 << stGenSettingVchipRegion5.stRegin5Dimension[n].u8Values_Defined)
    ...
    #define ShouldLockDim(n)  ( (stGenSettingVchipRegion5.stRegin5Dimension[n].u32CurrentOption) & (pstVChipRatingInfo->au16DimVal[(n)]) )

    U8 u8DimIdx;
    for( u8DimIdx = 0; u8DimIdx < REGION5_DIMENSION_NUM; u8DimIdx += 1)
    {
        //if( ShouldLockDim(0) && (pstVChipRatingInfo->u8DimVal0 < ValuesDefined(0)) )
        if( ShouldLockDim(u8DimIdx) && (pstVChipRatingInfo->au16DimVal[u8DimIdx] < ValuesDefined(u8DimIdx)) )
        {
            DBG_RRT5( printf("[RRT5]Lock dim %u (%x, %x)\n", u8DimIdx,
                        (U16)stGenSettingVchipRegion5.stRegin5Dimension[u8DimIdx].u32CurrentOption,
                        (U16)pstVChipRatingInfo->au16DimVal[u8DimIdx]
                      ); );
