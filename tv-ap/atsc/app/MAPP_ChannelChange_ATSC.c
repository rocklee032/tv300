...
#if 0 // Move to MApp_ChannelChange
...
static void MApp_ChChg_EnCh_Setup_Audio(void)
{
    ...
    DEBUG_ENABLE_CHANNEL_ATSC( printf("MApp_ChChg_EnCh_Setup_Audio()\n" ); );
...
static void MApp_ChChg_EnCh_Setup_Video(MS_VIRTUAL_CHANNEL * pVirCh)
{
    ...
    DEBUG_ENABLE_CHANNEL_ATSC( printf("MApp_ChChg_EnCh_Setup_Video()\n" ); );
...
void MApp_ChannelChange_EnableChannel_ATSC (void)
{
    ...
    U16 u16CurMainListProgIdx = MApp_ChanProc_GetCurMainListProgIndex();
    DEBUG_ENABLE_CHANNEL_ATSC( printf("u16CurMainListProgIdx=%u\n", u16CurMainListProgIdx ); );
    ...
    if( FALSE == msAPI_ChProc_ATSC_Get_VirCh_By_MainListIdx(u16CurMainListProgIdx, &g_ATSC_stCurVirCh) )
    {
        printf("\nError: Get VirCh failed! u16MainListProgIdx=%u\n", u16CurMainListProgIdx);
        return;
    }

    pstVirtualCh = &g_ATSC_stCurVirCh;
    ...
    if (MApp_ChanProc_IsCurProgIndexAir())
    {
       ...
       u16VirChIdx = VIR_CH_IDX_AIR(u16CurMainListProgIdx);
    }
    else
    {
        MApp_DB_ATSC_InitBaseAddr(ANT_CATV);
        u16VirChIdx = VIR_CH_IDX_CATV(u16CurMainListProgIdx - stChSettingDisp[ANT_AIR].u16MainListSrvNum);
    }
    //pstVirtualCh = MApp_ChanProc_GetChannelPointer( u16VirChIdx );
    MApp_ChanProc_GetSingleChannel(&g_ATSC_stCurVirCh, u16VirChIdx);
    pstVirtualCh = &g_ATSC_stCurVirCh;

    MApp_DB_ATSC_InitBaseAddr((EN_ANT_TYPE)ANT_TYPE);
  #else
    U16 u16VirChIdx = VIR_CH_IDX(u16CurMainListProgIdx);
    ...
    if (MApp_ChanProc_IsCurProgIndexAir())
    {
        u8CurrentPsyNumber = PHY_CH_IDX_AIR(u16CurMainListProgIdx);
    }
    else
    {
        u8CurrentPsyNumber = PHY_CH_IDX_CATV(u16CurMainListProgIdx - stChSettingDisp[ANT_AIR].u16MainListSrvNum);
    }
  #else
    u8CurrentPsyNumber = PHY_CH_IDX(u16CurMainListProgIdx);
  #endif
    ...
    MApp_ChanProc_Get_TPSetting( &g_ATSC_stCurVirCh, &stTPSettingAtscTmp);
...
//#if ENABLE_ATV_CHANNEL_CHANGE_FINETUNING
    ...
    else
    {
        ...
        //DEBUG_ENABLE_CHANNEL_ATSC( printf("pstVirtualCh=0x%X\n", pstVirtualCh ); );
        //DEBUG_ENABLE_CHANNEL_ATSC( printf("pstVirtualCh->u16PcrPid=0x%X\n", pstVirtualCh->u16PcrPid ); );
        //DEBUG_ENABLE_CHANNEL_ATSC( printf("pstVirtualCh_VCT=0x%X\n", pstVirtualCh_VCT ); );
        ...
        else if( pstVirtualCh_VCT != NULL )
        {
            if( pstVirtualCh_VCT->u16PcrPid )
            {
        ...
        DEBUG_ENABLE_CHANNEL_ATSC( printf(" g_ChChg_u16CurChAudioPid=0x%X\n", g_ChChg_u16CurChAudioPid ); );
    ...
    // Nosity PSIP
    MApp_Psip_Notify_ChannelChanged();
...
#if ENABLE_ATSC_CHLIST_DISP_TOGETHER
    if( stGenSetting.g_SysSetting.u16MainListLastWatched > (stChSettingDisp[ANT_AIR].u16MainListSrvNum + stChSettingDisp[ANT_CATV].u16MainListSrvNum)
...
#else
    if( stGenSetting.g_SysSetting.u16MainListLastWatched[ANT_TYPE] > stChSetting.u16MainListSrvNum
     || stGenSetting.g_SysSetting.u16MainListLastWatched[ANT_TYPE] == 0
      )
    {
