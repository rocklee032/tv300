...
static U8 iu8Loop_j;
static U8 iu8Buffer_k;
static U32 iu32Buffer_i;
...
static U8* MApp_Dmx_Parse_CAD_RRT5(U8 *pu8CAD, MS_EPG_RATING *pRating)
{
    ...
    for (iu8Loop_j = 0; iu8Loop_j < u8rated_dimensions; iu8Loop_j++)
    {
        ...
        if( pu8CAD[0] < REGION5_DIMENSION_NUM )
        {
            pRating->au168DownloadableRating[ pu8CAD[0] ] |= (0x01 << ((pu8CAD[1] & 0x0F) - 1));
        }
...
static BOOLEAN MApp_Dmx_PMT_CheckSoundTrackData(void)
{
    ...
    for(u8i = 0; u8i< u8AudLangNum; u8i++)
    {
...
static void MApp_Dmx_PAT_Parse(MS_PAT_ITEM_ATSC*pastPATItem_func, U8 *pu8Section)
{
    ...
    */
    ...
    for(u8Loop = 0; u8Loop < u8MaxChNumOfMutiSec; u8Loop++)
    {
        ...
        //printf("%u pastPATItem_func[u8ChIndex].u16PID=0x%X\n", __LINE__, pastPATItem_func[u8ChIndex].u16PID);
        ...
        //printf("%u pastVirtualCh[u8ChIndex].u16ProgramNumber=%u\n", __LINE__, pastVirtualCh[u8ChIndex].u16ProgramNumber);
        //printf("%u pastVirtualCh[u8ChIndex].u16PmtPid=%u\n", __LINE__, pastVirtualCh[u8ChIndex].u16PmtPid);
...
static BOOLEAN MApp_Dmx_PMT_CheckVersionChange(U8 *pu8Section)
{
    ...
    {
        if(!pstVirtualCh->astSoundTrack[u8SoundTrackIdx].u8AudPidInPMT)
        {
            u8AudioNuminVCT++;
            ...
            memcpy(&astSoundTrackVCT[iu8Loop_j], &pstVirtualCh->astSoundTrack[u8SoundTrackIdx], sizeof(MS_SOUND_TRACK_ATSC));
            iu8Loop_j++;
        }
    }
    ...
    while(pu8LoopPosition != NULL)
    {
        ...
        if(u8AudLangNum_New < MAX_AUD_LANG_NUM_ATSC)
        {
            ...
            for(iu8Loop_i=0; iu8Loop_i < u8AudLangNum; iu8Loop_i++)
            {
...
static void MApp_Dmx_EAS_Parse(MS_EAS_ST *pstEas, U8 *pu8Section)
{
    ...
    */
    ...
    if (u8ExceptionCount > MAX_COUNT_OF_EXCEPTION)
    {
...
static BOOLEAN MApp_Dmx_EIT_Check(MS_EIT_PARSE_PARAM_ATSC *pParse, U8 pu8Sec4K[])
{
  ...
  #if 0//(ATSC_EPG_ONLY_CUR_CHANNEL)
...
static MS_BOOL MApp_Dmx_RRT_CheckVersionChange(const U8* const _pu8Section)
{
    ...
    {
        DMX_DBG( DMX_RRT, printf("MApp_Dmx_RRT_CheckVersionChange => [VersionNo change]%u %u\n", u8CurVersionNo, stGenSettingVchipRegion5.u8VersionNo ));
        //MSAPI_MEMORY_FREE(pu8TempRRTRatingDes,BUF_ID_ATSC);
...
// For notify PSIP that channel changed
void MApp_Psip_Notify_ChannelChanged(void)
{
    // todo
}
