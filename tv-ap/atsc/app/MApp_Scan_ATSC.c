...
static MS_VIRTUAL_CHANNEL_INFO * pastVirtualChInfo_2 = NULL;

U32 gU32TVScanTotalTime;

//===============================================================================

void MApp_Scan_Clear_VirChInfo2(void)
{
...
BOOLEAN MApp_Scan_CopyTo_VirChInfo2(void)
{
    MApp_Scan_Clear_VirChInfo2();

    if( u16NumOfVirCh2bShifted == 0 )
    {
        printf("\nError at MApp_Scan_CopyTo_VirChInfo2\n");
        return FALSE;
    }
    ...
    //if( u16NumOfVirCh2bShifted )
    {
        ...
        if (pastVirtualChInfo_2 == NULL)
        {
            printf("\nError: pastVirtualChInfo_2 alloc memory failed!\n");
            return FALSE;
        }
...
void MApp_Scan_CopyFrom_VirChInfo2(void)
{
    if( u16NumOfVirCh2bShifted == 0 || pastVirtualChInfo_2 == NULL )
    {
        printf("\nError at MApp_Scan_CopyFrom_VirChInfo2\n");
        return ;
    }
...
void MApp_Scan_ATSC_Disable_VDEC(void)
{
    VDEC_Status stVDEC_Status;
    memset( &stVDEC_Status, 0, sizeof(stVDEC_Status) );

    if( FALSE == MApi_VDEC_GetStatus(&stVDEC_Status) )
    {
        return ;
    }

    if( stVDEC_Status.bInit )
    {
    #if( ATSC_CC != NO_CC )
        //stop cc parse
        {
...
BOOLEAN MApp_Scan_ATSC_Init(void)
{
    ...
    //  Enable blue-screen to Disable XC inputsource and mute screen
    msAPI_Scaler_SetBlueScreen( ENABLE, E_XC_FREE_RUN_COLOR_BLACK, NULL, MAIN_WINDOW);

    // Stop VDEC
    MApp_Scan_ATSC_Disable_VDEC();
    ...
    else //MANUAL_SCAN
    {
        ...
        if(u8RFCh < u8MaxRFCh)
        {
            ...
            else
            {
                ...
                u8PrePhyChIdx          = PHY_CH_IDX(MApp_ChanProc_GetPreProgIndexOfMainList());

                if(u8PrePhyChIdx==u8CurPhyChIdx)
                {
                    // use astChannelBuf[0].stSrvInfo as temp buffer to get previous virtual channel
                    MApp_ChanProc_GetSingleVirtualChannel(&g_stPreVirtualCh, MApp_ChanProc_GetPreProgIndexOfMainList());
                ...
                {
                    u8PreVirChInfoIdx   = stChSetting.astMainList[MApp_ChanProc_GetPreProgIndexOfMainList()].u8VirChInfoIdx;
                }
            }
            ...
            {
            #if 1 // Use API
                if( FALSE == MApp_Scan_CopyTo_VirChInfo2() )
                {
                    __ASSERT(0);
                    return FALSE;
                }
...
//******************************************************************************
//
#define DEBUG_CHECK_SERVICE(x)  //x
...
BOOLEAN MApp_Scan_ATSC_CheckServiceInfo(MS_VIRTUAL_CHANNEL *pstVirtualCh_func, MS_VIRTUAL_VCT_CHANNEL *p_stVirtualCh_VCT, U8 u8CurPatSrvItem)
{
    ...
    DEBUG_CHECK_SERVICE( PRINT_CURRENT_LINE(); );
...
*/
    ...
    DEBUG_CHECK_SERVICE( printf("pstVirtualCh_func->u16ScrambleChStatus=0x%X\n", pstVirtualCh_func->u16ScrambleChStatus); );
    ...
    {
        DEBUG_CHECK_SERVICE( printf("u16VideoPid=0x%X, VCT.u16VideoPid=0x%X,\n", pstVirtualCh_func->u16VideoPid, p_stVirtualCh_VCT->u16VideoPid); );

        BOOLEAN bVDECPlayed = FALSE;
        ...
        if ( pstVirtualCh_func->u16VideoPid ||p_stVirtualCh_VCT->u16VideoPid  )
        {
            ...
            // Init VDEC...
            VDEC_CodecType eVDEC_CodecType_New = E_VDEC_CODEC_TYPE_MPEG2;

            if( pstVirtualCh->stVirtualChNum.u8VideoStreamType == VIDEO_STREAM_TYPE_H264 )
                eVDEC_CodecType_New = E_VDEC_CODEC_TYPE_H264;
        #if( CHIP_SUPPORT_HEVC )
            else if( pstVirtualCh->stVirtualChNum.u8VideoStreamType == VIDEO_STREAM_TYPE_H265 )
                eVDEC_CodecType_New = E_VDEC_CODEC_TYPE_HEVC;
        #endif

        #if( (MEMORY_MAP == MMAP_32MB) && (ENABLE_32M_H264 == 0) )
            if( eVDEC_CodecType_New != E_VDEC_CODEC_TYPE_MPEG2 )
            {
                printf("\nWarning: Force VDEC = MPEG2\n");
            }
        #endif

            if( (stVDEC_Status.bInit == 0)
              ||(eVDEC_CodecType_New != msAPI_VID_GetCodecType())
              ||(fTableVersionChanged)
              )
            {
                msAPI_VID_SetMappingAVCParameter(eVDEC_CodecType_New);
        ...
        if ( u16AudioPID )
        {
            for(i=0 ; i<DESCRAMBLE_COUNT ; i++)
            {
                ...
                {
                #if(CHIP_FAMILY_MAZDA==CHIP_FAMILY_TYPE) // This is a temp patch for MAZDA
                    if(1)
                #else
                  #if (AUDIO_CPU_IS_R2)
...
BOOLEAN MApp_Scan_ATSC_AddSrv2DB(MS_PHYSICAL_CHANNEL *pstPhysicalCh_func, MS_VIRTUAL_CHANNEL_INFO *pstVirtualChInfo, MS_VIRTUAL_CHANNEL *pstVirtualCh_func, EN_ANT_TYPE enAntennaType)
{
    ...
    if(iu16Loop_i <= u16VirChMaxNum)
    {
        ...
        SCAN_DBINFO_ATSC( printf("AddSrv2DB>> u16PmtPid=0x%X\n", pstVirtualCh_func->u16PmtPid ); );
        ...
        case STATE_SCAN_GET_TABLE:
        {
            ...
            if (MApp_Dmx_GetScanTables_ATSC() == EN_PARSER_NO_ERROR)
            {
                ...
                for(u8Loop = 0; u8Loop < u8TotalTableItem; u8Loop++)
                {
                    ...
                    //printf("%u pstVirtualCh->u16PmtPid=%X\n", __LINE__, pstVirtualCh->u16PmtPid );
                ...
                {
                    MApp_ChanProc_GetSingleChannel(&g_ATSC_stCurVirCh, VIR_CH_IDX(MApp_ChanProc_GetCurMainListProgIndex()));
                }
            }
            else
            {
