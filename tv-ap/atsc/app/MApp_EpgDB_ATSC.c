...
void MApp_EPG_Demux_Reset(void)
{
    ...
    stEpgDBInfo.u8CntSourceNum = 1;
    stEpgDBInfo.au16SrcID[0] = MApp_ChanProc_GetCurChannelPointer()->u16SourceId;
#else
...
//    stEpgDBInfo.u8CntSourceNum = stChSetting.astPhysicalCh[stChSetting.astMainList[MApp_ChanProc_GetCurProgIndex()].u8PhysicalChIdx].u8NumOfVirCh;
    ...
    U16 u16PhyIdx = PHY_CH_IDX(MApp_ChanProc_GetCurProgIndex());
    U16 u16VirIdx = stChSetting.astPhysicalCh[u16PhyIdx].u16VirChInfoStartIdx;

#if (ENABLE_ATSC_EPG_DB_2016)
    U16 u16ChNum = MIN(NEW_MAX_TAB_NUM, stChSetting.astPhysicalCh[u16PhyIdx].u8NumOfVirCh);
#else
    U16 u16ChNum = MIN(MAX_TBL_NUM, stChSetting.astPhysicalCh[u16PhyIdx].u8NumOfVirCh);
#endif

    for (stEpgDBInfo.u8CntSourceNum = 0; stEpgDBInfo.u8CntSourceNum < u16ChNum; stEpgDBInfo.u8CntSourceNum++)
    {
        stEpgDBInfo.au16SrcID[stEpgDBInfo.u8CntSourceNum] = MApp_ChanProc_GetTempChannelPointer(u16VirIdx++)->u16SourceId;

        //printf("%d au16SrcID = %d\n", stEpgDBInfo.u8CntSourceNum, stEpgDBInfo.au16SrcID[stEpgDBInfo.u8CntSourceNum]);
    }
#endif
