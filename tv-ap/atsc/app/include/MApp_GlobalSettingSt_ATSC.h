...
#if !ATSC_A_C_DIFF_DB && (MAX_NUM_OF_AIR_VIR_CH != MAX_NUM_OF_CATV_VIR_CH)
#error "ATSC_A_C_DIFF_DB = DISABLE, MAX_NUM_OF_AIR_VIR_CH must equal MAX_NUM_OF_CATV_VIR_CH !!"
#endif
...
#define Support_Caption_Preview 0

typedef enum
{
    CAPTIONMENU_MODE_OFF =0,
    CAPTIONMENU_MODE_ON,
 #if 1//(PARSING_CC_WHENMUTE)
...
typedef struct _CaptionSettingOption_Type
{
    ...
    U8 u8CSOptionEdgeStyle;
    ///CC current program information edge color
    U8 u8CSOptionEdgeColor;
    ///CC current program information italics attr
    U8 u8CSOptionItalicsAttr;
    ///CC current program information underline attr
    U8 u8CSOptionUnderlineAttr;
} stCaptionSettingOption_Type;

typedef struct _CaptionSetting_Type
{
    U16 CaptionSettingCS;// check sum <<checksum should be put at top of the struct, do not move it to other place>>
    ///CC current program information DTV service mode
    U8 u8CaptionServiceModeDTV; // CC Mode for UI
    ///CC current program information ATV service mode
    U8 u8CaptionServiceModeATV; // CC Mode for UI
    ///CC current program information service mode
    U8 u8CaptionServiceMode;  // CC Service Mode for CC Lib
