...
void MApp_UI_NotifySysEvent(EnuSysEvent enSysEvent, void* pPara)
{
    ...
    #if(ENABLE_DVB)
        case E_SYS_EVENT_DTV_SERVICE_VER_CHANGE:
            DEBUG_UI_NOTIFY( printf("[E_SYS_EVENT_DTV_SERVICE_VER_CHANGE]\n"); );

            //gu8VerChange = TRUE;
            // If current OSD is channel-list, close it
            if( MApp_ZUI_GetActiveOSD() == E_OSD_CHANNEL_LIST )
            {
                //MApp_ZUI_ACT_ExecuteChannelListAction(EN_EXE_CLOSE_CURRENT_OSD);
                MApp_ZUI_ACT_ExecuteWndAction(EN_EXE_CLOSE_CURRENT_OSD);
            }
            // If current OSD is EPG, do what?
            else if( MApp_ZUI_GetActiveOSD() == E_OSD_EPG )
            {
            }
