...
#define DECODE_ONE_TIME_CHECK_POINT()  //printf("DeOn %u - %u\n", __LINE__, MsOS_GetSystemTime())
...
//==============================================================
    ...
    u32MADBuffAddr = (msAPI_AUD_GetDspMadBaseAddr(DSP_DEC));
    ...
    while(u32FileSize > 0)
    {
        ...
        if(es_info.aes_freeSpace > 0)
        {
            ...
            msAPI_MIU_Copy(u32FlashStart, es_info.aes_write_addr + u32MADBuffAddr, es_size, MIU_FLASH2SDRAM);
