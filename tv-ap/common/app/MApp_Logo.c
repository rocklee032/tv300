...
#define DEBUG_LOGO_TIME(x)              //x
#define DEBUG_BURST_INIT_GOP_TIME(x)    //x
...
U32 u32GopBuffer, MS_BOOL bHorStretch, MS_BOOL bVerStretch)
{
    ...
    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 1 at %u\n", MsOS_GetSystemTime()); );


    MApi_GOP_InitByGOP(&pGopInit, 0);
    //MApi_GOP_GWIN_SwitchGOP(0);

    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 2 at %u\n", MsOS_GetSystemTime()); );

    // Because panel is off, don't care garbage
    ...
    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 3 at %u\n", MsOS_GetSystemTime()); );

    MApi_GOP_GWIN_SetGOPDst(0, E_GOP_DST_OP0);

    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 4 at %u\n", MsOS_GetSystemTime()); );

  #if(GOP_SET_ALPHA_INVERSE)
    MApi_GOP_GWIN_SetAlphaInverse_EX(0, TRUE);
  #endif

    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 5 at %u\n", MsOS_GetSystemTime()); );
    ...
    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 6 at %u\n", MsOS_GetSystemTime()); );

    MApi_GOP_GWIN_CreateFBbyStaticAddr(u8FbId, u32DispX, u32DispY, u16Pitch, u16Height, E_MS_FMT_YUV422, u32GopBuffer);

    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 7 at %u\n", MsOS_GetSystemTime()); );

    MApi_GOP_GWIN_MapFB2Win(u8FbId, u8GwinId);

    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 8 at %u\n", MsOS_GetSystemTime()); );

    MApi_GOP_GWIN_SetWinDispPosition(u8GwinId, u32DispX, u32DispY);

    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 9 at %u\n", MsOS_GetSystemTime()); );

    MApi_GOP_GWIN_SetBlending(u8GwinId, FALSE, 0x3F);

    DEBUG_BURST_INIT_GOP_TIME( printf("BurstInitGOP - 10 at %u\n", MsOS_GetSystemTime()); );
}
#endif
...
BOOLEAN _MApp_Logo_LoadJpeg(U16 u16LogoBinID)
{
    ...
    DEBUG_LOGO_TIME( printf("Copy jpeg at %u\n", MsOS_GetSystemTime()); );
    ...
    DEBUG_LOGO_TIME( printf("Copy jpeg done at %u\n", MsOS_GetSystemTime()); );
...
void MApp_Logo_Display(BOOLEAN bDisplayLogo)
{
    ...
    if(bDisplayLogo)
    {
        ...
        DEBUG_LOGO_TIME( printf("Logo Disp at %u\n", MsOS_GetSystemTime()); );
        ...
        DEBUG_LOGO_TIME( printf("Logo Disp - 2 at %u\n", MsOS_GetSystemTime()); );

        // after clear frame buffer, flush cmd
        MApi_GFX_FlushQueue();

        DEBUG_LOGO_TIME( printf("Logo Disp - 3 at %u\n", MsOS_GetSystemTime()); );
        ...
        DEBUG_LOGO_TIME( printf("Logo Disp - 4 at %u\n", MsOS_GetSystemTime()); );
        ...
        DEBUG_LOGO_TIME( printf("Logo Disp - 5 at %u\n", MsOS_GetSystemTime()); );

        MApi_GOP_GWIN_Enable(u8GwinId, TRUE);

        DEBUG_LOGO_TIME( printf("Logo Disp done %u\n", MsOS_GetSystemTime()); );
    }
    else
    {
