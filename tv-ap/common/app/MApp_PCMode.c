...
#define MAX_HEIGHT_OF_DVI           (2160 + 10) // 2K
#else
...
static MS_BOOL _MApp_PCMode_Check_OOR(SCALER_WIN eWindow, XC_MODEPARSE_INPUT_INFO *psInputInfo)
{
    ...
    if( IsSrcTypeHDMI(psInputInfo->eSrcType) )
    {
        ...
        // Check 4Kx2K_60 timing
    #if(!ENABLE_HDMI_4K_2K_60)
        if( IS_IN_RANGE( psInputInfo->sDVI_HDMI_DE_Info.width, 3840, 4096)
          &&IS_IN_RANGE( psInputInfo->sDVI_HDMI_DE_Info.height, 2150, 2160)
          &&IS_IN_RANGE( psInputInfo->u16VFreqx10, 595, 605)
        )
        {
            // Don't support 4K@60
            //printf("");
        ...
        if(PCMODE_INFO[eWindow].eModeParseResult == XC_MODEPARSE_SUPPORT_MODE)
        {
            ...
            if(_MApp_PCMode_SetMode(eWindow))
            {
                ...
                if(IsHDMIInUse())
                {
                    ...
                    //PRINT_CURRENT_LINE();
                    printf("AUDIO_HDMI_RX_TYPE=%u\n", MApi_AUDIO_HDMI_RX_GetNonPCM() );
