...
{
    return stGenSetting.g_SysSetting.u8GameMode;
}
...
void MApp_MenuFunc_Set_GameMode( BOOLEAN bEnable )
{
    ...
    //PRINT_CURRENT_LINE();
    BOOLEAN bCurGameModeResult = MApi_XC_Sys_Get_GameMode();

    stGenSetting.g_SysSetting.u8GameMode = bEnable;
    printf("stGenSetting.g_SysSetting.u8GameMode=%u\n", stGenSetting.g_SysSetting.u8GameMode);

    printf("MApi_XC_Sys_Get_GameMode()=%u\n", MApi_XC_Sys_Get_GameMode());

    if( MApi_XC_Sys_Get_GameMode() != bCurGameModeResult )
    {
        // GameMode changed
        printf("GameMode changed => %u\n", MApi_XC_Sys_Get_GameMode());

        // Window Off
        U8 u8WinOnOff = DISABLE;
        MApp_Scaler_MainWindowOnOffEventHandler( SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW), &u8WinOnOff);

        // Set window again
        MApp_Scaler_MainWindowSyncEventHandler( SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW), 0);
