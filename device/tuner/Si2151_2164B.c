...
BOOLEAN mdev_GetSignalStrength(float* f_strength)
{
    ...
    *f_strength = custom_status->rssi;
    printf("RSSI=%f", *f_strength);
...
void devDigitalTuner_SetFreq ( double Freq,  RF_CHANNEL_BANDWIDTH eBandWidth)
{
    ...
    //do nothing , control by demod driver.    
...
static CUSTOM_Video_Sys_Enum Atv_system_enum_conversion(IF_FREQ eIF_Freq)
{
    CUSTOM_Video_Sys_Enum eSi_Std;
    switch(eIF_Freq)
    {
        case IF_FREQ_B:
        case IF_FREQ_G:                
            eSi_Std = SILABS_VIDEO_SYS_B;
            break;
        case IF_FREQ_I:    
            eSi_Std = SILABS_VIDEO_SYS_I;
            break;

        case IF_FREQ_DK:    
            eSi_Std = SILABS_VIDEO_SYS_DK;
            break;

        case IF_FREQ_L:    
            eSi_Std = SILABS_VIDEO_SYS_L;
            break;
        case IF_FREQ_L_PRIME:    
            eSi_Std = SILABS_VIDEO_SYS_LP;
            break;
        case IF_FREQ_MN:    
            eSi_Std = SILABS_VIDEO_SYS_M;
            break;
...
void devTunerSetFreq(U32 u32Freq)
{
    ...
    CUSTOM_Video_Sys_Enum CurrentStandard = Atv_system_enum_conversion(MDrv_IFDM_GetIF());


    if(CurrentStandard==SILABS_VIDEO_SYS_B)   //check B or G system
    {
        if(u32Freq>=300000000)  //over 300Mhz , system is G
            CurrentStandard=SILABS_VIDEO_SYS_GH;
    }



    SiLabs_API_TER_ATVConfig(front_end[fe]);

    front_end[fe]->Carrier_Cfg.freq = u32Freq;
    front_end[fe]->Analog_Cfg.video_sys= CurrentStandard;
    if((CurrentStandard==SILABS_VIDEO_SYS_L)||(CurrentStandard==SILABS_VIDEO_SYS_LP))//si_tv_system_lp
        front_end[fe]->Analog_Cfg.color = SILABS_COLOR_SECAM;
    else
        front_end[fe]->Analog_Cfg.color = SILABS_COLOR_PAL_NTSC;


    SiLabs_API_TER_Tuner_ATV_Tune(front_end[fe],0,0);
