...
//    LGS_SetMpegMode(0, 1,0);
    ...
    msAPI_Timer_Delayms(10); //add 20080616
//    LGS_SetManualMode();
...
#if (IS_MXL_TUNER)
    msAPI_Timer_Delayms(120); //waiting for tuner write
#else
    msAPI_Timer_Delayms(10); //waiting for tuner write
#endif
...
#elif (FRONTEND_DEMOD_TYPE == MONTAGE_M88DC2800_DEMOD)
    msAPI_Timer_Delayms(5); //waiting for tuner write
    M88DC2800_Config();
#elif (FRONTEND_DEMOD_TYPE == MONTAGE_M88DD2000_DEMOD)
    msAPI_Timer_Delayms(5); //waiting for tuner write
    M88DD2000_Config();
...
#elif (FRONTEND_DEMOD_TYPE == MSTAR_MSB1210_DEMOD)
    bEnable=bEnable;
#elif (FRONTEND_DEMOD_TYPE == LEGEND_8G52_DEMOD||FRONTEND_DEMOD_TYPE == LEGEND_8G80_DEMOD||FRONTEND_DEMOD_TYPE == LEGEND_8G75_DEMOD)
    bEnable=bEnable;
#elif (FRONTEND_DEMOD_TYPE == MONTAGE_M88DC2800_DEMOD)
    bEnable=bEnable;
#elif (FRONTEND_DEMOD_TYPE == MONTAGE_M88DD2000_DEMOD)
...
#elif (FRONTEND_DEMOD_TYPE == LEGEND_8G52_DEMOD||FRONTEND_DEMOD_TYPE == LEGEND_8G75_DEMOD)
    *quality = *quality;
    return E_RESULT_FAILURE;
...
BOOLEAN devCOFDM_Get_Modul_Mode(WORD *Modul_Mode)
{
    ...
    *Modul_Mode=*Modul_Mode;
    ...
    if(IsDVBTInUse())
    {
        ...
        #elif (FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
            return devDemod_Silab2164B_Get_DVBT_TPS_Parameter(Modul_Mode, TS_MODUL_MODE);
...
#elif (FRONTEND_DEMOD_TYPE == LEGEND_9x_DEMOD||FRONTEND_DEMOD_TYPE == LEGEND_8G52_DEMOD||FRONTEND_DEMOD_TYPE == LEGEND_8G80_DEMOD)
    MDrv_IIC_ReadBytes(U16SlaveID, u8AddrNum, paddr, u16size, pu8data);
    return TRUE;
