...
signed int   SiLabs_API_Demod_status_selection   (SILABS_FE_Context* front_end, CUSTOM_Status_Struct *status, unsigned char status_selection)
{
  ...
  if (front_end->standard == SILABS_SLEEP) {status->standard = SILABS_SLEEP; return 0;}
    ...
    status->standard = (CUSTOM_Standard_Enum)Custom_standardCode(front_end, front_end->SiDemod_FE->demod->rsp->dd_status.modulation);
    ...
    status->TS_clock_kHz    = front_end->SiDemod_FE->demod->rsp->dd_status.ts_clk_freq*10;
      ...
      status->standard = (CUSTOM_Standard_Enum)Custom_standardCode(front_end, front_end->SiDemod_FE->demod->prop->dd_mode.modulation);
    }
...
signed int   SiLabs_API_TER_Tuner_RSSI_status (SILABS_FE_Context *front_end, CUSTOM_Status_Struct *status)
{
	SILABS_TER_TUNER_Context *tuner_ter;
	SiTRACE("SiLabs_API_TER_Tuner_RSSI_status\n");
	tuner_ter = front_end->SiDemod_FE->tuner_ter;

	if (tuner_ter == NULL) {
		SiTRACE("No tuner_ter defined, SiLabs_API_TER_Tuner_RSSI_status can't be executed!\n");
	return 0;
	}
...
signed int   SiLabs_API_Text_status_selection    (SILABS_FE_Context* front_end, CUSTOM_Status_Struct *status, unsigned char *formatted_status, unsigned char status_selection)
{
  ...
  if (status->demod_die == 2) { safe_sprintf(formatted_status, " DEMOD B "); }
    ...
    switch (status->standard)
    {
        ...
        case SILABS_DVB_T : {
          safe_sprintf(formatted_status, "%s characteristics     %s ", formatted_status, Silabs_FFT_Text((CUSTOM_FFT_Mode_Enum)status->fft_mode));
          safe_sprintf(formatted_status, "%s %s "                , formatted_status, Silabs_Constel_Text((CUSTOM_Constel_Enum)status->constellation));
          safe_sprintf(formatted_status, "%s %s "                , formatted_status, Silabs_Coderate_Text((CUSTOM_Coderate_Enum)status->code_rate_hp));
          safe_sprintf(formatted_status, "%s %s "                , formatted_status, Silabs_GI_Text((CUSTOM_GI_Enum)status->guard_interval));
    ...
    case SILABS_ISDB_T: {
      ...
      break;
        ...
        case SILABS_DVB_C : {
          safe_sprintf(formatted_status, "%s characteristics     %s ", formatted_status, Silabs_Constel_Text((CUSTOM_Constel_Enum)status->constellation));
        ...
        case SILABS_DVB_C2: {
          ...
          safe_sprintf(formatted_status, "%s %s "                   , formatted_status, Silabs_Constel_Text((CUSTOM_Constel_Enum)status->constellation));
          safe_sprintf(formatted_status, "%s %s "                   , formatted_status, Silabs_Coderate_Text((CUSTOM_Coderate_Enum)status->code_rate));
          safe_sprintf(formatted_status, "%s %s "                   , formatted_status, Silabs_GI_Text((CUSTOM_GI_Enum)status->guard_interval));
          safe_sprintf(formatted_status, "%s %d kHz \n"             , formatted_status, status->bandwidth_Hz/1000);
        ...
        case SILABS_DVB_S : {
          ...
          safe_sprintf(formatted_status, "%s %s \n"                  , formatted_status, Silabs_Coderate_Text((CUSTOM_Coderate_Enum)status->code_rate));
          break;
        }
        case SILABS_DSS   : {
          safe_sprintf(formatted_status, "%s characteristics     %s ", formatted_status, Silabs_Constel_Text((CUSTOM_Constel_Enum)status->constellation));
        ...
        case SILABS_DVB_T2: {
          ...
          safe_sprintf(formatted_status, "%s %s "                   , formatted_status, Silabs_GI_Text((CUSTOM_GI_Enum)status->guard_interval));
          safe_sprintf(formatted_status, "%s %d kHz \n"             , formatted_status, status->bandwidth_Hz/1000);
          safe_sprintf(formatted_status, "%s %s "                   , formatted_status, Silabs_T2_Version_Text((CUSTOM_Pilot_T2_version_Enum)status->t2_version));
          ...
          safe_sprintf(formatted_status, "%s %s "                   , formatted_status, Silabs_Pilot_Pattern_Text((CUSTOM_Pilot_Pattern_Enum)status->pilot_pattern));
          safe_sprintf(formatted_status, "%s num_plp:%d "           , formatted_status, status->num_plp);
          safe_sprintf(formatted_status, "%s plp_id:%d "            , formatted_status, status->plp_id);
...
signed int   SiLabs_API_switch_to_standard       (SILABS_FE_Context *front_end, unsigned char standard, unsigned char force_full_init)
{
  ...
  front_end->init_ok = SiDemod_switch_to_standard (front_end->SiDemod_FE , Silabs_standardCode(front_end, (CUSTOM_Standard_Enum)standard), force_full_init);
...
signed int   SiLabs_API_lock_to_carrier          (SILABS_FE_Context *front_end, unsigned char standard, signed int freq, signed int bandwidth_Hz, unsigned char stream, unsigned int symbol_rate_bps, unsigned char constellation, unsigned char polarization, unsigned char band, signed int data_slice_id, signed int plp_id, unsigned char T2_lock_mode)
{
  ...
  printf("SiLabs_API_lock_to_carrier (front_end, %8s, %d, %d, %2s, %9d, %8s, %10s, %4s, %4d, %4d, %4d)\n", Silabs_Standard_Text((CUSTOM_Standard_Enum)standard), freq, bandwidth_Hz,Silabs_Stream_Text((CUSTOM_Stream_Enum)stream), symbol_rate_bps, \
      Silabs_Constel_Text((CUSTOM_Constel_Enum)constellation), Silabs_Polarization_Text((CUSTOM_Polarization_Enum)polarization), Silabs_Band_Text((CUSTOM_Band_Enum)band), data_slice_id , plp_id , T2_lock_mode);
...
#endif /* USB_Capability */
    if ( ((CUSTOM_TS_Mode_Enum)ts_mode != SILABS_TS_TRISTATE) | ((CUSTOM_TS_Mode_Enum)ts_mode != SILABS_TS_OFF) ) {
      front_end->active_TS_mode = (CUSTOM_TS_Mode_Enum)ts_mode;
    }
    return ts_mode;
...
signed int   SiLabs_API_TER_ATVConfig(SILABS_FE_Context* front_end)
{
	SiLabs_API_Tuner_I2C_Enable (front_end);

	front_end->SiDemod_FE->tuner_ter->prop->atv_video_mode.video_sys   = Silabs_videoSysCode (front_end, (CUSTOM_Video_Sys_Enum)front_end->Analog_Cfg.video_sys);
	//front_end->SiDemod_FE->tuner_ter->prop->atv_video_mode.color       = Silabs_colorCode    (front_end, front_end->Analog_Cfg.color);
	//atv_video_mode.video_sys, atv_video_mode.color
	if(Si2151_L1_SetProperty2(front_end->SiDemod_FE->tuner_ter, Si2151_ATV_VIDEO_MODE_PROP_CODE) != NO_Si2151_ERROR)
    {
		SiLabs_API_Tuner_I2C_Disable(front_end);
	...
	front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = front_end->Analog_Cfg.atv_agc_speed;
	if(Si2151_L1_SetProperty2(front_end->SiDemod_FE->tuner_ter, Si2151_ATV_AGC_SPEED_PROP_CODE) != NO_Si2151_ERROR)
    {
		SiLabs_API_Tuner_I2C_Disable(front_end);
		return ERROR_Si2151_SENDING_COMMAND;
    }
	...
	//front_end->SiDemod_FE->tuner_ter->prop->atv_artificial_snow.period = Si2151_ATV_ARTIFICIAL_SNOW_PROP_PERIOD_LONG;//long:0(def), short:1
	//front_end->SiDemod_FE->tuner_ter->prop->atv_artificial_snow.gain = Si2151_ATV_ARTIFICIAL_SNOW_PROP_GAIN_18DB;//default 0 -> 4
	//if(Si2151_L1_SetProperty2(front_end->SiDemod_FE->tuner_ter, Si2151_ATV_ARTIFICIAL_SNOW_PROP_CODE) != NO_Si2151_ERROR)
 //   {
 //      return ERROR_Si2151_SENDING_COMMAND;
 //   }
...
signed int   SiLabs_API_TER_Tuner_ATV_SetAgcSpeed  (SILABS_FE_Context *front_end,unsigned char agc_speed)
{
	switch(agc_speed)
	{
		case  0: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_AUTO; break;
		case  1: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_89; break;
		case  2: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_105; break;
		case  3: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_121; break;
		case  4: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_137; break;
		case  5: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_158; break;
		case  6: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_172; break;
		case  7: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_178; break;
		case  8: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_185; break;
		case  9: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_196; break;
		case 10: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_206; break;
		case 11: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_216; break;
		case 12: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_219; break;
		case 13: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_222; break;
		case 14: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_248; break;
		case 15: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_250; break;
		case 16: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_251; break;
		default: front_end->SiDemod_FE->tuner_ter->prop->atv_agc_speed.if_agc_speed = Si2151_ATV_AGC_SPEED_PROP_IF_AGC_SPEED_AUTO; break;
	}
...
//note: the define should be the same as SI2151_2164B.h
...
signed int   SiLabs_API_TER_Tuner_ATV_Tune       (SILABS_FE_Context *front_end,unsigned char tunint_check,unsigned int AtvTimeout)
{
  unsigned long freq;
  ...
  video_sys       = Silabs_videoSysCode    (front_end, (CUSTOM_Video_Sys_Enum)front_end->Analog_Cfg.video_sys);
  //trans           = Silabs_transmissionCode(front_end, front_end->Analog_Cfg.trans);//kwon,14,disable
  trans           = 0;//kwon,14
  color           = Silabs_colorCode       (front_end, (CUSTOM_Color_Enum)front_end->Analog_Cfg.color);
  ...
  SiTRACE("freq %ld, video_sys %d, trans %d\n", freq, video_sys, trans);
	...
	// 20151124 Half code 반영- 20150710,ATV 55.25MHz의 경우(SECAM-LP 에선 55.25MHz 미사용)
	if((front_end->SiDemod_FE->tuner_sat->i2c->address == SAT2_ADDRESS) || (front_end->SiDemod_FE->tuner_sat->i2c->address == SAT4_ADDRESS))
	{
		if(freq == 55250000){
				DIV_A = 207;DIV_B = 768;
		}
  ...
  switch (video_sys) {
    case SILABS_VIDEO_SYS_B  :
		front_end->SiDemod_FE->tuner_ter->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_B;
		freq += 2250000;

		// 20151123 ATV 사선bit 개선을 위한 CANCEL_TONE 적용 for 16_Single_T2S2_Half(5815M and AV2017)
		if((front_end->SiDemod_FE->tuner_sat->i2c->address == SAT2_ADDRESS) || (front_end->SiDemod_FE->tuner_sat->i2c->address == SAT4_ADDRESS))
		{
			if((freq == 69250000 + 2250000)||(freq == 140250000 + 2250000)||(freq == 48250000 + 2250000)){
				DIV_A = 200;DIV_B = 200;
			}
		}
      break;
    ...
    case SILABS_VIDEO_SYS_N  :
		front_end->SiDemod_FE->tuner_ter->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_M;
      break;
    case SILABS_VIDEO_SYS_GH :
		front_end->SiDemod_FE->tuner_ter->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_GH;
	//csm,13
		freq += 2750000;
		...
		if((front_end->SiDemod_FE->tuner_sat->i2c->address == SAT2_ADDRESS) || (front_end->SiDemod_FE->tuner_sat->i2c->address == SAT4_ADDRESS))
		{
			if((freq == 69250000 + 2750000)||(freq == 140250000 + 2750000)||(freq == 48250000 + 2750000)){
				DIV_A = 200;DIV_B = 200;
			}
		}
		break;
    ...
    case SILABS_VIDEO_SYS_I  :
		front_end->SiDemod_FE->tuner_ter->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_I;
	//csm,13
		freq += 2750000;
		break;
		//end
    case SILABS_VIDEO_SYS_L  :
		front_end->SiDemod_FE->tuner_ter->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_L;
	//csm,13
		freq += 2750000;
		break;
	case SILABS_VIDEO_SYS_DK:
		front_end->SiDemod_FE->tuner_ter->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_DK;
		//20130114,Issue: SECAM-DK/PAL-BG, 199.25 Mhz horizontal noise by ACI
		if((freq > 199000000) && (freq < 199999000))
		{
			front_end->SiDemod_FE->tuner_ter->prop->tuner_lo_injection.band_2 = Si2151_TUNER_LO_INJECTION_PROP_BAND_2_HIGH_SIDE  ; /* (default 'LOW_SIDE') */
		}
		else
		{
			front_end->SiDemod_FE->tuner_ter->prop->tuner_lo_injection.band_2 = Si2151_TUNER_LO_INJECTION_PROP_BAND_2_LOW_SIDE  ;
		}
		SiLabs_API_Tuner_I2C_Enable (front_end);
		if (Si2151_L1_SetProperty2(front_end->SiDemod_FE->tuner_ter, Si2151_TUNER_LO_INJECTION_PROP_CODE) != NO_Si2151_ERROR)
		{
			SiLabs_API_Tuner_I2C_Disable(front_end);
			return ERROR_Si2151_SENDING_COMMAND;
		}
    ...
    case SILABS_VIDEO_SYS_LP :
		front_end->SiDemod_FE->tuner_ter->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_L1;
      freq -= 2750000;
      break;
    default: {
  ...
  /* Center frequency end */
	...
	if(Si2151_L1_SetProperty2(front_end->SiDemod_FE->tuner_ter, Si2151_ATV_LIF_FREQ_PROP_CODE) != NO_Si2151_ERROR)
    {
      return ERROR_Si2151_SENDING_COMMAND;
    }

    AtvTimeout=AtvTimeout; /*avoid compile warning*/
    tunint_check=tunint_check; /*avoid compile warning*/
	//if(Si2151_ATVTune  (front_end->SiDemod_FE->tuner_ter, freq, video_sys, color, invert_signal, tunint_check,AtvTimeout) != NO_Si2151_ERROR)
	if(Si2151_Tune  (front_end->SiDemod_FE->tuner_ter, Si2151_TUNER_TUNE_FREQ_CMD_MODE_ATV, freq) != NO_Si2151_ERROR)
	{
		SiLabs_API_Tuner_I2C_Disable(front_end);
		return ERROR_Si2151_SENDING_COMMAND;
	}
...
signed int   SiLabs_API_TER_Tuner_SetAtvRfTop  (SILABS_FE_Context *front_end,unsigned char rf_top)
{
	switch(rf_top)
	{
		case  0: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_AUTO; break;
		case  1: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_P2DB; break;
		case  2: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_P1DB; break;
		case  3: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_0DB; break;
		case  4: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M1DB; break;
		case  5: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M2DB; break;
		case  6: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M3DB; break;
		case  7: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M4DB; break;
		case  8: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M5DB; break;
		case  9: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M6DB; break;
		case 10: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M7DB; break;
		case 11: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M8DB; break;
		case 12: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M9DB; break;
		case 13: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M10DB; break;
		case 14: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_M11DB; break;
		default: front_end->SiDemod_FE->tuner_ter->prop->atv_rf_top.atv_rf_top = Si2151_ATV_RF_TOP_PROP_ATV_RF_TOP_DEFAULT; break;
	}

	SiLabs_API_Tuner_I2C_Enable (front_end);

	if(Si2151_L1_SetProperty2(front_end->SiDemod_FE->tuner_ter, Si2151_ATV_RF_TOP_PROP_CODE) != NO_Si2151_ERROR)
	{
		SiLabs_API_Tuner_I2C_Disable(front_end);
		return ERROR_Si2151_SENDING_COMMAND;
	}
