...
static MS_BOOL volatile bBlockKeypadIsr = FALSE;

// In NOS, Use bBlockKeypadIsr is enough
#if 1 // NOS

    #define msKeypad_InitLock()
    #define msKeypad_Lock()
    #define msKeypad_UnLock()
...
static U8 volatile g_u8KeypadKey = 0xFF;
static U8 volatile g_u8KeypadRpt = 0;
static BOOLEAN volatile g_u8KeypadRes=MSRET_ERROR;
...
/******************************************************************************/
U8 msKeypad_Get_ADC_Channel(U8 u8Channel, U8 *pValue)
{
    return MHal_Keypad_Get_ADC_Channel(u8Channel, pValue);
}

#if 1
    #define ADCKeyValueOffest       0x0A
    #define msKeypad_CompareAdcValue( val_1, val_2) ( abs((val_1) - (val_2)) < ADCKeyValueOffest )
#else
    #define msKeypad_CompareAdcValue( val_1, val_2) ( (val_1) < (val_2) )
#endif
...
static BOOLEAN msKeypad_CH_GetKey(U8 Channel, U8 *pkey, U8* pflag)
{
    ...
    *pflag = 0;
    ...
    for ( i = 0; i < (KEYPAD_ISR_STABLE_NUM); i++ )
    {
        ...
        {
        #if 1
            if( msKeypad_CompareAdcValue(Key_Value, tADCKeyLevel[u8ChIdx][j]) )
        #else
...
void msKeypad_ClearBuffer(void)
{
    U8 i;
