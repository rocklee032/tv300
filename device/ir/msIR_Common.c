...
#include "msIR.h"
#include "msIR_Common.h"

#if (ENABLE_IR)
void msIR_ISR_1ms(void)
{
#if ((IR_MODE_SEL == IR_TYPE_SWDECODE_MODE) || (IR_MODE_SEL == IR_TYPE_SWDECODE_NEC_MODE))
    if(g_ucIrRepeatTimer > 0)
        g_ucIrRepeatTimer--;

  #if(CHIP_FAMILY_TYPE == CHIP_FAMILY_EULER)// Euler only
	if (g_ucIrRepeatTimer==0)
		IrRepeatStep =IrRepeatStep & BIT7;
  #endif
