...
extern BOOL g_bIsVideoPidChanged;
#endif
...
// mantis 1543572
#if ENABLE_CI_PLUS && MEDIASET_PREMIUM_SMART_CAM
            if(g_bIsVideoPidChanged)
            {
                msAPI_CI_ReInitial();
                g_bIsVideoPidChanged = FALSE;
            }
#endif
        }
        else
    ...
    msDebug_PrintChgSrcTime("Tune2RF done");
#endif
...
#elif((ENABLE_DVB_T2) && (FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT))
    if( eCurDemod == E_DEMODULATOR_TYPE_DVBT2)
    {
        devDemod_Silab2164B_Get_DVBT2_TPS_Parameter(pu16Modul_Mode, T2_MODUL_MODE);
        bRtn = TRUE;
    }
    else
#endif
