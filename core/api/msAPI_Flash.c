...
void msAPI_Flash_Init(void)
{
    ...
    DEBUG_FLASH_INIT( printf("msAPI_Flash_Init()\n"); );
    ...
    g_Flash_u32Size = u32FlashSize;

    s_Flash_u32WriteValidAddr_Start = 0x10000*FMAP_TOTAL_USE_BANK_END;
    s_Flash_u32WriteValidAddr_End = u32FlashSize - 1;

    DEBUG_FLASH_INIT( printf(" => g_Flash_u32Size=0x%x\n", g_Flash_u32Size); );
    DEBUG_FLASH_INIT( printf(" => s_Flash_u32WriteValidAddr_Start=0x%x\n", s_Flash_u32WriteValidAddr_Start); );
    DEBUG_FLASH_INIT( printf(" => s_Flash_u32WriteValidAddr_End=0x%x\n", s_Flash_u32WriteValidAddr_End); );
...
BOOLEAN msAPI_Flash_Read(U32 u32StartAddr, U32 u32ReadSize, U8 * user_buffer)
{
    //U32 u32FlashSize = 0;
    //MDrv_SERFLASH_DetectSize(&u32FlashSize);
    //ASSERT((u32StartAddr < g_Flash_u32Size));
    //ASSERT((u32ReadSize < g_Flash_u32Size));

    if( (u32StartAddr + u32ReadSize) >= g_Flash_u32Size )
    {
        printf("\nError: Read flash addr=0X%X, size=0x%X\n", u32StartAddr,  u32ReadSize);
    }
