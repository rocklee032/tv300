...
static BDMA_CpyType _MApi_BDMA_Convert_Type(MEMCOPYTYPE eType)
{
    ...
    default:
        printf("\nError: Unsupported BDMA_CpyType=%u\n", eType);
        break;
    }
...
{
    BDMA_Result eBDMA_Result = E_BDMA_OK;
    ...
    {
        case MIU_FLASH2SDRAM:
            if( TRUE != MDrv_SERFLASH_CopyHnd(u32Srcaddr, u32Dstaddr, u32Len, E_SPIDMA_DEV_MIU0, SPIDMA_OPCFG_DEF) )
            {
                printf("\nError: MDrv_SERFLASH_CopyHnd() failed!\n");
                eBDMA_Result = E_BDMA_FAIL;
            }
            break;

        case MIU_FLASH2VDMCU:
            if( TRUE != MDrv_SERFLASH_CopyHnd(u32Srcaddr, u32Dstaddr, u32Len, E_SPIDMA_DEV_VDMCU, SPIDMA_OPCFG_DEF) )
            {
                printf("\nError: MDrv_SERFLASH_CopyHnd() failed!\n");
                eBDMA_Result = E_BDMA_FAIL;
            }
        ...
        default: // Dram to Dram?
            //printf("BDMA(u32Srcaddr=0x%X, u32Dstaddr=0x%X, u32Len=0x%X)\n", u32Srcaddr, u32Dstaddr, u32Len);
            if( E_BDMA_OK != MDrv_BDMA_CopyHnd(u32Srcaddr, u32Dstaddr, u32Len, _MApi_BDMA_Convert_Type(eType), u8OpCfg) )
            {
                printf("\nError: MDrv_BDMA_CopyHnd() failed!\n");
                eBDMA_Result = E_BDMA_FAIL;
            }
            break;
    }
...
eRETCODE MDrv_DMA_LoadBin(BININFO *pBinInfo, U32 u32DstVA, U32 u32DecVA, U32 u32TmpVA)
{
    ...
    DEBUG_DMA_LOADBIN( printf("\n >> MDrv_DMA_LoadBin(0x%X, 0x%X(%s))\n",
                        pBinInfo->B_ID, pBinInfo->B_IsComp, GetComprressName(pBinInfo->B_IsComp)) );
    ...
    switch(pBinInfo->B_IsComp)
    {
    ...
    #endif
        {
        ...
        #if(ENABLE_DECOMP_ZDEC)
            if( MS_COMPRESS_ZDEC == pBinInfo->B_IsComp )
            {
                for(i=3;i<12;i++)
                {
            ...
            MsOS_Dcache_Flush( (MS_U32)(u32DstVA_Cached & (~15)),(MS_U32)((pBinInfo->B_Len+15) & (~15)));
            MsOS_FlushMemory();
...
eRETCODE MApi_BDMA_CopyFromResource(U32 u32FlashAddr, U32 destVA,U32 len)
{
    //printf("MApi_BDMA_CopyFromResource(u32FlashAddr=0x%X, destVA=0x%X, len=0x%x)\n", u32FlashAddr, destVA, len);
    ...
    // Check flash address
    if( u32FlashAddr >= g_Flash_u32Size )
    {
        printf("\nError: CopyFromResource: u32FlashAddr=0x%X\n", u32FlashAddr);
        //return FAILURE;
    }
  ...
  #if (ENABLE_DEBUG_BMP_MEM)
    {
        ...
        {
            PRINT_CURRENT_LINE();
            printf("u32FlashAddr=0x%X, destVA=0x%X, len=0x%X\n", u32FlashAddr, destVA, len);
        }
    }
  #endif
  ...
  #if(ENABLE_FLASH_ON_DRAM)
    if( msAPI_FlashOnDram_IsDataOnDram(u32FlashAddr, len ) )
    {
        if( FALSE == msAPI_FlashOnDram_CopyDataToDram(u32FlashAddr, u32PhyAdr, len ) )
        {
            return FAILURE;
        }
    }
  ...
  #endif
    {
        enBDMA_Result = MApi_BDMA_Copy(u32FlashAddr, u32PhyAdr, len, MIU_FLASH2SDRAM);
    ...
    msAPI_Flash_Read(u32FlashAddr, len, (U8 *)destVA);
#endif
