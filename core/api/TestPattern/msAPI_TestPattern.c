...
//=======================================================================
    ...
    #define TEST_PAT_PANEL_4K   1
#else
    #define TEST_PAT_PANEL_4K   0
#endif
...
U8 g_TestPattern_PixelAlign = 8;

#if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA) )
    #define PIX_AlIGN   32
    //#define PIX_AlIGN   g_TestPattern_PixelAlign
...
#else
    #define PIX_AlIGN   16  //16 for A6
    //#define PIX_AlIGN   g_TestPattern_PixelAlign  //16 for A6
#endif


#if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA) )
    #define TEST_PAT_USE_SRC_HDMI   0
#else
    #define TEST_PAT_USE_SRC_HDMI   0
#endif
    ...
    #define TEST_PAT_USE_BUF_ADDR   SCALER_FB_NO_VDEC_ADR

#elif( (CHIP_FAMILY_TYPE == CHIP_FAMILY_NASA) )

    #define TEST_PAT_USE_BUF_ADDR   SCALER_DNR_BUF_ADR
...
static void _DrawBMP_Init(void)
{
    ...
    else if(s_TestPattern_u16PanelWidth == 3840)
    {
       u16MemOffset = 3840;//(uc16PicWidth + PIX_AlIGN)&~(PIX_AlIGN-1);
       u16OPMFetch = 3840;//(uc16PicWidth)&~1;
    }
    else
    {
        PRINT_CURRENT_LINE();
        printf("\nError: Unknown panel size\n");
    }

    s_TestPattern_u16MemOffset = u16MemOffset;
    s_TestPattern_u16MemVLen = s_TestPattern_u16PanelHigh;

    s_TestPattern_u32DestBufMemAddr = TEST_PAT_USE_BUF_ADDR;
    ...
    MApi_XC_SetFrameBufferAddress( s_TestPattern_u32DestBufMemAddr, s_TestPattern_u32DestBufMemSize, MAIN_WINDOW);
    ...
    if( bUseScalerUtopia )
    {
    ...
    #if ( TEST_PAT_USE_SRC_HDMI )

        stXCSetWinInfo.enInputSourceType = INPUT_SOURCE_HDMI;

        MS_PQ_Hdmi_Info stPQHDMI_Info;
        memset(&(stPQHDMI_Info), 0, sizeof(MS_PQ_Hdmi_Info) );
        stPQHDMI_Info.bIsHDMI = FALSE; // g_HdmiPollingStatus.bIsHDMIMode;
        stPQHDMI_Info.enColorFmt = PQ_HDMI_COLOR_RGB;

        MDrv_PQ_SetHDMIInfo(PQ_MAIN_WINDOW, &stPQHDMI_Info);

        g_HdmiPollingStatus.bIsHDMIMode = FALSE; // Utopia will check this flag to use Mem422
        //MApi_XC_EnableForceRGBin(ENABLE, MAIN_WINDOW);
      #if 0
        stXCSetWinInfo.bPreHCusScaling = TRUE;
        stXCSetWinInfo.u16PreHCusScalingSrc = u16OPMFetch;
        stXCSetWinInfo.u16PreHCusScalingDst = u16OPMFetch;
        stXCSetWinInfo.bPreVCusScaling = TRUE;
        stXCSetWinInfo.u16PreVCusScalingDst = s_TestPattern_u16MemVLen;       ///<pre V customized scaling dst width
        stXCSetWinInfo.u16PreVCusScalingSrc = s_TestPattern_u16MemVLen;
      #endif
    #else
        ...
        stXCSetWinInfo.u16InputVFreq = 300;
        stXCSetWinInfo.u16InputVTotal = 2160+50;
        stXCSetWinInfo.u16DefaultHtotal = 4096+100;
    #else
...
//        MS_U8  u8DefaultPhase;          ///<Obsolete
    ...
    // Enable 2p mode
#if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA)&&(!TEST_PAT_USE_SRC_HDMI) )
    if( u16OPMFetch > 1920 )
    {
        MApi_XC_W2BYTEMSK(REG_SC_BK20_01_L, BIT(1), BIT(1));
    }
#endif
   ...
   ||( CHIP_FAMILY_TYPE == CHIP_FAMILY_WHISKY)   \
    ...
    MApi_XC_W2BYTEMSK(REG_SC_BK18_06_L, 0x0000, 0x0001 ); // CSC off
#else
    ...
    // Memory compress Off
#if ( CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA )
    MApi_XC_W2BYTEMSK(REG_SC_BK08_01_L, BIT0, BIT0 ); // reg_ce_en_bypass_en for ipm=1 ==> Compress mode off
    MApi_XC_W2BYTEMSK(REG_SC_BK08_40_L, BIT0, BIT0 ); // Compress flag clear=1  ==> Clear flag
    MApi_XC_W2BYTEMSK(REG_SC_BK08_40_L,    0, BIT0 ); //Compress flag clear=0  ==> Clear flag
#endif
...
#if(TEST_PAT_USE_SRC_HDMI)
    //MApi_XC_EnableForceRGBin(DISABLE, MAIN_WINDOW);
#endif
...
// 1920/256=7,  3840/256=15,
#define DRAW_LEVEL_REPEAT_NUM_H (s_TestPattern_u16PanelWidth/256) // 256 color

// 1080/256=4,  2160/256=8,
#define DRAW_LEVEL_REPEAT_NUM_V (s_TestPattern_u16PanelHigh/256) // 256 color
...
BOOL MApp_TestPattern_DrawPattern(U8 u8TPNo)
{
    ...
    //printf(" g_TestPattern_PixelAlign=%u\n", g_TestPattern_PixelAlign);
    //printf(" MApi_XC_IsSupport2StepScaling()=%u\n", MApi_XC_IsSupport2StepScaling());
...
#endif
    {
        ...
        switch(u8TPNo)
        {
            ...
            case E_TEST_PATTERN_ID_GRAY_LEVEL_H_0_255:
                {
                    bResult = msAPI_TestPattern_Draw_GrayLevel(0,0,1, DRAW_LEVEL_REPEAT_NUM_H);
                }
                break;

            case E_TEST_PATTERN_ID_GRAY_LEVEL_H_255_0:
                {
                    bResult = msAPI_TestPattern_Draw_GrayLevel(0,255,-1, DRAW_LEVEL_REPEAT_NUM_H);
                }
                break;

            case E_TEST_PATTERN_ID_GRAY_LEVEL_V_0_255:
                {
                    bResult = msAPI_TestPattern_Draw_GrayLevel(1, 0, 1, DRAW_LEVEL_REPEAT_NUM_V);
                }
                break;

            case E_TEST_PATTERN_ID_GRAY_LEVEL_V_255_0:
                {
                    bResult = msAPI_TestPattern_Draw_GrayLevel(1,255,-1, DRAW_LEVEL_REPEAT_NUM_V);
                }
                break;
