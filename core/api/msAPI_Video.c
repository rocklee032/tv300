...
U32 g_VDEC_u32FrameBuf_Addr = 0;
U32 g_VDEC_u32FrameBuf_Size = 0;
...
void _msAPI_VID_SetMemoryLayout(VDEC_InitParam *pInitParam)
{
    ...
    g_VDEC_u32FrameBuf_Addr = pInitParam->SysConfig.u32FrameBufAddr;
    g_VDEC_u32FrameBuf_Size = pInitParam->SysConfig.u32FrameBufSize;
...
void msAPI_VID_Command(U8 u8VIDcommand)
{
    ...
    DEBUG_VDEC_API( printf("msAPI_VID_Command(Cmd=%u)\n", u8VIDcommand); );
    ...
    switch (u8VIDcommand)
    {
        case MSAPI_VID_PLAY:
            ...
            msDebug_PrintChgSrcTime("VDEC play");
            g_Debug_u32Time_VDEC_WritePTR = 0;
            g_Debug_u32Time_VDEC_CheckDispInfoRdy = 0;
            g_Debug_u32Time_VDEC_IsWithValidStream = 0;
            g_Debug_u32Time_VDEC_GetDispInfo = 0;
        #endif
...
#if(DEBUG_CHG_SRC_TIME)
    if( g_Debug_u32Time_VDEC_GetDispInfo == 0 )
    {
        g_Debug_u32Time_VDEC_GetDispInfo = g_Debug_u32Time_ChangeSrc;
        msDebug_PrintChgSrcTime("VDEC GetDispInfo");
    }
#endif
...
BOOLEAN msAPI_VID_Init(BOOLEAN bEnableDropErrFrame, VDEC_SrcMode enSrcMode)
{
    ...
    DEBUG_VDEC_API( printf("msAPI_VID_Init(bEnableDropErrFrame=%u, enSrcMode=%u)\n", bEnableDropErrFrame, enSrcMode); );
...
#if(DEBUG_CHG_SRC_TIME)
    msDebug_PrintChgSrcTime("VDEC init done");
#endif
...
#if DTV_DYNAMIC_DNR_BUF
    U32 u32VDECMemFree = ALIGN(VDEC_FRAMEBUFFER_DTV_LEN-u32VDECMemUsage, 16);

    u32WxH = ALIGN_8(pstVidStatus->u16HorSize)*pstVidStatus->u16VerSize;

    #if H264_1080I_USE_RFBL
    if( ( eVdecCodecType == E_VDEC_CODEC_TYPE_H264
        #if (CHIP_SUPPORT_HEVC)
           || eVdecCodecType == E_VDEC_CODEC_TYPE_HEVC
        #endif
        ) && pstVidStatus->u8Interlace && pstVidStatus->u16VerSize > 1000 )
    {
        if(u32VDECMemFree > u32WxH*3)
        {
            MApi_XC_SetFrameBufferAddress( ALIGN_16(VDEC_FRAMEBUFFER_DTV_ADR+u32VDECMemUsage), u32VDECMemFree, MAIN_WINDOW);

            DEBUG_DTV_FBL(printf("DTV use RFBL\n"); );

            return FALSE; // use RFBL
        }
    }
    #endif

    if(u32VDECMemFree > u32WxH*3*2)
    {
        MApi_XC_SetFrameBufferAddress( ALIGN_16(VDEC_FRAMEBUFFER_DTV_ADR+u32VDECMemUsage), u32VDECMemFree, MAIN_WINDOW);
