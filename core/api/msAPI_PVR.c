...
#define MAX_FAT32_FILE_SIZE_KB          (U32)4118400    //maxmun fat32 record KB size of single file
#ifdef S3PLUS
...
static U32 u32EndTimeStamp = 0;
static BOOL bIsLimitJump = FALSE;

#define THRESHOLD_LENGTH     (6)

typedef struct
{
    U32 u32BitRate;
    U32 u32Size;
}PVR_RECORD_THRESHOLD;

PVR_RECORD_THRESHOLD threshold[THRESHOLD_LENGTH] =
{
    {2000000, 4096}, // 786432
    {1600000, 3072}, // 589824
    {1400000, 2048}, // 393216
...
_msAPI_PVR_RecordPath * msAPI_PVR_RecordPathOpen(U16 *pu16RecordPathName, enPvrFileMode enFileMode)
{
    ...
    if(enFileMode == E_FILE_MODE_RING)                  //TimeShift usually
    {
                 ...
                 msAPI_PVRFS_GetFileSystemType() == PVR_FS_LINUX_FAT)
        {
            ...
            {
                if ( FALSE == msAPI_PVRFS_FileClose(fhander) )
                    return NULL;

                if ( FALSE == msAPI_PVRFS_FileDelete(pu16RecordPathName) )
                    return NULL;
            }
...
#if (ENABLE_PVR_AESDMA || (ENABLE_CI_PLUS && ENABLE_CIPLUS_PVR))
void _PVR_AESDMAWaitDone(U32 u32MaxWaitTime)
{
    U32 u32DRVAESDMA_RESULT;
    U32 u32StartTime = MsOS_GetSystemTime();

    while (TRUE)
    {
        if ( msAPI_Timer_DiffTime_2(u32StartTime, MsOS_GetSystemTime()) >= u32MaxWaitTime )
        {
            printf("\nError: Wait AESDMA timeout!\n");
            break;
        }

        u32DRVAESDMA_RESULT = MDrv_AESDMA_GetStatus();

        if ( u32DRVAESDMA_RESULT & E_DRVAESDMA_EVENT_DMA_DONE )
        {
            break;
        }
...
/***************************************************************************************/
BOOL msAPI_PVR_RecordGetStream(_msAPI_PVR_RecordPath *pstPvrRecordPath, U32 *u32buffersize)
{
    U32 u32RecordWritePointer    = pstPvrRecordPath->u32BufferWritePointer;
    U32 u32RecordReadPointer     = pstPvrRecordPath->u32BufferReadPointer;
    U32 u32RecordPreWritePointer = u32RecordWritePointer;

    U32 u32Time = 0;
    U32 u32TimeDiff = 0;
    U32 u32BitRate = 0;
    static U32 u32BitRateTimer = 0;
    U32 u32BitrateBuffersize = 0;
    U32 u32Waterlevel = 0;
    BOOL bWrapAround = FALSE;

    U8 u8i = 0;
    U32 u32Checksize = 0;
    BOOL bWaitBuffer = FALSE;

    // 1. Get DMX Write Pointer
    MApi_DMX_Pvr_WriteGet(&u32RecordWritePointer);
    u32RecordWritePointer = _PA2VA(u32RecordWritePointer);
    pstPvrRecordPath->u32BufferWritePointer = u32RecordWritePointer;

    if (u32RecordWritePointer >= u32RecordPreWritePointer)
    {
        u32BitrateBuffersize = u32RecordWritePointer - u32RecordPreWritePointer;
    }
    else
    {
        u32BitrateBuffersize = pstPvrRecordPath->u32BufferLength - (u32RecordPreWritePointer - u32RecordWritePointer);
    }
    //printf("[RecordGetStream]: WP=%u, RP=%u, PWP=%u, offset=%u \n", u32RecordWritePointer, u32RecordReadPointer, u32RecordPreWritePointer, u32BitrateBuffersize);
    ...
    if (u32RecordReadPointer < u32RecordPreWritePointer)
    {
        // |         R           PW         W
        // +---------+-----------+----------+----------
        ...
        if ( (u32RecordReadPointer < u32RecordWritePointer) && (u32RecordWritePointer < u32RecordPreWritePointer) )
        {
            printf("[PVR][Record]!!!!!!OVERWRITE(1)!!!!!! \n");
            return FALSE;
        }
    }
    else if (u32RecordReadPointer > u32RecordPreWritePointer)
    {
        // |         PW         W          R
        // +---------+---------+----------+-----------
        ...
        // |                   PW         R      W
        // +-------------------+----------+------+----
        if (u32RecordWritePointer > u32RecordReadPointer)
        {
            printf("[PVR][Record]!!!!!!OVERWRITE(2)!!!!!! \n");
            return FALSE;
        }
        ...
        if (u32RecordWritePointer < u32RecordPreWritePointer)
        {
            printf("[PVR][Record]!!!!!!OVERWRITE(3)!!!!!! \n");
            return FALSE;
        }
    }

    // 3. Calculate Stream Bitrate
    if (u32BitrateBuffersize > 0)
    {
        u32Time = msAPI_Timer_GetTime0();
        u32TimeDiff = u32Time - u32BitRateTimer;
        u32BitRateTimer = u32Time;

        if ((u32TimeDiff > 0) && (u32TimeDiff != u32Time))
        {
            u32BitRate = u32BitrateBuffersize / u32TimeDiff;
            u32BitRate *= 1000;
        }
    }
    ...
    if (u32RecordWritePointer > u32RecordReadPointer)
    {
        u32Waterlevel = u32RecordWritePointer - u32RecordReadPointer;
        bWrapAround = FALSE;
    }
    else
    {
        u32Waterlevel = pstPvrRecordPath->u32BufferLength + pstPvrRecordPath->u32BufferStart - u32RecordReadPointer;
        bWrapAround = TRUE;
    }

    //if (u32BitRate > 0)
    //{
    //    printf("[RecordGetStream]: u32Waterlevel=%u, u32BitRate=%u \n", u32Waterlevel, u32BitRate);
    //}

    for (u8i = 0; u8i < THRESHOLD_LENGTH; u8i++)
    {
        if (u32BitRate > threshold[u8i].u32BitRate)
        {
            if (u32Waterlevel >= threshold[u8i].u32Size * pstPvrRecordPath->u8PacketSize)
            {
                u32Checksize = threshold[u8i].u32Size * pstPvrRecordPath->u8PacketSize;
                bWaitBuffer = FALSE;
                //printf("[RecordGetStream]: (1) u32Checksize=%u(%u)\n", u32Checksize, threshold[u8i].u32Size);
                break;
            }
        }
    }

    if ((TRUE == bWrapAround) && (THRESHOLD_LENGTH == u8i))
    {
        u32Checksize = u32Waterlevel;
        bWaitBuffer = FALSE;
        //printf("[RecordGetStream]: (2) u32Checksize=%u(%u)\n", u32Checksize, u32Checksize / pstPvrRecordPath->u8PacketSize);
    }

    // 5. Rutrun BufferSize And Add RecordReadPointer
    if ((u32Checksize == 0) || (bWaitBuffer == TRUE))
    {
        // Wating Buffer Add
        return FALSE;
    }

    *u32buffersize = u32Checksize;
    return TRUE;
}
...
enPvrApiStatus  msAPI_PVR_RecordPathInput(_msAPI_PVR_RecordPath * pstPvrRecordPath)
{
    ...
    if (FALSE == msAPI_PVR_RecordGetStream(pstPvrRecordPath, &u32WriteLength))
    {
        return E_PVR_API_STATUS_OK;;
    }
         ...
         || (msAPI_PVRFS_GetFileSystemType() == PVR_FS_LINUX_FAT) )
    {
        ...
        {
            u32WriteLength = u32WriteLength - (u32WriteLength % pstPvrRecordPath->u8PacketSize);
            bFsOverSize = TRUE;
        }
    }
    ...
    if ( (u32Time2 - u32Time1) >= 1000 )
    {
        printf("\r\n [Write %ld Bytes Data need %ldms,it's too long]\n", u32WriteLength, u32Time2 - u32Time1);
    }
    u32Time2 = msAPI_Timer_GetTime0();
    ...
    if (0 == msAPI_PVRFS_GetDriveFreeSpaceMB())
    {
    ...
    if(enUpdateMode == E_METADATA_UPDATE_FILE)
    {
        ...
        if (_PvrRecordMeataData.u16RecordedAvgRate == 0)
        {
            if(pstPvrPlaybackPath->enDirection != E_PLAYBACK_PATH_DIRECTION_FASTBACKWARD)
            {
                ...
                if(pstPvrPlaybackPath->pstPvrRecordPath)
                {
...
#if (PVR_TS_PACKET_SIZE == 192)
/***********************************************************************************************/
U32 msAPI_PVR_GetFileStartTimeStamp(void)
{
    return u32StartTimeStamp;
}

U32 msAPI_PVR_GetFileEndTimeStamp(void)
{
    return u32EndTimeStamp;
}

U32 msAPI_PVR_PlaybackGetStartTimeStamp(enPvrEncryptType enEncryptionType, U8 u8FileHandle)
{
    U32 u32ReadLength = 0;
    U32 u32StartTime = 0;
    ...
    if (0x47 == buff[4])
    {
        u32StartTime = ( (U32)buff[0]<<24) + ((U32)buff[1]<<16) + ((U32)buff[2]<<8) + (U32)buff[3];
        u32StartTime = u32StartTime/90000;
    }
    return u32StartTime;
}
...
enPvrApiStatus  msAPI_PVR_PlaybackPathSet(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath)
{
  ...
  //printf("timestamp nop; playback setbuf->setspeed; cmdq nop\n");
    ...
    #if 1
    {
        ...
        u32StartTimeStamp = msAPI_PVR_PlaybackGetStartTimeStamp(pstPvrPlaybackPath->pstPvrRecordPath->enEncryptionType, pstPvrPlaybackPath->hReadFile);
        u32EndTimeStamp = msAPI_PVR_PlaybackGetTotalTimeByTimeStamp(pstPvrPlaybackPath->pstPvrRecordPath->enEncryptionType, pstPvrPlaybackPath->hReadFile);
        #ifdef PVR_8051
...
/***************************************************************************************/
static U32 BinarySearchJumpTime(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath,U32 low,U32 high,U32 u16JumpSecond)
{
    U32 mid_time;
    U32 mid;
...
static U32 GetLinearFilePos(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath ,U32 u16JumpSecond, U32 u32FileValidPosStrKB, U32 u32FileValidPosEndKB)
{
    ...
    if (u16JumpSecond >= Jump_time && u32FileValidPosStrKB < u32FileValidPosEndKB)
    {
    ...
    temphigh = pstPvrPlaybackPath->u32FileLimitedSizeKB/PVR_TS_PACKET_SIZE;
    templow = 0;
    ...
    if (low >= high)
    {
        if (pstPvrPlaybackPath->enDirection == E_PLAYBACK_PATH_DIRECTION_FASTFORWARD)
        {
            ...
            if (pstPvrPlaybackPath->u32FilePositionKB/PVR_TS_PACKET_SIZE < high)
            {
                return BinarySearchJumpTime(pstPvrPlaybackPath,templow,high,u16JumpSecond);
            }
            else
            {
                return BinarySearchJumpTime(pstPvrPlaybackPath,low,temphigh,u16JumpSecond);
            }
        }
        else if (pstPvrPlaybackPath->enDirection == E_PLAYBACK_PATH_DIRECTION_JUMPFORWARD)
        {
            return BinarySearchJumpTime(pstPvrPlaybackPath,high,low,u16JumpSecond);
        }
        else
            return 0;
    }
    ...
    {
        return BinarySearchJumpTime(pstPvrPlaybackPath,low,high,u16JumpSecond);
    }
}

/***************************************************************************************/
enPvrApiStatus msAPI_PVR_PlaybackPathJumpToTime(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath, U32 u16JumpSecond)
{
...
enPvrApiStatus msAPI_PVR_PlaybackPathJumpForward(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath, U32 u16JumpSecond, enPvrJumpBufferMode enBufferMode)
{
    ...
    if ( (FALSE == msAPI_PVR_GetRetentionLimitJumpBackward())
         && (E_PVR_FILE_CHANGE_BACKWARD == MApp_PVR_GetFileChangeStatus()) )
    {
        LongLong fileLen;
        fileLen = msAPI_PVRFS_FileLength(pstPvrPlaybackPath->hReadFile);
        fileLen.Lo = ((fileLen.Lo - 100*1024) / 192 - 1) * 192;
        msAPI_PVRFS_FileSeek(pstPvrPlaybackPath->hReadFile, fileLen, PVRFS_SEEK_OPTION_FROM_BEGIN);
        pstPvrPlaybackPath->u32FilePositionKB = fileLen.Lo / 1024;

        mid_time = msAPI_PVR_PlaybackGetPosTimeByTimeStamp(pstPvrPlaybackPath->pstPvrRecordPath->enEncryptionType, pstPvrPlaybackPath->hReadFile, fileLen);
        MApi_DMX_Pvr_SetPlaybackStamp(mid_time*90);//mid_time ΢
        pstPvrPlaybackPath->u32FileLastPosKB = pstPvrPlaybackPath->u32FilePositionKB;
    }
    else
...
void msAPI_PVR_ClearBitStreamBuff(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath)
{
  ...
  //  BuffLen = pstPvrPlaybackPath->u32BufferLength;
    ...
    u32VDECCheckStarttime= msAPI_Timer_GetTime0();
    while(MApi_VDEC_Flush(E_VDEC_FREEZE_AT_CUR_PIC) != E_VDEC_OK)
    {
        ...
        if(msAPI_Timer_DiffTimeFromNow(u32VDECCheckStarttime) > 1000)
        {
            printf("2-1)Wait MApi_VDEC_Flush() time out!\n");
            break  ;
        }
    }
    FlushStarTime = msAPI_Timer_GetTime0();
    ...
    while (MApi_VDEC_IsAllBufferEmpty() != E_VDEC_OK)
    {
        ...
        if(msAPI_Timer_DiffTimeFromNow(u32VDECCheckStarttime) > 1000)
        {
            printf("2-2)Wait MApi_VDEC_IsAllBufferEmpty() time out!\n");
            break  ;
        }
...
BOOL msAPI_PVR_GetMeataDataLockInfo(void)
{
    _msAPI_PVR_RecordPath * pstPvrRecordPath;
    pstPvrRecordPath = &_PvrRecordMeataData;   //<---Linking to Record Path in Playback Mode
    msAPI_PVR_PlaybackPathUpdateMetadata(&_PvrPlaybackPath[0], E_METADATA_UPDATE_FILE);
    return pstPvrRecordPath->bIsChannelLocked;
}

BOOL msAPI_PVR_GetMeataDataParentalControl(void)
{
    _msAPI_PVR_RecordPath * pstPvrRecordPath;
    pstPvrRecordPath = &_PvrRecordMeataData;   //<---Linking to Record Path in Playback Mode
    msAPI_PVR_PlaybackPathUpdateMetadata(&_PvrPlaybackPath[0], E_METADATA_UPDATE_FILE);
    return pstPvrRecordPath->u8Parental_Control;
}

void msAPI_PVR_SetRetentionLimitJumpBackward(BOOL bRetentionLimitJump)
{
    bIsLimitJump = bRetentionLimitJump;
}

BOOL msAPI_PVR_GetRetentionLimitJumpBackward(void)
{
    return bIsLimitJump;
}
