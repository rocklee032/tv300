...
#if ((CHIP_SUPPORT_STR) &&(STR_ENABLE == ENABLE))
#include "MApp_STR.h"
#endif
...
#define DEBUG_AUDIO_INIT_TIME(x)    x
...
U32 g_Audio_u32MuteStatus;

#define AUDIO_MUTE_CLIENT_TO_MASK(client)   (1<<(client))
#define IS_AUDIO_MUTE_CLIENT_ACTIVE(client) ((g_Audio_u32MuteStatus&AUDIO_MUTE_CLIENT_TO_MASK(client))?1:0)
...
void msAPI_AUD_InitAudioSystem(THR_TBL_TYPE code *ThrTbl)
{
    ...
    DEBUG_AUDIO_INIT_TIME( printf("msAPI_AUD_InitAudioSystem() at %u\n", MsOS_GetSystemTime()); );
    ...
    TIMER_SET_DELAY_ID_START(E_DELAY_ID_AUDIO_DRV);
...
//======================================
    ...
    DEBUG_AUDIO_INIT_TIME( printf("call MApi_AUDIO_Initialize() at %u\n", MsOS_GetSystemTime()); );

    MApi_AUDIO_Initialize();

    DEBUG_AUDIO_INIT_TIME( printf("MApi_AUDIO_Initialize() done at %u\n", MsOS_GetSystemTime()); );
...
#if 0//(ENABLE_AUDIO_MUTE_CONTROL_NEW)
    m_bMHEGApMute = FALSE;
...
//========== For customer setting============
    ...
    DEBUG_AUDIO_INIT_TIME( printf("msAPI_AUD_InitAudioSystem() end at %u\n", MsOS_GetSystemTime()); );
...
/// @param wParam2  \b param2
/******************************************************************************/
void msAPI_AUD_AdjustAudioMute(AUDIOMUTETYPE eAudioMuteType, AUDIOMUTESOURCE_TYPE eAudioMuteSource)
{
#if (ENABLE_AUDIO_MUTE_CONTROL_NEW)

    EnuAudioMuteClient eMuteClient = (EnuAudioMuteClient)(eAudioMuteType&0x7FFF);
    BOOLEAN bMute = (eAudioMuteType&BIT15)?TRUE:FALSE;

    msAPI_AUD_Set_Mute( eMuteClient, bMute, eAudioMuteSource);
    ...
    */
    ...
    {
    #if (ENABLE_CHANNEL_CHANGE_MUTE_ON_OFF_DELAY)
        if ((eAudioMuteType%2)==0)   // Mute off
        {
        ...
        SetAudioMute((AUDIOMUTETYPE) eAudioMuteType, (AUDIOMUTESOURCE_TYPE) wParam2);

        if ((eAudioMuteType%2)==1)  // Mute on
        {
            if(!m_bByPassDelay) //fix laggardly issue of playing movie; MHEG Audio File Playback also needs no time delay when mute on/off
            {
...
void msAPI_AUD_AdjustAudioFactor2(ADJUST_TVAVFACTOR eFactor, WORD wParam1, WORD wParam2)
{
    ...
    */
    ...
    #if 1 // Use function
        msAPI_AUD_AdjustAudioMute((AUDIOMUTETYPE) wParam1, (AUDIOMUTESOURCE_TYPE)wParam2);
    #else
    {
        /* This switch statment can avoid the unnecessary delay time */
...
char* msAPI_AUD_Get_MuteClientName(EnuAudioMuteClient eAudioMuteClient)
{
    switch(eAudioMuteClient)
    {
        case E_AU_MUTE_CL_PERMANENT          : return "Permanent";
        case E_AU_MUTE_CL_MOMENT             : return "Moment";
        case E_AU_MUTE_CL_DURING_LIMITED_TIME: return "Time";
        case E_AU_MUTE_CL_SPDIF_TEMPMUTE     : return "SpdifTmp";
        case E_AU_MUTE_CL_INTERNAL_1         : return "Inter_1";
        case E_AU_MUTE_CL_INTERNAL_2         : return "Inter_2";
        case E_AU_MUTE_CL_INTERNAL_3         : return "Inter_3";
        case E_AU_MUTE_CL_INTERNAL_4         : return "Inter_4";
        case E_AU_MUTE_CL_BYSYNC             : return "Sync";
        case E_AU_MUTE_CL_HDMI_AVMUTE        : return "HDMI_AVMute";
        case E_AU_MUTE_CL_HDMI_CEC_ARC       : return "CEC_ARC";
        case E_AU_MUTE_CL_MHEGAP             : return "MHEG";
        case E_AU_MUTE_CL_PVR                : return "PVR";
        case E_AU_MUTE_CL_CI                 : return "CI";
        case E_AU_MUTE_CL_CIPVR              : return "CIPVR";
        case E_AU_MUTE_CL_TTS_Background     : return "TTS";
        case E_AU_MUTE_CL_KTS_BGM            : return "KTS";
        case E_AU_MUTE_CL_BYVCHIP            : return "VChip";
        case E_AU_MUTE_CL_BLOCK_CHANNEL      : return "BlockCh";
        case E_AU_MUTE_CL_BLOCK_INPUTSOURCE  : return "BlockInputSrc";
        case E_AU_MUTE_CL_BYUSER             : return "User";
...
void msAPI_AUD_PrintMuteClient(void)
{
    ...
    #if 1//(ENABLE_AUDIO_MUTE_CONTROL_NEW)
        U8 u8MuteClientIdx;
        for( u8MuteClientIdx = 0; u8MuteClientIdx < E_AU_MUTE_CL_NUMS; u8MuteClientIdx += 1)
        {
            if( g_Audio_u32MuteStatus & AUDIO_MUTE_CLIENT_TO_MASK(u8MuteClientIdx) )
            {
                printf(" %s,", msAPI_AUD_Get_MuteClientName( (EnuAudioMuteClient)u8MuteClientIdx ) );
            }
        }
...
{
#if 1
    if( IS_AUDIO_MUTE_CLIENT_ACTIVE(E_AU_MUTE_CL_BYUSER) )
        return TRUE;
...
BOOLEAN msAPI_AUD_SetAllAudioUnmute(void)
{
    ...
    g_Audio_u32MuteStatus = AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_PERMANENT);

    msAPI_AUD_Set_Mute(E_AU_MUTE_CL_PERMANENT, AU_MUTE_DISABLE, E_AUDIOMUTESOURCE_ACTIVESOURCE );
...
static void _AUD_Mute_Set_Driver(void);
...
static void SetAudioMute(AUDIOMUTETYPE eAudioMuteType, AUDIOMUTESOURCE_TYPE eAudioMuteSource)
{
    ...
    */
    ...
    _AUD_Mute_Set_Driver();

#endif
}

//todo: check mute condition for every dest
#if 1//(ENABLE_AUDIO_MUTE_CONTROL_NEW)
U32 msAPI_AUD_Get_MuteClientMask_Main(void)
{
    U32 u32MuteClientMask =
    (
        AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_PERMANENT          )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_MOMENT             )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_DURING_LIMITED_TIME)
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_1         )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_2         )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_3         )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_4         )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BYSYNC             )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_HDMI_AVMUTE        )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_HDMI_CEC_ARC       )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_MHEGAP             )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_PVR                )
       //|AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_CI                 ) // This client is only for scart
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_CIPVR              )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_TTS_Background     )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_KTS_BGM            )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BYVCHIP            )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BLOCK_CHANNEL      )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BLOCK_INPUTSOURCE  )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BYUSER             )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_ALIGN_VIDEO        )
    );

    return u32MuteClientMask;
}

BOOLEAN msAPI_AUD_Is_MainSpeakerMuted(void)
{
    if( g_Audio_u32MuteStatus & msAPI_AUD_Get_MuteClientMask_Main() )
        return TRUE;

    return FALSE;
}

MS_BOOL _AUD_Is_MainSpeakerNeedMute(MS_BOOL * pbMixGame1Mute)
{
    if( IS_AUDIO_MUTE_CLIENT_ACTIVE(E_AU_MUTE_CL_BYUSER) )
    {
        *pbMixGame1Mute = TRUE;
    ...
    MS_BOOL bSpeakerMute = FALSE;

    //printf("MainSpeakerNeedMute: %X, %X\n", g_Audio_u32MuteStatus, msAPI_AUD_Get_MuteClientMask_Main());
    ...
    { // Change mute control to GAME1, pass spkrMute flag to bMixGame1Mute
        ...
        *pbMixGame1Mute = bSpeakerMute;
        bSpeakerMute = FALSE; /*| m_bMuteByCecArc*/;
        //printf(" ==> spkrMute=%u\n", spkrMute);
        //printf(" ==> bMixGame1Mute=%u\n", bMixGame1Mute);
    }
#elif ENABLE_KEY_TO_SPEECH
    *pbMixGame1Mute = bSpeakerMute;
    bSpeakerMute = FALSE;//m_bByUserAudioMute /*| m_bMuteByCecArc*/;
#else
...
U32 msAPI_AUD_Get_MuteClientMask_Spdif(void)
{
    U32 u32MuteClientMask =
    (
        AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_PERMANENT          )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_MOMENT             )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_DURING_LIMITED_TIME)
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_SPDIF_TEMPMUTE     )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_1         )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_2         )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_3         )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_INTERNAL_4         )
       ...
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_MHEGAP             )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_PVR                )
       //|AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_CI                 ) // Need ask for CI
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_CIPVR              )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_TTS_Background     )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_KTS_BGM            )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BYVCHIP            )
...
MS_BOOL _AUD_Is_SpdifNeedMute(void)
{
    MS_BOOL bSpdifMute = FALSE;
    ...
    {
        // When TTS is On, source side will be muted,
        // so no need mute output side!
        if( IS_AUDIO_MUTE_CLIENT_ACTIVE(E_AU_MUTE_CL_SPDIF_TEMPMUTE) )
            bSpdifMute = TRUE;
        else
            bSpdifMute = FALSE;
    }
    ...
    {
        if( g_Audio_u32MuteStatus&msAPI_AUD_Get_MuteClientMask_Spdif() )
            bSpdifMute = TRUE;
        else
            bSpdifMute = FALSE;
    }
...
U32 msAPI_AUD_Get_MuteClientMask_Scart(void)
{
    U32 u32MuteClientMask =
    (
        AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_PERMANENT          )
...
//       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_SPDIF_TEMPMUTE     )
       ...
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_MHEGAP             )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_PVR                )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_CI                 )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_CIPVR              )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_TTS_Background     )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_KTS_BGM            )
       |AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BYVCHIP            )
...
MS_BOOL _AUD_Is_ScartoutNeedMute(void)
{
    if( g_Audio_u32MuteStatus & AUDIO_MUTE_CLIENT_TO_MASK(E_AU_MUTE_CL_BYUSER) )
    {
        return TRUE;
    }

    MS_BOOL bScartoutMute = FALSE;

    if( g_Audio_u32MuteStatus&msAPI_AUD_Get_MuteClientMask_Scart() )
        bScartoutMute = TRUE;
    else
        bScartoutMute = FALSE;
/*
...
{
    DEBUG_AUDIO_MUTE( PRINT_CURRENT_LINE(); );
    DEBUG_AUDIO_MUTE( printf("_AUD_Mute_Set_Driver()\n"); );

    MS_BOOL spkrMute, scartMute, spdifMute;

#if 1//(AUDIO_PATH_SUPPORT_TTS || ENABLE_KEY_TO_SPEECH)
    MS_BOOL bMixGame1Mute = FALSE;
#endif


    // Check speaker mute conition...

    #if 1
        spkrMute = _AUD_Is_MainSpeakerNeedMute(&bMixGame1Mute);
    ...
    // Check ScartOut mute conition...
#if 1
    scartMute = _AUD_Is_ScartoutNeedMute();
#else
    scartMute = m_bPermanentAudioMute|
                m_bMomentAudioMute|
                m_bBySyncAudioMute|
    ...
    // Check ScartOut mute conition...
#if 1
    spdifMute = _AUD_Is_SpdifNeedMute();
#else
    spdifMute = m_bMomentAudioMute|
            //when TV Connected to the amplifier,Some situations has no tts sound out
            #if(!(ENABLE_ATSC_TTS || ENABLE_KEY_TO_SPEECH))
    ...
    DEBUG_AUDIO_MUTE( printf(" => spkrMute=%u\n", spkrMute); );
    DEBUG_AUDIO_MUTE( printf(" => scartMute=%u\n", scartMute); );
    DEBUG_AUDIO_MUTE( printf(" => spdifMute=%u\n", spdifMute); );
    DEBUG_AUDIO_MUTE( printf(" => bMixGame1Mute=%u\n", bMixGame1Mute); );
    ...
    {
        //PRINT_CURRENT_LINE();
        DEBUG_AUDIO_MUTE( printf("---------- spkrMute ----------\n"); );
        DEBUG_AUDIO_MUTE( msAPI_AUD_PrintMuteClient(); );
...
AUDIOSTANDARD_TYPE msAPI_AUD_GetResultOfAutoStandardDetection(void)
{
...
#endif
    {
        ...
        if(m_eAudioStandard != E_AUDIOSTANDARD_L)
        {
            ...
            // Get Audio Status
            if ( MApi_AUDIO_SIF_GetAudioStatus(&m_eAudioStatus) == FALSE)
            {
                m_eAudioStatus = E_STATE_AUDIO_NO_CARRIER;
            }
...
void MW_AUD_AudioProcessor(void)
{
    ...
    #if 0//HDMI_DVI_AUDIO_MODE_CONFIG
       ||(IsHDMIInUse())
    #endif
       )
    {
   ...
   // Check the tuning module
    ...
    if( IsVDHasSignal() ) // VD has signal
    {
        //if( m_bBySyncAudioMute ) // Mute by Sync == 1
        if( IS_AUDIO_MUTE_CLIENT_ACTIVE(E_AU_MUTE_CL_BYSYNC) )
        {
        #if 1//(MS_BOARD_TYPE_SEL != BD_MSD309PX_HIS)
        ...
        else // Mute by Sync == 0
        {
            ...
            // todo: Move to msAPI_Audio_Standard_and_SoundMode_Task()
            CheckAudioStandard();
    ...
    {
        if( IS_AUDIO_MUTE_CLIENT_ACTIVE(E_AU_MUTE_CL_BYSYNC) ) // Mute by Sync == 1
        {
        }
        else // Mute by Sync == 0
        {
...
void MW_AUD_SetMadAddress(void)
{
...
#if (ENABLE_AUDIO_DEC_R2_BIN==TRUE)
    {
        ...
        if (bResult == PASS )
        {
        #if 0// ((CHIP_SUPPORT_STR) &&(STR_ENABLE == ENABLE)&&(ENABLE_STR_AUDIO_PATCH))
            if(IsSTREnable())
            {
                printf("[STR]skip audio load code\n");
                MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_CompressBin_LoadCode, FALSE, (MS_U32) MAPP_Audio_LoadCpuCode_STRRecover);
            }
            else
        #endif
            {
...
void MW_AUD_SetSoundMute(SOUND_MUTE_SOURCE eSoundMuteSource, SOUND_MUTE_TYPE eOnOff)
{
    ...
    TIMER_SET_DELAY_ID_START(E_DELAY_ID_AUDIO_DRV_SET_MUTE);
...
#define DEBUG_LOAD_AU_R2_TIME(x)    //x
MS_BOOL msAPI_Audio_LoadCpuCode(MS_U8 u8CpuIdx, MS_U32 u32CpuMemAddr, MS_U32 u32CpuMemSize )
{
    // Dest buffer
    ...
    DEBUG_LOAD_AU_R2_TIME( printf("Load AuR2 Start at %u\n", MsOS_GetSystemTime()); );
...
#if( (CHIP_SUPPORT_STR) &&(STR_ENABLE == ENABLE)&&(ENABLE_STR_AUDIO_PATCH))
    if( IsSTREnable() )
    {
        printf("[STR]skip audio load code\n");
        return TRUE;
      ...
      )
    {
        ...
        DEBUG_LOAD_AU_R2_TIME( printf("Read Flash Start at %u\n", MsOS_GetSystemTime()); );
        ...
        DEBUG_LOAD_AU_R2_TIME( printf("Read Flash End at %u\n", MsOS_GetSystemTime()); );
        ...
        if( (MS_COMPRESS7 == stBinInfo.B_IsComp) )
        {
            ...
            DEBUG_LOAD_AU_R2_TIME( printf("Decomp7 Start at %u\n", MsOS_GetSystemTime()); );
            ...
            DEBUG_LOAD_AU_R2_TIME( printf("Decomp7 End at %u\n", MsOS_GetSystemTime()); );
        ...
        else // MS_COMPRESS
        {
           ...
           ||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MELODY)   \
            ...
            DEBUG_LOAD_AU_R2_TIME( printf("Decomp Start at %u\n", MsOS_GetSystemTime()); );
            ...
            DEBUG_LOAD_AU_R2_TIME( printf("Decomp end at %u\n", MsOS_GetSystemTime()); );
    ...
    DEBUG_LOAD_AU_R2_TIME( printf("Load AuR2 End at %u\n", MsOS_GetSystemTime()); );
...
void msAPI_DolbyBulletin24_receive_SAD(U8* pu8SAD, U8 u8Len)
{
    ...
    for (idx = 0; idx < u8Len; idx+=SAD_LEN)
    {
        ...
        Digital_Out_Device_Capability_t stSAD;
        AUDIO_FORMAT_CODE eCodec = (AUDIO_FORMAT_CODE)((pu8SAD[idx]&0x78)>>3);
...
BOOLEAN msAPI_AUD_Bench_Check_Counter(void)
{
    ...
    do
    {
        msAPI_Timer_Delayms(10); // check counter per 10 ms
        u8BenchCheckCount++;
        ...
        if((u8BenchSeWhileFailCount >= 7) || (u8BenchSeISRFailCount >= 7)||(u8BenchDecWhileFailCount>=7)||(u8BenchDecTimerFailCount>=7))
        {
            ...
            msAPI_Timer_Delayus(u32Delay_Counter);
        }
        ...
        {
            msAPI_Timer_Delayms(1);    //Fix counter mis-detection problem
        }
...
{
    if( IS_AUDIO_MUTE_CLIENT_ACTIVE(eMuteClient) )
        return TRUE;
    else
        return FALSE;
}
...
{
    DEBUG_AUDIO_MUTE( PRINT_CURRENT_LINE(); );
    DEBUG_AUDIO_MUTE( printf("msAPI_AUD_Set_Mute(eMuteClient=%u(%s), bMute=%u, src=%u)\n", eMuteClient, msAPI_AUD_Get_MuteClientName(eMuteClient), bMute, eAudioMuteSource); );

    if ( (eAudioMuteSource != E_AUDIOMUTESOURCE_ACTIVESOURCE)
       &&(eAudioMuteSource !=(AUDIOMUTESOURCE_TYPE) m_eAudioSource) )
        return;

    U32 u32MuteClientMask = AUDIO_MUTE_CLIENT_TO_MASK(eMuteClient);
    U32 u32MuteStatusNew = g_Audio_u32MuteStatus;

    //DEBUG_AUDIO_MUTE( printf("u32MuteClientMask=%X\n", u32MuteClientMask); );

    if( bMute )
    {
        u32MuteStatusNew |= (u32MuteClientMask);
    }
    else
    {
        u32MuteStatusNew &= ~(u32MuteClientMask);
    ...
    DEBUG_AUDIO_MUTE( printf("MuteStatus=%X -> %X\n", g_Audio_u32MuteStatus, u32MuteStatusNew); );

    if( u32MuteStatusNew != g_Audio_u32MuteStatus ) // Mute status changed!
    {
        g_Audio_u32MuteStatus = u32MuteStatusNew;
...
//=============================================
#include "msAPI_Audio_Std_and_SoundMode.c"
//=============================================
