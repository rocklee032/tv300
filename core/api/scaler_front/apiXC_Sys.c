...
#define XC_SYS_INIT_CHECKPOINT()    //printf("xc_s %u %u\n", __LINE__, MsOS_GetSystemTime())

#define ENABLE_DEBUG_DRV_PANEL      0

#define DEBUG_PANEL_INIT(x)         //x
...
static MS_BOOL bSrcIsHD[2] = {FALSE, FALSE};// Daten FixMe, MAX_WINDOW will be removed and can be obtained from MApi_XC_GetInfo()
...
EnuXcHdmiEdidSel g_XC_Sys_au8EdidSel_HW[HDMI_PORT_NUM_MAX] = {XC_HDMI_EDID_Unknown, XC_HDMI_EDID_Unknown, XC_HDMI_EDID_Unknown, XC_HDMI_EDID_Unknown};
#endif
...
#define TCON_DUMP_TIMEOUT_TIME  1000
    ...
    while(1)
    {
        ...
        {
            msAPI_Timer_Delayms(u8DelayTime);
...
#if( ENABLE_DEBUG_DRV_PANEL )
extern MS_U8 g_DrvPnl_u8DebugFlag;
#endif
...
static void _MApi_XC_Sys_Init_XC(XC_INITDATA *pstXC_InitData)
{
...
#if(XC_MLOAD_ENABLE_POS_SEL == XC_MLOAD_EN_POS_AFTER_XC_DRV)
    MApi_XC_MLoad_Enable(TRUE);
#endif
    ...
    // Set extend panel info...
    {
        ...
        {
            DEBUG_PANEL_INIT( printf("Set ExPanel Info(u16VFreq=%u) OK\n", stPanelInfoEx.u16VFreq); );
        }
        else
...
static void _MApi_XC_DDCRam_Init_Hdmi(U8* pu8Edid, E_XC_DDCRAM_PROG_TYPE enProgType, U8 u8PhyAddr, U8 u8PhyAddrPos)
{
    ...
    DEBUG_EDID( PRINT_CURRENT_LINE(); );
    DEBUG_EDID( printf("_MApi_XC_DDCRam_Init_Hdmi(enProgType=0x%X, u8PhyAddr=0x%X, u8PhyAddrPos=0x%X)\n", enProgType, u8PhyAddr, u8PhyAddrPos ); );
...
{
    DEBUG_EDID( PRINT_CURRENT_LINE(); );
    DEBUG_EDID( printf("_MApi_XC_DDCRam_Init(eXcInitDDCType=%d, u8PortIndex0x%X)\n", eXcInitDDCType, u8PortIndex ); );
    ...
    {
        default:
        case E_XC_DDC_INIT_ALL: // Power on first time load all edid
            bNeedInitVGA = TRUE;
        ...
        #if ENABLE_EDID_SWITCH
            {
                ...
                for( u8HdmiIdx = 0; u8HdmiIdx < 4; u8HdmiIdx += 1 )
                {
                    ...
                    if( stGenSetting.stHDMISetting.aeHDMI_Edid_Sel[u8HdmiIdx] == E_USER_HDMI_EDID_1p4 )
                        g_XC_Sys_au8EdidSel[u8HdmiIdx] = XC_HDMI_EDID_1p4;
                    else
                        g_XC_Sys_au8EdidSel[u8HdmiIdx] = XC_HDMI_EDID_2p0;
    ...
    #if ENABLE_EDID_SWITCH
        if( g_XC_Sys_au8EdidSel_HW[0] == g_XC_Sys_au8EdidSel[0] )
        {
            DEBUG_EDID( printf("HDMI_1 skip load edid~\n"); );
        }
        else
    #endif
        {
        #if ENABLE_EDID_SWITCH
            g_XC_Sys_au8EdidSel_HW[0] = g_XC_Sys_au8EdidSel[0];
        #endif
    ...
    #if ENABLE_EDID_SWITCH
        if( g_XC_Sys_au8EdidSel_HW[1] == g_XC_Sys_au8EdidSel[1] )
        {
            DEBUG_EDID( printf("HDMI_2 skip load edid~\n"); );
        }
        else
    #endif
        {
        #if ENABLE_EDID_SWITCH
            g_XC_Sys_au8EdidSel_HW[1] = g_XC_Sys_au8EdidSel[1];
        #endif
    ...
    #if ENABLE_EDID_SWITCH
        if( g_XC_Sys_au8EdidSel_HW[2] == g_XC_Sys_au8EdidSel[2] )
        {
            DEBUG_EDID( printf("HDMI_3 skip load edid~\n"); );
        }
        else
    #endif
        {
        #if ENABLE_EDID_SWITCH
            g_XC_Sys_au8EdidSel_HW[2] = g_XC_Sys_au8EdidSel[2];
        #endif
    ...
    #if ENABLE_EDID_SWITCH
        if( g_XC_Sys_au8EdidSel_HW[3] == g_XC_Sys_au8EdidSel[3] )
        {
            DEBUG_EDID( printf("HDMI_4 skip load edid~\n"); );
        }
        else
    #endif
        {
        #if ENABLE_EDID_SWITCH
            g_XC_Sys_au8EdidSel_HW[3] = g_XC_Sys_au8EdidSel[3];
        #endif
...
#if(XC_MLOAD_ENABLE_POS_SEL == XC_MLOAD_EN_POS_AFTER_XC_SYS)
    MApi_XC_MLoad_Enable(TRUE);
#endif
...
void msAPI_Scaler_SetFreezeScreen( BOOLEAN bEnable, U16 u16ScreenUnMuteTime, SCALER_WIN eWindow)
{
    ...
    else
    {
        ...
        if( g_XC_Sys_abScreenFreeze[eWindow] )
        {
            ...
            //msAPI_Timer_Delayms(50); // dealy to avoid garbage screen
    ...
    else // Disable Mute
    {
        ...
        if (eScreenMuteTmp != s_eScreenMuteStatus[eWindow])
        {
            if( s_eScreenMuteStatus[eWindow] == E_SCREEN_MUTE_INIT  )
            {
                ...
                if(IsATVInUse())
                {
                    ...
                    else
                    {
                        ...
                        if(MApi_XC_Get_3D_Input_Mode(eWindow) != E_XC_3D_INPUT_MODE_NONE)
                        {
                            ...
                            msAPI_Timer_Delayms(u16PQDelyaTime);
            ...
            #endif //(ENABLE_SW_CH_FREEZE_SCREEN)
                {
                ...
                #if 1 // Daniel: Please don't remove this log!!!!!
                    ...
                    {
                        printf("u8FrameCount=%u\n", MHAL_XC_Get_FrameCount());
                    }
                #endif
...
void MApi_XC_Sys_Do_HPD(INPUT_SOURCE_TYPE_t enInputSourceType )
{
    ...
    else
    {
        ...
        // delay
        //msAPI_Timer_Delayms( HDMI_HPD_DELAY );
        msAPI_Timer_Delayms( HDMI_HPD_DELAY__RESET_TO_CLK_HIGH );
...
void MApi_XC_Sys_SetHPD( MS_BOOL bEnable, E_MUX_INPUTPORT enInputPortType )
{
    ...
    if(bEnable)
    {
        ...
        //msAPI_Timer_Delayms(300);
        msAPI_Timer_Delayms(HDMI_HPD_DELAY__CLK_HIGH_TO_HPD_HIGH);
    ...
    else
    {
        ...
        //msAPI_Timer_Delayms(200);
        msAPI_Timer_Delayms(HDMI_HPD_DELAY__HPD_LOW_TO_CLK_LOW);
...
{
    DEBUG_HDMI_HOT_PLUG( PRINT_CURRENT_LINE(); );
...
void MApi_XC_HPD_Do_EnterHDMI(INPUT_SOURCE_TYPE_t enINPUT_SOURCE_TYPE)
{
    ...
    // For only power on init first time HPD
#if(ENABLE_HDMI_FIRST_HDP_BEFORE_SRC_CHANGE && (CHIP_FAMILY_TYPE==CHIP_FAMILY_MAYA))
    if( MApp_Is_BeforeFirstTimeChangeSource() )
    {
        // Patch Maya HDMI2.0 5V detect issue
        //printf("Patch Maya HDMI2.0 5V detect issue %u\n", enHDMIport );
        MDrv_HDMI_SetHdcpEnable(enHDMIport, ENABLE);
    }
#endif
...
U16 MApi_XC_Sys_Get_Fpll_InVFreq(void)
{
    ...
    return (MST_XTAL_CLOCK_HZ*100*u8InVsFactor/u32Ivs);
}
...
U16 MApi_XC_Sys_Get_Fpll_OutVFreq(void)
{
    ...
    return (MST_XTAL_CLOCK_HZ*100*u8OutVsFactor/u32Ovs);
}
...
{
    printf(" iVT=%u,", MApi_XC_R2BYTE(0x013E));

    printf(" iVf=%u,", MApi_XC_Sys_Get_Fpll_InVFreq() );

    printf(" oVT=%u,", MApi_XC_R2BYTE(0x101A)+1 );
...
{
    DEBUG_EDID( PRINT_CURRENT_LINE(); );
    DEBUG_EDID( printf("MApi_XC_Sys_Set_UserEdidSel(eHDMI_Edid_Sel=%u)\n", eHDMI_Edid_Sel); );
...
{
    DEBUG_EDID( PRINT_CURRENT_LINE(); );
    DEBUG_EDID( printf("MApi_XC_Sys_Set_EdidSel(Src=%u, eEdidSel=%u)\n", enInputSourceType, eEdidSel); );
...
//    DEBUG_EDID_SWITCH( printf("MApi_XC_Sys_Reload_Edid(u8Edid=%u)\n", u8Edid); );
    DEBUG_EDID( printf("MApi_XC_Sys_Reload_Edid(Src=%u, eEdidSel=%u, bResetHPD=%u)\n", enInputSourceType, eEdidSel, bResetHPD); );
...
*/
    ...
    if( u8port_count > 0 )
    {
        DEBUG_HDMI_INPUT( printf(">> MDrv_DVI_Software_Reset(eInputPort=%u, BIT5)\n", eInputPort); );
...
#define DEBUG_XC_GAME_MODE(x)   //x

EnuXcSysGameMode g_XcSys_eGameMode = XC_SYS_GAME_MODE_OFF;

EnuXcSysGameMode MApi_XC_Sys_Get_GameMode(void)
{
    EnuXcSysGameMode eXcSysGameMode = XC_SYS_GAME_MODE_OFF;

    if( (g_XcSys_eGameMode == XC_SYS_GAME_MODE_OFF)
      ||(stGenSetting.g_SysSetting.u8GameMode == DISABLE)
      )
    {
        eXcSysGameMode = XC_SYS_GAME_MODE_OFF;
    }
    else
    {
        eXcSysGameMode = XC_SYS_GAME_MODE_ON;
    }

    //DEBUG_XC_GAME_MODE( PRINT_CURRENT_LINE(); );
    if( (eXcSysGameMode == XC_SYS_GAME_MODE_ON) )
    {
        DEBUG_XC_GAME_MODE( printf("GameMode is On\n"); );
    }
    else
    {
        DEBUG_XC_GAME_MODE( printf("GameMode is OFF\n"); );
    }

    return eXcSysGameMode;
}

void MApi_XC_Sys_Set_GameMode(EnuXcSysGameMode eXcSysGameMode)
{
    //DEBUG_XC_GAME_MODE( PRINT_CURRENT_LINE(); );

    g_XcSys_eGameMode = eXcSysGameMode;

    DEBUG_XC_GAME_MODE( printf("g_XcSys_eGameMode=%u\n", g_XcSys_eGameMode); );
}

BOOL MApi_XC_Sys_Is_SupportGameMode(void)
{
    if( g_XcSys_eGameMode == XC_SYS_GAME_MODE_OFF )
    {
        return FALSE;
...
void MApi_XC_Sys_Print_InputInfo(void)
{
...
void MApi_XC_Sys_Print_OutputInfo(void)
{
    printf("\n[OutputTiming]\n");

    // Input timing
    BOOL bIsFBL = MApi_XC_IsCurrentFrameBufferLessMode();
    BOOL bIsRFBL = MApi_XC_IsCurrentRequest_FrameBufferLessMode();


    // Output timing
    printf(" iVT=%u, oVT=%u\n", MApi_XC_R2BYTE(0x013E), MApi_XC_R2BYTE(0x101A)+1 );

    // FBL/RFBL mode
    printf("bIsFBL=%u, bIsRFBL=%u\n", bIsFBL, bIsRFBL);
    //if( bIsFBL || bIsRFBL )
    {
        U16 u16CapSizeV = MApi_XC_R2BYTE(0x010C);
        U16 u16DispSizeV = MApi_XC_R2BYTE(0x1016) - MApi_XC_R2BYTE(0x1014) + 1;

        // Cap size V
        printf(" u16CapSizeV = %u\n", u16CapSizeV);

        // Disp size V
        printf(" u16DispSizeV = %u\n", u16DispSizeV);
    }

    // Frame lock/ FPLL
    U16 u16Reg_3118_val = MDrv_Read2Byte(0x103118);
    BOOL bEnableFPLL = (u16Reg_3118_val&0x08)?1:0;
    BOOL bIsLockFreqMode = (MDrv_Read4Byte(0x1031E1)&BIT7)?TRUE:FALSE;

    printf("\n [Frame lock/FPLL] \n");
    printf(" bEnableFPLL = %u, bIsLockFreqMode=%u\n", bEnableFPLL, bIsLockFreqMode );
    ...
    printf(" InVFreq  = %u,", MApi_XC_Sys_Get_Fpll_InVFreq() );
    printf(" OutVFreq = %u\n", MApi_XC_Sys_Get_Fpll_OutVFreq() );
...
void MApi_XC_Sys_Print_AllInfo(void)
{
    ...
    // Post scaling
    printf(" PreH Ratio  = 0x%08X, PreV Ratio  = 0x%lX \n", MApi_XC_R4BYTE(0x0208), MApi_XC_R4BYTE(0x0210));
    printf(" PostH Ratio = 0x%08X, PostV Ratio = 0x%lX \n", MApi_XC_R4BYTE(0x230E), MApi_XC_R4BYTE(0x2312));
    ...
    // Mem fetch/Offset
    printf(" IPMFetch = %u, IPMOffset = %d\n", drvStatus.u16IPMFetch, drvStatus.u16IPMOffset);
    printf(" OPFetch  = %u, OPOffset  = %d\n", MApi_XC_R2BYTE(0x122E), MApi_XC_R2BYTE(0x122C));
