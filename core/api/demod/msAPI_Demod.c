...
void msAPI_Demod_Init(EnuInitDemodCase eInitDemodCase, EN_DEMODULATOR_TYPE eNewDemodTypeToInit)
{
    ...
    msDebug_PrintChgSrcTime("Demod Init done");
#endif
...
#define MAX_CMD_ARRAY 3
BOOLEAN msAPI_Demod_SetTSClockDivider(MS_U8 u8Divider)
{
    MS_U8 u8array[MAX_CMD_ARRAY];
    memset(u8array, 0, MAX_CMD_ARRAY*sizeof(MS_U8));
    u8array[0] = 0x01;
    u8array[1] = u8Divider;
    return MDrv_DMD_TSO_Clk_Control(u8array);
}

BOOLEAN msAPI_Demod_SetPhaseTuningNumber(MS_U8 u8TuningNumber)
{
    MS_U8 u8array[MAX_CMD_ARRAY];
    memset(u8array, 0, MAX_CMD_ARRAY*sizeof(MS_U8));
    u8array[0] = 0x03;
    u8array[1] = 0x01;
    u8array[2] = u8TuningNumber;
    return MDrv_DMD_TSO_Clk_Control(u8array);
}

MS_U8 msAPI_Demod_GetTSClockDivider(void)
{
    MS_U8 u8array[MAX_CMD_ARRAY];
    memset(u8array, 0, MAX_CMD_ARRAY*sizeof(MS_U8));
    u8array[0] = 0x04;
    if(MDrv_DMD_TSO_Clk_Control(u8array))
    {
        return u8array[1];
    }
    else
    {
        printf("msAPI_Demod_GetTSClockDivider Failed\n");
        return 0xFF;
    }
}

MS_U8 msAPI_Demod_GetPhaseTuningNumber(void)
{
    MS_U8 u8array[MAX_CMD_ARRAY];
    memset(u8array, 0, MAX_CMD_ARRAY*sizeof(MS_U8));
    u8array[0] = 0x04;
    ...
    {
        printf("msAPI_Demod_GetPhaseTuningNumber Failed\n");
        return 0xFF;
    }
}
#endif
