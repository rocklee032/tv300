...
#include "Board.h"
#define TMCCADDRESS (0x1500 + 0x04*2)
...
BOOLEAN KHAL_DEMOD_GetEmergencyAlertFlagStatus(void)
{
    ...
    if ((MDrv_DMD_ISDBT_MD_GetLock(id, DMD_ISDBT_GETLOCK_FEC_LOCK) == DMD_ISDBT_LOCK))
    {
        MDrv_DMD_ISDBT_MD_GetReg(id, TMCCADDRESS, &u8Data);
        if (u8Data & 0x01)
        {
            bRet = TRUE;
        }
