...
void msAPI_MHEG_AUD_PlayMHEG5AudioFile(void)
{
    ...
    // Permanent should not control by MEHG!!
...
void msAPI_MHEG5_StopDsmcc(BOOLEAN bWait)
{
	...
	M5DBG(printf("M5_MAILBOX_SEND start\n"));
    //M5_MAILBOX_SEND(MsgClass , Index , MsgType , ParameterCount , Parameter .....);
    M5_MAILBOX_SEND(    E_MBX_CLASS_TSP
    ...
    M5DBG(printf("M5_MAILBOX_SEND end\n"));
    if(bWait)
    {
        wait=0;
    ...
    M5DBG(printf("M5API_MBX_Clean start\n"));
    //Clean MB MSG
    M5API_MBX_Clean();
    M5DBG(printf("M5API_MBX_Clean done\n"));
