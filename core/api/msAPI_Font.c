...
BOOLEAN msAPI_Font_MVF_LoadFlashFontToSDRAM(U16 u16FontBinID)
{
    ...
    DEBUG_FONT_MEM( printf("fontaddr=0x%X\n", fontaddr); );
    ...
    DEBUG_LOAD_VF( printf("fontaddr_mailbox[%u]=0x%X\n", mvf_font_word_count+0, fontaddr_mailbox[mvf_font_word_count+0]); );

    DEBUG_LOAD_VF( printf("BinInfo.B_IsComp=0x%X\n", BinInfo.B_IsComp); );
    ...
    switch(BinInfo.B_IsComp)
    {
        ...
        case MS_COMPRESS7:
            {
                U32 u32DstVA = fontaddr;
                U32 u32DecVA = _PA2VA(((POOL_BUFFER_ADR+BUF_FOR_DECOMPRESS_OFFSET1) & MIU1) ? ((POOL_BUFFER_ADR+BUF_FOR_DECOMPRESS_OFFSET1) | MIU_INTERVAL) : (POOL_BUFFER_ADR+BUF_FOR_DECOMPRESS_OFFSET1));
                U32 u32TmpVA = _PA2VA(((POOL_BUFFER_ADR+BUF_FOR_DECOMPRESS_OFFSET2) & MIU1) ? ((POOL_BUFFER_ADR+BUF_FOR_DECOMPRESS_OFFSET2) | MIU_INTERVAL) : (POOL_BUFFER_ADR+BUF_FOR_DECOMPRESS_OFFSET2));
                if( SUCCESS != MDrv_DMA_LoadBin(&BinInfo, u32DstVA, u32DecVA, u32TmpVA ) )
                {
                ...
                DEBUG_LOAD_VF( printf("fontaddr_mailbox[%u]=0x%X\n", mvf_font_word_count+1, fontaddr_mailbox[mvf_font_word_count+1]); );
            }
            break;
...
void msAPI_Font_VariableInit(void)
{
    ...
    DEBUG_FONT_MEM( printf("FONT_START_ADR=0x%X\n", FONT_START_ADR); );
    DEBUG_FONT_MEM( printf("fontaddr_mailbox=0x%X\n", fontaddr_mailbox); );
    DEBUG_FONT_MEM( printf("fontaddr=0x%X\n", fontaddr); );
    DEBUG_FONT_MEM( printf("BMPfontaddr=0x%X\n", BMPfontaddr); );
