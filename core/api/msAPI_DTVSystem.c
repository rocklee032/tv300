...
extern EN_OP_TYPE MApp_CI_GetOperatorProfileType(void);
#endif
//---------------------------------------------------------------------------
...
#define DEBUG_CM_ARRANGE_LCN(x) //x
...
#if ENABLE_COMHEM
#define DTV_ONID_SWEDEN_2   0xA027
#endif
...
static U32 _u32ValidAudioIndex=0xFFFFFFFF;
#define _VAILDAUDIOINDEX _u32ValidAudioIndex
...
static WORD m_wfreq;
static WORD m_wTSID;
static WORD m_wService_ID[MAX_VC_PER_PHYSICAL];
static MEMBER_SERVICETYPE m_eService_Type[MAX_VC_PER_PHYSICAL];
#endif
...
void msAPI_CM_DebugArrange(void)
{
    ...
    WORD wdbInvalidService,wdbIsSpecialService,wdbIsSkipped,wdbInvalidCell,wdbUnconfirmedService;
    ...
    for(wdOrder=0; wdOrder < u16ProgramIndexTableArraySize; wdOrder++)
    {
        ...
        wdbInvalidService = pMuxTable[wdOrder].bInvalidService;
        wdbIsSpecialService = pMuxTable[wdOrder].bIsSpecialService;
        wdbIsSkipped = pMuxTable[wdOrder].bIsSkipped;
        wdbInvalidCell = pMuxTable[wdOrder].bInvalidCell;
        wdbUnconfirmedService = pMuxTable[wdOrder].bUnconfirmedService;
        ...
        printf("[DBG] wdbInvalidService=%d, wdbIsSpecialService=%d,wdbIsSkipped=%d,wdbInvalidCell=%d,wdbUnconfirmedService=%d\n",wdbInvalidService,wdbIsSpecialService,wdbIsSkipped,wdbInvalidCell,wdbUnconfirmedService);
    }
}
...
{
    if ( (FALSE == IsServiceTypeValid(bServiceType))
       ||(FALSE == IsPositionValid(bServiceType, wPosition))
       ||(INVALID_PROGRAM_POSITION == wPosition)
       )
    {
        return FALSE;
    }
...
BYTE msAPI_CM_Get_FirstPhysicalChannelNumber(void)
{
    ...
    const DTVPROGRAMID_M * pMuxTable = msAPI_DtvDB_Get_ProgIdTable_ReadOnly(eDtvDbSel);
...
BOOLEAN msAPI_CM_GetHpLP_WithID(WORD wTS_ID, WORD wON_ID, U8* pu8HpLp)
{
    ...
    const DTVPROGRAMID_M *pstProgIdTable = msAPI_DtvDB_Get_ProgIdTable_ReadOnly(eCurDbSel);
...
BOOLEAN msAPI_CM_GetAudioStreamInfo(MEMBER_SERVICETYPE bServiceType, WORD wPosition, AUD_INFO * pstAudioStreamInfo, BYTE cOrdinal)
{
    ...
    if( (TRUE == IsAudioPIDValid(stAudInfo[cOrdinal].wAudPID)) &&
        (GETBIT(_VAILDAUDIOINDEX,cOrdinal)) &&
...
BOOLEAN msAPI_CM_ArrangeAudioStreamInfo(MEMBER_SERVICETYPE bServiceType, WORD wPosition, U16 cValidAudIndex)
{
    ...
    msAPI_CM_Update_AudioValidInfo(0);
    bValidIndex=0;
    ...
    for(i=0; i < bAudioNum; i++)
    {
          ...
          )
        {
            ...
            SETBIT(_VAILDAUDIOINDEX,bValidIndex);
            bValidIndex++;
        }
        else
        {
            stAudInfoNew[bInvalidIndex]=stAudInfo[i];
            CLRBIT(_VAILDAUDIOINDEX,bInvalidIndex);
            bInvalidIndex--;
        }
    }
...
BOOLEAN msAPI_CM_GetServiceName(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BYTE * bChannelName)
{
    ...
    memset(aName, 0, sizeof(BYTE)*MAX_MULTI_LINGUAL_SERVICE_NAME*MAX_SERVICE_NAME);
    ...
    if(aLang[0] != 0xFF)
    {
     ...
     #if ENABLE_COUNTRY_SINGAPORE
       if(OSD_COUNTRY_SETTING == OSD_COUNTRY_SINGAPORE && (bRet == FALSE) &&(bChannelName[0] == NULL))
       {
           i = 0;
           memcpy(bChannelName,aName[i],MAX_SERVICE_NAME);
                //printf("index =%x lang %x name %s %x %x %x %x %x\n",i,aLang[i],
...
BOOLEAN msAPI_CM_AddProgram(DTV_CHANNEL_INFO *pDTVProgramData, BOOLEAN *bDBFull, BOOLEAN bSkipCheck)
{
    ...
    //DTVPROGRAMID_M *pMuxTable = msAPI_DtvDB_Get_ProgIdTable(eCurDbSel);
    const DTVPROGRAMID_M *pMuxTable = msAPI_DtvDB_Get_ProgIdTable_ReadOnly(eCurDbSel);
...
//    msDebug_ANSI_SetColorText(E_FONT_BACKGROUND_COLOR_GREEN);
    ...
    *bDBFull=FALSE;
    ...
    DEBUG_CM_ADD_PROG(msAPI_DtvSys_PrintServiceName( pDTVProgramData->bChannelName, MAX_SERVICE_NAME ););
...
#if(ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
    pDTVProgramData->u8AudioTrackIndex = DEFAULT_AUDIOLANG_SUBTITLELANG_DBINDEX;
    pDTVProgramData->u8SubtitleIndex = DEFAULT_AUDIOLANG_SUBTITLELANG_DBINDEX;
#endif
...
BOOLEAN msAPI_CM_UpdateProgram(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BYTE *pcBuffer, PROGRAMDATA_MEMBER eMember)
{
    ...
    TypDtvDbSel eCurDbSel = CUR_DB_SEL();
    //DTVPROGRAMID_M *pMuxTable = msAPI_DtvDB_Get_ProgIdTable(eCurDbSel);
    DTVPROGRAMID_M *pMuxTable = msAPI_DtvDB_Get_ProgIdTable_RW(eCurDbSel);
    DTVPROGRAMINDEX * pstProgramIndexTable = msAPI_DtvDB_Get_ProgramIndexTable(eCurDbSel);
    U16 u16ProgramIndexTableArraySize = msAPI_DtvDB_Get_ProgramIndexTableArraySize(eCurDbSel);
    DTVNETWORK * pstNetworkTable = msAPI_DtvDB_Get_NetWorkTable(eCurDbSel);
#else
...
static void DistinguishLCNPresentOrAbsent(MEMBER_SERVICETYPE bServiceType)
{
    ...
    WORD wOriinalLCN = 0;
    #if ENABLE_BAT_SCAN
    WORD wTsid,wOnid,wSid;
    MS_SI_LCN_INFO stBatLcnInfo[SI_MAX_MUX_IN_BAT];
    BYTE u8NumBatMux=0,i=0,j=0,u8LCN;
    BOOLEAN bVisible;

    memset(&stBatLcnInfo,0x00,sizeof(MS_SI_LCN_INFO)*SI_MAX_MUX_IN_BAT);
    for(wOrder =0;wOrder<SI_MAX_MUX_IN_BAT;wOrder++)
    {
        stBatLcnInfo[wOrder].u16OnId = INVALID_ON_ID;
        stBatLcnInfo[wOrder].u16TsId = INVALID_TS_ID;
    }
    MApp_SI_GetBATLcn(&u8NumBatMux,stBatLcnInfo);
    #endif
    ...
    for( wPosition = 0; wPosition < wProgramCount; wPosition++)
    {
        ...
        #if ENABLE_BAT_SCAN
        if(MApp_SI_Need2Parse_BAT() && stBatLcnInfo[0].u16OnId != INVALID_ON_ID && stBatLcnInfo[0].u16TsId != INVALID_TS_ID)
        {

            GetIDTable(pstProgramIndexTable[wOrder].cIDIndex, (BYTE *)&wOnid, E_DATA_ON_ID);
            if(wOnid == SKYNETWODKID)
            {
                wSid = pstProgramIndexTable[wOrder].wService_ID;
                GetIDTable(pstProgramIndexTable[wOrder].cIDIndex, (BYTE *)&wTsid, E_DATA_TS_ID);

                for(i=0;i<u8NumBatMux;i++)
                {
                    if(wTsid == stBatLcnInfo[i].u16TsId)
                    {
                        j=0;
                        while(stBatLcnInfo[i].stLcnInfo[j].u16ServiceID)
                        {
                            if(wSid == stBatLcnInfo[i].stLcnInfo[j].u16ServiceID)
                            {
                                u8LCN    = pstProgramIndexTable[wOrder].wLCN                = stBatLcnInfo[i].stLcnInfo[j].u16LCNNumber;
                                bVisible = pstProgramIndexTable[wOrder].bVisibleServiceFlag = stBatLcnInfo[i].stLcnInfo[j].u8fVisable;

                                msAPI_CM_UpdateProgram(bServiceType, wPosition, (U8*)&u8LCN, E_DATA_LCN);
                                msAPI_CM_UpdateProgram(bServiceType, wPosition, (U8*)&u8LCN, E_DATA_TS_LCN);
                                msAPI_CM_UpdateProgramVisibleAndSelectable(bServiceType, wPosition, bVisible,TRUE);
                                break;
                            }
                            j++;
                        }
        ...
        {
            #if (ENABLE_RCSRDS_C)
            if(OSD_COUNTRY_SETTING == OSD_COUNTRY_ROMANIA)
            {
            	//The radio LCN of the RCSRDS cert stream exceeds VALID_RANGE_LCN
            }
            else
            #endif
            {
    ...
    U16 u16IDTableArraySize = msAPI_DtvDB_Get_ProgIdTableArraySize(eDtvDbSel);
        ...
        for( wPosition = 0; wPosition < wProgramCount; wPosition++)
        {
            ...
            if( FALSE == bNative )
            {
                ...
                DEBUG_CM_ARRANGE_LCN( printf("Set ProgIdx(%u) LCN = %X\n", wOrder, INVALID_LOGICAL_CHANNEL_NUMBER); );
...
static void AssignNewLCN(MEMBER_SERVICETYPE bServiceType, E_LCN_ASSIGNMENT_TYPE eAssignmentType, WORD wBeginFromLCN, WORD wEndToLCN, WORD wBeginOfNewLCN, WORD wEndOfNewLCN, WORD wNewAssignableLCN)
{
    ...
    DEBUG_CM_ARRANGE_LCN( PRINT_CURRENT_LINE(); );
    DEBUG_CM_ARRANGE_LCN( printf("AssignNewLCN(bServiceType=%u, eAssignmentType=%u, wBeginFromLCN=%u, wEndToLCN=%u, wBeginOfNewLCN=%u, wEndOfNewLCN=%u, wNewAssignableLCN=%u)\n",
                                bServiceType, eAssignmentType, wBeginFromLCN, wEndToLCN, wBeginOfNewLCN, wEndOfNewLCN, wNewAssignableLCN); );
...
void AssignNewLCNforSorting(MEMBER_SERVICETYPE bServiceType, WORD wBeginFromLCN, WORD wEndToLCN, WORD wBeginOfNewLCN, WORD wEndOfNewLCN, WORD wLastLCN)
{
    ...
    DEBUG_CM_ARRANGE_LCN( PRINT_CURRENT_LINE(); );
    DEBUG_CM_ARRANGE_LCN( printf("AssignNewLCNforSorting(bServiceType=%u, wBeginFromLCN=%u, wEndToLCN=%u, wBeginOfNewLCN=%u, wEndOfNewLCN=%u, wNewAssignableLCN=%u)\n",
                                        bServiceType, wBeginFromLCN, wEndToLCN, wBeginOfNewLCN, wEndOfNewLCN, wNewAssignableLCN); );
...
static void AssignNewLCNforLCNSorting(WORD wBeginFromLCN, WORD wEndToLCN, WORD wBeginOfNewLCN, WORD wEndOfNewLCN, WORD wLastLCN)
{
    ...
    DEBUG_CM_ARRANGE_LCN( PRINT_CURRENT_LINE(); );
    DEBUG_CM_ARRANGE_LCN( printf("AssignNewLCNforLCNSorting(wBeginFromLCN=%u, wEndToLCN=%u, wBeginOfNewLCN=%u, wEndOfNewLCN=%u, wLastLCN=%u)\n", wBeginFromLCN, wEndToLCN, wBeginOfNewLCN, wEndOfNewLCN, wLastLCN); );
    ...
    wNewAssignableLCN = wBeginOfNewLCN;
    DEBUG_CM_ARRANGE_LCN( printf("wNewAssignableLCN=%u\n", wNewAssignableLCN ); );
    ...
    for( wOrder1 = 0; wOrder1 < u16ProgramIndexTableArraySize; wOrder1++ )
    {
        ...
        //printf("wOrder1=%u, eAssignmentType=0x%X\n", wOrder1, eAssignmentType);

        if( (eAssignmentType == (E_LCN_PRESENT|E_LCN_DUPLICATE))
          ||(eAssignmentType == E_LCN_ABSENT)
          ||(eAssignmentType == E_LCN_ZERO)
          )
        {
            wMinOrder    = wOrder1;
            wMinSID      = pstProgramIndexTable[wOrder1].wService_ID;
            ...
            else
            {
                for(wOrder2=wOrder1+1; wOrder2 < u16ProgramIndexTableArraySize; wOrder2++)
                {
                    ...
                    //printf("wOrder2=%u, eAssignmentType=0x%X\n", wOrder2, eAssignmentType);
                ...
                DEBUG_CM_ARRANGE_LCN( printf("wOrder=%u, Set pstProgramIndexTable[wOrder].wLCN=0x%X\n", wOrder, pstProgramIndexTable[wOrder].wLCN); );

                SetProgramTable(pstProgramIndexTable[wOrder].wPRIndex, (BYTE *)&(wNewAssignableLCN), E_DATA_LCN);

                //printf("wOrder=%u, eAssignmentType=0x%X\n", wOrder, pstProgramIndexTable[wOrder].eLCNAssignmentType);
...
#if(ENABLE_S2)
static BOOLEAN IsDvbsReserveLCN(void)
{
    BOOLEAN result = FALSE;
    if(IsS2InUse())
    {
        #if (ENABLE_BAT_SCAN)
        if(OSD_COUNTRY_SETTING == OSD_COUNTRY_NETHERLANDS)
        {
            result = TRUE;
        }
        #endif
        if(OSD_COUNTRY_SETTING == OSD_COUNTRY_ITALY)
        {
            result = TRUE;
        }
    }
...
static void ArrangeLogicalChannelNumber(MEMBER_SERVICETYPE bServiceType, E_MEMBER_LCN_TYPE eLcnType, BOOLEAN bSkipDupSrvRemove)
{
    ...
    {
        #if (ASTRA_HD_ENABLE || ASTRA_LCN_ENABLE)
        if (GetASTRA_HD_ENABLE() && stGenSetting.g_SysSetting.bIsAstraSat)
        {
            // printf("IsDvbsReserveLCN !\n");
            if( eLcnType != E_LCN_TYPE_MOVE)
            {
                DistinguishLCNPresentOrAbsent(bServiceType);
...
#if ENABLE_ZIGGO
    if(msAPI_SI_IsZiggo_Spec_Country(MApp_GetSICountry(eCountry)))
    {
    ...
    case E_FRANCE:
        {
            ...
            wBeginOfNewLCN = 800;
            wNewAssignableLCN = GetLastLCN( bServiceType, E_LCN_ABSENT|E_LCN_DUPLICATE|E_LCN_SAME_SERVICE ) + 1;
            if(bIsTNTLCN)  wBeginOfNewLCN = wNewAssignableLCN;
            AssignNewLCN( bServiceType, E_LCN_DUPLICATE, 1, 799, wBeginOfNewLCN, MAX_LCN, wNewAssignableLCN );
...
static void ArrangeLogicalChannelNumberforLCNSorting(E_MEMBER_LCN_TYPE eLcnType)
{
    ...
    DEBUG_CM_ARRANGE_LCN( PRINT_CURRENT_LINE(); );
    DEBUG_CM_ARRANGE_LCN( printf("ArrangeLogicalChannelNumberforLCNSorting(eLcnType=%u)\n", eLcnType); );
    ...
    DEBUG_CM_ARRANGE_LCN( printf("wLastLCN=%u\n", wLastLCN););
...
void msAPI_CM_DistinguishNotNativeON_ID_By_Country(EN_OSD_COUNTRY_SETTING eCountry )
{
    ...
    switch(eCountry)
    {
        ...
        #if ENABLE_COMHEM
            U16 SWEDEN_ONIDList[] =
                {DTV_ONID_SWEDEN, DTV_ONID_SWEDEN_2};
            DistinguishNotNativeON_IDS(E_SERVICETYPE_DTV, SWEDEN_ONIDList, sizeof(SWEDEN_ONIDList)/2);
            DistinguishNotNativeON_IDS(E_SERVICETYPE_RADIO, SWEDEN_ONIDList, sizeof(SWEDEN_ONIDList)/2);
            DistinguishNotNativeON_IDS(E_SERVICETYPE_DATA, SWEDEN_ONIDList, sizeof(SWEDEN_ONIDList)/2);
        #else
...
void msAPI_CM_GetFirstNormalProgramPosition(WORD cPCN,MEMBER_SERVICETYPE *peServiceType, WORD * pwPosition)
{
    WORD wPLPID;
    WORD wOrder;
    TypDtvDbSel eCurDbSel = CUR_DB_SEL();
    ...
    wPLPID = msAPI_CM_GetPLP_ID(*peServiceType,*pwPosition);

    if(msAPI_CM_GetProgramAttribute(*peServiceType,*pwPosition, E_ATTRIBUTE_IS_VISIBLE)==FALSE
      ||msAPI_CM_GetProgramAttribute(*peServiceType,*pwPosition, E_ATTRIBUTE_IS_DELETED)==TRUE
      )
    {
        WORD wPosition;
        WORD wProgramCount;
        ...
        BYTE u8ServiceType;
        DTVPROGRAMINDEX * pstProgramIndexTable = msAPI_DtvDB_Get_ProgramIndexTable(eCurDbSel);
        WORD u16IDTableArraySize = msAPI_DtvDB_Get_ProgIdTableArraySize(eCurDbSel);

        for(u8ServiceType = E_SERVICETYPE_DTV;u8ServiceType <= E_SERVICETYPE_DATA;u8ServiceType++)
        {
            wProgramCount = msAPI_DTV_GetProgramCount((MEMBER_SERVICETYPE)u8ServiceType, INCLUDE_ALL);

            for(wPosition = 0;wPosition < wProgramCount;wPosition++)
            {
                wOrder = ConvertPositionToOrder((MEMBER_SERVICETYPE)u8ServiceType, wPosition);
                ...
                if( cPCN == msAPI_DtvSys_Get_PCN_ByOrder(eCurDbSel, wOrder) && wPLPID == msAPI_CM_GetPLP_ID((MEMBER_SERVICETYPE)u8ServiceType,wPosition))
                {
                    if(msAPI_CM_GetProgramAttribute((MEMBER_SERVICETYPE)u8ServiceType,wPosition, E_ATTRIBUTE_IS_VISIBLE)==TRUE
                      && msAPI_CM_GetProgramAttribute((MEMBER_SERVICETYPE)u8ServiceType,wPosition, E_ATTRIBUTE_IS_DELETED)==FALSE
                      )
                    {
                        *peServiceType = (MEMBER_SERVICETYPE)u8ServiceType;
                        *pwPosition = wPosition;
                    }
                }
           }
         }
    ...
    if(bReArrangeLcn)
    {
        if(bUpdatePosition)
        {
            ...
            if(bServiceType == E_SERVICETYPE_RADIO)
                g_wCurrentRadioRealServicePosition=g_wCurrentRadioUserServicePosition = wPosition;
            #if NORDIG_FUNC //for Nordig Spec v2.0
            else if(bServiceType == E_SERVICETYPE_DATA)
                g_wCurrentDataRealServicePosition=g_wCurrentDataUserServicePosition = wPosition;
            #endif
            else
                g_wCurrentTVRealServicePosition=g_wCurrentTVUserServicePosition = wPosition;
        #endif
        }
    ...
    if(bReArrangeLcn)
    {
        ...
        if(bUpdatePosition)
        {
            ...
            else
            {
                ...
                {
                    msAPI_CM_GetFirstNormalProgramPosition(wPhysicalChannelNumber,&l_eServiceType, &l_wPosition);
                    if (wPosition != l_wPosition)
                    {
...
BOOLEAN msAPI_CM_GetServiceTypeAndPositionWithEpgTimer(const MS_EPG_TIMER * pstMsEpgTimer, MEMBER_SERVICETYPE *peServiceType, WORD * pwPosition)
{
    WORD wPosition;
    WORD wOrder;
    WORD cIDIndex;
    ...
    for(bServiceType = E_SERVICETYPE_DTV;bServiceType <= E_SERVICETYPE_DATA;bServiceType++)
    {
        wProgramCount = msAPI_DTV_GetProgramCount(bServiceType, INCLUDE_ALL);
        ...
        for(wPosition=0; wPosition < wProgramCount; wPosition++)
        {
            ...
            stChannelInfo.stCHAttribute.bIsOpService= DEFAULT_IS_OP_CHANNEL;
            msAPI_CM_GetProgramInfo(bServiceType, wPosition, &stChannelInfo);
            if( pstMsEpgTimer->bIsOpService != stChannelInfo.stCHAttribute.bIsOpService)
            {
                continue;
            }
        #endif
            ...
            if( pstMsEpgTimer->u16SrvID != pstProgramIndexTable[wOrder].wService_ID )
            {
                continue;
            }
            ...
            if( pstMsEpgTimer->u8PCN == msAPI_DtvSys_Get_PCN_ByOrder(eCurDbSel, wOrder) )
            {
                wRet = wOrder;
    ...
    if( wRet >= u16ProgramIndexTableArraySize )
    {
        return FALSE;
    }

    *peServiceType = msAPI_DtvSys_Get_ServiceType_ByOrder( eCurDbSel, wRet );
    ...
    *pwPosition = ConvertOrderToPosition(wRet);
...
/// @return BOOLEAN: Function execution result
//****************************************************************************
static U16 RemoveMismatchedProgram(MEMBER_SERVICETYPE bServiceType,
                                   WORD cRFChannelNumber,
                                   BYTE cPlp_ID,
                                   BYTE cHpLp,
                                   WORD *pwService_ID,
                                   MEMBER_SERVICETYPE *peService_Type,
                                   U16 u16CountOfServiceID,
                                   BYTE sat,
                                   WORD freq)
{
    WORD u16MaxMuxNum;
...
BOOLEAN msAPI_CM_AddProgramIDTable(DTVPROGRAMID *pstDtvIDTable, BOOLEAN bSave, U16 *pu16IDIndex)
{
    ...
    DTVPROGRAMID_M *pProgramIDTable = msAPI_DtvDB_Get_ProgIdTable_RW(eDtvDbSel);
    U16 u16IDTableArraySize = msAPI_DtvDB_Get_ProgIdTableArraySize(eDtvDbSel);
    //DTVPROGRAMINDEX * pstProgramIndexTable = msAPI_DtvDB_Get_ProgramIndexTable(eDtvDbSel);
    //U16 u16ProgramIndexTableArraySize = msAPI_DtvDB_Get_ProgramIndexTableArraySize(eDtvDbSel);
    DTVNETWORK * pstNetworkTable = msAPI_DtvDB_Get_NetWorkTable(eDtvDbSel);
...
BOOLEAN SortProgram(MEMBER_SERVICETYPE bServiceType)
{
    ...
    const DTVPROGRAMID_M *pMuxTable = msAPI_DtvDB_Get_ProgIdTable_ReadOnly(eCurDtvDbSel);
...
BOOLEAN GetProgramTable_DBSel(TypDtvDbSel eDbSel, DWORD wPRIndex, BYTE * pcBuffer, PROGRAMDATA_MEMBER eMember)
{
    ...
    #if (ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
        case E_DATA_SUBTITLE_IDX:
            return GetNVRAM(u32CurProgRamDiskAddr + offsetof(DTV_CHANNEL_INFO, u8SubtitleIndex),
                            pcBuffer, sizeof(BYTE));
        case E_DATA_AUDIOLANG_IDX:
            return GetNVRAM(u32CurProgRamDiskAddr + offsetof(DTV_CHANNEL_INFO, u8AudioTrackIndex),
                            pcBuffer, sizeof(BYTE));
    #endif
...
#if (ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
        case E_DATA_SUBTITLE_IDX:
            return SetNVRAM(u32CurProgRamDiskAddr+offsetof(DTV_CHANNEL_INFO, u8SubtitleIndex),
                            pcBuffer, sizeof(BYTE));
        case E_DATA_AUDIOLANG_IDX:
            return SetNVRAM(u32CurProgRamDiskAddr+offsetof(DTV_CHANNEL_INFO, u8AudioTrackIndex),
                            pcBuffer, sizeof(BYTE));
#endif
...
BOOLEAN FillProgramIDWithDefault(DTVPROGRAMID_M * pstDTVProgramID)
{
    ...
    pstDTVProgramID->SatelliteId = INVALID_SATELLITE_ID;
#endif
...
static BOOLEAN FillProgramDataWithDefault(BYTE * pcBuffer, PROGRAMDATA_MEMBER eMember)
{
    ...
    case E_DATA_ALL:
      ...
      #if(ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
        DTVProgramData.u8AudioTrackIndex = DEFAULT_AUDIOLANG_SUBTITLELANG_DBINDEX;
        DTVProgramData.u8SubtitleIndex = DEFAULT_AUDIOLANG_SUBTITLELANG_DBINDEX;
      #endif
...
static void UpdateIDInfo(TypDtvDbSel eDtvDbSel)
{
    ...
    DTVPROGRAMID_M *pstIDTable = msAPI_DtvDB_Get_ProgIdTable_RW(eDtvDbSel);
...
static void _RestInvalidMUXAndNetwork(void)
{
    ...
    DTVPROGRAMID_M* pMuxInfo = msAPI_DtvDB_Get_ProgIdTable_RW(eCurDbSel);//_astDTVProgramIDTable;
...
{
    _u32ValidAudioIndex=0xFFFFFFFF;
}
...
//****************************************************************************
U32 msAPI_CM_Get_AudioValidIndex(void)
{
    return _u32ValidAudioIndex;
}
...
/// - FALSE: NO
//****************************************************************************
BOOLEAN msAPI_CM_Update_AudioValidInfo(U32 u32AudioValidIndex)
{
    _u32ValidAudioIndex=u32AudioValidIndex;
    return TRUE;
}
...
void msAPI_CM_SetAudioStreamValidIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition, WORD wType, BYTE bISOLangIndex, U32 *pu32Index)
{
    AUD_INFO stAudInfo[MAX_AUD_LANG_NUM];
    WORD wOrder, wPRIndex;
    BYTE i;
...
BOOLEAN msAPI_CM_Is_TSExist(WORD wONID, WORD wTSID, BYTE *pcRFChannelNumber)
{
    ...
    //DTVPROGRAMINDEX * pstProgramIndexTable = msAPI_DtvDB_Get_ProgramIndexTable(eDtvDbSel);
    //DTVPROGRAMID_M * pstIDTable = msAPI_DtvDB_Get_ProgIdTable(eDtvDbSel);
    const DTVPROGRAMID_M * pstIDTable = msAPI_DtvDB_Get_ProgIdTable_ReadOnly(eDtvDbSel);
...
BOOLEAN msAPI_CM_CheckNetwork(WORD wNID, WORD *pwTS_ID,  BYTE cCountOfTS, U32 *u32NewTS, WORD *pwRemoveTSID, BYTE cMaxTSCnt, BOOLEAN *bRemove)
{
    ...
    U8 u8Multiple = ((MApp_GetSICountry(OSD_COUNTRY_SETTING) == SI_COUNTRY_NORWAY)? 10:1);
    ...
    for(i=0;i<u16IDTableArraySize;i++)
    {
        ...
        if( u16NetWorkIndex < u16NetWorkTableArraySize)// anvi  resolve mantis 0378496(Chakra3)
        {
            if(pstNetWorkTable[u16NetWorkIndex].wNetwork_ID == wNID)
            {
                ...
                {
                    if((pwTS_ID[j] /u8Multiple) == (pMuxTable[i].wTransportStream_ID /u8Multiple))
                    {
                        CLRBIT(u32GetTS,j);
                        break;
                    }
...
/*
    PRINT_CURRENT_LINE();
    if( eDtvDbSel == E_DTV_DB_SEL_DVBT )
    {
        msAPI_DTV_DB_DumpDB(E_DTV_DB_SEL_DVBT, CM_DUMP_INFO_ALL);
    }
*/
...
#if ENABLE_CI_PLUS_V1_4
    || EN_OP_TYPE_2 == MApp_CI_GetOperatorProfileType()
#endif
    )
    {
...
BOOLEAN msAPI_CM_ResetOriginalRFnumber(MEMBER_SERVICETYPE bServiceType, WORD wPosition)
{
    ...
    U16 u16IDTableArraySize = msAPI_DtvDB_Get_ProgIdTableArraySize(eDtvDbSel);
    //DTVPROGRAMID_M * pMuxTable = msAPI_DtvDB_Get_ProgIdTable(eDtvDbSel);
    DTVPROGRAMID_M * pMuxTable = msAPI_DtvDB_Get_ProgIdTable_RW(eDtvDbSel);
...
BOOLEAN msAPI_CM_UpdateLCDV2(U8* pu8Country, U8 u8RegionId)
{
    ...
    LOGICAL_CHANNEL_V2 * mLCDV2 = NULL;

    mLCDV2 = (LOGICAL_CHANNEL_V2 *)msAPI_Memory_Allocate( (sizeof(LOGICAL_CHANNEL_V2)*LCDV2ArrayMaxSize), BUF_ID_DTV_SYS_LCD_V2);
    if( mLCDV2 == NULL )
    {
        __ASSERT(0);
        printf("\nError: Malloc LCN2 temp buffer fail!\n");
        return FALSE;
    }

    memset(mLCDV2, 0, (sizeof(LOGICAL_CHANNEL_V2)*LCDV2ArrayMaxSize) );
    ...
    MSAPI_MEMORY_FREE( mLCDV2, BUF_ID_DTV_SYS_LCD_V2);
...
//BOOLEAN msAPI_CM_RemoveMux(WORD wNID, WORD *pwTS_ID,  BYTE cCountOfTS, BOOLEAN *bRemove)
{
    ...
    *bRemove = FALSE;
    ...
    for(u16IDTabIdx=0; u16IDTabIdx < u16IDTableArraySize; ++ u16IDTabIdx )
    {
        u16NetWorkIndex = pMuxTable[u16IDTabIdx].cNetWorkIndex;
        if( u16NetWorkIndex >= u16NetWorkTableArraySize )
            continue;
        ...
        if(pstNetWorkTable[u16NetWorkIndex].wNetwork_ID== wNID)
        {
            for(j=0;j<cCountOfTS;j++)
            {
                if(pwTS_ID[j] == pMuxTable[u16IDTabIdx].wTransportStream_ID)
                {
                    break;
                }
            }
            ...
            #if 1//ENABLE_S2
                RemoveMismatchedProgram(E_SERVICETYPE_DTV, u16IDTabIdx, 0, 0, &dummy1, &dummy2, 0, sat, freq);
                RemoveMismatchedProgram(E_SERVICETYPE_DATA, u16IDTabIdx, 0, 0, &dummy1, &dummy2, 0, sat, freq);
                RemoveMismatchedProgram(E_SERVICETYPE_RADIO, u16IDTabIdx, 0, 0, &dummy1, &dummy2, 0, sat, freq);
            #else
                RemoveMismatchedProgram(E_SERVICETYPE_DTV,i, 0, 0, &dummy1, &dummy2, 0);
            #endif

            #if ENABLE_DVBC
                pMuxTable[u16IDTabIdx].u32Frequency=INVALID_FREQUENCY;
            #endif
                pMuxTable[u16IDTabIdx].cRFChannelNumber=INVALID_PHYSICAL_CHANNEL_NUMBER;

                // Save to RamDisk
                SetIDTable_DBSel(eDtvDbSel, u16IDTabIdx, (BYTE*) &(pMuxTable[u16IDTabIdx]), E_DATA_ID_TABLE);

                // InActivce ID tab
                ActiveIDEntity_DBSel( eDtvDbSel, u16IDTabIdx, FALSE);
...
static U16 GetCountOfSameServiceWithServiceIDs(MEMBER_SERVICETYPE bServiceType, WORD wTransportStream_ID, WORD wOriginalNetwork_ID, WORD wService_ID)
{
    ...
    const DTVPROGRAMID_M * pIDTable = msAPI_DtvDB_Get_ProgIdTable_ReadOnly(CUR_DB_SEL());
...
AUDIOSTREAM_TYPE msAPI_CM_SIAudStreamType_To_AudStreamType_AD(SI_AUDIOSTREAM_TYPE eSIAudStreamType)
{
    switch(eSIAudStreamType)
    {
        case E_SI_AUDIOSTREAM_AC3:
            return E_AUDIOSTREAM_AC3_AD;

        case E_SI_AUDIOSTREAM_MPEG4:
            return E_AUDIOSTREAM_MPEG_AD;

        case E_SI_AUDIOSTREAM_AAC:
            return E_AUDIOSTREAM_AACP_AD;

        case E_SI_AUDIOSTREAM_AC3P:
            return E_AUDIOSTREAM_AC3P_AD;
...
AUDIOSTREAM_TYPE msAPI_CM_Get_AudStreamType_AD_By_AudInfo(AUD_INFO* pstAudInfo)
{
    ///< 0x00: Undefined, 0x01: Clean effects, 0x02: Hearing impaired, 0x03: Visual impaired commentary, 0x04~0xFF: Reserved.
    if( pstAudInfo->aISOLangInfo[0].bAudType == 0x03 )
    {
        return msAPI_CM_SIAudStreamType_To_AudStreamType_AD(pstAudInfo->eSIAudType);
    }
...
MS_U32 msAPI_CM_GetDefaultNetworkIDByOSDCountry(void)
{
    if(IsS2InUse())
    {
...
#if(ENABLE_S2)
BOOLEAN msAPI_CM_GetFreqByIDIndex(U16 u16IDIndex,BYTE *SatelliteId,U32 *u32Frequency)
{
    TypDtvDbSel eDtvDbSel = CUR_DB_SEL();
    ...
    if( IsS2InUse() )
    {
        if(SatelliteId == NULL || u32Frequency ==NULL)
            return FALSE;
        *SatelliteId = pMuxTable[u16IDIndex].SatelliteId;
        *u32Frequency = pMuxTable[u16IDIndex].u32S2Frequency;
    }
    else if( IsDVBCInUse() )
    {
        if(u32Frequency ==NULL)
            return FALSE;
        *u32Frequency = pMuxTable[u16IDIndex].u32Frequency;
    }
    else
    {
        return FALSE;
...
#if ENABLE_BAT_SCAN
EN_SI_BOUQUET_ID msAPI_Region2BouquetID(EN_REGION eRegion)
{
    switch(eRegion)
    {
        case E_REGION_NORTHLAND:
        case E_REGION_AUCKLAND:
        {
            return SI_REGION_AUCKLAND;
        }
        case E_REGION_WAIKATO:
        case E_REGION_ROTORUA:
        case E_REGION_BAY_OF_PLENTY:
        case E_REGION_TAUPO:
        {
            return SI_REGION_WAIKATO;
        }
        case E_REGION_GISBORNE:
        case E_REGION_NAPIER:
        case E_REGION_HASTINGS:
        case E_REGION_TARANAKI:
        case E_REGION_WANGANUI:
        case E_REGION_WANAWATU:
        {
            return SI_REGION_CENTRAL;
        }
        case E_REGION_WAIRARAPA:
        case E_REGION_WELLINGTON:
        {
            return SI_REGION_WELLINGTON;
        }
        case E_REGION_WEST_COAST:
        case E_REGION_CANTERBURY:
        {
            return SI_REGION_CHRISTCHURCH;
        }
        case E_REGION_SOUTHEM_LAKES:
        case E_REGION_OTAGO:
        case E_REGION_SOUTHLAND:
        {
            return SI_REGION_DUNEDIN;
        }
        case E_REGION_ALL_FAT:
        {
            return SI_REGION_ENGINEERING_TEST_BOUQUET;
        }
        default:
            return SI_REGION_UNKNOWN;
    }
}
void msAPI_SetRegion(EN_REGION eRegion)
{
    e_Region = eRegion;
}
EN_REGION msAPI_GetRegion(void)
{
    return e_Region;
}
#endif
...
#if (ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
BYTE msAPI_CM_GetSubtitleIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition)
{
    WORD wOrder;
    WORD wPRIndex;
    BYTE u8SubtitleIndex = 0;
    ...
    if( TRUE != GetProgramTable(wPRIndex, (BYTE *)&u8SubtitleIndex, E_DATA_SUBTITLE_IDX) )
    {
        return 0;
    }

    //if( FALSE == IsVideoPIDValid(u8SubtitleIndex) )
    //{
    //    return INVALID_PID;
    //}
    return u8SubtitleIndex;
}

BOOLEAN msAPI_CM_SetSubtitleIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BYTE u8SubtitleIndex)
{
    WORD wOrder,wPRIndex;
    TypDtvDbSel eDtvDbSel = CUR_DB_SEL();
    DTVPROGRAMINDEX* pstProgramIndexTable = msAPI_DtvDB_Get_ProgramIndexTable(eDtvDbSel);
    ...
    return SetProgramTable(wPRIndex, &u8SubtitleIndex, E_DATA_SUBTITLE_IDX);
}

BYTE msAPI_CM_GetAudioLangIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition)
{
    WORD wOrder;
    WORD wPRIndex;
    BYTE u8AudioLangIndex =0;
    TypDtvDbSel eDtvDbSel = CUR_DB_SEL();
    DTVPROGRAMINDEX* pstProgramIndexTable = msAPI_DtvDB_Get_ProgramIndexTable(eDtvDbSel);
    ...
    if( TRUE != GetProgramTable(wPRIndex, (BYTE *)&u8AudioLangIndex, E_DATA_AUDIOLANG_IDX) )
    {
        return 0;
    }
    ...
    //    return INVALID_PID;
    //}
    return u8AudioLangIndex;
}

BOOLEAN msAPI_CM_SetAudioLangIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BYTE u8AudioLangIndex)
{
    WORD wOrder,wPRIndex;
    TypDtvDbSel eDtvDbSel = CUR_DB_SEL();
    DTVPROGRAMINDEX* pstProgramIndexTable = msAPI_DtvDB_Get_ProgramIndexTable(eDtvDbSel);
    ...
    return SetProgramTable(wPRIndex, &u8AudioLangIndex, E_DATA_AUDIOLANG_IDX);
}
#endif

BOOLEAN msAPI_CM_UpdateProgramScrambWithServiceID(WORD wService_ID,BOOLEAN b8CAdescriptor)
{
    WORD wOrder,wPRIndex;
    CHANNEL_ATTRIBUTE stCHAttribute;
    MEMBER_SERVICETYPE bServiceType;
    ...
    MEMBER_SI_SERVICETYPE bCurServiceType = msAPI_SI_ToSI_Service_Type(msAPI_CM_GetCurrentServiceType());
    WORD wCurPosition = msAPI_CM_GetCurrentPosition(msAPI_CM_GetCurrentServiceType());
    
    WORD wTransportStream_ID = msAPI_SI_GetTS_ID(bCurServiceType,wCurPosition);
    WORD wOriginalNetwork_ID = msAPI_SI_GetON_ID(bCurServiceType,wCurPosition);

    for(bServiceType = E_SERVICETYPE_DTV;bServiceType < E_SERVICETYPE_UNITED_TV;bServiceType++)
    {
        wOrder = GetOrderOfSameServiceWithIDs(bServiceType,wTransportStream_ID,wOriginalNetwork_ID,wService_ID,1,TRUE);
        if(wOrder != INVALID_ORDER)
            break;
    }    
    ...
    stCHAttribute.bIsScramble = b8CAdescriptor;
    pstProgramIndexTable[wOrder].bIsScramble = b8CAdescriptor;
