...
BOOLEAN msAPI_OP_CheckOpCacheInfo(void)
{
    U32 i;
    ...
    {
        if ( (TRUE == _stOpCacheInfo[i].bUsed)
             && (0 == _stOpCacheInfo[i].stCiOpInfo.u32CicamIdentifier) )
        {
            ...
            if ( FALSE == msAPI_OP_Save((void *)&_stOpCacheInfo[i], E_OPCACHE_INFO) )
            {
                MW_DTV_CI_OPCACHE_DB_ERROR("[%s:%d] Save op fail\n", __FUNCTION__, __LINE__);
                return FALSE;
            }
...
BOOLEAN msAPI_OP_DeleteByIndex(U8 u8Index)
{
    ...
    return (msAPI_OP_Save((void *)&_stOpCacheInfo[u8Index], E_OPCACHE_INFO));
}
...
U8 msAPI_OP_GetOpCacheIndexByCicamId(U32 u32CicamId)
{
    MW_DTV_CI_OPCACHE_DB_FUNCTION("%s:%d\n",__PRETTY_FUNCTION__,__LINE__);
    U8 i = 0;
    MW_OPCACHE_INFO stOpCacheInfo;

    for (i = 0; i < msAPI_OP_GetOpCacheCount(); i++)
    {
        if (TRUE == msAPI_OP_GetOpCacheInfoByIndex(i, &stOpCacheInfo))
        {
            if (stOpCacheInfo.stCiOpInfo.u32CicamIdentifier == u32CicamId)
            {
                return i;
            }
        }
...
BOOLEAN msAPI_OP_UpdateCIOpInfo(const CI_OP_INFO *stCiOpInfo, U8 u8ProfileType)
{
    MW_OPCACHE_INFO stOpCacheInfo;
    BOOL bRet = TRUE;
    ...
    if(FALSE == msAPI_OP_GetOpCacheInfoByCicamId(stCiOpInfo->u32CicamIdentifier, &stOpCacheInfo))
    {
        if(msAPI_OP_GetOpCacheCount() < MAX_OP_CACHE_NUM)
        {
            ...
            stOpCacheInfo.u8ProfileType = u8ProfileType;
            MW_DTV_CI_OPCACHE_DB_DBG("u32ID:> %ld\n",stOpCacheInfo.u32ID);
            MW_DTV_CI_OPCACHE_DB_DBG("bUsed:> %d\n",stOpCacheInfo.bUsed);
            ...
            MW_DTV_CI_OPCACHE_DB_DBG("u8ProfileType:> %d\n",stOpCacheInfo.u8ProfileType);
