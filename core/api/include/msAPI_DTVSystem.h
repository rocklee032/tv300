...
#if( ENABLE_DVBS )
#define INVALID_SATELLITE_ID        0xFF // 8 bits
#endif
...
//        #define MAX_SERVICE_NAME           27//20+1+2//Brazil only need 20 chars+0xE Char+2 char for 10.1 channel
    ...
    //@@--Changed for Some long name Channel
        ...
        #if(ENABLE_ZIGGO)
	        #if (ENABLE_TDT)
	        #define MAX_DTVPROGRAM              460
	        #define MAX_SERVICE_NAME            60
	        #define MAX_DTVPROGRAM_DVBC         460 // MAX_DTV_C_PROGRAM
	        #else
	        #define MAX_DTVPROGRAM              520//600
	        #define MAX_SERVICE_NAME            52//32
	        #define MAX_DTVPROGRAM_DVBC         600//520 // MAX_DTV_C_PROGRAM
	        #endif
        #else
...
#define MAX_MULTI_LINGUAL_SERVICE_NAME      2
...
#define E_DATA_SUBTITLE_IDX                 0x33
#define E_DATA_AUDIOLANG_IDX                0x34
#endif
...
#elif  ENABLE_SBTVD_DTV_SYSTEM
#define DEFAULT_COUNTRY	                        ISDB_DEFAULT_COUNTRY
...
#define DEFAULT_CURRENT_REGION                  SI_REGION_UNKNOWN
...
#if ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB
#define DEFAULT_AUDIOLANG_SUBTITLELANG_DBINDEX        0xFF
#endif
...
#if (ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
    BYTE u8SubtitleIndex;
    BYTE u8AudioTrackIndex;
#endif
...
{
    E_REGION_MIN,
    E_REGION_NORTHLAND = E_REGION_MIN,
    E_REGION_AUCKLAND,
    E_REGION_WAIKATO,
    E_REGION_ROTORUA,
    E_REGION_BAY_OF_PLENTY,
    E_REGION_TAUPO,
    E_REGION_GISBORNE,
    E_REGION_NAPIER,
    E_REGION_HASTINGS,
    ...
    E_REGION_WEST_COAST,
    E_REGION_CANTERBURY,
    E_REGION_SOUTHEM_LAKES,
    E_REGION_OTAGO,
    E_REGION_SOUTHLAND,
    E_REGION_ALL_FAT,
    E_REGION_MAX = E_REGION_ALL_FAT,
    E_REGION_UNKNOWN = 0xFFFF,
}EN_REGION;
#endif
...
INTERFACE BOOLEAN msAPI_CM_GetServiceTypeAndPositionWithEpgTimer(const MS_EPG_TIMER * pstMsEpgTimer, MEMBER_SERVICETYPE *peServiceType, WORD * pwPosition);
...
INTERFACE U32 msAPI_CM_Get_AudioValidIndex(void);
INTERFACE BOOLEAN msAPI_CM_Update_AudioValidInfo(U32 u32AudioValidIndex);
...
INTERFACE void msAPI_CM_SetAudioStreamValidIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition,  WORD wType, BYTE bISOLangIndex,U32 *pu32Index);
...
INTERFACE MS_U32 msAPI_CM_GetDefaultNetworkIDByOSDCountry(void);

#if( ENABLE_CM_STORE_AUD_INFO_NEW_METHOD )
BOOLEAN msAPI_CM_Add_AudInfo(AUD_INFO * pstCMAudInfo, U8 u8Count);
#endif
INTERFACE BOOLEAN msAPI_CM_GetFreqByIDIndex(U16 u16IDIndex,BYTE *SatelliteId,U32 *u32Frequency);

#endif // #if( ENABLE_API_DTV_SYSTEM )

#if ENABLE_BAT_SCAN
INTERFACE EN_SI_BOUQUET_ID msAPI_Region2BouquetID(EN_REGION eRegion);
INTERFACE void msAPI_SetRegion(EN_REGION eRegion);
INTERFACE EN_REGION msAPI_GetRegion(void);
#endif

#if (ENABLE_AUDIOLANG_SUBTITLELANG_IN_CHDB)
INTERFACE BYTE msAPI_CM_GetSubtitleIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition);
INTERFACE BOOLEAN msAPI_CM_SetSubtitleIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BYTE u8SubtitleIndex);
INTERFACE BYTE msAPI_CM_GetAudioLangIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition);
INTERFACE BOOLEAN msAPI_CM_SetAudioLangIndex(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BYTE u8AudioLangIndex);
#endif

INTERFACE BOOLEAN msAPI_CM_UpdateProgramScrambWithServiceID(WORD wService_ID,BOOLEAN b8CAdescriptor);
