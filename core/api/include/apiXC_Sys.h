...
#define DEBUG_HDMI_INPUT(x)     //x

#define DEBUG_EDID(x)   //x

//---------------------------------------------------------------------------
#if( CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA)
    // Because 5v detect must be set before HPD,
    // and after change source , we can turn on the correct port
    #define ENABLE_HDMI_FIRST_HDP_BEFORE_SRC_CHANGE 0

#elif(ENABLE_EDID_SWITCH)
    // Turn on this option can speed up boot time => Maya need more test
...
#define XC_MLOAD_EN_POS_AFTER_XC_DRV        0
#define XC_MLOAD_EN_POS_AFTER_XC_SYS        1
#define XC_MLOAD_EN_POS_AFTER_PANEL_ENABLE  2
#define XC_MLOAD_EN_POS_AFTER_CHANGE_SOURCE 3

#if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_MAZDA)    \
   ||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MARLON)   \
   ||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MARLON2)   \
   )
    #define XC_MLOAD_ENABLE_POS_SEL XC_MLOAD_EN_POS_AFTER_CHANGE_SOURCE
#else
    #define XC_MLOAD_ENABLE_POS_SEL XC_MLOAD_EN_POS_AFTER_XC_SYS
#endif
