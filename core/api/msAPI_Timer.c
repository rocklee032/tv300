...
void msAPI_Timer_SyncToPmRTC(void)
{
    ...
    //if( msAPI_Timer_DiffTimeFromNow(preSyncTime) > PM_RTC_UPDATE_TIMEOUT )
    {
        ...
        DEBUG_RTC( printf("u32PmRTC_Time=%u\n", u32PmRTC_Time); );
...
{
    DEBUG_WAKE_UP( printf("msAPI_Timer_SetWakeupTime(%u)\n", u32Time); );
...
U32 msAPI_Timer_DiffTimeFromNow_2(U32 u32TaskTimer) //unit = ms
{
    U32 u32CurrentTime = MsOS_GetSystemTime();

    if (u32TaskTimer > u32CurrentTime)
    {
        return (0xFFFFFFFF - u32TaskTimer + u32CurrentTime + 1);
    }
    else
    {
        return MsOS_Timer_DiffTimeFromNow(u32TaskTimer);
...
U32 msAPI_PiuTimer_GetTime0(void)
{
    return MDrv_GetPiuTimerSystemTime();
}

U32 msAPI_PiuTimer_DiffTimeFromNow(U32 u32TaskTimer)
{
    U32 u32CurrentTime=MDrv_GetPiuTimerSystemTime();

    if(u32CurrentTime>u32TaskTimer)
        return (u32CurrentTime - u32TaskTimer);
    else
        return (0xFFFFFFFF-u32TaskTimer+1)+u32CurrentTime;
}
...
void msAPI_Timer_Delayus(U32 u32DelayTime)
{
    MsOS_DelayTaskUs(u32DelayTime);
}
