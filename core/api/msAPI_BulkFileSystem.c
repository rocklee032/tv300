            ...
            FileEntry *pFileDirEntry)
{
    ...
    if (msAPI_FSEnv_GetEnvironment( u8EnvironmentID ) == NULL)
    {
        __ASSERT(0);
        return INVALID_FILE_HANDLE;
    }
...
//    LongLong ringSize;
    ...
    if (BKFS_GetBulkFileEntryXDATA(BulkFileID) == NULL)
    {
        __ASSERT(0);
        return FALSE;
    }
...
U16 BKFS_CalculateBulksNum(U8 u8EnvironmentID)
{
    ...
    if ((u32SectorsOfDiskSize == 0) || (u32SectorsOfDiskSize > u32DiskAvailableSectors)) //Request all disk size or Request over quota
        ...
        u16BulksToCreate = (U16)(u32SectorsOfDiskSize/((U32)(u32FileTotalSize/u16BytesPerSector)));
...
//            memset(pBulkFile, 0, BULK_FILE_ENTRY_SIZE);
            memset(pBulkFile, 0, sizeof(BulkFileEntry));
