...
#define DB_FAC_BLOCK_MAGIC_ID           0xAABB
...
// Check tmp buffer size
#if( DB_FAC_SAVE_DATA_SIZE_MAX > DB_FAC_DATA_BUF_SIZE )
    #error "DB_FAC_SAVE_DATA_SIZE_MAX > DB_FAC_DATA_BUF_SIZE"
#endif
...
{
    BOOL bRtn;
    U8 u8Status = DB_FAC_BLOCK_STATUS_INVALID;
...
U8 _msAPI_DB_FAC_FindLast(void)
{
    ...
    for( u8BlockIdx = 0; u8BlockIdx < DB_FAC_BLOCK_NUM; ++ u8BlockIdx )
    {
        ...
        // Check magic id
        if( stBlockInfo.u16MagicId != DB_FAC_BLOCK_MAGIC_ID )
        {
            DEBUG_API_DB_FACTORY( printf(" Magic err\n"); );
            printf(" Magic err\n");
            continue;
        }
...
BOOL _msAPI_DB_FAC_CheckFlashBlockCRC(U8 u8BlockIdx)
{
    ...
    // Check data size ...
    if( (stBlockInfo.u16DataSize > DB_FAC_SAVE_DATA_SIZE_MAX)
      ||(stBlockInfo.u16DataSize == 0)
      )
    {
        return FALSE;
    }

    // Clear tmp buffer before read flash
    memset( s_DB_FAC_au8DataBuf, 0, sizeof(s_DB_FAC_au8DataBuf) );
...
{
    U8 u8LastBlockIdx = DB_FAC_BLOCK_INDEX_INVALID;
    ...
    while( bBlockCRCErrorCount < DB_FAC_BLOCK_NUM )
    {
        ...
        if( FALSE == _msAPI_DB_FAC_CheckFlashBlockCRC(u8LastBlockIdx) )
        {
            ...
            u8LastBlockIdx = DB_FAC_BLOCK_INDEX_INVALID;
        }
        else
    #endif
        {
...
BOOL msAPI_DB_Factory_Init(void)
{
    ...
    DEBUG_API_DB_FACTORY( printf("Factory use flash: 0x%X ~ 0x%X\n", DB_FAC_FLASH_START_ADDR, DB_FAC_FLASH_END_ADDR); );

    //msAPI_DB_Factory_PrintBlockInfo();

    //BUILD_ERROR_IF_TRUE( (sizeof(MS_FACTORY_SETTING) > (DB_FAC_BLOCK_SIZE)) );
...
BOOL msAPI_DB_Factory_LoadFromFlash(U8* pu8DataBuf, U32 u32DataSize)
{
    ...
    DEBUG_API_DB_FACTORY( printf("msAPI_DB_Factory_LoadFromFlash(u32DataSize=0x%X)\n", u32DataSize); );
    ...
    // Warning: Data size chang ~
    if( s_DB_FAC_stLastBlockInfo.u16DataSize != u32DataSize )
    {
        DB_FAC_ERROR( printf("\nDB_FAC: Warning: Data size changed: 0x%X => 0x%X\n", s_DB_FAC_stLastBlockInfo.u16DataSize, u32DataSize); );
    }
...
BOOL _msAPI_DB_FAC_WriteDataToFlash(U8* pu8DataBuf, U32 u32DataSize)
{
    ...
    //DEBUG_API_DB_FACTORY( printf("_msAPI_DB_FAC_WriteDataToFlash(u32DataSize=%u)\n", u32DataSize); );

    msDebug_ANSI_SetColorText_2(E_FONT_COLOR_BLACK, E_FONT_BACKGROUND_COLOR_GREEN);
    printf( "_msAPI_DB_FAC_WriteDataToFlash(u32DataSize=%u)\n", u32DataSize);
    msDebug_ANSI_AllAttrOffText();
    ...
    // Set magic id
    stNewBlockInfo.u16MagicId = DB_FAC_BLOCK_MAGIC_ID;
...
void msAPI_DB_Factory_PrintBlockInfo(void)
{
    ...
    for( i = 0; i < DB_FAC_BLOCK_NUM; i += 1 )
    {
        ...
        if( stBlockInfo.u8BlockStatus == DB_FAC_BLOCK_STATUS_GOOD )
        {
            printf(" Mag=%X", stBlockInfo.u16MagicId);
