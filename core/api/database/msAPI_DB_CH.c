...
BOOL _msAPI_DB_CH_WriteHeaderToFlash(U8 u8DBIdx)
{
    ...
    u32FlashAddr = DB_CH_VERSION_FLASH_ADDR(u8DBIdx);
    if( FALSE == MDrv_FLASH_Write(u32FlashAddr, sizeof(Stru_DB_CH_Header), (U8*)&stDB_CH_Header) )
    {
        printf("\nError: MDrv_FLASH_Write() failed!\n");
        return FALSE;
    }
    ...
    // Verify header on flash
    {
        ...
        memset( &st_DB_CH_Header_Tmp, 0, sizeof(st_DB_CH_Header_Tmp));
...

    {
        switch( _msAPI_DB_CH_Get_State() )
        {
            ...
            case E_DB_CH_STATE__WRITE_DATA:
            {
              ...
              #if 0 // Coverity:62633 Dead default in switch => eWriteDataResult must be between 0 and 3
              ...
              #else
                if (eWriteDataResult == DB_CH_WRITE_DATA_RESULT_STAY_SAME_STATE)
                {
                    // Continue write data...
                }
                else if ( (eWriteDataResult == DB_CH_WRITE_DATA_RESULT_EXIT_WITH_SUCCESSFUL)
                        ||(eWriteDataResult == DB_CH_WRITE_DATA_RESULT_EXIT_WITH_FAIL_AND_STOP) )
                {
                    _msAPI_DB_CH_Set_State(E_DB_CH_STATE__IDLE);
                    s_stDB_CH_Info.bWriteFlashStart = FALSE;
                }
                else if (eWriteDataResult == DB_CH_WRITE_DATA_RESULT_EXIT_WITH_FAIL_AND_RETRY)
                {
                    printf(" DB_CH: Retry write flash\n");
                    _msAPI_DB_CH_Set_State(E_DB_CH_STATE__ERASE_START);
                }
