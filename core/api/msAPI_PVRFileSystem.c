...
BOOL msAPI_CIPVRFS_FileCreate(U16* pu16FileName, U8 *pineventHandleID, U8 *urilicenseHandleID)
{
    ...
    if ((_enPVRFSFileSystemType == PVR_FS_N51FAT32) || (msAPI_PVRFS_IsLinuxFileSystemType(_enPVRFSFileSystemType)))
    {
        ...
        {
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#1 \n"););
            return FALSE;
        }
        ...
        if((!ret) || (ret && (entry.EntryType != ENTRY_TYPE_DIR)))
        {
            if (!msAPI_FCtrl_DirectoryCreate(u8EnvironmentID, PVRFolderName, u8FoldNameLen))
            {
                msAPI_FSEnv_UnRegister(u8EnvironmentID);
                __ASSERT(0);
                PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#2 \n"););
                return FALSE;
            }
            //check the forlder existed or not
            if(!msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, PVRFolderName , u8FoldNameLen, &entry))
            {
                msAPI_FSEnv_UnRegister(u8EnvironmentID);
                __ASSERT(0);
                PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#3 \n"););
                return FALSE;
            }
            if(!msAPI_FCtrl_DirectoryChange(u8EnvironmentID,&entry))
            {
                msAPI_FSEnv_UnRegister(u8EnvironmentID);
                __ASSERT(0);
                PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#4 \n"););
                return FALSE;
            }
        ...
        if((!ret) || (ret && (entry.EntryType != ENTRY_TYPE_DIR)))
        {
            if (!msAPI_FCtrl_DirectoryCreate(u8EnvironmentID, SecureFolderName, u8SecureFolderLength))
            {
                msAPI_FSEnv_UnRegister(u8EnvironmentID);
                __ASSERT(0);
                PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#5 \n"););
                return FALSE;
            }
            //check the forlder existed or not
            if(!msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecureFolderName , u8SecureFolderLength, &entry))
            {
                msAPI_FSEnv_UnRegister(u8EnvironmentID);
                __ASSERT(0);
                PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#6 \n"););
                return FALSE;
            }
            if(!msAPI_FCtrl_DirectoryChange(u8EnvironmentID,&entry))
            {
                msAPI_FSEnv_UnRegister(u8EnvironmentID);
                __ASSERT(0);
                PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#7 \n"););
                return FALSE;
            }
            u8Handle = msAPI_FCtrl_DirectoryOpen(&entry, OPEN_MODE_FOR_READ_WRITE);
            if(u8Handle != INVALID_FILE_HANDLE)
        ...
        if(msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecurePineventName, FS_strlen_U(SecurePineventName), &entry))
        {
             msAPI_FSEnv_UnRegister(u8EnvironmentID);
           __ASSERT(0);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#8 \n"););
            return FALSE;
        }
        //create target file
        ...
        if (u8pinHandle == INVALID_FILE_HANDLE)
        {
             msAPI_FSEnv_UnRegister(u8EnvironmentID);
           __ASSERT(0);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#9 \n"););
            return FALSE;
        }
        ...
        if(msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecureLicenseName, FS_strlen_U(SecureLicenseName), &entry))
        {
            ...
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            __ASSERT(0);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#10 \n"););
            return FALSE;
        }
        ...
        if (u8uriHandle == INVALID_FILE_HANDLE)
        {
            ...
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            __ASSERT(0);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#11 \n"););
            return FALSE;
        }
    ...
    PVR_TRACE(printf("[msAPI_CIPVRFS_FileCreate] Error#12 \n"););
    return FALSE;
}
...
BOOL msAPI_CIPVRFS_FileOpen(U16* pu16FileName, EN_PVRFS_OPEN_MODE mode, U8 *pineventHandleID, U8 *urilicenseHandleID)
{
    ...
    {
        PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#1 \n"););
        return FALSE;
    }
    ...
    U16 _pu16TimeShiftFileName[]={'T','I','M','E','S','H','I','F','T','.','T','S',0};
    BOOL bIsTimeshift = FALSE;
    if (0 == strcmp((char*)pu16FileName,(char*)_pu16TimeShiftFileName))
    {
        bIsTimeshift = TRUE;
    }
    ...
    if ( (_enPVRFSFileSystemType == PVR_FS_N51FAT32) || msAPI_PVRFS_IsLinuxFileSystemType(_enPVRFSFileSystemType) )
    {
        ...
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#2 \n"););
            __ASSERT(0);
            return FALSE;
        }
        ...
        if (!PVRFS_EnterPVRDirectory(u8EnvironmentID))
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#3 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            return FALSE;
        }
        ...
        if (!msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, PVRFolderName , u8FoldNameLen, &entry))
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#4 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
           __ASSERT(0);
            return FALSE;
        }

        if (ENTRY_TYPE_DIR != entry.EntryType)
        {
           PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#5 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            __ASSERT(0);
            return FALSE;
        }
        ...
        if (!msAPI_FCtrl_DirectoryChange(u8EnvironmentID,&entry))
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#6 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            __ASSERT(0);
            return FALSE;
        }
        ...
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#7 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            __ASSERT(0);
           return FALSE;
        }
        ...
        if (!msAPI_FCtrl_DirectoryChange(u8EnvironmentID,&entry))
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#8 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            __ASSERT(0);
           return FALSE;
        }
        ...
        switch (mode)
        {
            ...
            default:
            {
                PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#9 \n"););
                msAPI_FSEnv_UnRegister(u8EnvironmentID);
                __ASSERT(0);
                return FALSE;
            }
        ...
        if (!msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecurePineventName, FS_strlen_U(SecurePineventName), &entry))
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#10 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            return FALSE;
        }

        u8pinHandle = msAPI_FCtrl_FileOpen(&entry, N51FSMode);
        if (INVALID_FILE_HANDLE == u8pinHandle)
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#11 \n"););
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            return FALSE;
        }
        ...
        if (!msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecureLicenseName, FS_strlen_U(SecureLicenseName), &entry))
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#12 \n"););
            msAPI_FCtrl_FileClose(u8pinHandle);
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            return FALSE;
        }
        ...
        if (u8uriHandle == INVALID_FILE_HANDLE)
        {
            msAPI_FCtrl_FileClose(u8pinHandle);
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#13 \n"););
            return FALSE;
        }
        *pineventHandleID = u8pinHandle;
        *urilicenseHandleID = u8uriHandle;
    ...
    PVR_TRACE(printf("[msAPI_CIPVRFS_FileOpen] Error#14 \n"););
    return FALSE;
}
...
{
    if ( (INVALID_FILE_HANDLE == pineventHandleID) || (INVALID_FILE_HANDLE == urilicenseHandleID) )
    {
        return FALSE;
    }

    if ( (PVR_FS_N51FAT32 == _enPVRFSFileSystemType) || (msAPI_PVRFS_IsLinuxFileSystemType(_enPVRFSFileSystemType)) )
    {
...
BOOL msAPI_CIPVRFS_FileDelete(U16* pu16FileName)
{
    ...
    if ((_enPVRFSFileSystemType == PVR_FS_N51FAT32) || (msAPI_PVRFS_IsLinuxFileSystemType(_enPVRFSFileSystemType)))
    {
        ...
        if (INVALID_FS_ENVIRONMENT_INDEX == u8EnvironmentID)
        {
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#1 \n"););
            __ASSERT(0);
            return FALSE;
        }
        ...
        {
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#2 \n"););
            return FALSE;
        }
        ...
        {
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#3 \n"););
            __ASSERT(0);
            return FALSE;
        }
        ...
        {
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#4 \n"););
            __ASSERT(0);
            return FALSE;
        }
            ...
            if(msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecureFolderName , FS_strlen_U(SecureFolderName), &SecureFolderentry))
            {
                if(msAPI_FCtrl_DirectoryChange(u8EnvironmentID,&SecureFolderentry))
                {
                    ...
                    if(msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecurePineventName, FS_strlen_U(SecurePineventName), &fileentry))
                    {
                        ...
                        {
                            PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#5 \n"););
                        }
                    }
                    ...
                    if(msAPI_FCtrl_EntrySearchByName(u8EnvironmentID, SecureLicenseName, FS_strlen_U(SecureLicenseName), &fileentry))
                    {
                        if (!msAPI_FCtrl_FileDelete(&fileentry))
                        {
                            PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#6 \n"););
                        }
                    }
                }
                if (!msAPI_FCtrl_FileDelete(&SecureFolderentry))
                {
                    PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#7 \n"););
                }
            }
        }
        ...
        {
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
            PVR_TRACE(printf("[msAPI_CIPVRFS_FileDelete] Error#8 \n"););
            __ASSERT(0);
            return FALSE;
        }
        msAPI_FSEnv_UnRegister(u8EnvironmentID);
