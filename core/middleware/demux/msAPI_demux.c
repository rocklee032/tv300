...
U8 g_Demux_u8CurVideoDmxId = DMX_DMXID_NULL;

U16 g_Demux_u16CurCCPID;
U16 g_Demux_u16CurCCFID;
...
void msAPI_DMX_Print_Filter(U8 u8FilterIdx, const _MApp_Dmx_Ts_Filter* pst_Dmx_Ts_Filter)
{
    ...
    if( u8FilterIdx < MSAPI_DMX_SEC4K_FILTER_END )
    {
        // Addr
        U32 u32BufSize = 0;
        U32 u32Adr = msAPI_DMX_Get_SecBufAddr(u8FilterIdx, pst_Dmx_Ts_Filter->u32SectionFltType, &u32BufSize);
...
void msAPI_DMX_Print_AllFilter(void)
{
    ...
    for( u8FilterIdx=0; u8FilterIdx < MSAPI_DMX_MAX_FILTER_NUMBER; u8FilterIdx += 1)
    {
        ...
        if( pst_Dmx_Ts_Filter->bUsed )
        {
            ...
            msAPI_DMX_Print_Filter(u8FilterIdx, pst_Dmx_Ts_Filter);

            printf("\n");
        }
    }

    printf("g_Demux_u8CurVideoDmxId=%u\n", g_Demux_u8CurVideoDmxId );
...
DMX_FILTER_STATUS msAPI_DMX_Close(U8 u8FltID)
{
    U8 i;
/*
    if( u8FltID == g_Demux_u16CurCCFID )
    {
...
*/
    for (i = 0; i < MSAPI_DMX_MAX_FILTER_NUMBER; i++)
    {
        if (_DmxTsFilter[i].bUsed && _DmxTsFilter[i].u8FltID == u8FltID)
        {
            ...
            if( g_Demux_u8CurVideoDmxId == u8FltID )
            {
                g_Demux_u8CurVideoDmxId = DMX_DMXID_NULL;
                //PRINT_CURRENT_LINE();
            }
...
DMX_FILTER_STATUS msAPI_DMX_StartFilter(U16 u16Pid, U32 u32FltType, U8 *pu8FID)
{
    ...
    if (*pu8FID == MSAPI_DMX_INVALID_FLT_ID)        // not open yet
    {
        ...
        if( enFtTypeNew == DMX_FILTER_TYPE_VIDEO )
        {
            //PRINT_CURRENT_LINE();
            g_Demux_u8CurVideoDmxId = *pu8FID;
        }
...
DMX_FILTER_STATUS msAPI_DMX_Open(MS_DMX_SECTIONMONITOR_PARA * pstSectionMonitor, U32 *u32StartAddress, U32 *u32Size)
{
    ...
    *(pstSectionMonitor->pu8FID) = MSAPI_DMX_INVALID_FLT_ID;
    ...
    {
        //printf("DMX: src is file, pstSectionMonitor->u32SectionFltType=0x%X\n", pstSectionMonitor->u32SectionFltType);
        eFilterType=DMX_FILTER_SOURCE_TYPE_FILE;
    }
    ...
    if(pstSectionMonitor->u32SectionFltType == MSAPI_DMX_FILTER_TYPE_SUBTITLE)
    {
        ...
        //printf("FilterType = DMX_FILTER_TYPE_PES\n");
    }
...
MSAPI_DMX_SCRAMBLING_SOURCE msAPI_DMX_ScramblingGet(U16 u16VideoPID, U16 u16AudioPID, U16 u16TimeoutMilliSec, BOOLEAN bEnPesScramble)
{
    ...
    (printf("\r\n [msAPI_DMX_ScramblingGet] u16VideoPID=%x, u16AudioPID=%x, timeout=%u, PesScramble=%x, VdFit=%x, AuFit=%x\r\n",
    u16VideoPID, u16AudioPID, u16TimeoutMilliSec, bEnPesScramble, u8VdFid, u8AuFid));
#endif
