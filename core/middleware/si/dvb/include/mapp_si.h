...
#define SI_VERSION                100 //0~999                          //LIB version
...
#define MSIF_SI_CHANGELIST        {'@','1','1','f','d','3','3','e'}    //for truck update code
...
#endif
    TAG_EIED            = 0xFC,         // emergency_information_descriptor
...
typedef struct
{
    ...
    U8 reserved      :4;
    U8 u8Modulation;
} DESC_DVBC_DEL_SYS; // Cable delivery system descript
...
} DESC_DVBS_DEL_SYS; // Satellite delivery system descript

typedef struct
{
    union
    {
        DESC_DVBC_DEL_SYS  stDCDS;
        DESC_DVBS_DEL_SYS  stDSDS;
    };
    BOOLEAN fFilled;        //flag to distinguish the descriptor is parsed and the structure is filled or not
    U32  FEC_inner         :4;
    ...
    U16  u16Onid;
    U16  u16Nid;
} DESC_DEL_SYS; // Cable and Satellite delivery system descript
...
typedef struct
{
    ...
    DESC_DEL_SYS stSatelliteDeliveryInfoList;
    ///service info
...
typedef struct
{
    ...
    U16 wBouquet_ID;
    MS_SERVICE_LIST_INFO stServiceList[SI_MAX_VC_PER_PHYSICAL];
    MS_LCN_INFO stLcnInfo[SI_MAX_VC_PER_PHYSICAL];
} MS_BAT_SERVICE_LIST_INFO;

typedef struct _BAT_SERVICE_LIST_DEL_SYS_DATA
{
    MS_BAT_SERVICE_LIST_INFO stBAT;
    BOOLEAN bUsed;
    struct _BAT_SERVICE_LIST_DEL_SYS_DATA *next;
} BAT_SERVICE_LIST_DEL_SYS_DATA;
...
typedef struct
{
    BOOLEAN bStartEndFlag;
    BOOLEAN bSignalLevel;
    WORD wService_ID;
    WORD wArea_Code;
}SI_EWBS_INFO;

typedef struct _ISDB_EWBS_DATA
{
    SI_EWBS_INFO stEWBSInfo;
    BOOLEAN bUsed;
    struct _ISDB_EWBS_DATA *next;
} ISDB_EWBS_DATA;
...
INTERFACE BOOLEAN MApp_SI_SetTimeZone_Minutes(U16 u16Minutes);
...
#if 1//ENABLE_DVBC
/*DVB-S*/
INTERFACE BOOLEAN  MApp_SI_GetNextSatParameter(DESC_DEL_SYS *desc,MS_SERVICE_LIST_INFO** pServiceListInfo, MS_SERVICE_LCN_INFO** pLCNInfo, U16* pu16ServiceNumber,U16 *RemainFreq,BOOLEAN bUsed);
INTERFACE BOOLEAN  MApp_SI_GetSatParameterById(DESC_DEL_SYS *desc, U16 u16Tsid);
#endif
/*DVB-C*/
#if 1//ENABLE_DVBC
INTERFACE BOOLEAN  MApp_SI_GetNextCableParameter(DESC_DEL_SYS *desc,MS_SERVICE_LIST_INFO** pServiceListInfo, MS_SERVICE_LCN_INFO** pLCNInfo, U16* pu16ServiceNumber, U16 *RemainFreq , BOOLEAN bUsed);
...
// EWBS function
INTERFACE void MApp_SI_AddISDBEWBSData(SI_EWBS_INFO *desc, BOOLEAN bUsed);
INTERFACE BOOLEAN MApp_SI_GetISDBEWBSData(SI_EWBS_INFO *desc,BOOLEAN bUsed);
INTERFACE void MApp_SI_FreeISDBEWBSData(void);
...
INTERFACE BOOLEAN  MApp_SI_GetNextBatService(MS_BAT_SERVICE_LIST_INFO *desc,BOOLEAN bUsed );
INTERFACE void MApp_SI_FreeBatServiceDesc(void);
