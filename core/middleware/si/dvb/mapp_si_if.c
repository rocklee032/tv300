...
static code TimeZone2Country stTimeZone2Country[]=
{
    ...
    {SI_TIMEZONE_PHILIPPINES,             SI_COUNTRY_PHILIPPINES},
};
...
U16 msAPI_SI_GetPhysicalChannelNumberByID(WORD wONID, WORD wTSID)
{
...
U8 msAPI_SI_RemoveQuickInstallMismatchedProgram(SI_SHORT_DTV_CHANNEL_INFO *pastVirtualCh,U8 u8NumOfVch,BYTE bIDIndex, MEMBER_SI_SERVICETYPE eServiceType)
{
    ...
    if(wServiceID)
        msAPI_Memory_Free(wServiceID, BUF_ID_SI_REMOVE_QUICK_PROG);

    return u8Result;
}
#endif

U16 msAPI_SI_RemoveMismatchedProgram(U16 u16RFChannelNumber, U16 wTS_ID, U8 u8PLP_ID,U8 u8HpLp,  U16 *pwService_ID, MEMBER_SI_SERVICETYPE *peService_Type, WORD cCountOfServiceID)
{
...
#if 1//(ENABLE_S2)
    BYTE SatelliteId = 0xFF;
    U32 u32S2Frequency = 0;
#endif

  #if(ENABLE_S2)
    if( IsS2InUse() )
    {
        if( FALSE == msAPI_CM_GetFreqByIDIndex(u16RFChannelNumber, &SatelliteId, &u32S2Frequency) )
        {
            return 0;
        }
    }
    ...
    if(_bM7FastScanFlag)
    {
        aeServiceType=(MEMBER_SERVICETYPE*)msAPI_Memory_Allocate(sizeof(MEMBER_SERVICETYPE)*MAX_DTV_S2_PROGRAM, BUF_ID_SI_REMOVE_PROG);
    }
    else
    ...
    result = msAPI_CM_HaveRemoveProgram(u16RFChannelNumber,wTS_ID,u8PLP_ID, u8HpLp, pwService_ID, aeServiceType, cCountOfServiceID,SatelliteId,u32S2Frequency);
  #else
    result = msAPI_CM_RemoveMismatchedProgram(u16RFChannelNumber, wTS_ID,u8PLP_ID, u8HpLp, pwService_ID, aeServiceType, cCountOfServiceID,SatelliteId,u32S2Frequency);
  #endif

    msAPI_CM_Set_TS_Update(FALSE,wTS_ID);

    msAPI_Memory_Free(aeServiceType, BUF_ID_SI_REMOVE_PROG);
...
BOOLEAN msAPI_SI_AddProgram(SI_DTVPROGRAMID *stDtvIDTable, SI_DTV_CHANNEL_INFO *pSIDTVProgramData, BOOLEAN *bDBFull, BOOLEAN bSkipCheck)
{
    ...
    SI_PROG_DEBUG(printf("\n ---------------------------------\n"););
    SI_PROG_DEBUG(printf(" >>>> msAPI_SI_AddProgram()\n"););
    SI_PROG_DEBUG(printf(" cRFChannelNumber=%u,", stDtvIDTable->cRFChannelNumber););
...
#endif
    {
        SI_PROG_DEBUG(printf("%d,%d,%d,%d, u8PLP_ID=0x%X,", stDtvIDTable->u32Frequency, stDtvIDTable->u32SymbRate, stDtvIDTable->QamMode,stDtvIDTable->enBandWidth,stDtvIDTable->u8PLP_ID););
    }

    SI_PROG_DEBUG(printf(" ONID:%d, NID:%d, ", stDtvIDTable->wOriginalNetwork_ID, stDtvIDTable->wNetwork_ID););
    SI_PROG_DEBUG(printf(" wLCN :%d, wTS_LCN:%d, sLCN:%d\n", pSIDTVProgramData->wLCN, pSIDTVProgramData->wTS_LCN, pSIDTVProgramData->wSimu_LCN););
    ...
    SI_PROG_DEBUG(printf("wLCN:%d, VideoPID:%d, ", pSIDTVProgramData->wLCN, DTVProgramData.wVideo_PID););
    ...
    SI_PROG_DEBUG(printf("TSID:%d, SID:%d, Scram:%d, ", stDtvIDTable->wTransportStream_ID, pSIDTVProgramData->wService_ID, pSIDTVProgramData->stCHAttribute.bIsScramble););
...
*/
    ...
    SI_PROG_DEBUG(printf("Service Name: "););
    SI_PROG_DEBUG(msAPI_DtvSys_PrintServiceName(pSIDTVProgramData->bChannelName, SI_MAX_SERVICE_NAME););
    SI_PROG_DEBUG(printf("\n"););
...
BOOLEAN msAPI_SI_IsSpecificCountry(EN_SI_COUNTRY_SETTING eCountry, E_SI_SPECIFIC_COUNTRY eSpecificCountry)
{
    ...
    {
        case EPG_EVENT_OVERLAP_SPECIFIC_COUNTRY:
        ...
        case SI_NORDIC_SPECIFIC_COUNTRY:
            if( (eCountry == SI_COUNTRY_FINLAND) ||
                ...
                (eCountry == SI_COUNTRY_SPAIN) ||
                (eCountry == SI_COUNTRY_NORWAY)
             )
            {
              #if ENABLE_ZIGGO
                if(msAPI_SI_IsZiggo_Spec_Country(eCountry))
                {
                    return FALSE;
                }
                else
        ...
        case SI_SUPPORT_DVB_C_NETWORK_UPDATE_SPECIFIC_COUNTRY:
            ...
            if (IsDVBCInUse())
            {
              ...
              #if ENABLE_STOFA
                if(eCountry == SI_COUNTRY_DENMARK)
                {
                    return TRUE;
                }
              #endif
       ...
       case SI_EPGINFO_WITHOUT_CHECK_TIME:
        ...
        case SI_FORCE_PRD_COUNTRYCODE:
            ...
            return FALSE;
        case SI_SUMSUNG_SPECIFIC_COUNTRY:
            if(eCountry == SI_COUNTRY_THAILAND)
            {
                return TRUE;
            }
            return FALSE;
#if ENABLE_CI_PLUS && MEDIASET_PREMIUM_SMART_CAM // mantis 1501405: [Marlon U01][DVBT][Mediaset Premium SMART CAM (Italy)]
        case SI_SDT_WAIT_15000MS_WHEN_CHANNEL_CHANGE:
...
{
//printf("msAPI_SI_Action_CheckServiceType_DVB u8OriginalType %x\n",u8OriginalType);
    *pNewType=u8OriginalType;
#if ENABLE_STOFA
    if(OSD_COUNTRY_SETTING == OSD_COUNTRY_DENMARK)
    {
        if(u8OriginalType == E_TYPE_ACHD_DTV ||u8OriginalType == E_TYPE_AC_RADIO)   //Invalid Service Type
        {
            return FALSE;
        }
    }
...
BOOLEAN msAPI_SI_SaveLCDV2(const LOGICAL_CHANNEL_V2 *pstLCNv2, U16 u16MiniChannelId, BOOLEAN bSortFlag, BOOLEAN bReset)
{
    ...
    if(bSortFlag == 1)
    {
        ...
        LOGICAL_CHANNEL_V2 * pstLOGICAL_CHANNEL_V2_Temp = NULL;

        printf("\r\n >>>> sort LCD V2 data base");

        pstLOGICAL_CHANNEL_V2_Temp = (LOGICAL_CHANNEL_V2 *)msAPI_Memory_Allocate( sizeof(LOGICAL_CHANNEL_V2), BUF_ID_SI_DEFAULT);
        if( pstLOGICAL_CHANNEL_V2_Temp == NULL )
        {
            __ASSERT(0);
        ...
        if(u16MiniChannelId != 0xFFFF)
        {
            ...
            if( u8tempIdx < LCDV2ArrayMaxSize )   //Fixed Coverity 59184
            {
                ...
                memcpy(pstLOGICAL_CHANNEL_V2_Temp, &_aLCDV2[u8tempIdx], sizeof(LOGICAL_CHANNEL_V2));
                memcpy(&_aLCDV2[u8tempIdx], &_aLCDV2[0], sizeof(LOGICAL_CHANNEL_V2));
                memcpy(&_aLCDV2[0], pstLOGICAL_CHANNEL_V2_Temp, sizeof(LOGICAL_CHANNEL_V2));
            }
        }
        else
        {
            ...
            if(u8MiniIndex != 0)
            {
                ...
                memcpy(pstLOGICAL_CHANNEL_V2_Temp, &_aLCDV2[u8MiniIndex], sizeof(LOGICAL_CHANNEL_V2));
                memcpy(&_aLCDV2[u8MiniIndex], &_aLCDV2[0], sizeof(LOGICAL_CHANNEL_V2));
                memcpy(&_aLCDV2[0], pstLOGICAL_CHANNEL_V2_Temp, sizeof(LOGICAL_CHANNEL_V2));
            }
        }

        MSAPI_MEMORY_FREE(pstLOGICAL_CHANNEL_V2_Temp, BUF_ID_SI_DEFAULT);
...
BOOLEAN msAPI_SI_UpdateProgramInfo(MEMBER_SI_SERVICETYPE bServiceType, WORD wPosition, SI_SHORT_DTV_CHANNEL_INFO* pastVirtualCh, BYTE *bChanelNameBuffer, E_SI_DATA_MEMBER eMember)
{
    ...
    switch(eMember)
    {
        ...
        case E_SI_DATA_PMT_SERVICE_INFO:
        {
            ...
            msAPI_SI_Copy_SI_AudInfo_To_CM_AudInfo(pastVirtualCh->stAudInfo, ChannelInfo.stAudInfo, maxAudio);
...
EN_SI_BOUQUET_ID MApp_SI_GET_BouquetID(void)
{
#if ENABLE_BAT_SCAN
    return GET_OSD_MENU_REGION();
#else
    return SI_REGION_UNKNOWN;
#endif
}

BOOLEAN MApp_SI_Need2Parse_BAT(void)
{
#if ENABLE_BAT_SCAN
    if(IsS2InUse())
    {
...
BOOLEAN msAPI_SI_IsZiggo_Spec_Country(EN_SI_COUNTRY_SETTING eCountry)
{
#if ENABLE_ZIGGO
    if(Mapp_SI_Get_Ziggo_ENABLE() &&
        ((SI_COUNTRY_AUSTRIA== eCountry) 
      || (SI_COUNTRY_HUNGARY== eCountry) 
      || (SI_COUNTRY_POLAND== eCountry) 
      || (SI_COUNTRY_ROMANIA== eCountry) 
      || (SI_COUNTRY_NETHERLANDS == eCountry) 
      || (SI_COUNTRY_SWITZERLAND== eCountry)
      || (SI_COUNTRY_IRELAND == eCountry))
      )
    {
    #if ENABLE_DVBC
        if(IsDVBCInUse())
...
#endif //#if(ENABLE_DVB || ENABLE_OAD )
