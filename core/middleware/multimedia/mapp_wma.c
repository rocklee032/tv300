...
#define WMA_HEADER_DESNAME_FIELD_LEN 2
#define WMA_HEADER_DATA_TYPE_FIELD_LEN 2
#define WMA_HEADER_DATA_LENGTH_FIELD_LEN 2
...
typedef struct
{
    ...
    U32 u32PayloadObjOffset;
    U32 u32PacketHeaderSize;
    U32 u32PayloadHeaderSize;
    U32 u32PaddingSize;
} ST_WMA_INFO;
...
static U32 MApp_WMA_ParsingASFPacketHeader(U32 dwReqAddress,ST_WMA_INFO *pWmaInfo)
{
    ...
    {
        i += (1 << (pWmaInfo->u8PacketLType-1));
        if ((1 << (pWmaInfo->u8PacketLType - 1)))
        {
            memcpy(&pWmaInfo->u32PacketSize, &pu8Data[i], (1 << (pWmaInfo->u8PacketLType - 1)));
        }
    }
    if(pWmaInfo->u8SequenceLType)
    {
    ...
    pWmaInfo->u32PaddingSize = 0; // init pstWMA->u32PaddingSize!
    if(pWmaInfo->u8PaddingLType)
    {
        i += (1 << (pWmaInfo->u8PaddingLType-1));
        if ((1 << (pWmaInfo->u8PaddingLType - 1)))
        {
            memcpy(&pWmaInfo->u32PaddingSize, &pu8Data[i], (1 << (pWmaInfo->u8PaddingLType - 1)));
        }
    }
    ...
    pWmaInfo->u32PacketHeaderSize = i;
...
static U32 MApp_WMA_ParsingASFPayloadPTime(U32 dwReqAddress,ST_WMA_INFO *pWmaInfo, U32 u32IndexNum, U32 u32PayloadPosInFile)
{
    ...
    else  //non-compressed payload data
    {
        ...
        else
        {
            ...
            {
                pWmaInfo->u32PayloadHeaderSize = i;
                pWmaInfo->u32PayloadLength = pWmaInfo->u32PacketSize -  pWmaInfo->u32PacketHeaderSize - pWmaInfo->u32PayloadHeaderSize - pWmaInfo->u32PaddingSize;
            }
        }
    }
...
{
    U8 *pu8Data = (U8 *)dwReqAddress;
    U32 i=0; //notice that the i should added after reference.
    stWmaInfo.u16CurrentStreamId = pu8Data[i] & 0x7F;
    i += 1;    //Jump Stream Number
    ...
    pWmaInfo->u32PayloadObjOffset = 0; // init pstWMA->u32PayloadObjOffset!
    if(pWmaInfo->u8OffseIMOLType)
    {
        if ((1 << (pWmaInfo->u8OffseIMOLType - 1)))
        {
             memcpy(&pWmaInfo->u32PayloadObjOffset, &pu8Data[i], (1 << (pWmaInfo->u8OffseIMOLType - 1)));
        }
        i += (1 << (pWmaInfo->u8OffseIMOLType-1));
    }
    if(pWmaInfo->u8ReplicatedDLType)
    ...
    if (pWmaInfo->u32ReplicatedDLength > pWmaInfo->u32PacketSize)
    {
        printf("pstWMA->u32ReplicatedDLength: %ld is abnormal.\n", pWmaInfo->u32ReplicatedDLength);
        return 0xFFFFFFFF;
    }
    ...
    else  //non-compressed payload data
    {
        ...
        if (pWmaInfo->u32ReplicatedDLength < 0x1000)
        {
            memcpy(&pWmaInfo->u32PayloadLength,&pu8Data[i],4);
            i += pWmaInfo->u32ReplicatedDLength;
            ...
            else
            {
                if (pWmaInfo->u32PayloadLength > pWmaInfo->u32PacketSize)
                {
                    pWmaInfo->u32PayloadHeaderSize = i;
                    pWmaInfo->u32PayloadLength = pWmaInfo->u32PacketSize -  pWmaInfo->u32PacketHeaderSize - pWmaInfo->u32PayloadHeaderSize - pWmaInfo->u32PaddingSize;
                    //printf("Case 4: Calculated PayloadLength = %ld.\n", pstWMA->u32PayloadLength);
                }
            }
        }
        else
        {
            pWmaInfo->u32PayloadLength = 0xFFFFFFFF;
        }
    }
...
static U32 MApp_WMA_AddASFPacketForDSP(U32 dwReqAddress,U32 u32ReqBytes)
{
    ...
    while(u32ReqBytes>0)
    {
        ...
        else
        {
            ...
            if(stWmaInfo.u16FormatTag != CODEC_FORMAT_MP3 && stWmaInfo.u32PayloadObjOffset == 0)
            {
                U32 u32BlockAlign = stWmaInfo.u16BlockAlign;
                //----Payload header for DSP----//
                _pu8ASFPayloadHeader[0]=0x02;
                _pu8ASFPayloadHeader[1]=0x80;
                _pu8ASFPayloadHeader[2]=0xFD;
                _pu8ASFPayloadHeader[3]=0x7F;
                memcpy(&_pu8ASFPayloadHeader[4],&u32BlockAlign,4);
                memset(&_pu8ASFPayloadHeader[8],0,8);
                _pu8ASFPayloadHeader[16]=0;   //Left size
                //---------------------------//
        ...
        for(u32Index=0;u32Index<(pWmaInfo->u32HeaderSize+0x100);u32Index++,i++)     //u32HeaderSize + 0x100 to parsing Data Object
        {
                ...
                (pu8Data[i+12]==0x0C)&&(pu8Data[i+13]==0x20)&&(pu8Data[i+14]==0x53)&&(pu8Data[i+15]==0x65))
            {
                ...
                (pu8Data[i+36] == 0x5F) && (pu8Data[i+37] == 0x5C) && (pu8Data[i+38] == 0x44) && (pu8Data[i+39] == 0x2B))
                {
                    ...
                    u32ExtraDataOffset = 0x60;

                    if (pWmaInfo->u16FormatTag == 0xFFFE) // 0xFFFE -> WAVE_FORMAT_EXTENSIBLE
                    {
                        memcpy(&pWmaInfo->u16FormatTag, &pu8Data[i + u32ExtraDataOffset + 6], 2);
                        u32ExtraDataOffset += 22; // 22 -> size of WAVEFORMATEXTENSIBLE
                    }

                    if(pWmaInfo->u16FormatTag == 0x160)
                    {
                        pWmaInfo->u8Version = 1;
                        memcpy(&pWmaInfo->u16Encopt,&pu8Data[i + u32ExtraDataOffset+2],2);
                    }
                    else if(pWmaInfo->u16FormatTag == 0x161)
                    {
                        pWmaInfo->u8Version = 2;
                        memcpy(&pWmaInfo->u16Encopt,&pu8Data[i + u32ExtraDataOffset+4],2);
                    }
                    else if(pWmaInfo->u16FormatTag == CODEC_FORMAT_WMA_PRO || pWmaInfo->u16FormatTag == CODEC_FORMAT_MP3)
                    {
                        pWmaInfo->u8Version = 3;
                        memcpy(&pWmaInfo->u32ChannelMask,&pu8Data[i + u32ExtraDataOffset+2],4);
                        memcpy(&pWmaInfo->u16Encopt,&pu8Data[i + u32ExtraDataOffset+14],2);
                    }
                    else
                    {
            ...
            if(!pWmaInfo->bExContentFound) // it is optional
            {
                ...
                )
                {
                    ...
                    for(u16Index = 0; u16Index < u16ContentCount; ++u16Index)
                    {
                        ...
                        //Descriptor Structure
                        //-------------------------------------------------------------------------------------------------------//
                        // desname length  2 byte  | name: varies  n byte |data type  2 byte | data or value length  2 byte |  value: varies  n byte //
                        //-------------------------------------------------------------------------------------------------------//
                        if(bResetIndex == TRUE)
                        {
                            u32DataOffset += WMA_HEADER_DATA_TYPE_FIELD_LEN + WMA_HEADER_DATA_LENGTH_FIELD_LEN + u16DataLen;
                        }
                        else
                        {
                            u32DataOffset += WMA_HEADER_DESNAME_FIELD_LEN + u16DesNameLen + WMA_HEADER_DATA_TYPE_FIELD_LEN + WMA_HEADER_DATA_LENGTH_FIELD_LEN + u16DataLen;
                        }
                    }
                    ...
                    for(u32Skip=0x10;u32Skip<u32HeaderSize;u32Skip++)
                    {
                        ...
                        if(!pWmaInfo->bHE_MetadataObjectFound)
                        {
                            ...
                            )
                            {
                                ...
                                memcpy(&u16ObjectSize,&pu8Data[i+u32Skip+16], sizeof(U16));
                                memcpy(&u16Attribute,&pu8Data[i+u32Skip+24], sizeof(U16));
