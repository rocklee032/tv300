...
RestartInitial:
    ...
    else if(enCurrentMusicType == EN_MUSIC_AAC)
    {
        ...
        if (M4AParser_Get_M4A_Info(&stM4aInfo) == TRUE)
        {
            ...
            if(stM4aInfo.enCodec==EN_MUSIC_AAC && stM4aInfo.u32StsdSampleRate > 48000)
            {
                AAC_ERR(printf("Error : Unsupport Sample rate : %d > 48000\n", stM4aInfo.u32StsdSampleRate));
                return FALSE;
            }
#endif
    ...
    if (m_OggInfo.bSeekToCurrentTime==TRUE)
    {
        ...
        else if(m_stMp3Info.u32SampleRate>0)
        {
            while(1)   //use binary search to find closest page
            {
                ...
                if(_MApp_Music_OggGetNextPage(m_u32Mp3FileHandle,&g_OggTemp.CurrentPage) < 0)
                {
                    OGG_DBG2(printf("_MApp_Music_OggGetNextPage() return < 0\n"));
                }
...
static BOOLEAN _MApp_Music_FLACTimeOffset(U32 u32GotoTimeMs)
{
    ...
    FLAC_DBG(printf("\nu32GotoTimeMs: %lu\n",u32GotoTimeMs));
    U64 u64SampleRate = m_stFlacInfo.u32SampleRate;
    U64 u64GotoTimeMs = u32GotoTimeMs/1000;
    u64SeekToSample = u64GotoTimeMs*u64SampleRate;
    u32FileLength = m_stMp3Info.u32FileSize;
