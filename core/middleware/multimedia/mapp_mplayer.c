...
BOOLEAN MApp_MPlayer_RecordPath(BOOLEAN bPop)
{
    ...
    else
    {
        ...
        if(MApp_MPlayer_QueryFileInfo(E_MPLAYER_INDEX_CURRENT_PAGE,
                            MApp_MPlayer_QueryCurrentFileIndex(E_MPLAYER_INDEX_CURRENT_PAGE),
                            &stFileInfo) != E_MPLAYER_RET_OK)
        {
            __ASSERT(0);
            return FALSE;
        }
...
#else
        if(MApp_MPlayer_ConnectDrive(stPlayFileInfo.u16Drive) == E_MPLAYER_RET_OK)
        {
            bChangePath = TRUE;
        }
        else
                                              ...
                                              &fileInfo) == E_MPLAYER_RET_OK)
                {
                    ...
                    if (IS_PHOTO_FILE(fileInfo.u8ExtFileName))
                    {
                        ...
                        if (MApp_Photo_DecodeFile_Init(&m_MPlayerInfo[E_MPLAYER_FOREGROUND_PLAYER].curPlayingFileEntry, FALSE) == TRUE)
                        {
                            ...
                            {
                                m_photoInfo.eOrientation = eOrientation;
                            }
                            else
                            {
                                m_photoInfo.eOrientation = E_PHOTO_ORIENT_ROTATE_0;
                            }
...
BOOLEAN MApp_MPlayer_ProbeMovieHeaderOnly(enumMPlayerFileIndexBegin eBegin, U16 u16FileIdx)
{
    ...
    if(MApp_MPlayer_QueryFileInfo(E_MPLAYER_INDEX_CURRENT_DIRECTORY, u16FileIdx, &fileInfo) == E_MPLAYER_RET_OK)
    {
    ...
    {
        bRet = FALSE
        MPLAYER_DBG(printf("MApp_MPlayer_QueryFileInfo = %d \n",eRet));
    }
