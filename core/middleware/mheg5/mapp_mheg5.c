...
void MApp_MHEG5_Dsmcc_StartTime_Update()
{
    u32DSMCC_Start_Timer = msAPI_Timer_GetTime0();
}

void MApp_MHEG5_Dsmcc_StartTime_Reset()
{
    u32DSMCC_Start_Timer = 0;
}

U32 MApp_MHEG5_Dsmcc_StartTime_Get()
{
...
{
    U32 u32Dsmcc_StartTime = MApp_MHEG5_Dsmcc_StartTime_Get();

    if ( (MApp_MHEG5_CheckMHEG5Status() == SI_MHEG5_DATA_ONLY)
       &&(msAPI_Timer_DiffTimeFromNow_2(u32Dsmcc_StartTime) < (1*1000))//display at least 1 seconds
       )
    {
        return;
    }
    ...
    switch (u8MailBox[1])
    {
        ...
        case MB_VIDEO_SET_STREAM_WITH_PID:
            ...
            wOriNetwork_ID = ( (((U16)u8MailBox[8]) & 0x00FF) | ((((U16)u8MailBox[9]) << 8) & 0xFF00) );
            {
                ...
                else if(u8MailBox[10] == M5_VIDEO_TYPE_H265)
                {
            ...
            if ((g_u16MHEG5_AudioPID != INVALID_PID))
            {
                ...
                //Audio mute type has been changed,and it is muted in MApp_ChannelChange_DisableAV
