...
#if 1   //add for adjust CC position take effect
        stIntCustomerVariables stCustomer;
        memset(&stCustomer,0x00,sizeof(stIntCustomerVariables));
        stCustomer.u8CCEnableSVS = 1;
        CC_BrazilDecoderFuns.InitBrazilVar(&stCustomer);
#endif
...
void MApp_ClosedCaption_Open_Filter(U16 u16PID)
{
    ...
    DEBUG_CC_DEMUX( printf("MApp_ClosedCaption_Open_Filter(u16PID=0x%X)\n", u16PID); );

    g_Demux_u16CurCCPID = u16PID;
    ...
    DEBUG_CC_DEMUX( printf("_bCCFileIn=%u\n", _bCCFileIn); );

    _u16PesDataLength = 0;

    //printf("*MApp_Dmx_GetFid(EN_PES_FID)=%u\n", (*MApp_Dmx_GetFid(EN_PES_FID)) );

    MApp_Dmx_PES_Monitor(u16PID, MApp_ClosedCaption_PES_Cb, _bCCFileIn);

    DEBUG_CC_DEMUX( printf("*MApp_Dmx_GetFid(EN_PES_FID)=%u\n", (*MApp_Dmx_GetFid(EN_PES_FID)) ); );

    g_Demux_u16CurCCFID = (*MApp_Dmx_GetFid(EN_PES_FID));

    DEBUG_CC_DEMUX( printf("g_Demux_u16CurCCPID=%u\n", g_Demux_u16CurCCPID ); );
    DEBUG_CC_DEMUX( printf("g_Demux_u16CurCCFID=%u\n", g_Demux_u16CurCCFID ); );
    DEBUG_CC_DEMUX( msAPI_DMX_Print_AllFilter(); );
}

void MApp_ClosedCaption_Close_Filter(void)
{
    DEBUG_CC_DEMUX( printf("MApp_ClosedCaption_Close_Filter()\n"); );
    ...
    g_Demux_u16CurCCPID = MSAPI_DMX_INVALID_PID;
    g_Demux_u16CurCCFID = MSAPI_DMX_INVALID_FLT_ID;
}
...
void MApp_ClosedCaption_Monitor(void)
{
    U32 u32Err;
    ...
    {
        // Should open CC filter ...
        if( *MApp_Dmx_GetFid(EN_PES_FID) == MSAPI_DMX_INVALID_FLT_ID ) // Not open yet
        {
            printf("1 Open CC filter\n");
            MApp_ClosedCaption_Open_Filter(u16CCPid);
        }
        else // Already open
        {
            // Check filein status
            if( MApp_IsCCFileIn() != MApp_PVR_IsPVRStatePlaybacking() )
            {
                printf("2 ReOpen CC filter\n");
                MApp_ClosedCaption_Close_Filter();
                MApp_ClosedCaption_Open_Filter(u16CCPid);
            }
        }
    }
    ...
    // Check Final FID status
    U8 u8PesFID = *MApp_Dmx_GetFid(EN_PES_FID);
    if( u8PesFID != MSAPI_DMX_INVALID_FLT_ID )
    {
        if( MApp_CC_GetInfo(CC_SELECTOR_STATUS_CODE) != STATE_CAPTION_PARSER )
        {
        ...
        {
            //printf("Call msAPI_DMX_Proc(%u)\n", u8PesFID);
            msAPI_DMX_Proc(u8PesFID);
        }
    }
