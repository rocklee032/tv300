...
static U16 _TTX_ReadVpsCni(void)
{
    ...
    for (ucDataIdx = 0; ucDataIdx < 7; ucDataIdx++)
    {
        msAPI_Timer_Delayms(45);        // 1-frame period pluse 5ms margin. VPS is received once each frame.
...
void msAPI_TTX_CloseRenderObj(void)
{
...
BOOLEAN g_TTX_bEnableISR = FALSE;
BOOLEAN g_TTX_bIsInISR = FALSE;

void msAPI_TTX_ISR_1ms(void)
{
#ifdef MSOS_TYPE_LINUX
#else
    ...
    static U8 _s_u8TTXTimer = 0;

    g_TTX_bIsInISR = TRUE;

    ++ _s_u8TTXTimer;

    if( _s_u8TTXTimer >= 6 ) // Every 6ms do job
    {
        _s_u8TTXTimer = 0;
