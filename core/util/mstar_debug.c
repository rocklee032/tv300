...
U8 msDebug_Get_FileSysEnvironment(U8 u8UsbPortNO, U8 u8DriveID)
{
    ...
    if( (u8UsbPortNO == 1) || (u8UsbPortNO == 2) )
    {
        ...
        for( u8DeviceIdx = u8DeviceIdx_Start; u8DeviceIdx < u8DeviceIdx_End; u8DeviceIdx++)
        {
            ...
            if( eMsdStatus == MSD_STATUS_INSERTED )
            {
                if ( msAPI_FCtrl_ActiveDevice(u8DeviceIdx) == FALSE )
                    return INVALID_FS_ENVIRONMENT_INDEX;
...
#define MIPS_BT_PATCH       1

#define DEBUG_MIPS_BT(x)    //x
void msDebug_mips_BackTrace_2(void* u32PC, void* u32Stack, void* u32RA)
{
    unsigned long *addr;
    unsigned long *ra;
    unsigned long *sp;
    ...
    u32RA_1st = (U32)u32RA;//(((MHAL_SavedRegisters *) pHalReg)->d[31]);
    pu32SP_1st = (unsigned long *)u32Stack;//(((MHAL_SavedRegisters *) pHalReg)->d[29]);
    ...
    printf(" %s -e AP.elf -f 0x%x ", MIPS_BT_CMD_ADDR2LINE, u32RA_1st);

    ra = (unsigned long *)u32RA_1st;
    sp = pu32SP_1st;

    unsigned long *calling;
#if( MIPS_BT_PATCH )
    U32 u32OpCode;
#endif
    //unsigned long * apulCalling[MIPS_BT_DEPTH_MAX+1] = {0};
    ...
    for (depth = 0; (depth < MIPS_BT_DEPTH_MAX) && (*sp != 0xDEADBEEF); ++depth)
    {
        ...
        if( (U32)ra != u32RA_1st )
        {
            ...
            DEBUG_MIPS_BT( printf("ra=%X\n", ra); );

            for( i = 0; i < 10; i += 1)
            {
                calling = calling - 1; // addr -4
                u32OpCode = *(calling);
                DEBUG_MIPS_BT( printf("%u calling=%X,op=%X\n", i, calling, u32OpCode); );

                if( (u32OpCode&0xFE000000)==0x0C000000 )
                {
                    bFound = 1;
                    break;
                }
            ...
            //DEBUG_MIPS_BT( printf(" => calling=%X\n", calling ); );
        }
    #endif
...
__attribute__((noinline)) void* MIPS_Get_PC(void)
{
    void *pc;
...
/*
void msDebug_mips_BT_NonISR(void)
{
    msDebug_mips_BackTrace_2(MIPS_Get_PC(), MIPS_Get_SP(), MIPS_Get_RA());
}
*/

// This function is for ISR
void msDebug_mips_BackTrace(MS_U32 pHalReg)
{
#if 1
    msDebug_mips_BackTrace_2( (void*)((MHAL_SavedRegisters *) pHalReg)->pc,
                              (void*)((MHAL_SavedRegisters *) pHalReg)->d[29],
                              (void*)((MHAL_SavedRegisters *) pHalReg)->d[31]);
#else
    unsigned long *addr;
    unsigned long *ra;
    unsigned long *sp;
...
// msAPI_Timer_Delayms() will call this function frist
// Then get this function return value, and delay

#define ENABLE_DEBUG_TOTAL_DELAY_TIME   0

#if ( ENABLE_DEBUG_TOTAL_DELAY_TIME )
static U32 s_MsDebug_u32TotalDelay = 0;
#endif

#define ENABLE_DEBUG_AUDIO_TOTAL_DELAY_TIME   0
#if ENABLE_DEBUG_AUDIO_TOTAL_DELAY_TIME
U16 g_MsDebug_u32AudioTotalDelay = 0;
#endif

#if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_MARLON)   \
   ||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MAZDA)    \
   ||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MARLON2)  \
   )
    #define DEBUG_DELAY_TIME_LOW_BOUND  5
#else
    #define DEBUG_DELAY_TIME_LOW_BOUND  10
#endif
...
MS_U32 MsOS_DelayTask_Debug(MS_U32 u32Ms)
{
   ...
   #if ENABLE_DEBUG_AUDIO_TOTAL_DELAY_TIME
    if( (g_Timer_eCurDelayId >= E_DELAY_ID_AUDIO_DRV)
      &&(g_Timer_eCurDelayId <= E_DELAY_ID_AUDIO_APP)
      )
    {
        g_MsDebug_u32AudioTotalDelay += u32Ms;
    }
   #endif
    ...
    if( u32Ms < DEBUG_DELAY_TIME_LOW_BOUND )
    {
        bPrintDelayTime = FALSE;
    }
    ...
    if( bPrintDelayTime )
    {
    ...
    #else
        printf("[%u Delay %u]", g_Timer_eCurDelayId, u32Ms );
    #endif
        ...
        {
            printf("[Au %u]", g_MsDebug_u32AudioTotalDelay );
        }
      #endif
