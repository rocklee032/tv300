...
U32 u32PiuSystemTime=0;
U16 u16TickerRemainder=0;
U32 u32PreviousPiuTick=0;


void MDrv_UpdatePiuTimerSystemTime(void)
{
    
    U32 u32PiuTick = MHAL_PIU_TIMER_Get_CurTick(PIU_TIMER_1);

    if(u32PiuTick>u32PreviousPiuTick)
    {   
        u32PiuSystemTime += ((u32PiuTick-u32PreviousPiuTick) +u16TickerRemainder)/ 12000ul;
        u16TickerRemainder = ((u32PiuTick-u32PreviousPiuTick) +u16TickerRemainder)%12000ul;
    }
    else
    {
        u32PiuSystemTime +=((0xFFFFFFFF-u32PreviousPiuTick)+1+u32PiuTick+u16TickerRemainder)/ 12000ul;
        u16TickerRemainder = ((0xFFFFFFFF-u32PreviousPiuTick)+1+u32PiuTick+u16TickerRemainder)%12000ul;

    }
    u32PreviousPiuTick = u32PiuTick;

}


U32 MDrv_GetPiuTimerSystemTime(void)
{

    MDrv_UpdatePiuTimerSystemTime();
    return u32PiuSystemTime;
}
