...
static void TimerISR(void)
{
...
#if(ENABLE_IR)// IR ISR start ------------------------------------------------------------------------------
    msIR_ISR_1ms();
#endif// IR ISR End ------------------------------------------------------------------------------


#if 0 // Move to msKeypad_ISR_1ms() ((KEYPAD_TYPE_SEL != KEYPAD_TYPE_NONE) && (KEYPAD_USE_NEW_CHECK))
    if(g_ucKeyPadRepeatTimer < 0xFFFF)
        g_ucKeyPadRepeatTimer++;
#endif

    //-------------------------------
#if 0 //Move to MDrv_TVTool_1ms_ISR()  (ENABLE_UART2)
    if (g_Uart2CheckTick)
    {
        --g_Uart2CheckTick;
...
#if 0 //Move to MDrv_TVTool_1ms_ISR()(!BLOADER)
...
#if 0// Move to MDrv_TVTool_1ms_ISR()  (ENABLE_UART1_INTERRUPT == ENABLE)
    ...
    #if (!BLOADER)
    #if 0 // Move to  msAPI_Timer_1ms_ISR()  ENABLE_TTX
    {
  ...
  #if 0 // Move to MApi_XC_HDMI_ISR_1ms()  (!BLOADER)
