...
#ifndef ENABLE_FORCE_VSP_BILINEAR_WHEN_RFBL
    #define ENABLE_FORCE_VSP_BILINEAR_WHEN_RFBL 0
#endif

#ifndef PQ_MADI_MAPPING_NEW_METHOD
    #define PQ_MADI_MAPPING_NEW_METHOD  0
#endif
...
#define DEBUG_FRAME_COUNT(x)    //x

#define DEBUG_GAMEMODE(x)       x
...
void MDrv_PQ_Init(MS_PQ_Init_Info *pstPQInitInfo)
{
...
#if(ENABLE_PQ_BIN)
    // Clear header ...
    memset(stPQBinHeaderInfo, 0, sizeof(stPQBinHeaderInfo));
    ...
    if(pstPQInitInfo->u8PQBinCnt)
    {
        ...
        //Sub first
        for(i = 2; i < pstPQInitInfo->u8PQBinCnt; i++) // For Sub_Std, Sub_Ex
        {
        ...
        #if(ENABLE_PQ_BIN_EX)
            if( u8ID == PQ_BIN_EXT_SUB)
                MDrv_PQBin_LoadCommTable(MDrv_PQBin_Ex_GetPanelIdx(), &stPQBinHeaderInfo[u8ID]);
            else
        #endif
        ...
        //Main second
        for(i = 0; i < ((pstPQInitInfo->u8PQBinCnt <= 2) ? pstPQInitInfo->u8PQBinCnt : 2); i++) // For Main_Std, Main_Ex
        {
            u8ID = pstPQInitInfo->stPQBinInfo[i].u8PQID;

            if( u8ID == PQ_BIN_STD_MAIN )
            {
                gbPQBinEnable = 1;
                //PRINT_CURRENT_LINE();
            }
        #if(ENABLE_PQ_BIN_EX)
            else if( u8ID == PQ_BIN_EXT_MAIN )
            {
                gbPQBin_Ex_Enable = 1;
                //PRINT_CURRENT_LINE();
            }
...
#endif // ENABLE_PQ_LOAD_TABLE_INFO
...
#if( ENABLE_SCALER_GAMEMODE)

#define PQ_GAMEMODE_IDX_ON  0

// 480/576p:   MADi/UC_CTL/PostCC/PostCC_NP/DHD
// 480i/576i:  MADi_Motion/ DFK/SST/EODi
// SwDriver/BWS : No Need change

#define PQ_GAME_MODE_SUPPORT_SD_P_MODE  (GAME_MODE_SUPPORT_SD_P_MODE) // 480p/576p
#define PQ_GAME_MODE_SUPPORT_SD_I_MODE  0 // 480i/576i

#define PQ_GAME_MODE_SET_SW_DRIVER      0

typedef struct
{
#if(PQ_GAME_MODE_SUPPORT_SD_P_MODE||PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    U8 u8TabIdx_MADi; // For 480P/480i
    U8 u8TabIdx_UC_CTL; // For 480p/480i
    U8 u8TabIdx_PostCCS; // For 480p/480i
    U8 u8TabIdx_PostCCS_NP; // For 480p/480i
    U8 u8TabIdx_DHD; // For 480p/480i
#endif

#if(PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    U8 u8TabIdx_MADi_Motion; // For 480i
    U8 u8TabIdx_MADi_DFK; // For 480i
    U8 u8TabIdx_MADi_SST; // For 480i
    U8 u8TabIdx_MADi_EODi; // For 480i
    U8 u8TabIdx_UCDi; // For 480i
#endif

#if(PQ_GAME_MODE_SET_SW_DRIVER)
    U8 u8TabIdx_SwDriver;
#endif
    //U16 u16TabIdx_BWS;

} StuGRuleGameModeInfo;

StuGRuleGameModeInfo g_PQ_stGRuleGameModeInfo;

U8 MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(U16 u16PqIpIdx)
{
    U8 i;

    for( i = 0; i < PQ_GRULE_GAME_MODE_IP_NUM_Main; i += 1 )
    {
        if( u16PqIpIdx == MST_GRule_GAME_MODE_IP_Index_Main[i] )
        {
            return i;
        }
    }
...
MS_BOOL MDrv_PQ_Is_IP_ControlByGameMode(/*U16*/PQ_IPTYPE_Main u16PqIpIdx)
{
    switch(u16PqIpIdx)
    {
    #if(PQ_GAME_MODE_SUPPORT_SD_P_MODE||PQ_GAME_MODE_SUPPORT_SD_I_MODE)
        ...
        case PQ_IP_UC_CTL_Main:
        case PQ_IP_PostCCS_Main:
        case PQ_IP_PostCCS_NP_Main:
        case PQ_IP_DHD_Main:
    #endif
        ...
        case PQ_IP_MADi_SST_Main:
        case PQ_IP_MADi_EODiW_Main:
        case PQ_IP_UCDi_Main:
    #endif

    #if(PQ_GAME_MODE_SET_SW_DRIVER)
        case PQ_IP_SwDriver_Main:
    #endif
...
void MDrv_PQ_Print_GameModeInfo(StuGRuleGameModeInfo* pstGRuleGameModeInfo)
{
#if(PQ_GAME_MODE_SUPPORT_SD_P_MODE||PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    DEBUG_GAMEMODE( printf(" u8TabIdx_MADi=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_MADi, MDrv_PQ_GetTableName(PQ_IP_MADi_Main, pstGRuleGameModeInfo->u8TabIdx_MADi)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_UC_CTL=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_UC_CTL, MDrv_PQ_GetTableName(PQ_IP_UC_CTL_Main, pstGRuleGameModeInfo->u8TabIdx_UC_CTL)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_PostCCS=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_PostCCS, MDrv_PQ_GetTableName(PQ_IP_PostCCS_Main, pstGRuleGameModeInfo->u8TabIdx_PostCCS)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_PostCCS_NP=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_PostCCS_NP, MDrv_PQ_GetTableName(PQ_IP_PostCCS_NP_Main, pstGRuleGameModeInfo->u8TabIdx_PostCCS_NP)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_DHD=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_DHD, MDrv_PQ_GetTableName(PQ_IP_DHD_Main, pstGRuleGameModeInfo->u8TabIdx_DHD)); );
#endif

#if(PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    DEBUG_GAMEMODE( printf(" u8TabIdx_MADi_Motion=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_MADi_Motion, MDrv_PQ_GetTableName(PQ_IP_MADi_Motion_Main, pstGRuleGameModeInfo->u8TabIdx_MADi_Motion)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_MADi_DFK=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_MADi_DFK, MDrv_PQ_GetTableName(PQ_IP_MADi_Motion_Main, pstGRuleGameModeInfo->u8TabIdx_MADi_DFK)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_MADi_SST=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_MADi_SST, MDrv_PQ_GetTableName(PQ_IP_MADi_Motion_Main, pstGRuleGameModeInfo->u8TabIdx_MADi_SST)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_MADi_EODi=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_MADi_EODi, MDrv_PQ_GetTableName(PQ_IP_MADi_Motion_Main, pstGRuleGameModeInfo->u8TabIdx_MADi_EODi)); );
    DEBUG_GAMEMODE( printf(" u8TabIdx_UCDi=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_UCDi, MDrv_PQ_GetTableName(PQ_IP_UCDi_Main, pstGRuleGameModeInfo->u8TabIdx_UCDi)); );
#endif

#if(PQ_GAME_MODE_SET_SW_DRIVER)
    DEBUG_GAMEMODE( printf(" u8TabIdx_SwDriver=%u(%s)\n", pstGRuleGameModeInfo->u8TabIdx_SwDriver, MDrv_PQ_GetTableName(PQ_IP_SwDriver_Main, pstGRuleGameModeInfo->u8TabIdx_SwDriver)); );
#endif
}

MS_BOOL MDrv_PQ_Get_GRule_GameModeInfo(PQ_WIN eWindow, StuGRuleGameModeInfo* pstGRuleGameModeInfo)
{
    if( eWindow != PQ_MAIN_WINDOW )
        return FALSE;

    memset( pstGRuleGameModeInfo, 0xFF, sizeof(StuGRuleGameModeInfo));

    U16 u16GameModeIdx = PQ_GAMEMODE_IDX_ON; // Currently, only one setting is for GameMode-On
    MS_U8 u8GRuleIPIndex = 0;

    UNUSED(u16GameModeIdx);
    UNUSED(u8GRuleIPIndex);

#if(PQ_GAME_MODE_SUPPORT_SD_P_MODE||PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_MADi_Main) )
        pstGRuleGameModeInfo->u8TabIdx_MADi = MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_MADi_Main) );

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_UC_CTL_Main) )
        pstGRuleGameModeInfo->u8TabIdx_UC_CTL = PQ_IP_UC_CTL_OFF_Main;

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_PostCCS_Main) )
        pstGRuleGameModeInfo->u8TabIdx_PostCCS = PQ_IP_PostCCS_OFF_Main; //MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_PostCCS_Main) );

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_PostCCS_NP_Main) )
        pstGRuleGameModeInfo->u8TabIdx_PostCCS_NP = PQ_IP_PostCCS_NP_OFF_Main;//MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_PostCCS_NP_Main) );

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_DHD_Main) )
        pstGRuleGameModeInfo->u8TabIdx_DHD = PQ_IP_DHD_OFF_Main; //MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_DHD_Main) );
#endif

#if(PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_MADi_Motion_Main) )
        pstGRuleGameModeInfo->u8TabIdx_MADi_Motion = MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_MADi_Motion_Main) );

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_MADi_DFK_Main) )
        pstGRuleGameModeInfo->u8TabIdx_MADi_DFK = MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_MADi_DFK_Main) );

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_MADi_SST_Main) )
        pstGRuleGameModeInfo->u8TabIdx_MADi_SST = MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_MADi_SST_Main) );

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_MADi_EODiW_Main) )
        pstGRuleGameModeInfo->u8TabIdx_MADi_EODi = MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_MADi_EODiW_Main) );

    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_UCDi_Main) )
        pstGRuleGameModeInfo->u8TabIdx_UCDi = PQ_IP_UCDi_OFF_Main;
#endif

#if(PQ_GAME_MODE_SET_SW_DRIVER)
    if( MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_MADi_Main) )
    {
        u8GRuleIPIndex = MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_SwDriver_Main);
        if( u8GRuleIPIndex != 0xFF )
        {
            pstGRuleGameModeInfo->u8TabIdx_SwDriver = MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, u8GRuleIPIndex );
        }
    }
#endif

    //pstGRuleGameModeInfo->u8TabIdx_BWS = MDrv_PQ_GetGRule_TableIndex(eWindow, PQ_GRule_GAME_MODE_Main, u16GameModeIdx, MDrv_PQ_Get_GameMode_ArrayIdx_ByIpIdx(PQ_IP_BWS_Main) );

#if 1
    DEBUG_GAMEMODE( printf("GameModeInfo:\n"); );
    DEBUG_GAMEMODE( MDrv_PQ_Print_GameModeInfo(pstGRuleGameModeInfo); );
#endif

    return TRUE;
}

MS_BOOL MDrv_PQ_Get_MADi_ForGameMode(PQ_WIN eWindow,
...
{
    DEBUG_GAMEMODE( printf("MDrv_PQ_Get_MADi_ForGameMode()\n"); );

    if( (eWindow != PQ_MAIN_WINDOW) )
        return FALSE;


    if( (MApi_XC_Sys_Get_GameMode() == XC_SYS_GAME_MODE_OFF) )
        return FALSE;


    //printf("1 u16TabIdx_MADi = %u\n", u16TabIdx_MADi);
    //printf("1 u16TabIdx_UCCTL = %u\n", u16TabIdx_UCCTL);
    //printf("1 u16TabIdx_MADi_Motion = %u\n", u16TabIdx_MADi_Motion);
    //printf("1 u16TabIdx_MADi_DFK = %u\n", u16TabIdx_MADi_DFK);

    // Get MADi TabIdx from GRule-GameMode
    memset( &g_PQ_stGRuleGameModeInfo, 0xFF, sizeof(StuGRuleGameModeInfo));

    if( FALSE == MDrv_PQ_Get_GRule_GameModeInfo(eWindow, &g_PQ_stGRuleGameModeInfo) )
        return FALSE;


#if( PQ_GAME_MODE_SUPPORT_SD_P_MODE||PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    if( g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi != 0xFF )
    {
        *pu16TabIdx_MADi = g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi;
        DEBUG_GAMEMODE( printf("[GameMode] u16TabIdx_MADi = %u\n", *pu16TabIdx_MADi); );
    }
    if( g_PQ_stGRuleGameModeInfo.u8TabIdx_UC_CTL != 0xFF )
    {
        *pu16TabIdx_UCCTL = g_PQ_stGRuleGameModeInfo.u8TabIdx_UC_CTL;
        DEBUG_GAMEMODE( printf("[GameMode] u16TabIdx_UCCTL = %u\n", *pu16TabIdx_UCCTL); );
    }
#endif

#if( PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    if( g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_Motion != 0xFF )
    {
        *pu16TabIdx_MADi_Motion = g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_Motion;
        DEBUG_GAMEMODE( printf("[GameMode] u16TabIdx_MADi_Motion = %u\n", *pu16TabIdx_MADi_Motion); );
    }
    if( g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_DFK != 0xFF )
    {
        *pu16TabIdx_MADi_DFK = g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_DFK;
        DEBUG_GAMEMODE( printf("[GameMode] u16TabIdx_MADi_DFK = %u\n", *pu16TabIdx_MADi_DFK); );
    }
    if( g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_SST != 0xFF )
    {
        *pu16TabIdx_MADi_SST = g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_SST;
        DEBUG_GAMEMODE( printf("[GameMode] u16TabIdx_MADi_SST = %u\n", *pu16TabIdx_MADi_SST); );
    }
    if( g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_EODi != 0xFF )
    {
        *pu16TabIdx_MADi_EODiW = g_PQ_stGRuleGameModeInfo.u8TabIdx_MADi_EODi;
        DEBUG_GAMEMODE( printf("[GameMode] u16TabIdx_MADi_EODiW = %u\n", *pu16TabIdx_MADi_EODiW); );
    }
#else
...
MS_BOOL MDrv_PQ_Load_GRule_GameMode(PQ_WIN eWindow, StuGRuleGameModeInfo* pstGRuleGameModeInfo)
{
    if( eWindow != PQ_MAIN_WINDOW )
        return FALSE;


    DEBUG_GAMEMODE( printf("MDrv_PQ_Load_GRule_GameMode()\n"); );

#if(PQ_GAME_MODE_SUPPORT_SD_P_MODE||PQ_GAME_MODE_SUPPORT_SD_I_MODE)
    // MADi load at MDrv_PQ_SetMemFormat()
    // UC_CTRL load at MDrv_PQ_SetMemFormat()

    if( pstGRuleGameModeInfo->u8TabIdx_PostCCS != 0xFF )
    {
        DEBUG_GAMEMODE( printf("Load PostCCS=%u\n", pstGRuleGameModeInfo->u8TabIdx_PostCCS); );
        MDrv_PQ_LoadTable(eWindow, pstGRuleGameModeInfo->u8TabIdx_PostCCS, PQ_IP_PostCCS_Main);
    }
    if( pstGRuleGameModeInfo->u8TabIdx_PostCCS_NP != 0xFF )
    {
        DEBUG_GAMEMODE( printf("Load PostCCS_NP=%u\n", pstGRuleGameModeInfo->u8TabIdx_PostCCS_NP); );
        MDrv_PQ_LoadTable(eWindow, pstGRuleGameModeInfo->u8TabIdx_PostCCS_NP, PQ_IP_PostCCS_NP_Main);
    }
    if( pstGRuleGameModeInfo->u8TabIdx_DHD != 0xFF )
    {
        DEBUG_GAMEMODE( printf("Load DHD=%u\n", pstGRuleGameModeInfo->u8TabIdx_DHD); );
        MDrv_PQ_LoadTable(eWindow, pstGRuleGameModeInfo->u8TabIdx_DHD, PQ_IP_DHD_Main);
    }
#endif
    ...
    // PQ_IP_MADi_Motion_Main  load at at MDrv_PQ_SetMemFormat()
    // PQ_IP_MADi_DFK_Main  load at at MDrv_PQ_SetMemFormat()
    // PQ_IP_MADi_SST_Main  load at at MDrv_PQ_SetMemFormat()
    // PQ_IP_MADi_EODiW_Main  load at at MDrv_PQ_SetMemFormat()

  #if PQ_UC_CTL
    if( pstGRuleGameModeInfo->u8TabIdx_UCDi != 0xFF )
    {
        DEBUG_GAMEMODE( printf("Load UCDi=%u\n", pstGRuleGameModeInfo->u8TabIdx_UCDi); );
        ...
        MDrv_PQ_LoadTable(eWindow, pstGRuleGameModeInfo->u8TabIdx_UCDi, PQ_IP_UCDi_Main);
...
#if( PQ_GAME_MODE_SET_SW_DRIVER )
    //printf("GameMode.u16TabIdx_SwDriver = %u\n", g_PQ_stGRuleGameModeInfo.u16TabIdx_SwDriver);
    if( pstGRuleGameModeInfo->u8TabIdx_SwDriver != 0xFF )
    {
        DEBUG_GAMEMODE( printf("Load SwDriver=%u\n", pstGRuleGameModeInfo->u8TabIdx_SwDriver); );
        MDrv_PQ_LoadTable(eWindow, pstGRuleGameModeInfo->u8TabIdx_SwDriver, PQ_IP_SwDriver_Main);
    }
#endif
...
void MDrv_PQ_LoadSettings_Ex(PQ_WIN eWindow)
{
    ...
    {
    #if(ENABLE_PQ_BIN_EX)
        if(gbPQBin_Ex_Enable)
        {
            MDrv_PQBin_LoadTableBySrcType(_u16PQSrcType[eWindow],
                                          PQ_BIN_IP_ALL,
                                          MDrv_PQBin_Ex_GetPanelIdx(),
...
void MDrv_PQ_LoadSettings(PQ_WIN eWindow)
{
    ...
    if(eWindow == PQ_MAIN_WINDOW)
    {
    ...
    #if( ENABLE_SCALER_GAMEMODE)
        if( (MApi_XC_Sys_Get_GameMode() != XC_SYS_GAME_MODE_OFF)
          &&MDrv_PQ_Is_IP_ControlByGameMode(PQ_IP_SwDriver_Main)
          )
        {
            // Load PQ_IP_SwDriver_Main at MDrv_PQ_Load_GRule_GameMode()
            //printf("GameMode.u16TabIdx_SwDriver = %u\n", g_PQ_stGRuleGameModeInfo.u16TabIdx_SwDriver);
            //MDrv_PQ_LoadTable(eWindow, g_PQ_stGRuleGameModeInfo.u16TabIdx_SwDriver, PQ_IP_SwDriver_Main);
        }
        else
    #endif
        {
        ...
        */
        ...
        MDrv_PQ_LoadSettings_Ex(eWindow);

        PQTAB_DBG(printf("Load Std/Ex...done\n"));
    ...
    else if(eWindow == PQ_SUB_WINDOW)
    {
        ...
        // Load sub_ex setting
        MDrv_PQ_LoadSettings_Ex(PQ_SUB_WINDOW);

        PQTAB_DBG(printf("Load sub Std/Ex...done\n"));
    }
#endif
    else
    {
        ...
        MS_U16 u16SizeAfterScaling)
{
    ...
    //PRINT_CURRENT_LINE();
    SRULE_DBG( printf("_MDrv_PQ_LoadScalingTable(eXRuleType=%u, u8XRuleIP=%d, bPreV_ScalingDown=%u, u16InputSize=%u, u16SizeAfterScaling=%u)\n",
                     eXRuleType, (MS_U16)u8XRuleIP, bPreV_ScalingDown, u16InputSize, u16SizeAfterScaling); );
    ...
    SRULE_DBG( printf(" IPIdx=%d(%s), ratio=%ld\n", u16IPIdx, MDrv_PQ_GetIPName(u16IPIdx), u32Ratio); );


    // Check special rule: RFBL ==> VSP_Y/C only use Bilinear
#if(ENABLE_FORCE_VSP_BILINEAR_WHEN_RFBL)
    if( (eXRuleType == E_XRULE_VSP)&&MApi_XC_IsCurrentRequest_FrameBufferLessMode() )
    {
        SRULE_DBG( printf(" FORCE_VSP_BILINEAR_WHEN_RFBL\n"); );
        if( u16IPIdx == PQ_IP_VSP_Y_Main )
        {
            SRULE_DBG( printf("  u16TabIdx = PQ_IP_VSP_Y_Bilinear_Main\n"); );
            u16TabIdx = PQ_IP_VSP_Y_Bilinear_Main;
        }
        else if( u16IPIdx == PQ_IP_VSP_C_Main )
        {
            SRULE_DBG( printf("  u16TabIdx = PQ_IP_VSP_C_Bilinear_Main\n"); );
            u16TabIdx = PQ_IP_VSP_C_Bilinear_Main;
        }
    }
    else
#endif
    if( bPreV_ScalingDown )
    {
        if( bInterlace)
        {
...
#endif
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //SRULE_DBG(printf("(e)tabidx=%u(%s)\n", u16TabIdx, MDrv_PQ_GetTableName(u16IPIdx, u16TabIdx ) ););
            }
        }
    }
    ...
    {
        return (u16TabIdx == (MS_U16)PQ_IP_VSD_Bilinear_Main) ; // PreVSDMode: 0:Cb, 1:Bilinear
    }
    else if( eXRuleType == E_XRULE_HSD)
    {
        return (u16TabIdx != (MS_U16)PQ_IP_HSD_Y_CB_Main); // PreHSDMode - 0:Cb, 1:Adv
    }
...
#if (PQ_ENABLE_MULTI_LEVEL_AUTO_NR)
...
PQ_DEINTERLACE_MODE MDrv_PQ_SetMemFormat(PQ_WIN eWindow, MS_BOOL bMemFmt422, MS_BOOL bFBL, MS_U8 *pu8BitsPerPixel)
{
    ...
    //SCALER_WIN eSCALER_WIN = (eWindow == PQ_SUB_WINDOW)?SUB_WINDOW:MAIN_WINDOW;
    ...
    if(!bFBL)
    {
        ...
        if( ((_stMode_Info[eWindow].u16input_hsize >= 1280) && (_stMode_Info[eWindow].u16input_vsize >= 720))
          &&(_bOSD_On)
     #if( ENABLE_SCALER_GAMEMODE )
          &&(MApi_XC_Sys_Get_GameMode() == XC_SYS_GAME_MODE_OFF) //  When Game mode is On, Don't use OSD GRule
     #endif
          )
        {
    ...
    #if( ENABLE_SCALER_GAMEMODE )
        //  When Game mode is On: Patch some IP
        if( (MApi_XC_Sys_Get_GameMode() != XC_SYS_GAME_MODE_OFF)&&(eWindow == PQ_MAIN_WINDOW) )
        {
            if( MDrv_PQ_Get_MADi_ForGameMode(eWindow,
                                        &u16TabIdx_MADi,
                                        &u16TabIdx_MADi_Motion,
                                        &u16TabIdx_MADi_DFK,
                                        &u16TabIdx_MADi_SST,
                                        ...
                                        &u16TabIdx_UCCTL) )
            {
                MDrv_PQ_Load_GRule_GameMode( eWindow, &g_PQ_stGRuleGameModeInfo );
            }
        }
    #endif

    #if( PQ_UC_CTL && ENABLE_DMP )
        if( (eWindow==PQ_MAIN_WINDOW) &&(u16TabIdx_UCCTL != PQ_IP_UC_CTL_OFF_Main) )
        {
    ...
    DEBUG_FRAME_COUNT( printf(" MDrv_PQ_GetBPPInfoFromMADi() => u8FrameCount=%u\n", u8FrameCount); );
    ...
    //BOOLEAN bIsBlackVideoEnable = MApi_XC_IsBlackVideoEnable(eSCALER_WIN);
    //BOOLEAN bIsInputSourceDisabled = MApi_XC_IsInputSourceDisabled(eSCALER_WIN);
    //printf("InputSourceDisabled=%u, BlackVideoEnable=%u\n", bIsInputSourceDisabled, bIsBlackVideoEnable);
...
void MDrv_PQ_ReduceBW_ForOSD(PQ_WIN eWindow, MS_BOOL bOSD_On)
{
    ...
    // When scaler is FBL mode, don't need reduce scaler BW
    if( _stMode_Info[eWindow].bFBL )
    {
        //printf("FBL skip reduecBW~\n");
        return;
    }
...
#if(ENABLE_SCALER_GAMEMODE)
    // When GameMode is On, Disable this function
    if( MApi_XC_Sys_Get_GameMode() != XC_SYS_GAME_MODE_OFF )
    {
        return;
    }
...
MS_BOOL MDrv_PQ_IOCTL(PQ_WIN eWindow, MS_U32 u32Flag, void *pBuf, MS_U32 u32BufSize)
{
    ...
    switch(u32Flag)
    {
        case E_PQ_IOCTL_HSD_SAMPLING:
        {
            ...
            SRULE_DBG(printf("[SRule] GetHSDSampling() => ret=%u, u32ratio=%u, bADVMode=%u\n", bret, u32ratio, bADVMode ); );
        ...
        case E_PQ_IOCTL_PQ_SUGGESTED_FRAMENUM:
        {
            if( u32BufSize == sizeof(MS_U16) )
            {
                *((MS_U16 *)pBuf) = _u16PQSuggestedFrameNum[eWindow];
                DEBUG_FRAME_COUNT( printf(" IOCTL_PQ_SUGGESTED_FRAMENUM => _u16PQSuggestedFrameNum[]=%u\n", _u16PQSuggestedFrameNum[eWindow]); );
                bret = TRUE;
            }
            else
            {
...
MS_BOOL MDrv_PQ_Set_DisplayType_Sub(MS_U16 u16DisplayWidth, PQ_DISPLAY_TYPE enDisplaType, MS_BOOL bSkipCommTable)
{
        ...
        PQTableInfo.u8PQFileType = PQ_FILE_SUB;
        ...
        PQTableInfoEx.u8PQFileType = PQ_FILE_SUB_EX;
        ...
        if (_bCustomerSubPQEn)
        {
            if ( devPanel_FullHD() )
            {
                ...
                PQTableInfoColor.u8PQFileType = PQ_FILE_SUB_;
...
MS_BOOL MDrv_PQ_Set_DisplayType_Main(MS_U16 u16DisplayWidth, PQ_DISPLAY_TYPE enDisplaType, MS_BOOL bSkipCommTable)
{
    ...
    switch(enDisplaType)
    {
        ...
        #if( ENABLE_SCALER_GAMEMODE && (PQ_GRULE_GAME_MODE_ENABLE) )
            if ( bIs1366Panel )
                PQTableInfo.pGRule_Level[E_GRULE_GAMEMODE] = (void*)MST_GRule_1366_GAME_MODE_Main;
          #if( PQ_SUPPORT_4K_PANEL )
            else if( bIs4KPanel )
                PQTableInfo.pGRule_Level[E_GRULE_GAMEMODE] = (void*)MST_GRule_4K2K_GAME_MODE_Main;
          #endif
            else
                PQTableInfo.pGRule_Level[E_GRULE_GAMEMODE] = (void*)MST_GRule_1920_GAME_MODE_Main;
        #endif
    ...
    #if( ENABLE_SCALER_GAMEMODE && (PQ_GRULE_GAME_MODE_ENABLE) )
        PQTableInfo.u8PQ_GRule_Num[E_GRULE_GAMEMODE] = PQ_GRULE_GAME_MODE_NUM_Main;
        PQTableInfo.u8PQ_GRule_IPNum[E_GRULE_GAMEMODE] = PQ_GRULE_GAME_MODE_IP_NUM_Main;
        PQTableInfo.pGRule_IP_Index[E_GRULE_GAMEMODE] = (void*)MST_GRule_GAME_MODE_IP_Index_Main;
        PQTableInfo.pGRule_Array[E_GRULE_GAMEMODE] = (void*)MST_GRule_GAME_MODE_Main;
    #endif

        PQTableInfo.u8PQFileType = PQ_FILE_MAIN;
        ...
        PQTableInfoEx.u8PQFileType = PQ_FILE_MAIN_EX;
        ...
        if (_bCustomerMainPQEn)
        {
            if (1)//( devPanel_FullHD() )
            {
                ...
                PQTableInfoColor.u8PQFileType = PQ_FILE_MAIN_;
...
#if PQ_MADI_MAPPING_NEW_METHOD
/*
#define PQ_MADI_PMODE   0
#define PQ_MADI_24      1
#define PQ_MADI_25      2
#define PQ_MADI_26      3
#define PQ_MADI_27      4
#define PQ_MADI_RFBL    5
*/

#define MADI_OPW    BIT0 // OP Write
#define MADI_RFBL   BIT1
...
typedef struct
{
    ...
    U8 u8PqMADiIdx;
    U8 u8BitsPerPixel;
    PQ_DEINTERLACE_MODE ePqDeIntMode;
    U8 u8FrameCount;
    U8 u8DelayTime;
...
TypPqMADiInfo g_PQ_astMADiInfoTable[] =
{
    // 24 2R
    { MS_MADI_24_2R,            PQ_IP_MADi_24_2R_Main,          20, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_24_2R_880,        PQ_IP_MADi_24_2R_880_Main,      20, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },

    // 24 4R
    { MS_MADI_24_4R,            PQ_IP_MADi_24_4R_Main,          20, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_24_4R_880,        PQ_IP_MADi_24_4R_880_Main,      20, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },

    // 25 2R
    { MS_MADI_25_2R,            PQ_IP_MADi_25_2R_Main,          24, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_25_2R_884,        PQ_IP_MADi_25_2R_884_Main,      24, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_25_2R_880,        PQ_IP_MADi_25_2R_880_Main,      24, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },

    // 25 4R
    { MS_MADI_25_4R,            PQ_IP_MADi_25_4R_Main,          24, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_25_4R_884,        PQ_IP_MADi_25_4R_884_Main,      24, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_25_4R_880,        PQ_IP_MADi_25_4R_880_Main,      24, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },

    // 25 4R MC
    { MS_MADI_25_4R_MC,         PQ_IP_MADi_25_4R_MC_Main,       24, PQ_DEINT_3DDI_HISTORY, 4, 60, MADI_OPW },
  #if( (CHIP_FAMILY_TYPE != CHIP_FAMILY_MAZDA)&&(CHIP_FAMILY_TYPE != CHIP_FAMILY_MAYA) )
    { MS_MADi_25_4R_MC_Yonly,   PQ_IP_MADi_25_4R_MC_Yonly_Main, 24, PQ_DEINT_3DDI_HISTORY, 4, 60, MADI_OPW },
  #endif
    { MS_MADI_25_4R_MC_NW,      PQ_IP_MADi_25_4R_MC_NW_Main,    24, PQ_DEINT_3DDI_HISTORY, 4, 60, MADI_OPW },


    // 25 4R 6F MC
  #if( (CHIP_FAMILY_TYPE != CHIP_FAMILY_MAZDA)&&(CHIP_FAMILY_TYPE != CHIP_FAMILY_MAYA) )
    { MS_MADI_25_6F_4R_MC,      PQ_IP_MADi_25_6F_4R_MC_Main,        24, PQ_DEINT_3DDI_HISTORY, 6, 125, MADI_OPW },
    { MS_MADI_25_6F_4R_MC_Yonly,PQ_IP_MADi_25_6F_4R_MC_Yonly_Main,  24, PQ_DEINT_3DDI_HISTORY, 6, 125, MADI_OPW },
  #endif
  #if( (CHIP_FAMILY_TYPE==CHIP_FAMILY_MAYA) || (CHIP_FAMILY_TYPE==CHIP_FAMILY_MAZDA) )
    { MS_MADI_25_6F_4R_MC_I,    PQ_IP_MADi_25_6F_4R_MC_I_Main,      24, PQ_DEINT_3DDI_HISTORY, 6, 125, MADI_OPW },
    { MS_MADI_25_6F_4R_MC_P,    PQ_IP_MADi_25_6F_4R_MC_P_Main,      24, PQ_DEINT_3DDI_HISTORY, 6, 125, MADI_OPW },
  #endif

    // 25 4R 8F MC
  #if( (CHIP_FAMILY_TYPE != CHIP_FAMILY_MILAN)&&(CHIP_FAMILY_TYPE != CHIP_FAMILY_WHISKY) )
    { MS_MADI_25_8F_4R_MC,      PQ_IP_MADi_25_8F_4R_MC_Main,        24, PQ_DEINT_3DDI_HISTORY, 8, 125, MADI_OPW },
  #endif
  #if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_MARLON) )
    { MS_MADI_25_8F_4R_MC_Yonly,PQ_IP_MADi_25_8F_4R_MC_Yonly_Main,  24, PQ_DEINT_3DDI_HISTORY, 8, 125, MADI_OPW },
  #endif

    // 25 6R MC
    { MS_MADI_25_6R_MC,         PQ_IP_MADi_25_6R_MC_Main,       24, PQ_DEINT_3DDI_HISTORY, 8, 125, MADI_OPW },
    { MS_MADI_25_6R_MC_NW,      PQ_IP_MADi_25_6R_MC_NW_Main,    24, PQ_DEINT_3DDI_HISTORY, 8, 125, MADI_OPW },
    { MS_MADI_25_14F_6R_MC,     PQ_IP_MADi_25_14F_6R_MC_Main,   24, PQ_DEINT_3DDI_HISTORY, 14, 125, MADI_OPW },

    // 25 8R MC
    { MS_MADI_25_12F_8R_MC,     PQ_IP_MADi_25_12F_8R_MC_Main,   24, PQ_DEINT_3DDI_HISTORY, 12, 125, MADI_OPW },
    { MS_MADI_25_14F_8R_MC,     PQ_IP_MADi_25_14F_8R_MC_Main,   24, PQ_DEINT_3DDI_HISTORY, 14, 125, MADI_OPW },
    { MS_MADI_25_16F_8R_MC,     PQ_IP_MADi_25_16F_8R_MC_Main,   24, PQ_DEINT_3DDI_HISTORY, 16, 125, MADI_OPW },

    // 26 2/4R
    { MS_MADI_26_2R,            PQ_IP_MADi_26_2R_Main,          20, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_26_4R,            PQ_IP_MADi_26_4R_Main,          20, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },

    // 27 2/4R
    { MS_MADI_27_2R,            PQ_IP_MADi_27_2R_Main,          16, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },
    { MS_MADI_27_4R,            PQ_IP_MADi_27_4R_Main,          16, PQ_DEINT_3DDI_HISTORY, 4, 60, 0 },

    // P Mode
    { MS_MADI_P_MODE8,          PQ_IP_MADi_P_MODE8_Main,            16, PQ_DEINT_OFF,       3, 60, 0 },
    { MS_MADI_P_MODE10,         PQ_IP_MADi_P_MODE10_Main,           20, PQ_DEINT_OFF,       3, 60, 0 },
    { MS_MADI_P_MODE8_2BYTE_3D, PQ_IP_MADi_P_MODE8_2BYTE_3D_Main,   16, PQ_DEINT_OFF,       3, 60, 0 },
  #if( (CHIP_FAMILY_TYPE==CHIP_FAMILY_MARLON) )
    { MS_MADI_P_MODE8_NO_DNR,   PQ_IP_MADi_P_MODE8_NO_DNR_Main,     16, PQ_DEINT_OFF,       3, 60, 0 },
  #endif
  #if( (CHIP_FAMILY_TYPE==CHIP_FAMILY_MAYA) )
    { MS_MADI_P_MODE_RY8,       PQ_IP_MADi_RY8_MODE_Main,           24, PQ_DEINT_OFF,       3, 60, 0  },
  #endif

    // P Mode Motion
    { MS_MADI_P_MODE_MOT8,       PQ_IP_MADi_P_MODE_MOT8_Main,           20, PQ_DEINT_OFF,   3, 60, 0 },
    { MS_MADI_P_MODE_MOT10,      PQ_IP_MADi_P_MODE_MOT10_Main,          24, PQ_DEINT_OFF,   3, 60, 0 },
  #if( (CHIP_FAMILY_TYPE==CHIP_FAMILY_MAZDA)||(CHIP_FAMILY_TYPE==CHIP_FAMILY_MARLON) )
    { MS_MADI_P_MODE_MOT10_NO_DNR, PQ_IP_MADi_P_MODE_MOT10_NO_DNR_Main, 24, PQ_DEINT_OFF,   3, 60, 0 },
  #endif
    { MS_MADI_P_MODE_MOT10_4Frame, PQ_IP_MADi_P_MODE_MOT10_4Frame_Main, 24, PQ_DEINT_OFF,   4, 125, MADI_OPW },
    { MS_MADI_P_MODE_MOT10_8Frame, PQ_IP_MADi_P_MODE_MOT10_8Frame_Main, 24, PQ_DEINT_OFF,   8, 125, MADI_OPW },

  #if(PQ_SUPPORT_MADI_P_MODE_MOT10_NR_Yonly)
    { MS_MADI_P_MODE_MOT10_NR_Yonly,PQ_IP_MADi_P_MODE_MOT10_NR_Yonly_Main, 24, PQ_DEINT_OFF, 3, 60, 0 },
  #endif

    // P Mode MC
  #if( (CHIP_FAMILY_TYPE != CHIP_FAMILY_MAYA) )
    { MS_MADI_P_MODE_MC_3Frame,     PQ_IP_MADi_P_MODE_MC_3Frame_Main,       24, PQ_DEINT_OFF,   3, 125, MADI_OPW },
  #endif
  #if( (CHIP_FAMILY_TYPE==CHIP_FAMILY_MAZDA)||(CHIP_FAMILY_TYPE==CHIP_FAMILY_MAYA) )
    { MS_MADI_P_MODE_MC_4Frame,     PQ_IP_MADi_P_MODE_MC_4Frame_Main,       24, PQ_DEINT_OFF,   4, 125, MADI_OPW },
    { MS_MADI_P_MODE_MC_6Frame,     PQ_IP_MADi_P_MODE_MC_6Frame_Main,       24, PQ_DEINT_OFF,   6, 125, MADI_OPW },
  #endif
    { MS_MADI_P_MODE_MC_6Frame_6R,  PQ_IP_MADi_P_MODE_MC_6Frame_6R_Main,    24, PQ_DEINT_OFF,   6, 125, MADI_OPW },
  #if( (CHIP_FAMILY_TYPE==CHIP_FAMILY_MELODY) )
    { MS_MADI_P_MODE_MC_8Frame_6R,  PQ_IP_MADi_P_MODE_MC_8Frame_6R_Main,    24, PQ_DEINT_OFF,   8, 125, MADI_OPW },
  #endif
    { MS_MADI_P_MODE_MC_12Frame_8R, PQ_IP_MADi_P_MODE_MC_12Frame_8R_Main,   24, PQ_DEINT_OFF,  12, 125, MADI_OPW },


 #if( PQ_ENABLE_RFBL )
     // RFBL
    { MS_MADI_RFBL_2D,          PQ_IP_MADi_RFBL_2D_Main,        24,  PQ_DEINT_3DDI_HISTORY, 4, 60, MADI_RFBL }, // For P Mode
    { MS_MADI_RFBL_25D,         PQ_IP_MADi_RFBL_25D_Main,       24,  PQ_DEINT_3DDI_HISTORY, 4, 60, MADI_RFBL },
    { MS_MADI_RFBL_3D,          PQ_IP_MADi_RFBL_3D_Main,        24,  PQ_DEINT_3DDI_HISTORY, 4, 60, MADI_RFBL },
    // RFBL Motion
   #if( (CHIP_FAMILY_TYPE==CHIP_FAMILY_MAZDA)||(CHIP_FAMILY_TYPE==CHIP_FAMILY_MAYA) )
    { MS_MADI_RFBL_P_MODE8_MOT, PQ_IP_MADi_RFBL_P_MODE8_MOT_Main,  24, PQ_DEINT_OFF,        2, 40, MADI_RFBL },
    { MS_MADI_RFBL_P_MODE10_MOT,PQ_IP_MADi_RFBL_P_MODE10_MOT_Main, 24, PQ_DEINT_OFF,        2, 40, MADI_RFBL },
   #endif
    // RFBL MC
   #if( (CHIP_FAMILY_TYPE != CHIP_FAMILY_MAZDA)&&(CHIP_FAMILY_TYPE != CHIP_FAMILY_MAYA) )
    { MS_MADI_RFBL_MC,          PQ_IP_MADi_RFBL_MC_Main,        24,  PQ_DEINT_3DDI_HISTORY, 4, 60, MADI_RFBL|MADI_OPW },
   #endif
   #if( (CHIP_FAMILY_TYPE != CHIP_FAMILY_MAZDA)&&(CHIP_FAMILY_TYPE != CHIP_FAMILY_MAYA) )
    { MS_MADI_RFBL_P_MODE_MC,   PQ_IP_MADi_RFBL_P_MODE_MC_Main, 24,  PQ_DEINT_OFF,          2,120, MADI_RFBL|MADI_OPW },
   #endif
 #endif

    // P Mode 444
    { MS_MADI_P_MODE8_444,      PQ_IP_MADi_P_MODE8_444_Main,    24, PQ_DEINT_OFF,           2, 40, 0 },
    { MS_MADI_P_MODE10_444,     PQ_IP_MADi_P_MODE10_444_Main,   32, PQ_DEINT_OFF,           2, 40, 0 },
};

BOOLEAN MDrv_PQ_Get_MADiInfo(U8 u8PqMADiIdx, TypPqMADiInfo* pstPqMADiInfo)
{
    U8 i = 0;

    for( i = 0; i < (sizeof(g_PQ_astMADiInfoTable)/sizeof(TypPqMADiInfo)); i += 1 )
    {
        if( g_PQ_astMADiInfoTable[i].u8PqMADiIdx == u8PqMADiIdx )
        {
            *pstPqMADiInfo = g_PQ_astMADiInfoTable[i];
            return TRUE;
        }
    }

    printf("\nError: Unknown u8PqMADiIdx=%u\n", u8PqMADiIdx);
                                              ...
                                              )
{
    ...
    if(bMemFmt422)
    {
        ...
        TypPqMADiInfo stPqMADiInfo;
        memset(&stPqMADiInfo, 0, sizeof(stPqMADiInfo));
        if( FALSE == MDrv_PQ_Get_MADiInfo(u16TabIdx_MADi, &stPqMADiInfo) )
        {
            printf("\nError: Get MADi info failed! (u16TabIdx_MADi=%u)\n", u16TabIdx_MADi);
            stPqMADiInfo.u8MsMADiIdx = MS_MADI_P_MODE_MOT10;
            stPqMADiInfo.u8BitsPerPixel = 24;
            stPqMADiInfo.ePqDeIntMode = PQ_DEINT_OFF;
            stPqMADiInfo.u8FrameCount = 3;
            stPqMADiInfo.u8DelayTime = 60;
            stPqMADiInfo.u8MiscFlag = 0;
        }
        else
        {
            PQMADi_DBG( printf("u16TabIdx_MADi=%u => %u\n", u16TabIdx_MADi, stPqMADiInfo.u8MsMADiIdx ); );
        }

        //PQMADi_DBG( printf("24_4R PQ_DEINT_3DDI_HISTORY \n"); );
        PQMADi_DBG( printf("u8BitsPerPixel=%u\n", stPqMADiInfo.u8BitsPerPixel ); );
        PQMADi_DBG( printf("ePqDeIntMode=%u\n", stPqMADiInfo.ePqDeIntMode ); );
        PQMADi_DBG( printf("u8FrameCount=%u\n", stPqMADiInfo.u8FrameCount ); );
        PQMADi_DBG( printf("u8DelayTime=%u\n", stPqMADiInfo.u8DelayTime ); );
        PQMADi_DBG( printf("u8MiscFlag=%u\n", stPqMADiInfo.u8MiscFlag ); );

        // Patch some special MADi mode
        switch( stPqMADiInfo.u8MsMADiIdx )
        {
            case MS_MADI_P_MODE_MC_3Frame:
                ...
                {
                    printf("\n!!Error: Cannot set MS_MADI_P_MODE_MC_3Frame for mirror 24p(at least 4 frame number). Fix your PQ excel!!\n");
                }
                break;
            ...
            case MS_MADi_25_4R_MC_Yonly:
                if( MApi_XC_GetMirrorModeType() != MIRROR_NORMAL )
                {
                    printf("\n!!Error: Cannot set MS_MADI_25_4R_MC(4 fields mode) for Mirror(at least 6 fields). Fix your PQ excel!!\n");
                }
                break;
            ...
            case MS_MADI_P_MODE_MOT10:
                if( (MApi_XC_GetMirrorModeType() == MIRROR_V_ONLY) || (MApi_XC_GetMirrorModeType() == MIRROR_HV) )
                {
                    *pu8FrameCount = 3; // For tearing issue.
                }
            #if ( ENABLE_SCALER_GAMEMODE )
                else if( MApi_XC_Sys_Get_GameMode() != XC_SYS_GAME_MODE_OFF )
                {
                    *pu8FrameCount = 2; // For small frame delay
                }
            #endif
                break;
        ...
        *pu8BitsPerPixel = stPqMADiInfo.u8BitsPerPixel;
        *peDeInterlaceMode = stPqMADiInfo.ePqDeIntMode;;
        _u16PQDelyaTime = stPqMADiInfo.u8DelayTime;;

        if( stPqMADiInfo.u8MiscFlag&MADI_RFBL )
        {
            MApi_XC_EnableRequest_FrameBufferLess(ENABLE);
            PQMADi_DBG(printf("[RFBL=1]"););
        }
        else
        {
            MApi_XC_EnableRequest_FrameBufferLess(DISABLE);
            PQMADi_DBG(printf("[RFBL=0]"););
        }

        if( stPqMADiInfo.u8MiscFlag&MADI_OPW )
        {
            MApi_XC_Set_OPWriteOffEnable(DISABLE, eSCALER_WIN);
            PQMADi_DBG(printf("[OPW=1]"););
        }
        else
        {
            PQMADi_DBG(printf("[OPW=0]"););
        }
        ...
        else if(u16TabIdx_MADi == Hal_PQ_get_madi_idx((eWindow == PQ_MAIN_WINDOW) ? TRUE : FALSE, MS_MADI_P_MODE8))
        {
            ...
            *peDeInterlaceMode = PQ_DEINT_2DDI_BOB;
            ...
            //if(MApi_XC_GetIPAutoNoSignal((SCALER_WIN)eWindow)==ENABLE) // Why this condition??
            if( (MApi_XC_GetMirrorModeType() == MIRROR_V_ONLY) || (MApi_XC_GetMirrorModeType() == MIRROR_HV) )
            {
                *pu8FrameCount = 3; // For tearing issue.
            }
        ...
        */
        ...
        else if(u16TabIdx_MADi == Hal_PQ_get_madi_idx((eWindow == PQ_MAIN_WINDOW) ? TRUE : FALSE, MS_MADI_P_MODE8_NO_DNR))
        {
            *pu8BitsPerPixel = 16;
            *peDeInterlaceMode = PQ_DEINT_2DDI_BOB;
            _u16PQDelyaTime = 40;
    ...
    // If memory format is 444, Force disable UC
    if( !bMemFmt422 ) // If 444
    {
        // Force UC Off
    #if PQ_UC_CTL
...
{
    DEBUG_FRAME_COUNT( printf("PQ_SuggestFrameNum(u8FrameCount=%u)\n", u8FrameCount); );
    ...
    if(MApi_XC_IsFieldPackingModeSupported())
    {
        ...
        if(bSetFrameCount )
        {
            ...
            DEBUG_FRAME_COUNT( printf("_u16PQSuggestedFrameNum[%u] = %u\n", eWindow, u8FrameCount); );
        }
    }
    return TRUE;
}
...
#if (PQ_QM_YPBPR)
MS_U16 QM_InputSourceToIndex_YPbPr(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;
...
/*
    ...
    else if (QM_IsYPbPr_1920x1080_24P(eWindow)
        || QM_IsYPbPr_1920x1080_30P(eWindow)
        || QM_IsYPbPr_1920x1080_25P(eWindow) ){
...
#if (PQ_QM_DTV)
MS_U16 QM_InputSourceToIndex_DTV(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;

    if( !QM_IsSourceDTV(enInputSourceType) )
        return u16SrcType;
...
MS_U16 QM_InputSourceToIndex_MM(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;

    if( !QM_IsSourceMultiMedia(enInputSourceType) )
        return u16SrcType;
...
#if (PQ_QM_CVBS)
MS_U16 QM_InputSourceToIndex_Scart(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;

    if( !QM_IsSourceScart(enInputSourceType) )
        return u16SrcType;

    PQ_VIDEOSTANDARD_TYPE eStandard = QM_GetATVStandard(eWindow);

    if (QM_IsSourceScartRGB(enInputSourceType, eWindow))
    {
        MS_U16 u16Input_VFreq = QM_GetInputVFreq(eWindow);
        if (u16Input_VFreq > 550)
        {
...
MS_U16 QM_InputSourceToIndex_ATV(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;

    if( !QM_IsSourceATV(enInputSourceType) )
        return u16SrcType;
...
MS_U16 QM_InputSourceToIndex_SV(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;

    if( !QM_IsSourceSV(enInputSourceType) )
        return u16SrcType;
...
MS_U16 QM_InputSourceToIndex_AV(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    MS_U16 u16SrcType = 0;

    if( !QM_IsSourceAV(enInputSourceType) )
        return u16SrcType;
...
MS_U16 QM_InputSourceToIndex(PQ_WIN eWindow, MS_U8 enInputSourceType)
{
    ...
    {
    #if (PQ_QM_YPBPR)
        u16SrcType = QM_InputSourceToIndex_YPbPr(eWindow, enInputSourceType);
    #endif
    }
    else if (QM_IsSourceDTV(enInputSourceType))
    {
    #if (PQ_QM_DTV)
        u16SrcType = QM_InputSourceToIndex_DTV(eWindow, enInputSourceType);
    #endif
    }
    else if (QM_IsSourceMultiMedia(enInputSourceType))
    {
    #if (PQ_QM_MM_VIDEO) || (PQ_QM_MM_PHOTO)
        u16SrcType = QM_InputSourceToIndex_MM(eWindow, enInputSourceType);
    #endif
    }
    else if (QM_IsSourceScart(enInputSourceType))
    {
    #if (PQ_QM_CVBS)
        u16SrcType = QM_InputSourceToIndex_Scart(eWindow, enInputSourceType);
    #endif
    }
    else if (QM_IsSourceATV(enInputSourceType))
    {
    #if (PQ_QM_CVBS)
        u16SrcType = QM_InputSourceToIndex_ATV(eWindow, enInputSourceType);
    #endif
    }
    else if (QM_IsSourceSV(enInputSourceType))
    {
    #if (PQ_QM_CVBS)
        u16SrcType = QM_InputSourceToIndex_SV(eWindow, enInputSourceType);
    #endif
    }
    else // AV
    {
    #if (PQ_QM_CVBS)
        u16SrcType = QM_InputSourceToIndex_AV(eWindow, enInputSourceType);
    #endif
    }
...
MS_BOOL MDrv_PQ_Is_IpNeedSkip(U8 u8PqFileType, U16 u16PqIpIdx)
{
    if( u8PqFileType == PQ_FILE_MAIN )
    {
    #if( MARLON_PATCH__FORCE_SCALER_GAMMA_OFF )
        if ( u16PqIpIdx == PQ_IP_Gamma_Main )
        {
        ...
        if( MApi_XC_Sys_Get_GameMode() != XC_SYS_GAME_MODE_OFF )
        {
            if( MDrv_PQ_Is_IP_ControlByGameMode((PQ_IPTYPE_Main)u16PqIpIdx) )
            {
                printf("Skip ip %u(%s)\n", u16PqIpIdx, MDrv_PQ_GetIPName(u16PqIpIdx) );
                return TRUE;
            }
        }
    #endif
