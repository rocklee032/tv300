...
#if(ENABLE_STR_PQ_PATCH)// ((CHIP_SUPPORT_STR) &&(STR_ENABLE == ENABLE))
...
static void _MDrv_PQ_LoadCommTable(void)
{
    ...
    for (i=0; i<_PQTableInfo.u8PQ_IP_Num; i++){
        ...
        if (ip_Info.u8TabType == PQ_TABTYPE_VIP_IHC_CRD_SRAM || ip_Info.u8TabType == PQ_TABTYPE_VIP_ICC_CRD_SRAM)
        {
            continue;
        }
...
static void _MDrv_PQ_LoadTableBySrcType(MS_U16 u16PQSrcType, MS_U8 u8PQIPIdx)
{
    ...
    //PRINT_CURRENT_LINE();
    //printf("_MDrv_PQ_LoadTableBySrcType(u16PQSrcType=%u, u8PQIPIdx=%u)\n", u16PQSrcType, u8PQIPIdx );

#if (PQ_ENABLE_PIP)
 #if 1
    if( (_PQTableInfo.u8PQFileType == PQ_FILE_SUB)
      ||(_PQTableInfo.u8PQFileType == PQ_FILE_SUB_EX)
      )
    {
    ...
    //printf("QMIPtype_size=%u\n", QMIPtype_size);

    for(i=0; i<QMIPtype_size; i++, u8PQIPIdx++)
    {
        //printf("{i=%u, u8PQIPIdx=%u}", i, u8PQIPIdx);
        ...
        //if( _PQTableInfo.u8PQFileType == PQ_FILE_MAIN )
        {
            if( MDrv_PQ_Is_IpNeedSkip( _PQTableInfo.u8PQFileType, u8PQIPIdx ) )
            {
                //printf("QMAP_%u skip ip_%u by drvPQ\n", _PQTableInfo.u8PQFileType, u8PQIPIdx);
                continue;
            }
        }
