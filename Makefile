...
ifeq ($(BIN_FORMAT),COMPRESS)
	...
	COMPRESS_FORMAT=1
else ifeq ($(BIN_FORMAT),COMPRESS_ZDEC)
	COMPRESS_FORMAT=2
endif
...
ifeq ($(OS_TYPE),nos_arm)
	...
	#FPU_FLAGS = -mfpu=vfpv3 -mfloat-abi=softfp
	FPU_FLAGS += -mno-apcs-float
else
    FPU_FLAGS =
endif

ifeq ($(OS_TYPE),nos_arm)
    YOGA_GLOBAL_LDFLAGS = -g -nostdlib -Wl,--gc-sections -Wl,-static
else
...
ifeq ($(PARA), 1)
    ...
    #CC_OPTS0  += -c $(INCLUDE) $(AEON_FLAG) $(CC_TVOPTS) $(YOGA_GLOBAL_CFLAGS) $(MALLOC_CFLAGS)
    CC_OPTS0  += -c $(AEON_FLAG) $(CC_TVOPTS) $(YOGA_GLOBAL_CFLAGS) $(MALLOC_CFLAGS) $(INCLUDE)
    ...
    ifeq ($(OS_TYPE),nos_arm)
	    ...
	    CC_OPTS0 += -fno-omit-frame-pointer -mapcs-frame
...
ifeq ($(OS_TYPE),nos_mips)
    ...
    CC_DEBUG_OPTION = -g -gdwarf-2
    ifeq ($(VERSION),Debug)
      #  CC_OPTS  = $(CC_OPTS0) $(CC_OPTIMIZE) -g -gdwarf-2
    else
      #  CC_OPTS  = $(CC_OPTS0) $(CC_OPTIMIZE)   -gdwarf-2
    endif
...
CC_OPTS  = $(CC_OPTS0) $(CC_OPTIMIZE) $(CC_DEBUG_OPTION)
...
ifeq ($(PARA), 1)
    ...
    ifeq ($(OS_TYPE),nos_mips)
		...
		LD_OPTS += -g -mlittle-endian -nostdlib -Wl,--gc-sections -Wl,--no-enum-size-warning -Wl,-static -Wl,-Map,$(AP_MAP)
		LD_OPTS += -fPIC
...
#	@rm $(FILE_HK_CPU_HEAP_END)
	@awk '{if($$1>=100) { ;} else {print "Error:HEAP_SIZE("$$1"KB) is less than 100 KB";exit 1}}' $(FILE_HK_CPU_HEAP_SIZE)
...
#		@$(shell $(LZSSDIR)/lzss.out C 14336 $(BINPATH)/$(AP_NAME).bin $(BINPATH)/$(AP_COMPRESS_NAME).bin;)
		...
		@$(shell $(MSCOMPDIR)/mscomp_zdec -S 14336 -e $(BINPATH)/$(AP_NAME).bin $(BINPATH)/$(AP_COMPRESS_NAME).bin > $(BINPATH)/zlib.log; )
		@$(shell cp $(BINPATH)/$(AP_COMPRESS_NAME).bin $(ROOT)/boot/sboot/bin/$(AP_NAME).bin )
		@$(shell cp $(BINPATH)/$(AP_NAME).map $(ROOT)/boot/sboot/bin)
    else
...
debug_tool:
	@echo "[Rule - debug_tool]"
	$(OBJDUMP) -W $(AP_ELF) > $(BINPATH)/AP_DWARF.txt
...
#	@$(CC) -v >> $(FILE_TOOLCHAIN_INFO)
	@echo "CC_TVOPTS = $(CC_TVOPTS)" > $(FILE_BUILD_OPTION)
	@echo "AEON_FLAG = $(AEON_FLAG)" >> $(FILE_BUILD_OPTION)
	@echo "FPU_FLAGS = $(FPU_FLAGS)" >> $(FILE_BUILD_OPTION)
	@echo "CC_OPTS0 = $(CC_OPTS0)" >> $(FILE_BUILD_OPTION)
	@echo "CC_OPTS = $(CC_OPTS)" >> $(FILE_BUILD_OPTION)
	@echo "LD_OPTS = $(LD_OPTS)" >> $(FILE_BUILD_OPTION)
	@echo "LD_LIB = $(LD_LIB)" >> $(FILE_BUILD_OPTION)
	@echo "DRV_BSP_INC = $(DRV_BSP_INC)" >> $(FILE_BUILD_OPTION)
	@echo "INC_DIR = $(INC_DIR)" >> $(FILE_BUILD_OPTION)
	@echo "WARN_FLAGS = $(WARN_FLAGS)" >> $(FILE_BUILD_OPTION)
	@echo "YOGA_GLOBAL_CFLAGS = $(YOGA_GLOBAL_CFLAGS)" >> $(FILE_BUILD_OPTION)
	@echo "YOGA_GLOBAL_LDFLAGS = $(YOGA_GLOBAL_LDFLAGS)" >> $(FILE_BUILD_OPTION)
